...
model_dir = "/root/.cache/modelscope/hub/qwen/Qwen-1_8B-Chat"
name = model_dir
tokenizer = AutoTokenizer.from_pretrained(name, trust_remote_code=True)
generation_config = GenerationConfig.from_pretrained(name, trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained(name, device_map="auto", trust_remote_code=True).eval()
...
def text_completion(input_text: str, stop_words) -> str:  # 作为一个文本续写模型来使用
    ...
    pdb.set_trace()
    output = model.generate(input_ids, do_sample=False, stop_words_ids=stop_words_ids)
