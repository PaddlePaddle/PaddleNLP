...
class LlamaModel(LlamaPretrainedModel):
    ...
    def set_state_dict(self, state_dict, use_structured_name=True):
        ...
        self.embed_tokens.weight.set_value(paddle.to_tensor(state_dict["embed_tokens.weight"]))
        self.norm.weight.set_value(paddle.to_tensor(state_dict["norm.weight"]))

        for idx in range(self.config.num_hidden_layers):
            unfused_state_dict = {}
            unfused_state_dict["self_attn.q_proj.weight"] = state_dict["layers.{}.self_attn.q_proj.weight".format(idx)]
            unfused_state_dict["self_attn.k_proj.weight"] = state_dict["layers.{}.self_attn.k_proj.weight".format(idx)]
            unfused_state_dict["self_attn.v_proj.weight"] = state_dict["layers.{}.self_attn.v_proj.weight".format(idx)]
            ...
            self.transformer_block.linear_weights[idx].set_value(
                paddle.to_tensor(state_dict["layers.{}.self_attn.o_proj.weight".format(idx)])
            )

            unfused_state_dict["mlp.gate_proj.weight"] = state_dict["layers.{}.mlp.gate_proj.weight".format(idx)]
            unfused_state_dict["mlp.up_proj.weight"] = state_dict["layers.{}.mlp.up_proj.weight".format(idx)]
            ...
            self.transformer_block.ffn2_weights[idx].set_value(
                paddle.to_tensor(state_dict["layers.{}.mlp.down_proj.weight".format(idx)])
            )

            self.transformer_block.ln_scales[idx].set_value(
                paddle.to_tensor(state_dict["layers.{}.input_layernorm.weight".format(idx)])
            )

            self.transformer_block.ffn_ln_scales[idx].set_value(
                paddle.to_tensor(state_dict["layers.{}.post_attention_layernorm.weight".format(idx)])
            )
