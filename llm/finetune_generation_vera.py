...
import paddle
print('paddle.framework.in_dynamic_or_pir_mode()', paddle.framework.in_dynamic_or_pir_mode())
...
from paddlenlp.peft import LoRAConfig, LoRAModel, PrefixConfig, PrefixModelForCausalLM, VeRAModel, VeRAConfig
...
def main():
    ...
    if model_args.vera:
        ...
        if model_args.lora_path is None:
            ...
            print('model_args.rslora', model_args.rslora)
            print('model_args.lora_rank', model_args.lora_rank)
            print('lora_alpha', 2 * model_args.lora_rank if not model_args.rslora else 4)
        ...
        print('vera model', model)
        model.mark_only_vera_as_trainable(notfreezeB=True)
        model.print_trainable_parameters()
        
        print('mark_only_lora_as_trainable后可训练的层')
    ...
    if training_args.do_train:
        ...
        # 训练完输出layers.0.q_proj weight loraa weight lorab veraa verab
