...
class LoReftSupervisedDataset(ReftDataset):
    def preprocess(self, kwargs):
        ...
        self.trigger_tokens = "the correct answer is "
    ...
    def postprocess(self, kwargs):
        ...
        if self.task in ["gsm8k"] and self.original_data_split == "train" and self.test_split == "validation":
            self.task_dataset = self.task_dataset.select(range(original_dataset_size - 300))
        if self.task in ["gsm8k"] and self.original_data_split == "validation":
            self.task_dataset = self.task_dataset.select(range(original_dataset_size - 300, original_dataset_size))
    ...
    def tokenize(self, data_item):
        ...
        if self.original_data_split == "train":
            ...
            if self.task == "ultrafeedback_pair" and self.data_split == "train":
                ...
                input_pad_tensor = paddle.full((input_pad_length,), self.tokenizer.pad_token_id, dtype=paddle.long)
                output_pad_tensor = paddle.full((output_pad_length,), IGNORE_INDEX, dtype=paddle.long)

                base_input_ids = paddle.concat((base_input_ids, input_pad_tensor), axis=0)
