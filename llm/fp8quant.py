...
from paddle.quantization import FP8PTQ, QAT, QuantConfig
...
from paddleslim.quant.observers.fp8uniform import FP8UniformObserverLayer 
...
def apply_ptq(quant_args, trainer, ptq_dataloader):
    ...
    if quant_args.quant_type == "a8w8":
        activation = FP8UniformObserver()
        weight = FP8UniformObserver()
    elif quant_args.quant_type == "weight_only_int4":
        activation = None
    ...
    for cur_name, cur_layer in trainer.model.named_sublayers():
        if isinstance(cur_layer, FP8UniformObserverLayer):
            if "_observer" not in cur_name:
                print(cur_name)
                _min = cur_layer._min
                _max = cur_layer._max
                _abs_max = max(-_min, _max)
                print("_abs_max:",_abs_max)
                print("cur_layer.scales:",cur_layer.scales())
                print("cur_layer._float8_type:",cur_layer._float8_type)
                print("cur_layer._qmax:",cur_layer._qmax)
    '''
    '''
    weight_scales = {}
    act_scales = {}
