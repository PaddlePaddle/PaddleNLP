...
class Config:
    ...
    def read_from_config(self):
        ...
        tag = os.getenv("tag")
        if "paddlenlp_version" in config:
            if tag not in config["paddlenlp_version"]:
                logger.warning(
                    f"Current image paddlenlp version {tag} doesn't match the model paddlenlp version {config['paddlenlp_version']} "
                )
        ...
        if self.use_cache_kv_int8 is None or self.use_cache_kv_int8 == 0:
