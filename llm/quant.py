...
def create_qat_model(quant_args, model, dtype):
    ...
    elif quant_args.quant_algo == "weight_only_int4":
        activation = None
        weight = FakeQuanterChannelWiseAbsMaxObserver(bit_length=4, dtype="float32")
    elif quant_args.quant_algo == "weight_only_int8":
        activation = None
        weight = FakeQuanterChannelWiseAbsMaxObserver(bit_length=8, dtype="float32")
    else:
        raise ValueError("quant_algo should be one of ['a8w8', 'weight_only_int4', 'weight_only_int8']")
...
def apply_smooth(quant_args, trainer, ptq_dataloader, ptq_model_config):
    trainer.model.config.quantization_config.quant_algo = quant_args.quant_algo
