...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        self.reference_free = kwargs.pop("reference_free", None)
        if ref_model:
    ...
    def get_batch_logps(self, batch, logits):
        """DPO logprobs"""
        ...
        if hasattr(self.model.config, "dpo") and self.model.config.dpo:
            ...
            if self.reference_free:
                ...
                with paddle.no_grad():
                    ...
                    reference_chosen_logps, reference_rejected_logps = self.get_batch_logps(batch, ref_logits)
            policy_logits = model(**dpo_inputs)[0]
            policy_chosen_logps, policy_rejected_logps = self.get_batch_logps(batch, policy_logits)
