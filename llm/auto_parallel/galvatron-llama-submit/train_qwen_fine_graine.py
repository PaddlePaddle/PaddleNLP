...
from paddlenlp.transformers import LlamaConfig, LlamaForCausalLMFineGrained, LlamaPretrainingCriterionFineGrained
from paddlenlp.transformers import LlamaForCausalLMFineGrainedFinal, LlamaPretrainingCriterionFineGrainedFinal
MODEL_CLASSES = {
    ...
    "llama_fine_grained": (LlamaConfig, LlamaForCausalLMFineGrained, LlamaPretrainingCriterionFineGrained),
    "llama_fine_grained_final": (LlamaConfig, LlamaForCausalLMFineGrainedFinal, LlamaPretrainingCriterionFineGrainedFinal)
}
...
from paddlenlp.experimental.galvatron.runtime.comm_group import GranularityRuntimeArguments, generate_meshs_coarse_grained, generate_meshs_fine_grained, get_redistributed_flag
...
def main():
    parser = PdArgumentParser((ModelArguments, DataArguments, PreTrainingArguments, RuntimeProfilerArguments, GranularityRuntimeArguments))
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        model_args, data_args, training_args, runtime_profiler_args, granularity_args = parser.parse_json_file(json_file=os.path.abspath(sys.argv[1]))
    else:
        model_args, data_args, training_args, runtime_profiler_args, granularity_args = parser.parse_args_into_dataclasses()
    ...
    training_args.print_config(granularity_args, "GranularityRuntime")
    ...
    if granularity_args.granularity_type == 'coarse_grained':
        num_hidden_layers = model_args.num_hidden_layers
        gpu_nums = dist.get_world_size()
        pp_size = training_args.pipeline_parallel_degree
        ...
        no_recompute_layers = []
        for idx in range(len(recompute_list)):
            if recompute_list[idx] == 0:
                no_recompute_layers.append(idx)
        config.no_recompute_layers = no_recompute_layers
        if pp_size == 1:
            input_label_mesh_list = [mesh_list[0]]
        else:
            input_label_mesh_list = [mesh_list[0], mesh_list[-1]]
        redistributed_flag = get_redistributed_flag(mesh_list)
        
    elif granularity_args.granularity_type == 'fine_grained':
        gpu_nums = dist.get_world_size()
        fine_grained_config_path = granularity_args.fine_grained_config_path
        mesh_list, pp_stage_idx_list, recompute_list, sharding_stage_list = generate_meshs_fine_grained(
            gpu_nums, fine_grained_config_path
        )
        
        max_sharding_stage = -1
        for sharding_stage in sharding_stage_list:
            if sharding_stage > max_sharding_stage:
                max_sharding_stage = sharding_stage
        from paddlenlp.trainer.trainer_utils import ShardingOption
        if max_sharding_stage == 3:
            training_args.sharding = [ShardingOption.FULL_SHARD]
        elif max_sharding_stage == 2:
            training_args.sharding = [ShardingOption.SHARD_GRAD_OP]
        elif max_sharding_stage == 1:
            training_args.sharding = [ShardingOption.SHARD_OP]
        else:
            max_sharding_stage.sharding = [""]
        ...
        for idx in range(len(recompute_list)):
            if recompute_list[idx] == 0:
                ...
                training_args.recompute = True # ​If there is any layer with recomputation, this value must be set to true.​​
        config.no_recompute_layers = no_recompute_layers
    
        redistributed_flag = get_redistributed_flag(mesh_list)
        
        import json
        with open(fine_grained_config_path, 'r') as f:
            fine_grained_config = json.load(f)
            training_args.gradient_accumulation_steps = fine_grained_config['gradient_accumulation_steps']
            training_args.per_device_train_batch_size = fine_grained_config['global_batch_size'] // fine_grained_config['gradient_accumulation_steps'] // mesh_list[0].shape[0]
            training_args.global_batch_size = fine_grained_config['global_batch_size']
            print(f'[linguangming] gradient_accumulation_steps is {training_args.gradient_accumulation_steps}, global_batch_size is {training_args.global_batch_size}, per_device_train_batch_size is {training_args.per_device_train_batch_size}')
            pp_size = fine_grained_config['pp_size']
            training_args.pipeline_parallel_degree = pp_size
            training_args.sharding_parallel_degree = mesh_list[0].shape[0]
            training_args.tensor_parallel_degree = mesh_list[0].shape[1]
            if pp_size == 1:
                input_label_mesh_list = [mesh_list[0]]
            else:
                input_label_mesh_list = [mesh_list[0], mesh_list[-1]]
    else:
        raise ValueError(f"Unsupported granularity type: {granularity_args.granularity_type}")
    ...
    with paddle.LazyGuard():
        model = model_class.from_config(config, dtype="float32", 
                                        meshs=mesh_list, pp_division=pp_stage_idx_list, 
                                        redistributed_flag=redistributed_flag,
                                        sharding_stage_list=sharding_stage_list)
