...
def main():
    ...
    config_class, model_class, criterion_class = MODEL_CLASSES[model_args.model_type] # 此处选择模型
    ...
    pp = training_args.pipeline_parallel_degree
    tp = training_args.tensor_parallel_degree
    dp = training_args.dataset_world_size
        
    pp_division = []
    num_layers = config.num_hidden_layers
    ave_layers = num_layers // pp
    for i in range(pp):
        if i == pp - 1:
            pp_division.append(list(range(i * ave_layers, num_layers)))  # 最后一个分区包含剩余的层
        else:
            pp_division.append(list(range(i * ave_layers, (i + 1) * ave_layers)))
    print("[auto-parallel] pp_division:", pp_division)
