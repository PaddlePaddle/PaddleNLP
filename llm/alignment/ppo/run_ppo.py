...
def create_actor_models(
    ...
    with timers_scope_runtimer("Actor eval model loading time"):
        ...
        if reshard_controller is not None:
            reshard_controller.set_rollout_env("[create actor_eval_model]")
            ...
            print(f"actor_eval_model_config: {actor_eval_model_config}")
            reshard_controller.set_train_env("[after init actor_eval_model]")
        else:
