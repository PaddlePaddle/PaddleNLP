...
if __name__ == "__main__":
    ...
    train_batch_sampler = P.io.DistributedBatchSampler(train_ds, batch_size=args.bsz, shuffle=True)
    dev_batch_sample = P.io.DistributedBatchSampler(dev_ds, batch_size=args.bsz, shuffle=False)
    ...
    lr_scheduler = P.optimizer.lr.LambdaDecay(
        ...
        get_warmup_and_linear_decay(
            max_steps, int(args.warmup_proportion * max_steps)))
    ...
    opt = P.optimizer.AdamW(lr_scheduler,
                            ...
                            grad_clip=P.nn.ClipGradByGlobalNorm(1.0) if args.language == 'ch' else None)
        ...
        with P.amp.auto_cast(enable=args.use_amp):
            ...
            for epoch in range(args.epoch):
                ...
                for step, d in enumerate(train_data_loader, start=1):
                    # input_ids:        P.Tensor(bsz, seq)
                    # token_type_ids:   P.Tensor(bsz, seq)
                    # start_positions:  P.Tensor(bsz)
                    # end_positions:    P.Tensor(bsz)
                    # answerable_label:    P.Tensor(bsz)
