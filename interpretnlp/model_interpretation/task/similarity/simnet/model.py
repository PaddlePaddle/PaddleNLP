...
class SimNet(nn.Layer):
    ...
    def forward_interpret(self, 
        ...
        logits, addiational_info = self.model.forward_interpreter(
            query, title, query_seq_len, title_seq_len,
            noise=noise, i=i, n_samples=n_samples)
...
class LSTMModel(nn.Layer):
    ...
    def forward_interpreter(self, query, title, query_seq_len, title_seq_len,
                            noise=None, i=None, n_samples=None):
        assert query_seq_len is not None and title_seq_len is not None
        # Shape: (batch_size, num_tokens, embedding_dim)
        ...
        q_att = paddle.matmul(fc_out, embedded_query,
                              transpose_y=True).squeeze(axis=[1])   # (bsz, query_len)
        q_att = F.softmax(q_att, axis=-1)
        t_att = paddle.matmul(fc_out, embedded_title,
