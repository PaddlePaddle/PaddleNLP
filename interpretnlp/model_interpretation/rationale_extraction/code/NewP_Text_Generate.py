...
def generate_for_senti(args, evid_dict, ratio):
    ...
    ex_r['context_token'] = [[evid_dict['char_attri'][x][0] for x in char_attri[math.ceil(length * rationale_ratio):]]]
    ex_r['label'] = label
    return r, ex_r
...
def generate_for_similarity(args, evid_dict, ratio):
    ...
    r['context_idx'] = [[int(x) for x in q_char_attri[: math.ceil(q_length * q_rationale_ratio)]], [int(x) for x in t_char_attri[: math.ceil(t_length * t_rationale_ratio)]]]
    r['context_token'] = [[evid_dict['query_char_attri'][x][0] for x in q_char_attri[: math.ceil(q_length * q_rationale_ratio)]], 
    ...
    ex_r['context_idx'] = [[int(x) for x in q_char_attri[math.ceil(q_length * q_rationale_ratio):]], [int(x) for x in t_char_attri[math.ceil(t_length * t_rationale_ratio):]]]
    ex_r['context_token'] = [[evid_dict['query_char_attri'][x][0] for x in q_char_attri[math.ceil(q_length * q_rationale_ratio):]], 
                        [evid_dict['title_char_attri'][x][0] for x in t_char_attri[math.ceil(t_length * t_rationale_ratio):]]]
    ex_r['label'] = label
    return r, ex_r
...
def generate_for_MRC(args, evid_dict, ratio):
    ...
    data_R_dict['context_token'] = [[evid_dict['char_attri'][x][0] for x in char_attri[: math.ceil(length * rationale_ratio)]]]
    ...
    Rdata_noR_dict['context_token'] = [[evid_dict['char_attri'][x][0] for x in char_attri[math.ceil(length * rationale_ratio):]]]
