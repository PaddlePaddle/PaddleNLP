...
| [文本分类](#文本分类)      | `Taskflow("text_classification")`  |     | ✅        | ✅        |            | ✅          | 基于微调和提示学习策略的文本分类推理工具      |
...
### 文本分类
<details><summary>&emsp; 适配微调和提示学习（小样本）的通用文本分类推理工具 </summary><div>

文本分类不同与其他任务，暂时不提供开箱即用功能。文本分类taskflow支持动态图和静态图模型作为模型参数输入。

#### 微调模型

```
>>> from paddlenlp import Taskflow
>>> cls = Taskflow("text_classification", model="finetune", task_path=<local_saved_model>, problem_type="multi_class", id2label={0: "negative", 1: "positive"})
>>> cls('房间依然很整洁，相当不错')
[{'text': '房间依然很整洁，相当不错', 'predictions: [{'label': 'positive', 'score': 0.80}]}]
```
...
```
>>> from paddlenlp import Taskflow
>>> cls = Taskflow("text_classification", model="prompt", task_path=<local_saved_static_model>, is_static_model=True, problem_type="multi_label", id2label={ 0: "体育", 1: "经济", 2: "娱乐"})
>>> cls('这是一条体育娱乐新闻的例子')
[{'text': '这是一条体育娱乐新闻的例子', 'predictions: [{'label': '体育', 'score': 0.80}, {'label': '娱乐', 'score': 0.90}]}]
```

#### 可配置参数说明
* `model`：选择任务使用的模型，默认为`finetune`, 支持`finetune`, `prompt`。
* `task_path`：自定义任务路径，默认为None。
* `is_static_model`：task_path中是否为静态图模型参数，默认为False。
* `problem_type`：文本分类任务类型，默认为`multi_class`, 支持`multi_class`, `multi_label`。
* `multilabel_threshold`：模型对标签预测的概率在0～1之间，返回结果去掉小于这个阈值的结果，默认为0.5。
