...
__global__ void probs_topk_guided_unzip_kernel(
  ...
  const int this_row = blockIdx.x * blockDim.x + threadIdx.x; // 一个线程处理一行topk
  ...
  __nv_bfloat16 *guided_unzipped_probs_1d = reinterpret_cast<__nv_bfloat16 *>(guided_unzipped_probs_1d_out);
  // 使用该行的num_expert规模的行映射信息填充寄存器组，非法值为-1
  #pragma unroll
  for(int i =0; i < num_experts; i++){
    token_expert_rowmap[i] = zipped_expertwise_rowmap[this_row * num_experts + i];
  }
...
void dispatch_probs_topk_guided_unzip(
  ...
  dim3 grid, block;
  block.x = 256;
  grid.x = (total_zipped_tokens_num + block.x - 1) / block.x; // 每一个thread处理一个probs_topk
  if (num_experts == 4 && topk == 8) {
    probs_topk_guided_unzip_kernel<8,4><<<grid, block, 0, probs_topk.stream()>>>(
...
std::vector<paddle::Tensor> probs_topk_guided_unzip(
  ...
  //------------------------ 输出1张量 ------------------------
  auto guided_unzipped_probs_1d =
      paddle::empty({total_unzipped_tokens_num}, probs_topk.dtype(), probs_topk.place());

  dispatch_probs_topk_guided_unzip(
      probs_topk, expert_routemap_topk, zipped_expertwise_rowmap, guided_unzipped_probs_1d, num_experts, rows, cols);
