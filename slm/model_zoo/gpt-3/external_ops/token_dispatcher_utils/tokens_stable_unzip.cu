...
__global__ void tokens_unzip_stable_kernel(
    ...
    // 多阶段算法说明：
    // 1.原子获取上一个block写的、合法的本block cumsum，如果是第一个block则直接开始
    // 2.线程1使用cumsum值处理块内route_topk和probs_topk, 并更新local_cumsum并发送，更新自己的shared数组
    // 3.启动所有thread开始向量化搬运CUMSUM_BLOCK_SIZE个token
    ...
    if(threadIdx.x == 0)[[unlikely]]{
      ...
      for(int i = 0; i < CUMSUM_BLOCK_SIZE; i++){
        #pragma unroll
        for (int j = 0; i < num_experts; j++) {
          local_expert_rowmap[i][j] = -1; // 以非法值初始化，方便后续shared mem写入
          local_expert_probs[i][j] = (probs_T) 0;
        }
      }
      // 将乱序访存限制在寄存器级别，后续shared_mem规整写入
      for(int row = block_row_base; row < block_row_base + CUMSUM_BLOCK_SIZE; row++){
        ...
        for(int k = 0; k < topk; k++){
          ...
          local_expert_rowmap[internal_row][expert] = local_cumsum[expert] + expert_offset[expert];
      ...
      for(int i = 0; i < num_experts; i++){
        ...
        global_expertwise_block_cumsum[(blockIdx.x + 1) * num_experts + i] = proposed_offset;
      ...
      for(int i = 0; i < CUMSUM_BLOCK_SIZE; i++){
        ...
        for(int j = 0; j < num_experts; j++){
          const int proposed_row = (local_expert_rowmap[i][j] == -1) ? -1 : (local_expert_rowmap[i][j] + cumsum_offset[j]);
          shared_expert_rowmap[i][j] = proposed_row;
          shared_expert_probmap[i][j] = local_expert_probs[i][j];
        }
      }
    } // 至此， local_cumsum及其对应的rowmap、probs已经更新完毕，需要依据上一个block提供的信息将cumsum_offset更新并传递
      // 至此，本线程块内的shared_mem已经规整完毕，接下来是向量化的数据搬运
    __syncthreads(); // 其余线程等到了thread0， 具体工作已经与上一个block沟通完毕、存储在 shared中
    // ------------------------- 所有block内线程 -------------------------
    for(int row = block_row_base; row < block_row_base + CUMSUM_BLOCK_SIZE; row++){
      ...
      for(int expert = 0; expert < num_experts; expert++){
        ...
        vectorized_memcpy(&X[row * token_length],
                        ...
                        token_length);
        probs_unzipped[unzipped_row_idx] = shared_expert_probmap[internal_row][expert];
        expert_idx_unzipped[unzipped_row_idx] = expert;
      }
    }
}
// ---------------------------- Dispatch ---------------------------------
void dispatch_tokens_unzip_stable(const paddle::Tensor &X,
                                  ...
                                  const int max_tokens_per_expert) {
  dim3 grid, block;
  grid.x = (total_zipped_tokens_num + CUMSUM_BLOCK_SIZE - 1) / CUMSUM_BLOCK_SIZE;
  block.x = 256;
