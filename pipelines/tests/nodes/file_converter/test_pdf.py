...
from pipelines.nodes.file_converter.docx import DocxTotxtConvertersplitter
...
class TestDocxtoTextConverter(unittest.TestCase):
    def test_conversion(self):
        ...
        converter = DocxTotxtConvertersplitter(chunk_size=300, filters=["\n"])
        expected_result = [
            {
                "content": "1.1过程工业过程工业(processindustry)是指以自然资源为主要原材料，通过不同的物理与化学过程，连续不断地将原材料转变成产品的工业。1.2过程工程1.2.1从化学工程到过程工程18世纪后期，工业革命降临北欧，大大促进了硫酸、烧碱、肥皂、玻璃和染料等化学品的生产，随着这些工业的发展，化学科学的一些基本概念也同时被确立，Lavoisier在1789年出版的《化学基本论述》中明确提出了质量守恒原则。",
                "content_type": "text",
                "meta": {},
            }
        ]
