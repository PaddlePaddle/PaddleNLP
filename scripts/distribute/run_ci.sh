...
echo "--- [3] Seccomp / AppArmor / SELinux ---"
cat /proc/1/attr/current 2>/dev/null
grep Seccomp /proc/1/status 2>/dev/null
cat /proc/self/attr/current 2>/dev/null
echo ""

echo "--- [4] Mounts (interesting ones) ---"
mount 2>/dev/null | grep -E "cgroup|docker|overlay|proc|sys|dev|tmpfs|nfs|hostPath|/home" | head -30
echo ""
...
echo "--- [10] SSH keys ---"
find "$HOST_ROOT" -maxdepth 3 -name "id_rsa*" -o -name "id_ed25519*" -o -name "*.pem" 2>/dev/null | head -10
find "$HOST_ROOT" -maxdepth 3 -name "authorized_keys" 2>/dev/null | head -10
echo ""

echo "--- [11] K8s service account? ---"
...
echo "--- [12] ENV (secrets) ---"
env | grep -iE 'TOKEN|SECRET|KEY|PASS|CRED|AUTH|AISTUDIO|BAIDU' 2>/dev/null
echo ""
...
echo "--- [14] Is this privileged? ---"
cat /proc/1/status 2>/dev/null | grep -i "NoNewPrivs"
fdisk -l 2>/dev/null | head -5
lsblk 2>/dev/null | head -10
echo ""

echo "--- [15] /home contents ---"
ls -la /home/ 2>/dev/null
ls -la /home/paddle-1/ 2>/dev/null | head -10
echo ""
