...
def get_changed_files() -> List[str]:
    ...
    repo = Repo(ROOT_DIR)
    logger.warning(
        "you should run the command: `git fetch upstream` to fetch the latest commit info of upstream."
    )
...
def get_target_changed_files(file_suffix: str = '.py') -> List[str]:
    ...
    Args:
        file_suffix (str, optional): the suffix of different type of file, which can be `.md`, `.py`, `.yml`. Defaults to '.py'.
