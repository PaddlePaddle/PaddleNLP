...
def _dynabert_training(self, ofa_model, model, teacher_model, train_dataloader,
    ...
    def evaluate(model, criterion, data_loader, width_mult=1.0):
        ...
        if width_mult == 100:
            logger.info("teacher model, eval loss: %.5f, acc: %.5f" %
                        (np.mean(losses), acc))
        else:
            logger.info("width_mult: %s, eval loss: %.5f, acc: %.5f" %
                        (str(width_mult), np.mean(losses), acc))
        model.train()
        return acc
    ...
    logger.info("width_mult: %s, Best accuracy: %.4f" %
                (str(width_mult), best_acc))
    return ofa_model
