...
我们在KUAKE-QIC数据集部分数据（训练集数据规模:500）进行实验,筛选稀疏数据数量和筛选有效数据数量均设为100条，使用不同的数据增强方法进行评测：
|  |Accuracy(%)   |
| ---------| ------------ |
|训练集(500)|73.50|
|训练集(500)+有效增强集(100,duplicate) |73.61|
|训练集(500)+有效增强集(100,substitute) |**74.32**|
|训练集(500)+有效增强集(100,insert) |73.81|
|训练集(500)+有效增强集(100,delete) |74.27|
|训练集(500)+有效增强集(100,swap) |73.66|
...
我们在KUAKE-QIC数据集部分数据（训练集数据规模:500）进行实验,筛选稀疏数据数量设为100条，筛选待标注数据数量为50和100条。我们比较了使用稀疏数据方案的策略采样和随机采样的效果，下表结果表明使用稀疏数据方案的策略采样能够有效指导训练数据扩充，在标注更少的数据情况下获得更大提升的效果：

|  |Accuracy(%)   |
| ---------| ------------ |
|训练集(500)|73.50|
|训练集(500)+策略采样集(50) |76.88|
|训练集(500)+随机采样集(50) |74.32|
|训练集(500)+策略采样集(100) |**77.64**|
|训练集(500)+随机采样集(100) |76.37|
...
我们在KUAKE-QIC数据集部分数据（训练集数据规模:500）进行实验，取20%的数据进行脏数据处理，也即20%训练数据为标签错误数据，应用脏数据清洗策略进行评测：
