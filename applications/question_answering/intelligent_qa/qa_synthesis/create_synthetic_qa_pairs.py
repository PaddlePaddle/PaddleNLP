            ...
            for predict_dict, paragraph in zip(predicts, paragraph_list):
                ...
                for prompt in schema:
                    if prompt in predict_dict:
                        ...
                        answers += [
                            answer_dict['text'] for answer_dict in answer_dicts
                        ]
                        probabilitys += [
...
def create_fake_question(json_file_or_pair_list,
                         out_json=None,
    ...
    for index, line_dict in enumerate(tqdm(all_lines)):
        ...
        for j, pair in enumerate(answers):
            ...
            if (i + 1) % batch_size == 0 or (all_sample_num and
                                             (i + 1) == all_sample_num) or (
                                                 (index + 1) == num_all_lines
                                                 and j == len(answers) - 1):
                ...
                for context, answer, answer_probability, true_question in zip(
                    ...
                    answer_probability_buffer_temp += [answer_probability
                                                       ] * num_return_sequences
...
if __name__ == '__main__':
    ...
    answer_generator = Taskflow("information_extraction",
                                schema=schema,
    ...
    synthetic_context_answer_pairs = answer_generation_from_paragraphs(
        ...
        schema=schema,
        wf=open(
            '/root/project/data/faq-qa/yiqing-answer-agnostic-unsupervised-faq-qa/yiqing_data.json.synthetic_answer.json',
            'w',
            encoding='utf-8'))
    ...
    if args.do_filtration:
        filtration_model = Taskflow("information_extraction",
                                    schema=['答案'],
                                    task_path=args.filtration_model_path,
                                    batch_size=args.batch_size,
                                    position_prob=args.filtration_position_prob)
        filtration(synthetic_answer_question_pairs,
                   batch_size=16,
