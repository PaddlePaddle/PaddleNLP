...
def parse_args():
    ...
    parser.add_argument("--do_map_async",
                        action="store_true",
...
def coverage_rate(true_file_path, generate_file_path, bleu_n_size, bleu_threshold, num_return_sequences, all_sample_num=None, is_log_file=False, log_dir=None, is_multiprocessing=True, is_map_async=True):
    true_questions = []
    with open(true_file_path, 'r', encoding='utf-8') as rf:
        for i, json_line in enumerate(tqdm(rf.readlines())):
            ...
            true_questions.append(line_dict['question'][0] if isinstance(line_dict['question'], list) else line_dict['question'])
    ...
    with open(generate_file_path, 'r', encoding='utf-8') as rf:
        ...
        for i, json_line in enumerate(tqdm(rf.readlines())):
            if i >= all_sample_num * num_return_sequences:
                break
            line_dict = json.loads(json_line)
            group.append(line_dict['question'][0] if isinstance(line_dict['question'], list) else line_dict['question'])
    ...
    generate_question_groups = [[generate_question if generate_question.strip()!='' else 'none' for generate_question in generate_question_group] for generate_question_group  in generate_question_groups]
    for true_question, generate_question_group in tzip(true_questions, generate_question_groups):
        if is_multiprocessing:
            if is_map_async:
                ...
                pool_results.append(pool.apply_async(worker_apply_async, args=(true_question, generate_question_group, bleu_n_size, bleu_threshold, i)))
    ...
    if is_log_file and log_dir:
        with open(os.path.join(log_dir, 'positive_pair.txt'), 'w', encoding='utf-8') as wf:
            for pair in positive:
                wf.write(pair[0] + '\t' + pair[1] + '\n' if len(pair)==2 else pair[0] + '\t' + pair[1] + str(pair[2]) + '\n')
        with open(os.path.join(log_dir, 'negative_pair.txt'), 'w', encoding='utf-8') as wf:
            for pair in negative:
                wf.write(pair[0] + '\t' + pair[1] + '\n' if len(pair)==2 else pair[0] + '\t' + pair[1] + str(pair[2]) + '\n')
    assert len(positive) + len(negative) == all_sample_num, 'the number of positive pairs ' + str(len(positive)) + ' plus the number of negative pairs ' + str(len(negative)) + ' should be equal to all_sample_num' + str(all_sample_num)
    return len(positive) / (len(positive) + len(negative))

if __name__ == '__main__':
    args = parse_args()
    rate = coverage_rate(true_file_path=args.true_file_path, 
