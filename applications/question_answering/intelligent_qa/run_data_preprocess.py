...
def convert_from_json_to_answer_extraction_format(json_file,
                                                  output_path,
                                                  domain=None,
                                                  do_answer_prompt=True,
                                                  do_len_prompt=False,
                                                  do_domain_prompt=False):
    with open(json_file,
...
def convert_from_json_to_question_generation_format(json_file,
                                                    output_path,
                                                    tokenizer=None):
    with open(json_file,
              'r', encoding='utf-8') as rf, open(output_path,
...
if __name__ == '__main__':
    ...
    answer_extraction_target_file_path = os.path.join(
        args.target_dir, 'answer_extraction',
        os.path.basename(args.source_file_path))
    ...
    question_generation_target_file_path = os.path.join(
        args.target_dir, 'question_generation',
        os.path.basename(args.source_file_path))
    if not os.path.exists(
            os.path.dirname(question_generation_target_file_path)):
        os.makedirs(os.path.dirname(question_generation_target_file_path))
    convert_from_json_to_question_generation_format(
        json_file=args.source_file_path,
        output_path=question_generation_target_file_path)

    filtration_target_file_path = os.path.join(
        args.target_dir, 'filtration', os.path.basename(args.source_file_path))
    ...
    convert_from_json_to_filtration_format(
        json_file=args.source_file_path,
        output_path=filtration_target_file_path)
