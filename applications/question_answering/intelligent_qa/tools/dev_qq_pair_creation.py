...
def create_test_qq_pair(ori_path=None,
                        trans_path=None,
                        write_path=None,
                        trans_query_answer_path=None,
                        test_sample_num=None):
    assert trans_path
    trans_rf = open(trans_path, 'r', encoding='utf-8')
    wf = open(write_path, 'w', encoding='utf-8')
    if trans_path.endswith('.json'):
        trans_q_list = extract_q_from_json_file(trans_path, None,
                                                test_sample_num,
                                                trans_query_answer_path)
    else:
        trans_q_list = [
            line.strip() for i, line in enumerate(trans_rf.readlines())
            if not test_sample_num or i < test_sample_num
        ]
    ...
    if not ori_path or ori_path in ['NONE', 'None', 'none']:
        ...
        if ori_path.endswith('.json'):
            origin_q_list = extract_q_from_json_file(ori_path, None,
                                                     test_sample_num)
        else:
            origin_q_list = [
                line.strip() for i, line in enumerate(origin_rf.readlines())
                if not test_sample_num or i < test_sample_num
            ]
