...
- [5. 更进一步：结合业务分析经验，定制情感分析](#5.)
  - [5.1 打通数据标注到训练样本构建](#5.1)
    ...
    - [5.1.2 样本构建：评价维度抽取相关任务](#5.1.2)
    - [5.1.3 样本构建升级1：加强维度聚合能力](#5.1.3)
    - [5.1.4 样本构建升级2：加强隐性观点抽取能力](#5.1.4)
  - [5.2 模型训练](#5.2)
  - [5.3 模型测试](#5.3)
  - [5.4 模型预测](#5.4)
    - [5.4.1 使用训练后的模型进行预测](#5.4.1)
    - [5.4.2 维度聚合预测和分析](#5.4.2)
...
除语句级情感分析之外，本项目同时支持评价维度级情感分析，包括评价维度抽取（Aspect Term Extraction）、观点抽取（Opinion Term Extraction）、评价维度级情感分析（Aspect Based Sentiment Classification）等等。可以通过设置相应的schema进行对应信息的抽取，其调用示例如下。
...
更进一步地，在某些业务场景中，特别是一些垂域场景，用户可能比较关注固定的某些评价维度。在这种情况下，可以预先提供相应的评价维度集合，则本项目将只会在该评价维度集上进行情感分析，分析和抽取该集合中各个维度的信息。

针对固定评价维度的情感分析示例如下，需要将评价维度集合传入参数 `aspects` 中，后续将只针对这些维度进行分析。可以看到在示例中，传入了维度 `房间`，`位置` 和 `价格`，针对 `房间` 和 `价格` 均分析到了观点词和情感倾向，但是`位置`由于在样本中并未提及，因此相应观点词为空，情感倾向为 `未提及`。
...
在情感分析处理之后，可以根据情感分析的保存结果进行可视化展示，帮助用户更友好地分析业务特点。默认情况下，可视化功能支持围绕评价维度、观点、评价维度+观点、评价维度+情感、指定评价维度+观点分析功能。在各项分析中，均支持词云和直方图两类图像展示。特别说明的是，当前只支持如下schema分析后的数据，后续将支持自定义不同分析类型的可视化。
...
**(1) 评价维度分析**
通过评价维度信息，可以查看客户对于产品/服务的重点关注方面. 可以通过`plot_aspect_with_frequency`函数对评价维度进行可视化，当前可通过参数`image_type`分别指定`wordcloud`和'histogram'，通过词云和直方图的形式进行可视化。
...
**(3) 评价维度+观点分析**
结合评价维度和观点两者信息，可以更加具体的展现客户对于产品/服务的详细观点，分析某个评价维度的优劣，从而能够帮助商家更有针对性地改善或提高自己的产品/服务质量。可以通过`plot_aspect_with_opinion`函数对评价维度+观点进行可视化，同时可通过设置参数`sentiment`按照情感倾向展示不同分析结果，以更好进行情感分析，若设置为`all`，则会展示正向和负向所有的评价维度；若为`positive`，则会仅展示正向的评价维度；若为`negative`，则会仅展示负向的评价维度。如果在绘制直方图时，通过设置参数`top_n`，可以展示频率最高的top n个评价维度。
 ...
 **(4) 评价维度+情感分析**
挖掘客户对于产品/服务针对评价维度的情感极性，帮助商家直观地查看客户对于产品/服务的某些评价维度的印象。可以通过`plot_aspect_with_sentiment`函数对评价维度+情感进行可视化。如果在绘制直方图时，通过设置参数`top_n`，可以展示频率最高的top n个评价维度。
...
**(5) 对给定评价维度进行观点分析**
通过指定评价维度，更加细致查看客户对于产品/服务某个评价维度的观点。可以帮助商家更加细粒度地分析客户对于产品/服务的某个评价维度的印象。下面图片示例中，展示了客户对于评价维度"房间"的观点。可以通过`plot_opinion_with_aspect`函数，对给定的评价维度进行观点分析。默认情况下，不会自动生成该类图像，需要开发者手动调用`plot_opinion_with_aspect`进行可视化分析。
...
- 评价维度聚合：结合业务经验，支持传入同义的评价维度集合，可以增强模型对于数据聚合的能力。
...
#### **5.1.2 样本构建：评价维度抽取相关任务**

针对抽取式的任务，比如评价维度抽取、观点抽取、评价维度分类任务等，可以使用如下命令将label-studio导出数据转换为模型训练数据：
...
#### **5.1.3 样本构建升级1：加强评价维度聚合能力**

在用户对产品或服务进行评论时，对某一些评价维度可能会有不同的说法，这会在后续对评价维度分析时可能会带来困扰。如以下示例中的"价格","价钱"和"费用"。
...
针对这种情况，针对评价维度相关任务，本项目同时支持用户结合业务经验，通过设置同义的评价维度词表，加强模型的评价维度聚合能力。具体来讲，本项目期望通过以下两点，支持对评价维度聚合能力的建设。

- 支持针对用户给定的评价维度进行情感分析
- 支持用户提供同义的评价维度词表，用以加强模型对用户领域评价维度同义词的理解能力
...
可以通过以下命令，使用synonym_file指定凝聚业务经验的同义评价维度集合，利用同义评价维度生成对应的数据样本：
...
另外，本项目同时支持加强对隐性观点功能抽取的能力，这里需要说明一点，本项目中定义隐性观点是指没有对应评价维度的纯观点词，如以下示例中的"比较便宜"便是隐性观点。

```
蛋糕味道不错，外观很漂亮，而且比较便宜
```

本项目支持用户提供一个隐性观点映射文件，用户可以根据自己的业务场景定义隐性观点词，以下给出了酒店场景的示例。其格式为，第1个单词为隐性观点对应的评价维度，后续按照情感情感倾向对隐性观点词进行了归类，同一类的以"[ ]"方式放到一块。
...
下面就 `隔音` 与 `价格` 两个维度进行分析，抽取样本中与这两个维度相关的情感信息，代码如下：
...
下图展示了关于模型对于评价维度聚合能力支持的样本，在分析之前设定固定的评价维度结合`["隔音", "价格"]`，可以看到尽管语料中同时出现了`隔音`、`隔声`、`价格`、`价钱`和`费用`，但是经过定制后的情感分析模型依然能够准确识别出对于维度 `隔音` 和 `价格`的情感信息，从而起到评价维度聚合的作用。
...
下面就`价格` 和 `卫生` 两个维度进行分析隐性观点，抽取样本中与这两个维度相关的情感信息，代码如下：
...
下图展示了关于模型对于隐性观点抽取的样本，可以看到，虽然以下这些样本中，并未出现`价格` 和 `卫生`，但模型依然正确识别除了这两个维度的情感信息。
