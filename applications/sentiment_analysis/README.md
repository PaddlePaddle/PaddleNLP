...
可以看到，顾客在口感、包装和味道 三个维度上对薯片进行了评价，顾客在味道和口感两个方面给出了好评，但是在包装上给出了负面的评价。由此看来，针对一个产品或服务，用户可从多个维度进行评论，每个维度都会有相应的观点。

基于这样的考虑，本项目提出了一套评论观点抽取的情感分析能力，即对于给定文本，首先会抽取该文本中的评论维度和观点，然后分析不同维度的情感极性。
...
以上提到的解决方案如图1所示，整个情感分析的过程大致包含两个阶段，依次是评论维度和观点抽取模型，细粒度情感分类模型。对于给定的一段文本，首先基于前者抽取出文本语句中潜在的评论维度以及该维度相应的评论观点，然后将评论维度、观点以及原始文本进行拼接，传给细粒度情感分类模型以识别出该评论维度的情感色彩。
