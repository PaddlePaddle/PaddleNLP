...
class Predictor(object):
    def __init__(self, args):
        ...
        self.ext_label2id, self.ext_id2label = self._load_dict(args.ext_label_path)
        self.cls_label2id, self.cls_id2label = self._load_dict(args.cls_label_path)
    ...
    def predict(self, text):
        ...
        ext_input_ids, ext_token_type_ids = self.convert_example_to_feature(text, max_seq_len=args.max_seq_len)
        ...
        for ap in aps:
            ...
            aspect_text = self._concate_aspect_and_opinion(text, aspect, opinion_words)
            cls_input_ids, cls_token_type_ids = self.convert_example_to_feature(text=aspect_text, text_pair=text, max_seq_len=args.max_seq_len)
            ...
            result = {"aspect": aspect, "opinions": opinion_words, "sentiment": self.cls_id2label[pred_id]}
    ...
    def convert_example_to_feature(self, text, text_pair=None, max_seq_len=256):
        if text_pair is None:
            encoded_inputs = self.tokenizer(list(text), is_split_into_words=True, max_seq_len=max_seq_len)
        else:
            encoded_inputs = self.tokenizer(text, text_pair=text_pair, max_seq_len=max_seq_len)
