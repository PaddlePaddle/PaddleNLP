...
def parse_args():
    ...
    parser.add_argument('--trans_query_answer_path',
                        type=str,
                        default=None,
                        help='the target query-answer file path for extract_trans_from_fake_question')  
    parser.add_argument('--dev_sample_num',
                        type=int,
                        default=None,
                        help='the test sample number when convert_json_to_data, if None, treat all lines as dev samples')
    ...
    with open(json_file, 'r', encoding='utf-8') as rf:
        ...
        for i, json_line in enumerate(rf.readlines()):
            ...
            if not test_sample_num or i < test_sample_num:
                if query_answer_path:
                    qeury_answer_wf.write(question.replace('\n', ' ').replace('\t', ' ').strip() + '\t' + answer.replace('\n', ' ').replace('\t', ' ').strip() + '\n')
                if out_file:
...
def create_test_qq_pair(ori_path=None, trans_path=None, write_path=None, trans_query_answer_path=None, test_sample_num=None):
    assert trans_path
    ...
    if trans_path.endswith('.json'):
        ...
        trans_q_list = [line.strip()  for i, line in enumerate(trans_rf.readlines()) if not test_sample_num or i<test_sample_num]
    ...
    if not ori_path or ori_path in ['NONE', 'None', 'none']:
        ...
        if ori_path.endswith('.json'):
            ...
            origin_q_list = [line.strip()  for i, line in enumerate(origin_rf.readlines()) if not test_sample_num or i<test_sample_num]
...
if __name__ == '__main__':
    ...
    if args.do_create_test_qq_pair:
        create_test_qq_pair(ori_path=args.qq_pair_source_ori_file_path, 
