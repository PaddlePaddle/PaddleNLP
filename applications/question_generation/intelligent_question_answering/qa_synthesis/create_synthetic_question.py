...
def create_fake_question(json_file, out_json, num_return_sequences, all_sample_num=None,  batch_size=8):
    with open(json_file, 'r', encoding='utf-8') as rf, open(out_json, 'w', encoding='utf-8') as wf:
        ...
        for i, json_line in enumerate(tqdm(all_lines)):
            ...
            if (i+1) % batch_size == 0 or (all_sample_num and (i+1) == all_sample_num or (i+1)) or (i+1) == num_all_lines:  
                ...
                for context, answer, true_question, result in zip(context_buffer_temp, answer_buffer_temp, true_question_buffer_temp, result_one_two_buffer):
                    ...
                    for fake_quesitons_token, fake_quesitons_score in zip(fake_quesitons_tokens, fake_quesitons_scores):
                        out_dict = {'context':context, 'answer':answer, 'question':fake_quesitons_token, 'true_question':true_question, 'score':fake_quesitons_score}
...
if __name__ == '__main__':
    ...
    create_fake_question(args.source_file_path, args.target_file_path, args.num_return_sequences, args.all_sample_num, args.batch_size)