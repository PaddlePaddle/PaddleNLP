...
def convert_example(example,
    ...
    if mode == 'train' or mode == 'pretrain':
        ...
        assert count == 7 or count == 2, str(count) + ' is not in [2, 7], temp_tokens: ' + ' '.join(temp_tokens) + 'source: ' + source
        index = -1
        for i in range(0, count):
            index = tokenized_example['input_ids'].index(tokenizer.cls_token_id, index + 1)
            index_list.append(index)
        if template == 4:
            tokenized_example['token_type_ids'] = [2] * (index_list[1]-index_list[0]) + [3] * (index_list[4]-index_list[1]) + [0] * (index_list[6]-index_list[4]) + [1] * (len(tokenized_example['input_ids'])-index_list[6])
