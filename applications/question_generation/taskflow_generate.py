...
def parse_args():
    ...
    parser.add_argument("--adversarial_training", type=str, default=None, help="The adversarial training strategy during training, select from ['None', 'fgm', 'pgd'].")
    parser.add_argument("--template", type=int, default=1, help="The template used during training, select from [0, 1, 2, 3].")
...
def evaluation(model, data_loader, args, tokenizer):
    ...
    # result1 = question_generator([{"context": "年基准利率4.35%。 从实际看,贷款的基本条件是: 一是中国大陆居民,年龄在60岁以下; 二是有稳定的住址和工作或经营地点; 三是有稳定的收入来源; 四是无不良信用记录,贷款用途不能作为炒股,赌博等行为; 五是具有完全民事行为能力。", "answer": "年基准利率4.35%", }])
    for step, inputs in enumerate(data_loader, 1):
        ...
        task_ids, task_scores = question_generator.task_instance.out_run_model(
