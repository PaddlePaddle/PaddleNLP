...
def parse_args():
    ...
    parser.add_argument('--save_dir', type=str, default='./unimo/pretrain/checkpoints', help='The directory where the checkpoints will be saved.')
    ...
    parser.add_argument('--max_dec_len', type=int, default=80, help='The maximum sequence length of decoding.')
    parser.add_argument('--min_dec_len', type=int, default=3, help='The minimal sequence length of decoding.')
    parser.add_argument('--max_target_len', type=int, default=80, help='The maximum target sequence length of training.')
    parser.add_argument('--max_title_len', type=int, default=80, help='The maximum title sequence length of training.')
    ...
    parser.add_argument("--do_pretrain", action='store_true', help="Whether to pretrain the model.")
    parser.add_argument("--do_predict", action='store_true', help="Whether to eval and predict.")
    parser.add_argument("--adversarial_training", type=str, default=None, help="The adversarial training strategy during training, select from ['None', 'fgm'].")
    parser.add_argument("--template", type=int, default=0, help="The template used during training, select from [0, 1, 2, 3].")
    ...
    if args.do_pretrain:
        ...
        optimizer = AdamW(learning_rate=lr_scheduler,
                          ...
                          grad_clip=paddle.nn.ClipGradByGlobalNorm(
                              args.max_grad_norm))
        if args.adversarial_training == 'fgm':
            fgm = FGM(model)
        elif args.adversarial_training == 'pgd':
            pgd = PGD(model)
        step = 0
        total_time = 0.0
        best_bleu4 = 0
        for epoch in range(args.epochs):
            ...
            for inputs in train_data_loader:
                ...
                if args.adversarial_training == 'fgm':
                    ...
                    loss_adv = F.cross_entropy(logits_adv,
                                               labels,
                                               soft_label=True)
                    ...
                    for t in range(pgd.K):
                        pgd.attack(is_first_attack=(
                            t == 0))  # 在embedding上添加对抗扰动，并在第一次attck时备份参数
                        # 前K-1次的梯度清零，最后一次恢复最开始的梯度；
                        if t != pgd.K - 1:
                        ...
                        loss_adv = F.cross_entropy(logits_adv,
                                                   ...
                                                   soft_label=True)
                        loss_adv.backward(
                        )  # 前K-1次反向传播的目的是为下一次扰动的计算提供新的梯度，最后一次反向传播的作用是在原梯度基础上累加对抗训练的梯度
                    pgd.restore_emb()  # 恢复embedding参数
                optimizer.step()
                lr_scheduler.step()
                optimizer.clear_grad()
