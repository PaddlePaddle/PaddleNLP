...
class UTCPredictor(object):
    def __init__(self, args):
        self.args = args
        self.tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path)
        self.template, self.input_handles = self.post_init()
    ...
    def post_init(self):
        template = UTCTemplate(self.tokenizer, self.args.max_length)
        inputs = ["input_ids", "token_type_ids", "position_ids", "attention_mask", "omask_positions", "cls_positions"]
        return template, inputs

    def predict(self, input_data: list):
        encoded_inputs = self.preprocess(input_data)
        infer_result = self.infer_batch(encoded_inputs)
        result = self.postprocess(input_data, infer_result)
    ...
    def preprocess(self, input_data):
        inputs = [self.template(x) for x in input_data]
        return inputs
    ...
    def postprocess(self, input_data, infer_data):
        threshold = 0.5
        ...
        for idx, label_id in label_ids:
            choices = input_data[idx]["choices"]
            labels[idx].append([choices[label_id], probs[idx][label_id].tolist()])
        return {"label": labels}
    ...
    def printer(self, result, input_data):
        ...
        for i in range(len(label)):
            ...
            logger.info("labels: {}".format(json.dumps(label[i], ensure_ascii=False)))
            logger.info("-----------------------------")
