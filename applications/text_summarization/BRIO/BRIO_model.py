...
from pegasus import PegasusScorer
...
# class label_smoothing_loss(paddle.nn.Layer):
...
#         input = F.log_softmax(input, axis=2)
#         k = input.shape[2]
#         target_prob = paddle.ones_like(input, dtype=input.type) * self.epsilon * 1 / k
#         mask =torch.arange(k).unsqueeze(0).unsqueeze(0).


class BRIO(PretrainedModel):
    ...
    def forward(
        ...
        output = output.reshape(
            [batch_size, -1, output.shape[1], output.shape[2]]
        ...
        if normalize:
            ...
            scores = paddle.take_along_axis(_output, candidate_id, 3).squeeze(-1)  # [bz, cand_num, seq_len]
            # scores = torch.gather(input=_output, dim=3, index=candidate_id).squeeze(-1)
        else:
    ...
    #     num_beam_groups: Optional[int] = 0,
