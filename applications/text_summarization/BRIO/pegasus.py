...
class PegasusEncoder(PegasusPretrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs):
        ...
        encoder_input = inputs_embeds + inputs_embed_pos
...
class PegasusDecoder(PegasusPretrainedModel):
    ...
    def forward(
        ...
        decoder_output = self.decoder(
            tgt=decoder_input,  # (bs*cand)*output_len*hidsize
            memory=encoder_output,  # (bs*cand)*input_len*hidsize
            tgt_mask=decoder_attention_mask,  # (bs*cand)*output_len
            memory_mask=memory_mask,  # (bs*cand)*input_len
            cache=cache,
        )
...
#             encoder_hidden_states = paddle.repeat_interleave(encoder_hidden_states, cand_num, dim=0)
#             attention_mask = paddle.repeat_interleave(attention_mask, cand_num, dim=0)
#             decoder_input_ids = decoder_input_ids.reshape([-1, decoder_input_ids.size(-1)])
#             if decoder_attention_mask is not None:
#                 decoder_attention_mask = decoder_attention_mask.reshape([-1, decoder_attention_mask.size(-1)])
#         else:
#             encoder_hidden_states = encoder_outputs[0]
#
#         decoder_output, new_cache = self.decoder(
#             decoder_input_ids, decoder_attention_mask, encoder_hidden_states, attention_mask, cache
#         )
#         return decoder_output, new_cache
