...
def parse_args():
    ...
    parser.add_argument("--mix_ratio", default=0, type=float, help="mixture ratio for synthetic input")
...
def do_train(args):
    ...
    train_set = load_dataset("json", data_files=args.train_file, split="train", streaming=True)
    dev_set = load_dataset("json", data_files=args.eval_file, split="train", streaming=True)
    ...
    with main_process_first(desc="train dataset map pre-processing"):
        train_set = train_set.map(trans_func, batched=True, remove_columns=remove_columns)
    with main_process_first(desc="dev dataset map pre-processing"):
        dev_set = dev_set.map(trans_func, batched=True, remove_columns=remove_columns)

    train_set = IterDataset(train_set)
    dev_set = IterDataset(dev_set)
    ...
    train_data_loader = DataLoader(
        dataset=train_set, batch_size=args.train_batch_size, num_workers=0, collate_fn=batchify_fn, return_list=True
    )

    dev_data_loader = DataLoader(
        dataset=dev_set, batch_size=args.eval_batch_size, num_workers=0, collate_fn=batchify_fn, return_list=True
    )

    if paddle.distributed.get_world_size() > 1:
        model = paddle.DataParallel(model)

    len_train_data_loader = 2081211
    if args.max_steps > 0:
        num_training_steps = args.max_steps
        num_train_epochs = math.ceil(num_training_steps / len_train_data_loader)
    else:
        num_training_steps = len_train_data_loader * args.epoch
