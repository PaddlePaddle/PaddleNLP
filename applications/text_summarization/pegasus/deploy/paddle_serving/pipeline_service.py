...
def convert_example(example, tokenizer, max_seq_len=512):
    ...
    input_dict = {}
    input_dict["input_ids"] = np.asarray(tokenized_example['input_ids'],
                                         dtype='int32')
    return input_dict
...
class PegasusOp(Op):
    ...
    def init_op(self):
        self.tokenizer = PegasusChineseTokenizer.from_pretrained(
    ...
    def preprocess(self, input_dicts, data_id, log_id):
        ...
        input_dict = convert_example(list(data),
                                     self.tokenizer,
                                     max_seq_len=128)
    ...
    def postprocess(self, input_dicts, fetch_dict, data_id, log_id):
        ...
        for sample in outputs:
            ...
            for idx, beam in enumerate(sample):
                if idx >= len(sample) // 2:
                    break
                res = self.tokenizer.decode(beam,
