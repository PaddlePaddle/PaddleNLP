...
class ErnieReader(BaseReader):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # load_model
        self.model = ppnlp.transformers.ErnieGramForQuestionAnswering.from_pretrained(model_name_or_path)
        self.model.eval()
        tokenizer = ppnlp.transformers.ErnieGramTokenizer.from_pretrained(model_name_or_path)
    ...
    def _get_predictions_and_aggregate(self, dataset, tensor_names: List, baskets: List[SampleBasket]):
        ...
        for i, batch in enumerate(data_loader):
            ...
            (input_ids, padding_mask, segment_ids, passage_start_t, start_of_word, labels, id, seq_2_start_t, span_mask) = batch
    ...
    def logits_to_preds(
        ...
        **kwargs,
        ...
        pos_mask_tensor = paddle.tensor.triu((paddle.ones((max_seq_len, max_seq_len), dtype=paddle.get_default_dtype()) * -888), diagonal=1)
        ...
        start_end_matrix = paddle.to_tensor(start_end_matrix, place=paddle.CUDAPlace(0))
