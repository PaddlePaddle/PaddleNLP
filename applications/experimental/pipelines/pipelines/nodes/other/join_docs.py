...
class JoinDocuments(BaseComponent):
    ...
    * reciprocal_rank_fusion: combines the documents based on their rank in multiple nodes.
    ...
    def __init__(self,
        ...
        self.set_config(
            join_mode=join_mode, weights=weights, top_k_join=top_k_join)
    ...
    def run(self, inputs: List[dict],
        ...
        sorted_docs = sorted(
            scores_map.items(), key=lambda d: d[1], reverse=True)
    ...
    def _calculate_comb_sum(self, results):
        ...
        weights = self.weights if self.weights else [1 / len(results)] * len(
            results)
