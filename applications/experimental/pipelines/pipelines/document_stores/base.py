...
class BaseKnowledgeGraph(BaseComponent):
    ...
    def run(self,
        ...
        result = self.query(
            sparql_query=sparql_query, index=index, headers=headers)
...
class BaseDocumentStore(BaseComponent):
    ...
    def add_eval_data(
        ...
        if file_path.suffix == ".json":
            ...
            for docs, labels in eval_data_from_jsonl(
                ...
                if labels:
                    self.write_labels(
                        labels, index=label_index, headers=headers)
    ...
    def run(self,
        ...
        doc_objects = [
            Document.from_dict(
                d, field_map=field_map, id_hash_keys=id_hash_keys)
            for d in documents
        ]
        self.write_documents(
            documents=doc_objects, index=index, headers=headers)
        return {}, "output_1"
