...
parser.add_argument("--text_tag_fle", type=str, required=True, help="The full path of similar text pair file")
...
if __name__ == "__main__":
    ...
    text_list = gen_id2corpus(args.text_tag_fle)
    text_list = [{idx: text} for idx, text in text_list.items()]
    ...
    with open(recall_result_file, 'w', encoding='utf-8') as f:
        for batch_index, batch_query_embedding in enumerate(query_embedding):
            recalled_idx, cosine_sims = final_index.knn_query(
                batch_query_embedding.numpy(), top_k)
            ...
            for row_index in range(batch_size):
                ...
                for idx, doc_idx in enumerate(recalled_idx[row_index]):
                    f.write("{}\t{}\n".format(id2corpus[doc_idx]['label'], 1.0 -
                                              cosine_sims[row_index][idx]))
