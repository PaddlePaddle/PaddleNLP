...
void SequencePreTokenizer::AppendPreTokenizer(PreTokenizer* pretokenizer) {
  ...
  } else if (typeid(*pretokenizer) == typeid(WhitespaceSplitPreTokenizer)) {
    auto cast_pretokenizer =
        dynamic_cast<WhitespaceSplitPreTokenizer*>(pretokenizer);
    pretokenizer_ptr =
        std::make_shared<WhitespaceSplitPreTokenizer>(*cast_pretokenizer);
...
void to_json(nlohmann::json& j,
  ...
  for (auto& ptr : sequence_pretokenizer.pretokenzer_ptrs_) {
    ...
    } else if (typeid(*ptr) == typeid(WhitespaceSplitPreTokenizer)) {
      jitem = *dynamic_cast<WhitespaceSplitPreTokenizer*>(ptr.get());
...
#define TRY_APPEND_PRETOKENIZER(PRETOKENIZER_TYPE)           \
  ...
  for (auto& pretokenizer_json : j.at("pretokenizers")) {
    ...
    TRY_APPEND_PRETOKENIZER(WhitespaceSplitPreTokenizer);
