...
from paddlenlp.transformers import BartModel, BartForPretraining, BartPretrainingCriterion
...
MODEL_CLASSES = {"bart": (BartForPretraining, BartTokenizer)}
...
def do_train(args):
    ...
    default_global_tokens_num = default_global_batch_size * (
        args.encoder_max_seq_len + args.decoder_max_seq_len)
    ...
    if args.model_name_or_path in pretrained_models_list:
        ...
        model = BartForPretraining(BartModel(**model_config))
    else:
        model = BartForPretraining.from_pretrained(args.model_name_or_path)

    # Create the critrion for the gpt model
    criterion = BartPretrainingCriterion()
    ...
    while True:
        ...
        for f_id in range(num_files):
            ...
            train_data_loader, valid_data_loader, test_data_loader = create_pretrained_dataset(
                ...
                decoder_max_seq_len=args.decoder_max_seq_len)
            ...
            for step, batch in enumerate(train_data_loader()):
                ...
                with paddle.amp.auto_cast(
                    ...
                    encoder_prediction_scores, decoder_prediction_scores = model(
                        input_ids=batch['encoder'][0],
                        decoder_input_ids=batch['decoder'][0],
                        masked_positions=batch['encoder'][-1])
                    loss = criterion(encoder_prediction_scores,
                                     batch['encoder'][2],
                                     decoder_prediction_scores,
                                     batch['decoder'][-1], batch['decoder'][1])
