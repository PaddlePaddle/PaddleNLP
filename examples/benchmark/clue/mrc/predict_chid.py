...
def process_test_data(input_dir, tokenizer, max_seq_length, max_num_choices):
    ...
    test_example_file = os.path.join(
        input_dir, 'test_examples_{}.pkl'.format(str(max_seq_length)))
    test_feature_file = os.path.join(
        input_dir, 'test_features_{}.pkl'.format(str(max_seq_length)))

    test_features = generate_input(
        predict_file,
        None,
        test_example_file,
        test_feature_file,
    ...
    all_input_ids = paddle.to_tensor(
        [f.input_ids for f in test_features], dtype="int64")
    all_input_masks = paddle.to_tensor(
        [f.input_masks for f in test_features], dtype="int64")
    all_segment_ids = paddle.to_tensor(
        [f.segment_ids for f in test_features], dtype="int64")
    all_choice_masks = paddle.to_tensor(
        [f.choice_masks for f in test_features], dtype="int64")
...
if __name__ == "__main__":
    ...
    test_data, all_example_ids, all_tags, test_features = process_test_data(
