...
from .utils import DIFFUSERS_DYNAMIC_MODULE_NAME, PPNLP_MODULES_CACHE, logging
...
def init_hf_modules():
    ...
    """
    # This function has already been executed if PPNLP_MODULES_CACHE already is in the Python path.
    if PPNLP_MODULES_CACHE in sys.path:
        return

    sys.path.append(PPNLP_MODULES_CACHE)
    os.makedirs(PPNLP_MODULES_CACHE, exist_ok=True)
    init_path = Path(PPNLP_MODULES_CACHE) / "__init__.py"
...
def create_dynamic_module(name: Union[str, os.PathLike]):
    ...
    """
    init_hf_modules()
    dynamic_module_path = Path(PPNLP_MODULES_CACHE) / name
...
def get_relative_imports(module_file):
    ...
    relative_imports = re.findall("^\s*import\s+\.(\S+)\s*$",
                                  content,
                                  flags=re.MULTILINE)
...
def find_pipeline_class(loaded_module):
    ...
    for cls_name, cls in cls_members.items():
        if cls_name != DiffusionPipeline.__name__ and issubclass(
...
def get_cached_module_file(
    ...
    submodule_path = Path(PPNLP_MODULES_CACHE) / full_submodule
