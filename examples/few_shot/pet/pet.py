...
def do_train(args):
    ...
    trans_func = partial(convert_example_fn, tokenizer=tokenizer, max_seq_length=args.max_seq_length)

    trans_test_func = partial(
        convert_example_fn, tokenizer=tokenizer, max_seq_length=args.max_seq_length, is_test=True
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(train_data_loader, start=1):
            ...
            if args.rdrop_coef > 0:
                ...
                ce_loss = (
                    ...
                    + mlm_loss_fn(prediction_scores_2, masked_lm_labels)
                ) * 0.5
        ...
        test_accuracy, total_num = evaluate_fn(model, tokenizer, public_test_data_loader, label_norm_dict)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--seed", type=int, default=1000, help="random seeds for initialization")
    parser.add_argument("--output_dir", default="./output", type=str, help="The output directory where to save output")
    parser.add_argument(
    ...
    parser.add_argument("--index", required=True, type=str, default="0", help="must be in [0, 1, 2, 3, 4, all]")
    parser.add_argument(
        "--language_model",
        type=str,
