...
def parse_args():
    ...
    parser.add_argument("--negative_num", default=1, type=int, help="Random negative sample number for efl strategy")
    ...
    parser.add_argument("--save_steps", type=int, default=100000, help="Inteval steps to save checkpoint")
    parser.add_argument(
        "--rdrop_coef",
        default=0.0,
        type=float,
...
def do_train():
    ...
    train_ds, public_test_ds, test_ds = load_dataset(
        "fewclue", name=args.task_name, splits=("train_0", "test_public", "test")
    )
    ...
    train_ds = processor.get_train_datasets(train_ds, TASK_LABELS_DESC[args.task_name])

    public_test_ds = processor.get_dev_datasets(public_test_ds, TASK_LABELS_DESC[args.task_name])
    ...
    public_test_data_loader = create_dataloader(
        public_test_ds, mode="eval", batch_size=args.batch_size, batchify_fn=batchify_fn, trans_fn=trans_func
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(train_data_loader, start=1):
            ...
            prediction_scores = model(input_ids=src_ids, token_type_ids=token_type_ids)
        ...
        test_public_accuracy, total_num = do_evaluate(
            model, tokenizer, public_test_data_loader, task_label_description=TASK_LABELS_DESC[args.task_name]
        )

        print("epoch:{}, dev_accuracy:{:.3f}, total_num:{}".format(epoch, test_public_accuracy, total_num))
        ...
        output_file = os.path.join(args.output_dir, str(epoch) + predict_file[args.task_name])
