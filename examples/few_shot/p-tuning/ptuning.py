...
def do_train():
    ...
    train_ds, dev_ds, public_test_ds = load_dataset(
        "fewclue", name=args.task_name, splits=("train_0", "dev_0", "test_public")
    )

    # Task related transform operations, eg: numbert label -> text_label, english -> chinese
    transform_fn = partial(transform_fn_dict[args.task_name], label_normalize_dict=label_norm_dict)
    ...
    dev_data_loader = create_dataloader(
        dev_ds, mode="eval", batch_size=args.batch_size, batchify_fn=batchify_fn, trans_fn=trans_func
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(train_data_loader, start=1):
            ...
            prediction_scores = model(
                input_ids=src_ids, token_type_ids=token_type_ids, masked_positions=masked_positions
            )
        ...
        dev_accuracy, total_num = evaluate_fn(model, tokenizer, dev_data_loader, label_norm_dict)
        print("epoch:{}, dev_accuracy:{:.3f}, total_num:{}".format(epoch, dev_accuracy, total_num))
