...
class LukeForEntityTyping(LukePretrainedModel):
    def __init__(self, luke, num_labels):
        ...
        self.dropout = nn.Dropout(self.luke.config['attention_probs_dropout_prob'])
...
if __name__ == '__main__':
    ...
    model = LukeForEntityTyping.from_pretrained(args.model_type, num_labels=num_labels)
    ...
    model.luke.embeddings.word_embeddings = nn.Embedding(model.luke.config['vocab_size'] + 1, 
                                                         model.luke.config['hidden_size'])
    model.luke.embeddings.word_embeddings.weight.set_value(paddle.concat([word_emb, marker_emb]))
    trainer = Trainer(
