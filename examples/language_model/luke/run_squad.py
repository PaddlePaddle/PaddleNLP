...
parser.add_argument("--n_best_size",
                    type=int,
                    default=20,
                    help="n-best logits from a list")
parser.add_argument("--wiki_link_db_file",
                    type=str,
                    default="enwiki_20160305.pkl",
                    help="Wikipedia entity dataset")
parser.add_argument("--model_redirects_file",
                    type=str,
                    default="enwiki_20181220_redirects.pkl",
                    help="Wikipedia entity dataset")
parser.add_argument("--link_redirects_file",
                    type=str,
                    default="enwiki_20160305_redirects.pkl",
                    help="Wikipedia entity dataset")
parser.add_argument("--with_negative",
                    action='store_true',
                    help="Whether to evaluate the model.")
...
parser.add_argument("--min_mention_link_prob", type=float, default=0.01, help="Min mention link prob")              
...
def evaluate(args, model):
    ...
    all_predictions = write_predictions(args, examples, features, all_results,
                                        ...
                                        False, LukeTokenizer.from_pretrained(args.model_type))
    SQuad_postprocess(
...
if __name__ == '__main__':
    ...
    if args.do_train:
        train_dataloader, _, _, _ = load_examples(args, data_file='train-v1.json')
        num_train_steps = len(
            train_dataloader
