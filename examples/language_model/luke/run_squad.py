...
class DataGenerator(Dataset):
    def __init__(self, features, args):
        ...
        self.all_word_attention_mask = [
            f['word_attention_mask'] for f in features
        ]
        self.all_entity_ids = [f['entity_ids'] for f in features]
        self.all_entity_position_ids = [
            f['entity_position_ids'] for f in features
        ]
        self.all_entity_segment_ids = [
            f['entity_segment_ids'] for f in features
        ]
        ...
        self.all_example_index = [f['example_indices']
                                  for f in features] if args.evaluate else None
...
def load_examples(args, evaluate=False):
    ...
    if evaluate:
        ...
        dataloader = DataLoader(
            data_generator, batch_size=args.train_batch_size, shuffle=True)
...
def evaluate(args, model):
    ...
    all_predictions = write_predictions(args, examples, features, all_results,
                                        20, 30, False)
    SQuad_postprocess(
