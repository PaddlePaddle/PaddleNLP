...
class SquadExample:
    ...
    def __init__(self,
        ...
        for answer in answers:
            self.start_positions.append(char_to_word_offset[answer[
                "answer_start"]])
            self.end_positions.append(char_to_word_offset[min(
                answer["answer_start"] + len(answer["text"]) - 1,
...
class SquadProcessor:
    ...
    def get_dev_examples(self, data_dir, filename=None):
        """get dev examples"""
        with open(
                os.path.join(data_dir, self.dev_file
                             if filename is None else filename)) as reader:
            input_data = json.load(reader)["data"]
        return self._create_examples(input_data)
    ...
    def _create_examples(self, input_data):
        ...
        return [
            ...
            for entry in input_data for para in entry["paragraphs"]
            for qa in para["qas"]
        ]
