...
def load_examples(args, evaluate=False):
    ...
    for feature in features:
        ...
        data['word_segment_ids'] = feature.token_type_ids
    ...
    def collate_fn(o):
        ...
        def create_padded_sequence(o, attr_name, padding_value, max_len):
            ...
            if attr_name == 'entity_position_ids':
                ...
                res = value + [[padding_value] * len(value[0])] * (max_len -
                                                                   len(value))
                return res
            if len(value) > max_len:
                return value[:max_len]
        ...
        ret = dict(
            word_ids=create_padded_sequence(o, "word_ids",
                                            args.tokenizer.pad_token_id,
                                            args.max_seq_length),
            word_attention_mask=create_padded_sequence(o, "word_attention_mask",
                                                       0, args.max_seq_length),
            word_segment_ids=create_padded_sequence(o, "word_segment_ids", 0,
                                                    args.max_seq_length),
            entity_ids=create_padded_sequence(o, "entity_ids", 0,
                                              args.max_entity_length),
            entity_attention_mask=create_padded_sequence(
                o, "entity_attention_mask", 0, args.max_entity_length),
            entity_position_ids=create_padded_sequence(
                o, "entity_position_ids", -1, args.max_entity_length),
            entity_segment_ids=create_padded_sequence(
                o, "entity_segment_ids", 0, args.max_entity_length), )
        if args.no_entity:
...
def save_train(args):
    ...
    with open(
            os.path.join(args.output_data_dir, 'train.json'), 'w',
            encoding='utf-8') as f:
...
def save_eval(args):
    ...
    with open(
            os.path.join(args.output_data_dir, 'eval_data.json'),
            'w',
            encoding='utf-8') as f:
