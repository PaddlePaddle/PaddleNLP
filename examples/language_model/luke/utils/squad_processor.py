...
def collate_fn(batch):
    def create_padded_sequence(k, padding_value):
        ...
        for each_batch in batch:
            # pad to max length
            new_data.append(each_batch[k] + [padding_value] * (max_len - len(
                each_batch[k])))
...
def load_examples(args, data_file='train-v1.json'):
    ...
    wiki_link_db = WikiLinkDB(
        os.path.join(args.data_dir, args.wiki_link_db_file))
    model_redirect_mappings = joblib.load(
        os.path.join(args.data_dir, args.model_redirects_file))
    link_redirect_mappings = joblib.load(
        os.path.join(args.data_dir, args.link_redirects_file))
...
class DataGenerator(Dataset):
    ...
    def __getitem__(self, item):
        ...
        start_positions = [
            self.features[item].start_positions[0]
            if self.is_train else self.features[item].start_positions
        ]
        end_positions = [
            self.features[item].end_positions[0]
            if self.is_train else self.features[item].end_positions
        ]
