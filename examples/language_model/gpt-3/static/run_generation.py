...
def do_generation(args):
    ...
    white_list = ["softmax", "layer_norm", "gelu", "fused_softmax_mask_upper_triangle", "elementwise_add"]
    black_list = ["reduce_sum", "c_softmax_with_cross_entropy", "elementwise_div"]
    ...
    with paddle.static.program_guard(main_program, startup_program):
        with paddle.utils.unique_name.guard():
            with paddle.static.device_guard("gpu:0" if topo.pp_info.size > 1 else None):
            ...
            if args.use_amp:
                amp_lists = AutoMixedPrecisionLists(
                    custom_white_list=set(white_list), custom_black_list=set(black_list)
                )
                # Cast model with while loop to fp16
                from utils.fp16_util import cast_model_to_fp16_block

                fp16_var_names = cast_model_to_fp16_block(main_program, amp_lists, False)
                ...
                for var_name in fp16_var_names:
                    ...
                    cast_var = block.create_var(name=var_name, dtype="float16", shape=var.shape, persistable=True)
                    for op in block.ops:
    ...
    if model_path in pretrained_models_list and model_path in model_urls:
        ...
        if os.path.exists(dygraph_path):
            if args.sharding_degree > 1:
                ...
                init_static_with_params(model, paddle.load(dygraph_path, return_numpy=True), topo, main_program)
                flag_loaded = True
        if not flag_loaded:
            logger.error("No checkpoint load.")
    ...
    ### check resutls
    text = ["Question: Where is the capital of China? Answer:", "Question:Who is the CEO of Apple? Answer:"]
    ...
    if args.save_inference_model_then_exist:
        ...
        inference_save_path = os.path.join(
            save_inference_model_dir, "rank_" + str(fleet.worker_index()), "step_" + str(0)
        )
        ...
        paddle.static.save_inference_model(inference_save_path, feeds, fetchs, exe, program=main_program)
