...
flags.DEFINE_string("input_groundtruth_path", "", "The root path to parent folder of all ground truth files.")
...
def get_field_xpaths(all_data_dict, vertical_to_process, website_to_process, n_pages, max_variable_nodes_per_website):
    ...
    for index in tqdm(all_data_dict, desc="Processing %s" % website_to_process, total=n_pages):
        ...
        for field in page_data:
            ...
            for value in page_data[field]["values"]:
                xpaths, current_xpath_data, current_page_nodes_in_order, is_truth_value_list = get_value_xpaths(
                    dom_tree, value, overall_xpath_dict, website_to_process, field[6:]
                )

                # Assert each truth value can be founded in >=1 nodes.
                assert len(xpaths) >= 1, "%s;\t%s;\t%s;\t%s; is not found" % (website_to_process, field, index, value)
    ...
    node_variability = sorted(
        [(xpath, len(text_set)) for xpath, text_set in overall_xpath_dict.items()], key=lambda x: x[1], reverse=True
    )
