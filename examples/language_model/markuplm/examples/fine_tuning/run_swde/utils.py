...
class SwdeDataset(Dataset):
    def __init__(
        ...
        raise ValueError
        """
        self.tensors = [all_input_ids, all_attention_mask, all_token_type_ids, all_xpath_tags_seq, all_xpath_subs_seq]
...
def get_swde_features(root_dir, vertical, website, tokenizer, doc_stride, max_length, prev_nodes, n_pages):
    ...
    for index in tqdm.tqdm(raw_data, desc=f"Processing {vertical}-{website}-{n_pages} features ..."):
        ...
        while True:
            ...
            splited_token_ids_seq = (
                [tokenizer.cls_token_id] + all_token_ids_seq[start_pos:end_pos] + [tokenizer.sep_token_id]
            )
            ...
            splited_xpath_subs_seq = (
                [padded_xpath_subs_seq] + all_xpath_subs_seq[start_pos:end_pos] + [padded_xpath_subs_seq]
            )
