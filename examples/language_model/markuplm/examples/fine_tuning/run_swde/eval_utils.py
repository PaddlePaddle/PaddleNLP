...
def page_hits_level_metric(vertical, target_website, sub_output_dir, prev_voted_lines):
...
def site_level_voting(vertical, target_website, sub_output_dir, prev_voted_lines):
    ...
    for field, xpth_freq in field_xpath_freq_dict.items():
        frequent_xpath = sorted(xpth_freq.items(), key=lambda kv: kv[1], reverse=True)[0][0]  # Top 1.
...
def page_level_constraint(vertical, target_website, lines, sub_output_dir):
    ...
    for line in lines:
        ...
        for index, score in enumerate(raw_scores):
            ...
            if tags[index] not in page_field_max[html_path] or score >= page_field_max[html_path][tags[index]]:
    ...
    return site_level_voting(vertical, target_website, sub_output_dir, voted_lines)
