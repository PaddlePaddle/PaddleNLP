...
def do_something(train_websites, test_websites, args, config, tokenizer):
    ...
    sub_output_dir = os.path.join(
        args.output_dir, args.vertical, f"seed-{args.n_seed}_pages-{args.n_pages}", "-".join(train_websites)
    )
...
def main():
    ...
    parser.add_argument(
        ...
        default="book",
        type=str,
        help="Which vertical to train and test" "Now we haven't supported multi-verticals in one program",
    )
    parser.add_argument("--n_seed", default=2, type=int, help="number of seed pages")
    parser.add_argument(
        "--n_pages", default=2000, type=int, help="number of pages in each website, set a small number for debugging"
    )
    parser.add_argument(
        "--prev_nodes_into_account",
        default=4,
        type=int,
        help="how many previous nodes before a variable nodes will we use" "large value means more context",
    )
    parser.add_argument(
        "--model_name_or_path",
        default=None,
    ...
    parser.add_argument("--warmup_ratio", default=0.0, type=float, help="Linear warmup ratio over all steps")
