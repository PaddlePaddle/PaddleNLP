...
def compute_pos(f, t_gold, addition, t_pred):
    ...
    if e_gold is None:
        if len(t_pred) != 1:
            ...
            if (e_pred is not None) or (addition == 1 and e_prev is not None) or (addition == 0 and e_prev is None):
                return 0
            else:
                return 1
    else:
...
def get_raw_scores(dataset, preds, tag_preds, root_dir):
    ...
    for websites in dataset:
        for w in websites["websites"]:
            f = os.path.join(root_dir, websites["domain"], w["page_id"][0:2], "processed_data", w["page_id"] + ".html")
            for qa in w["qas"]:
                qid = qa["id"]
                ...
                pos_scores[qid] = max(
                    compute_pos(f, t, a, t_pred) for t, a in zip(gold_tag_answers, additional_tag_information)
                )
