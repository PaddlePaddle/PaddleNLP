...
def set_hyrbid_parallel_seed(basic_seed, dp_rank, mp_rank, pp_rank):
    ...
    meta_parallel.model_parallel_random_seed(basic_seed + dp_rank + 1000 * mp_rank)
...
def parameters_classify(model):
    ...
    print("decay_gate_params len: {}, decay_expert_params len: {}, decay_other_params len: {}".format(len(decay_gate_params), len(decay_expert_params), len(decay_other_params)))
    print("gate_params len: {}, expert_params len: {}, other_params len: {}".format(len(gate_params), len(expert_params), len(other_params)))
...
def do_train(args):
    ...
    if args.resume_dir is not None:
        global_step, loss_scale, data_meta = load_checkpoint(args, model, optimizer, lr_scheduler, tokenizer, dp_rank, mp_rank, pp_rank)        
        pass_num = data_meta["pass_num"]
        file_id = data_meta["file_id"]
        start_epoch = data_meta["start_epoch"]        

    if args.use_pure_fp16:
        scaler = paddle.amp.GradScaler(init_loss_scaling=loss_scale if args.resume_dir is not None else args.scale_loss)
        scaler = fleet.distributed_scaler(scaler)
        scaler._unscale = MethodType(unscale_method, scaler)
        ...
        for f_id in range(file_id, num_files):
            ...
            for step, batch in enumerate(train_data_loader()):
                ...
                if (global_step % args.save_steps == 0 or global_step >= args.max_steps):
                    ...
                    save_checkpoint(args, global_step, model, optimizer, lr_scheduler, tokenizer, loss_scale, dp_rank, mp_rank, pp_rank, pass_num, file_id, epoch) 
