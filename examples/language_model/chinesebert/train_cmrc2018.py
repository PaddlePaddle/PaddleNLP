...
def evaluate(model, data_loader, args, output_dir="./"):
    ...
    eval_results = get_result(
        ground_truth_file=ground_truth_file, prediction_file=os.path.join(output_dir, "all_predictions.json")
    )
...
def main(args):
    ...
    decay_params = [
        p.name for n, p in model.named_parameters() if not any(nd in n for nd in ["bias", "norm", "LayerNorm"])
    ]
    ...
    for _ in range(args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            model.train()
            with auto_cast(args.use_amp, custom_white_list=["layer_norm", "softmax", "gelu"]):
                ...
                loss = loss_fn(logits, (start_positions, end_positions)) / args.gradient_accumulation_steps
                tr_loss += loss.item()
...
def parse_args():
    ...
    parser.add_argument(
        "--scheduler_type", choices=["linear", "cosine", "poly"], default="linear", type=str, help="scheduler_type."
    )
    parser.add_argument(
        "--doc_stride",
        type=int,
