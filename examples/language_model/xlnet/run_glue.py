...
def do_train(args):
    ...
    #     XLNetModel(**model_class.pretrained_init_configuration[args.model_name_or_path])
    # )
    ...
    #     lr_lambda=lambda current_step, num_warmup_steps=warmup_steps,
    ...
    tic_train = time.time()
    model.train()
    # params = [p.cpu().numpy() for p in model.parameters()]
    # np.save('./pp_params.txt', params)
    for epoch in range(args.num_train_epochs):
        for step, batch in enumerate(train_data_loader):
            input_ids, segment_ids, attention_mask, labels = batch
            print()
            print("global_step: {}    lr_scheduler: {}".format(global_step, lr_scheduler.get_lr()))
            # print("input_ids:\r\n{}".format(input_ids))
            # print("segment_ids:\r\n{}".format(segment_ids))
            ...
            # pd_out = list()
            # pd_out.append(logits.cpu().numpy())
            # print("logits:\r\n{}".format(logits))
            loss = loss_fct(logits, labels)
            # pd_out.append(loss.cpu().numpy())
            # np.save('./pp_out.txt', pd_out)
            print("loss:\r\n{}".format(loss), flush=True)
            ...
            # pd_param_grad = [para.grad for para in model.parameters()]
            # np.save('./pp_grad.txt', pd_param_grad)
            optimizer.step()
            lr_scheduler.step()
