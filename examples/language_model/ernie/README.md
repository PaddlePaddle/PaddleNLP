...
### Fleet 4D Hybrid Parallelism Pretraining
为了探索更优的语言模型，本示例采用了飞桨Fleet 4D 混合并行策略，可以将模型扩展到100多层transformer网络, 千亿参数规模。
100多层transformer网络结构，计算复杂，训练时需要占用T级显存资源。
...
其中，如果想要训练 128层 transformer，230B参数配置的模型，推荐机器配置为 32 * 8 V100 32G，即32台* 8 卡 32G V100机器。
整体配置如下：
```shell
python -m paddle.distributed.fleet.launch \
    --log_dir output_dir/log \
