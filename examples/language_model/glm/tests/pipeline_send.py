...
from paddlenlp.trainer.trainer import device_guard
...
def distributed_all_gather(tensor: Any, offload=False) -> Any:
    try:
        if isinstance(tensor, (tuple, list)):
            return type(tensor)(distributed_all_gather(t, offload) for t in tensor)
        if isinstance(tensor, dict):
            return {k: distributed_all_gather(v, offload) for k, v in tensor.items()}
