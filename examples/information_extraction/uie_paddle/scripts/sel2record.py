...
def main():
    ...
    parser.add_argument('-g', dest='gold_folder', help='标准答案（Gold）文件夹')
    parser.add_argument(
        '-p', dest='pred_folder', nargs='+', help='多个不同的预测（Pred）文件夹')

    parser.add_argument(
        '-c', '--config', dest='map_config', help='Offset 匹配策略的配置文件')
    parser.add_argument(
        '-d',
        dest='decoding',
        default='spotasoc',
        help='使用 SpotAsoc 结构的解析器进行结构表达式解析')
    parser.add_argument(
        '-v',
        '--verbose',
        dest='verbose',
