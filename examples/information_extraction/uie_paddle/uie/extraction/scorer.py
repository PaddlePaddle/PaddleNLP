...
class RecordMetric(Metric):
    ...
    def is_equal(gold, pred):
        ...
        if len(gold['roles']) != len(pred['roles']):
            return False
        for gold_role, pred_role in zip(
                sorted(gold['roles']), sorted(pred['roles'])):
    ...
    def count_instance(self, gold_list, pred_list):
        if self.match_mode == 'set':
            raise NotImplementedError(
                f'{self.__class__.__name__} do not support the match model `set`'
            )
        ...
        for pred in pred_list:
            for gold_index in gold_indexes:
                if non_found[gold_index] and self.is_equal(
                        gold_list[gold_index], pred):
                    self.tp += 1
