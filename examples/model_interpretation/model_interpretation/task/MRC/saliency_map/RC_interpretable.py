...
def get_args():
    ...
    parser.add_argument('--max_seq_len', type=int, default=512, help='max sentence length, should not greater than 512')
    ...
    parser.add_argument('--ans_path', type=str, required=True, help="the path of the file which stores the predicted answer from last step")
    parser.add_argument('--ans_idx_path', type=str, required=True, help="the path of the file which stores the predicted answer index from last step")
...
def ch_per_example(args, scores_in_one_example, prev_context_tokens, dev_ds, prev_example_idx, ans_dic, ans_idx_dic, out_handle):
    total_score = []
    for idx, score in enumerate(scores_in_one_example):
        if idx == len(scores_in_one_example) - 1:
            total_score.extend(score)
        else:
            total_score.extend(score[: args.doc_stride])
    assert len(prev_context_tokens) == len(total_score)
    token_score_dict = {}
    for idx, token in enumerate(prev_context_tokens):
        token_score_dict[idx] = (token, total_score[idx])
    ...
    for token, score in sorted(token_score_dict.items(),
                                key=lambda x: x[1][1], reverse=True):
        result['char_attri'][token] = score

    out_handle.write(json.dumps(result, ensure_ascii=False) + '\n')
    

def en_per_example(sub_word_id_dict, inter_score, result, ans_dic, ans_idx_dic, out_handle):
...
def extract_attention_scores(args, 
    ...
    with P.no_grad():
        ...
        _, start_logits, end_logits, cls_logits, attentions, embedded = model.forward_interpret(*fwd_args, **fwd_kwargs)
    ...
    if args.language == 'ch':
        if prev_example_idx is None or prev_example_idx == example_idx:
            ...
            ch_per_example(args, scores_in_one_example, prev_context_tokens, dev_ds, prev_example_idx, ans_dic, ans_idx_dic, out_handle)
            scores_in_one_example = [context_norm_score.numpy().tolist()]
        prev_example_idx = example_idx
        prev_context_tokens = context_tokens
    else:
        en_per_example(sub_word_id_dict, context_norm_score, result, ans_dic, ans_idx_dic, out_handle)
    return prev_example_idx, prev_context_tokens, scores_in_one_example

def extract_integrated_gradient_scores(args, 
                                    dev_ds, 
                                    model, 
                                    result,
                                    fwd_args, 
    ...
    for i in range(args.n_samples): 
        ...
        _, start_logits, _, _, _, embedded = model.forward_interpret(*fwd_args, **fwd_kwargs,
                                                                            noise='integrated',
                                                                            i=i,
        ...
        _, _, end_logits, _, _, embedded = model.forward_interpret(*fwd_args, **fwd_kwargs,
                                                                        noise='integrated', 
                                                                        i=i, 
                                                                        n_samples=args.n_samples)
    ...
    trapezoidal_grads = (embedded_grads_tensor[1:] + embedded_grads_tensor[:-1]) / 2    # Tensor(n_samples-1, 1, seq_len, embed_size)
...
if __name__ == "__main__":
    ...
    with P.amp.auto_cast(enable=args.use_amp), \
        ...
        for step, d in tqdm(enumerate(dataloader)):
            ...
            result = {}
            input_ids, segment_ids, example_idx = d
            ...
            if args.language == 'en':
                ...
                for context_word in context_list:
                    sub_word_id = get_subword_ids(context_word)
                    sub_word_id_dict.append((context_word, len(list(sub_word_id))))
                    
            if args.inter_mode == "attention":
                prev_example_idx, prev_context_tokens, scores_in_one_example = extract_attention_scores(args, 
                                                                                                        model, 
                                                                                                        result, 
                                                                                                        fwd_args, 
                                                                                                        fwd_kwargs, 
                ...
                prev_example_idx, prev_context_tokens, scores_in_one_example = extract_integrated_gradient_scores(args, 
                                                                                                                dev_ds, 
                                                                                                                model, 
                                                                                                                result,
                                                                                                                fwd_args, 
        ...
        if args.language == 'ch':
            ...
            ch_per_example(args, scores_in_one_example, context_tokens, dev_ds, -1, ans_dic, ans_idx_dic, out_handle)
