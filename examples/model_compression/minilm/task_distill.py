...
def do_train(args):
    ...
    for epoch in range(num_train_epochs):
        for step, batch in enumerate(train_data_loader):
            ...
            # self-attention
            loss_attn = loss_fct(
                F.log_softmax(student.outputs.scaled_qks[-1]),
                F.softmax(teacher.outputs.scaled_qks[-1]))

            # value-relation
            v_t = teacher.outputs.vs[-1]
            v_s = student.outputs.vs[-1]
            vv_t = paddle.tensor.matmul(
                x=v_t, y=v_t, transpose_y=True) / math.sqrt(head_dim)
            vv_s = paddle.tensor.matmul(
                x=v_s, y=v_s, transpose_y=True) / math.sqrt(head_dim)
            loss_vr = loss_fct(F.log_softmax(vv_s), F.softmax(vv_t))
            pad_seq_len = input_ids.shape[1]

            loss = (loss_attn + loss_vr) / (head_num * pad_seq_len)
