...
parser.add_argument(
    "--batch_size", type=int, required=False, default=2, help="Batch images")
...
parser.add_argument(
    "--num_classes", type=int, required=False, default=2, help="num_classes")
...
dev_ds.map(
    partial(
        prepare_dev_features_single, tokenizer=tokenizer, args=args),
    batched=False,
...
with paddle.no_grad():
    for batch_idx, batch in tqdm(
            enumerate(dev_data_loader), total=len(dev_data_loader)):
