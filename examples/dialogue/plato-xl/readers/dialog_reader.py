...
class DialogReader(object):
    ...
    def add_cmdline_args(cls, parser):
        ...
        group.add_argument(
            "--use_role",
            action="store_true",
            help="Whether to use role embedding. ")
        group.add_argument(
            "--position_style",
            type=str,
            choices=["relative", "continuous"],
            default="continuous",
            help="Setting position ids' style. Must be one of ['relative', 'continuous']. "
        )
    ...
    def __init__(self, args):
        ...
        self.fields = ["input_ids", "type_ids", "pos_ids"]
        self.num_numerical_fields = len(self.fields)
        self.fields += ["tgt_start_idx", "data_id"]
        self.sort_key = lambda record: [len(record.input_ids)]

        if self.use_role:
            self.fields += ["role_ids"]
    ...
    def _convert_example_to_record(self, example):
        ...
        for s in example.src.split("[SEP]"):
            ...
            s_input_ids = self.tokenizer.convert_tokens_to_ids(
                s_tokens) + [self.eos_id]
            s_input_ids_list.append(s_input_ids)

        # trim src
        idx = len(s_input_ids_list) - 1
        total_token_num = 1
        while idx >= 0:
            total_token_num += len(s_input_ids_list[idx])
            if total_token_num > self.max_src_len:
                if self.truncate_first_turn and idx == 0:
                    truncated_ids = s_input_ids_list[idx][:self.max_src_len -
                                                          total_token_num]
        ...
        if self.use_role and len(role_id_list) == 0:
            for i in range(len(s_input_ids_list)):
                role_id_list.append((len(s_input_ids_list) - i) % 2)
        for i, s_input_ids in enumerate(s_input_ids_list[idx + 1:], idx + 1):
            src_input_ids += s_input_ids
            src_pos_ids += list(range(1, len(s_input_ids) + 1))
            if self.use_role:
                src_role_ids += [role_id_list[i]] * len(s_input_ids)

        src_input_ids = [self.bos_id] + src_input_ids
        src_type_ids = [0] * len(src_input_ids)
        src_pos_ids = [0] + src_pos_ids
        assert len(src_input_ids) == len(src_type_ids) == len(src_pos_ids), \
            "The length of src_input_ids, src_type_ids and src_pos_ids must be the same. "
        if self.use_role:
        ...
        pos_ids = src_pos_ids
        tgt_start_idx = len(input_ids)

        assert len(input_ids) == len(type_ids) == len(pos_ids), \
            "not len(input_ids) == len(type_ids) == len(pos_ids)"

        if self.position_style == "relative":
            ctx_len = len(input_ids)
            pos_ids = [
                self.max_tgt_len + ctx_len - i - 1 for i in range(ctx_len)
            ]
        elif self.position_style == "continuous":
            pos_ids = list(range(len(input_ids)))
        else:
            raise ValueError(
                "Invalid position style setting. Expected one of [relative, continuous], but excepted %s. ",
                self.position_style)
    ...
    def _gen_self_attn_mask(self,
        ...
        if is_unidirectional:
            ...
            for index, input_ids in enumerate(batch_input_ids):
                input_mask_data[index, :len(input_ids) + shift_len, :len(
                    input_ids) + shift_len] = 1.0
        return input_mask_data.astype(dtype)
    ...
    def _pad_batch_records(self, batch_records, is_infer):
        ...
        batch["input_ids"] = pad_batch_data(batch_input_ids, pad_id=self.pad_id)
        ...
        batch["generation_mask"] = self._gen_self_attn_mask(
            batch_input_ids, batch_tgt_start_idx=batch_tgt_start_idx)

        tgt_ids = np.array(
            [[[self.bos_id]]] * len(batch_input_ids), dtype="int32")
        if self.continuous_position:
            tgt_pos = np.array(batch_tgt_start_idx, dtype="int32")
        else:
            tgt_pos = np.zeros_like(batch_tgt_start_idx, dtype="int32")
