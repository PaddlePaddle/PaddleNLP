...
def read_data(filenames):
    ...
    for filename in filenames:
        data = pd.read_json(f"/ssd2/hesijun/workspace/paddle_bot/answer/{filename}", lines=True)
        filters = data["答案"].isin(["timeout", "skip due to low quality"])
        data = data[~filters]
        total_data.append(data)
    concat_data = pd.concat(total_data)
    for i, row in concat_data.iterrows():
        yield {"source": row["问题"], "target": row["答案"]}
