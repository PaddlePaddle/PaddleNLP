...
private:
  ...
  void run_split_on_punc(const wstring& text, vector<wstring>* output) const {
    output->clear();
    size_t i = 0;
...
public:
  ...
  void Tokenize(const wstring& text, vector<wstring>* output_tokens) const {
    vector<wstring> tokens;
...
public:
  ...
  int64_t GetNumSpecialTokensToAdd(const bool pair) const {
    if (pair) {
      return 3;
    } else {
      return 2;
  ...
  int BatchEncode(
    ...
    for (size_t i = 0; i < batch_size; i++) {
      if (stride > 0 && has_text_pair) {
        ...
        cout << "Tokenizer op to precoss QA task data needs to be done."
             << endl;
        return 0;
  ...
  int64_t GetClsTokenID() const { return cls_token_id_; }

  int64_t GetSepTokenID() const { return sep_token_id_; }

  int64_t GetUnkTokenID() const { return unk_token_id_; }

  int64_t GetMaskTokenID() const { return mask_token_id_; }
...
int main(int argc, char* argv[]) {
  ...
  Vocab vocab;
  LoadVocab(FLAGS_vocab_file, &vocab);
  BertTokenizer tokenizer(vocab, false);
