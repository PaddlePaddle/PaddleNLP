...
def term_linking(wordtag_result, LABEL_TO_SCHEMA):
    for item in wordtag_result["items"]:
        if item["wordtag_label"] not in LABEL_TO_SCHEMA:
            continue
        flag, _ = termtree.find_term(item["item"])
        if flag is False:
            continue
        ...
        for target_type_raw in target_type_can:
            ...
            flag, term_id = termtree.find_term(
                item["item"], target_type)
            if flag is False:
                continue
            term_id = list(
                filter(lambda d: termtree[d].node_type == "term",
                        term_id))
            if len(term_id) == 0:
                continue
            if target_src is not None:
                term_id = list(
                    filter(
                        lambda d: termtree[d].base.startswith(
                            target_src.lower()), term_id))
                if len(term_id) == 0:
                    continue
            term_id.sort(key=lambda d: (termtree[
                d].termtype == target_type or target_type in termtree[
                    d].subtype, termtree[d].term == item["item"]),
                            reverse=True)
