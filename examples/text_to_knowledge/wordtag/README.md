...
- **覆盖中文全词类，更丰富的知识标注结果**
  - 中文文本知识标注工具使用的词类体系为覆盖中文全部词汇的词类体系，包括各类实体词与非实体词（如概念、实体/专名、语法词等）。wordtag开源版对部分类目（如组织机构等），做了更细类目的划分识别（如，医疗卫生机构、体育组织机构），对仅使用文本信息难以细分的类目（如人物类、作品类、品牌名等），不做更细粒度的词类识别。
  - wordtag开源版使用的词类体系是我们在实践中对百科文本解析应用较好的一个版本，不同类型文本的词类分布不同，用户可以利用[TermTree](../termtree)定制自己的词类体系和训练样本，构建自己的wordtag应用版，以获得更好的适配效果。
- **整合TermTree linking结果，获得更丰富的标注知识**
  - 如上图示例所示，各个切分标注结果中，除词类标注外，还整合了TermTree的linking结果，用户可以结合[TermTree](../termtree)数据共同使用，如利用TermTree中的subtype获得更细的上位粒度；利用TermTree中term的百科信息获得更加丰富的知识等；同时，TermTree数据是可插拔的，用户可将自定义的词表按照TermTree字段定义好，挂接/整合到TermTree体系中，即可使用自己的Term数据定制下游任务。
...
wordtag提供从文本到TermTree的链接方法，即Term-Linking，只需将term词类体系与TermTree数据加载到工具中，即可在解析结果中得到term-linking结果。

为了能够适配领域中的不同实体集（例如，不同的企业有不同的人物实体集合，不同的小说站有不同的小说实体集合），我们将term-linking拆分为两个步骤：

- 第一步是基于词类的linking，主要解决“同名概念词/实体词”、“不同类的同名词”消歧问题，这一步只使用文本本身特征和词类特征，不使用图谱中的SPO知识（从而支持切换不同领域图谱）；
- 第二步是同类同名实体词的linking，主要解决同类下不同属性值的实体消歧问题，这一步需要使用实体词的SPO知识（一般用于实体特征表示计算，以及文本-实体相似度计算）。

“wordtag+TermTree”的开源版提供了第一步的解决示例，第二步由于依赖于特定图谱的SPO知识，无法提供通用工具，未来可能提供通用解决方案。
...
3. 提供根据TermPath及TermCorpus完成细粒度term及subterm消歧的解决方案。
