...
解语（Text to Knowledge）是首个中文全词类描述的知识库及知识标注框架，拥有可覆盖所有中文词汇的概念体系、全词类文本知识标注工具，以及更适用于中文挖掘任务的预训练语言模型。

全词类知识库和知识标注能够帮助你面对更加多元的应用场景，方便地融合自有知识体系，显著提升中文文本解析和挖掘效果，并能够更容易地利用知识增强机器学习模型效果。
...
中文文本知识标注工具（wordtag）整合了传统中文解析的**分词**、**词性标注**、**命名实体识别**的能力，能够将任意中文句子解析为**完整的词类序列**。结合百科知识树（TermTree），可为应用提供一套通用的知识关联（term-linking）框架，方便应用适配关联自己的领域知识图谱，更好地将知识用于中文NLP任务。
...
虽然近年来，深度学习模型尤其是预训练语言模型的广泛使用大幅提升了各项中文NLP任务效果，但在实际的工业应用中，单独使用DNN模型往往达不到应用需求，还需要结合规则模型以提升精度以及解决恶劣case，如，知识图谱构建、query解析、语义一致性判定等应用。
...
wordtag的标注结果中，区分了“人物类\_实体”和“人物类\_概念”，以及“作品类\_实体”和“作品类\_概念”，使得模板生成更为精准。同时，TermTree中也区分了命名实体词(eb)与非实体词(cb)，这样，可以利用TermTree分别进行实体扩展（e.g., 周星驰->王晶）和概念扩展(e.g., 电影->小说)，生成更加丰富多样的模板，支持更细化的应用场景。

### 应用场景B：词类知识增强的深度学习模型

词类特征同时也是一类重要的文本特征，可为原始文本token提供有效的边界信息、归组信息，减少样本中的噪音，防止模型过拟合；还可作为层次泛化特征，弥补统计共现特征的不足。

在DNN模型应用中，可将wordtag产出的词类作为embedding特征，直接叠加到文本token上，作为DNN模型的输入；在BERT等模型中，也可以使用K-BERT等方式，将词类作为文本序列中的一部分，利用position id和可见性矩阵控制token和词类特征之间的可见性，作为DNN模型的输入。

###应用场景C：知识图谱关联（term-linking）

随着知识图谱技术的普及和越来越多领域知识图谱数据的发布，如何利用知识提升NLP任务效果，成为近年来NLP研究的热点方向。文本与图谱知识结合的前提是将图谱中的实体准确link到文本上，这是知识图谱应用的一大难点。现有的方案多是基于某个特定图谱实现的，缺乏通用的图谱关联解决方案。我们尝试使用“**wordtag+TermTree**”提供一套通用的图谱关联（term-linking）技术框架。
...
1. 发布百科知识树（TermTree）正式版数据，建立知识共建社区，支持用户提交领域词表/领域图谱 & 定制化TermTree；
