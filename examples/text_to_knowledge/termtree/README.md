...
1. 直接作为词表使用，利用termtype和subtype筛选应用所需的词表。
2. 结合文本知识标注工具集（wordtag等）使用，用于文本词类特征生成、挖掘/解析pattern生成、文本一致性判定等等，参见wordtag说明。
3. 用于样本构建和优化：利用词类信息筛选&扩充样本、样本纠错、样本类别细化。

## TermTree的后续规划

1. 数据覆盖扩展到全量百度百科词条，提升TermType归类准确率，便于应用方筛选构建应用适配的TermTree；
2. 基于TermPath知识库和TermCorpus语料库计算term的向量表示，用于实体链接消歧；
3. 建立知识共建社区，支持用户提交自己的term词表，生成定制版TermTree。
