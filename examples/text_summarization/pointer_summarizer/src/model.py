...
def paddle2D_scatter_add(x_tensor, index_tensor, update_tensor, dim=0):
    ...
    if dim == 0:
        index_tensor = paddle.concat(x=[index_tensor, paddle.to_tensor([[x] for x in range(dim1)] * dim0)], axis=1)
    elif dim == 1:
        index_tensor = paddle.concat(x=[
            paddle.flatten(paddle.to_tensor([[[x]] * dim1 for x in range(dim0)]), start_axis=0, stop_axis=1),
            index_tensor], axis=1)
...
class Encoder(paddle.nn.Layer):
    def __init__(self):
        ...
        self.embedding = nn.Embedding(config.vocab_size, config.emb_dim, weight_attr=paddle.ParamAttr(
                initializer=I.Normal(std=config.trunc_norm_init_std)))

        # initialized lstm weights
        self.lstm = nn.LSTM(config.emb_dim, config.hidden_dim, num_layers=1, direction='bidirect',
                            weight_ih_attr=paddle.ParamAttr(initializer=I.Uniform(
                                low=-config.rand_unif_init_mag, high=config.rand_unif_init_mag)),
...
class Attention(paddle.nn.Layer):
    ...
    def forward(self, s_t_hat, encoder_outputs, encoder_feature, enc_padding_mask, coverage):
        ...
        attn_dist = attn_dist_ / (paddle.reshape(normalization_factor, [-1, 1]) +
...
class Decoder(paddle.nn.Layer):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(config.emb_dim, config.hidden_dim, num_layers=1, direction='forward',
                            weight_ih_attr=paddle.ParamAttr(initializer=I.Uniform(
                                low=-config.rand_unif_init_mag, high=config.rand_unif_init_mag)),
                            bias_ih_attr=paddle.ParamAttr(initializer=I.Constant(
                                value=0.0)))
        ...
        self.out2 = nn.Linear(config.hidden_dim, config.vocab_size, weight_attr=paddle.ParamAttr(initializer=I.Normal(
            std=config.trunc_norm_init_std)))
    ...
    def forward(self, y_t_1, s_t_1, encoder_outputs, encoder_feature, enc_padding_mask,
        ...
        if not self.training and step == 0:
            ...
            s_t_hat = paddle.concat((paddle.reshape(h_decoder, [-1, config.hidden_dim]),
                                     paddle.reshape(c_decoder, [-1, config.hidden_dim])), 1)  # B x 2*hidden_dim
