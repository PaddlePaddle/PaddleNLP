# PaddleNLP GPU 算子库
> paddlenlp_gpu_ops 是一个专为 PaddleNLP 量身打造的 GPU 算子库，它集成了一系列常用的自然语言处理（NLP）算子，并提供了 CUDA 和 Triton 两种高效的实现方式，旨在充分利用 GPU 的卓越计算能力，为 NLP 任务加速。
...
## 使用 paddlenlp_gpu_ops 库
以下是如何在代码中使用 `CUDA` 和 `Triton` 算子的示例：
```python
# 导入并使用 CUDA 算子
from paddlenlp_gpu_ops.cuda.selective_scan import selective_scan_fn
xxx = selective_scan_fn(xxx)

# 导入并使用 Triton 算子
from paddlenlp_gpu_ops.triton.inf_cl import cal_flash_loss
