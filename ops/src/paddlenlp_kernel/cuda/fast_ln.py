...
@unified
def fast_ln(x,scale,bias,epsilon):
    # The output variable's dtype use default value 'float32',
    # and the actual dtype of output variable will be inferred in runtime.
    if in_dynamic_or_pir_mode():
        outs = _C_ops._run_custom_op("fast_ln", x,scale,bias,epsilon)
        res = []
        start_idx = 0
        ...
        for key, value in ins_map.items():
            ...
            if value is not None:
                ins[key] = value
        helper = LayerHelper("fast_ln", **locals())
        ...
        helper.append_op(type="fast_ln", inputs=ins, outputs=outs, attrs={'epsilon' : epsilon})
...
@unified
def fast_rms_norm(x,scale,epsilon):
    # The output variable's dtype use default value 'float32',
    # and the actual dtype of output variable will be inferred in runtime.
    if in_dynamic_or_pir_mode():
        outs = _C_ops._run_custom_op("fast_rms_norm", x,scale,epsilon)
        res = []
        start_idx = 0
        res.append(outs[start_idx])
        start_idx += 1
        ...
        for key, value in ins_map.items():
            ...
            if value is not None:
                ins[key] = value
        helper = LayerHelper("fast_rms_norm", **locals())

        outs['y'] = helper.create_variable(dtype='float32')
        outs['invvar'] = helper.create_variable(dtype='float32')
        helper.append_op(type="fast_rms_norm", inputs=ins, outputs=outs, attrs={'epsilon' : epsilon})
        res = [outs[out_name] if out_name in outs.keys() else None for out_name in outs_list]
        return res[0] if len(res)==1 else res
...
so_path = os.path.join(cur_dir, "lib/fast_ln_pd.so")
