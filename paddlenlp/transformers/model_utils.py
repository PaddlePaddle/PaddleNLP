...
class PretrainedModel(Layer, GenerationMixin, ConversionMixin):
        ...
        if pretrained_model_name_or_path is not None:
            ...
            # pretrained_model_name_or_path is dir
                ...
                if use_safetensors is not False and os.path.isfile(
                    ...
                    os.path.join(pretrained_model_name_or_path, subfolder, _add_variant(PYTORCH_WEIGHTS_INDEX_NAME, variant))
                ):
                    if from_hf_hub or convert_from_torch:
                        archive_file = os.path.join(pretrained_model_name_or_path, subfolder, _add_variant(PYTORCH_WEIGHTS_INDEX_NAME, variant))
                    else:
                        raise ValueError(
                    ...
                    if from_hf_hub or convert_from_torch:
                        archive_file = os.path.join(pretrained_model_name_or_path, subfolder, _add_variant(PYTORCH_WEIGHTS_NAME, variant))
                    else:
                        raise ValueError(
                ...
                resolved_archive_file = get_file(pretrained_model_name_or_path, pretrained_model_name_or_path, 
                                                subfolder, cache_dir=cache_dir, from_aistudio=from_aistudio, 
                                                from_hf_hub=from_hf_hub
                                                )
                ...
                resolved_archive_file = get_file(pretrained_model_name_or_path, [resource_file_url], 
                                                 subfolder, cache_dir=cache_dir, from_aistudio=from_aistudio, 
                                                 from_hf_hub=from_hf_hub
                                                )
            else:
                if use_safetensors is not False:
                    filenames = [_add_variant(SAFE_WEIGHTS_INDEX_NAME, variant), _add_variant(SAFE_WEIGHTS_NAME, variant)]
                else:
                    filenames = [_add_variant(PADDLE_WEIGHTS_INDEX_NAME, variant), _add_variant(PADDLE_WEIGHTS_NAME, variant), _add_variant(PYTORCH_WEIGHTS_INDEX_NAME, variant), _add_variant(PYTORCH_WEIGHTS_NAME, variant)]
                resolved_archive_file = get_file(pretrained_model_name_or_path, filenames,
                                                 subfolder, cache_dir=cache_dir, from_aistudio=from_aistudio, 
                                                 from_hf_hub=from_hf_hub
                                                )
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        ...
        if convert_from_torch and state_dict is None:
            ...
            elif resolved_archive_file.endswith(PADDLE_WEIGHTS_NAME) or resolved_archive_file.endswith(PADDLE_WEIGHTS_INDEX_NAME):
