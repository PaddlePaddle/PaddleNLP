...
class LlamaAttentionAuto(nn.Layer):
    ...
    def forward(
        ...
        if self.config.rope:
            ...
            if self.config.context_parallel_degree > 1:
                ...
                cos, sin = self.rotary_emb(
                    ...
                    dist.auto_parallel.api.dtensor_to_local(value_states, value_states.process_mesh, value_states.placements), seq_len=kv_seq_len
                )
                new_query_states, new_key_states = apply_rotary_pos_emb(
                    dist.auto_parallel.api.dtensor_to_local(query_states, query_states.process_mesh, query_states.placements),
                    dist.auto_parallel.api.dtensor_to_local(key_states, key_states.process_mesh, key_states.placements),
        ...
        print(f'[linguangming] LlamaAttentionAuto forward,  attention_mask is None? {attention_mask is None}')
        if (
...
class LlamaModelAuto(nn.Layer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        print(f'decoder attention mask is {attention_mask == None}')
        output_attentions = (
        ...
        if self.config.use_flash_attention:
            ...
            if is_casual:
                attention_mask = None
                print(f'[linguangming] set attention_mask to None')
                print(f'[linguangming] LlamaModelAuto line 803, attention_mask is {attention_mask}')
        ...
        for idx, (decoder_layer) in enumerate(self.layers):
            ...
            if not is_pp_enable():
                ...
                # attention_mask_input = attention_mask # [NOTE] 修改为如下
                attention_mask_input = None
                print(f'[linguangming] LlamaModelAuto line 830, attention_mask_input is {attention_mask_input}')
            else:
                ...
                print(f'[linguangming] LlamaModelAuto line 853, attention_mask_input is {attention_mask_input}')
            ...
            if (
                ...
                # print(f'[linguangming] LlamaModelAuto line 875, attention_mask_input is {attention_mask_input}')
...
def is_casual_mask(attention_mask):
    ...
    """
    is_causal = paddle.triu(attention_mask) == attention_mask
    is_causal = paddle.all(is_causal)
    return is_causal
...
def scaled_dot_product_attention(
    ...
    if config.use_flash_attention and flash_attention:
        ...
        # print(f'[linguangming] version is {version}')
        if version != "0.0.0" and version <= "2.5.2":
            ...
            print(f'[linguangming] scaled_dot_product_attention attn_mask is None? {attention_mask is None}')
