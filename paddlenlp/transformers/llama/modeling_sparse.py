...
def _batch_make_pose_mask(block_length, seq_length, sparsity_levels=[0.3, 0.5, 0.8, 0.9]):
    num_blocks = len(block_length)
    total_length = sum(block_length)  # Calculate total length by summing all subsequence lengths

    mask = paddle.tril(
        paddle.ones((total_length, total_length), dtype=bool)
    )  # Initialize with lower triangular matrix (True)
    start_indices = paddle.cumsum(
        paddle.concat([paddle.to_tensor([0]), block_length[:-1]])
    )  # Calculate start indices of each subsequence

    for i in range(num_blocks):
        for j in range(num_blocks):
            if i == j:
                continue  # Full attention within sub-sequences
            distance = abs(i - j)
            ...
            length_i = block_length[i]
            start_j = start_indices[j]
            length_j = block_length[j]

            if start_i > start_j:
                mask_indices = paddle.to_tensor(
                    np.random.choice(
                        paddle.arange(length_i * length_j), size=int(length_i * length_j * sparsity), replace=False
                    )
                )
                for idx in mask_indices:
                    row = start_i + idx // length_j
                    col = start_j + idx % length_j
                    if row > col:
                        mask[row, col] = False

    attn_mask = paddle.zeros((seq_length, seq_length), dtype=bool)
    attn_mask[-total_length:, -total_length:] = mask
    # [1, 1, seq_length, seq_length]
    return attn_mask[None, None, :, :]


def _make_pose_mask(block_lengths, max_length, sparsity_levels=[0.3, 0.5, 0.7, 0.9]):
    # def _make_pose_mask(block_lengths, sparsity_levels=[0,0,0,0]):
    num_blocks = block_lengths.shape[-1]
    ...
    if total_length == max_length:
        ...
        for i in range(num_blocks):
            for j in range(num_blocks):
                ...
                # if start_i > start_j:
                #     mask_indices = paddle.to_tensor(np.random.choice(paddle.arange(length_i * length_j), size=int(length_i * length_j * sparsity), replace=False))
                #     for idx in mask_indices:
                #         row = start_i + idx // length_j
                #         col = start_j + idx % length_j
                #         if row > col:
                ...
                if start_i > start_j:
                    ...
                    if not any(dim == 0 for dim in mask_indices.shape):
                        rows = start_i + mask_indices // length_j
                        cols = start_j + mask_indices % length_j
...
class LlamaModel(LlamaPretrainedModel):
    ...
    @staticmethod
    def _batch_prepare_block_attention_mask(block_lengths, input_shape, dtype):
        batch_size, seq_length = input_shape  # target_length: seq_len
        attn_mask = paddle.zeros([seq_length, seq_length], dtype=bool).expand([batch_size, 1, seq_length, seq_length])
        for i, block_length in enumerate(block_lengths):
            block_mask = _make_pose_mask(block_length, seq_length)
            attn_mask[i, :, :, :] = block_mask

        if get_env_device() == "npu":
            x = paddle.to_tensor(0.0, dtype="float16")
            y = paddle.to_tensor(paddle.finfo(dtype).min, dtype="float16")
            attn_mask = attn_mask.astype("float16")
            attn_mask = paddle.where(attn_mask, x, y).astype(dtype)
        else:
            attn_mask = paddle.where(attn_mask, 0.0, paddle.finfo(dtype).min).astype(dtype)
        return attn_mask

    @staticmethod
    def _prepare_block_attention_mask(block_lengths, max_length, dtype):
        attn_mask = _make_pose_mask(block_lengths, max_length)
        if get_env_device() == "npu":
            x = paddle.to_tensor(0.0, dtype="float16")
            y = paddle.to_tensor(paddle.finfo(dtype).min, dtype="float16")
            attn_mask = attn_mask.astype("float16")
    ...
    def forward(
        ...
        **kwargs,
        ...
        if position_ids is None:
            position_ids = paddle.arange(seq_length, dtype="int64").expand((batch_size, seq_length))
        # attention_mask = self._prepare_decoder_attention_mask(                                          # 重写这部分函数，获取对应的attention mask
        #     attention_mask, (batch_size, seq_length), cache_length, inputs_embeds.dtype
        # )  # [bs, 1, seq_len, seq_len]
        # attention_mask = self._prepare_block_attention_mask(block_lengths, (batch_size, seq_length), inputs_embeds.dtype)  # [bs, 1, seq_len, seq_len]
        attention_mask = self._prepare_block_attention_mask(
            block_lengths, max_length, inputs_embeds.dtype
        )  # [1, 1, seq_len, seq_len]
...
class LlamaForCausalLM(LlamaPretrainedModel):
    ...
    def forward(
        ...
        # print("This is LLamaforcausal!!")
        # print(block_lengths.shape)
        # print(block_lengths)
        # print(position_ids)
        # print(f"causal:{self.config}")
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
