...
class DISCOCache:
    ...
    def __init__(
        ...
        for i in range(num_layers):
            self.layer_eviction_managers[i] = LayerwiseEvictionManager(
                cache_size=self.layer_budget[i], window_size=window_size, gamma=gamma
            )
    ...
    def _evict_tokens(self, layer_idx: int, attention_weights: Optional[Tensor] = None):
        ...
        if attention_weights is not None:
            scores = eviction_manager.compute_scores(key_cache, value_cache, attention_weights)
        else:
...
class LayerwiseEvictionManager:
    ...
    def compute_scores(self, key_states: Tensor, value_states: Tensor, attention_weights: Tensor) -> Tensor:
        ...
        if scores.shape[-1] > 5:
            ...
            scores = F.avg_pool1d(scores_reshaped, kernel_size=kernel_size, stride=1, padding=kernel_size // 2)
            # Reshape back
            scores = scores.squeeze(1).reshape(original_shape)

        return scores

    def compute_distance_scores(self, key_states: Tensor, value_states: Tensor) -> Tensor:
...
def get_scores_with_kv_fusion(key_states: Tensor, value_states: Tensor, window_size: int = 32) -> Tensor:
    """
