...
class LlamaAttentionAuto(nn.Layer):
    ...
    def forward(
        ...
        if self.config.sequence_parallel:
            ...
            attn_output = dist.reshard(attn_output, get_mesh(self.ipp), [dist.Shard(1), dist.Shard(0)])
...
class LlamaModelAuto(nn.Layer):
    def __init__(self, config):
        ...
        self.layers = nn.LayerList(
            [LlamaDecoderLayerAuto(config, False, get_layer_ipp(i)) for i in range(config.num_hidden_layers)]
        )
...
class LlamaForCausalLMAuto(PretrainedModel):
    ...
    def forward(
        ...
        if self.config.sequence_parallel:
            hidden_states = dist.reshard(hidden_states, get_mesh(-1), [dist.Shard(1), dist.Replicate()])
            # [S, B, H] -> [B, S, H]
