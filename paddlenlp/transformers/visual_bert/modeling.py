...
class VisualBertPreTrainedModel(PretrainedModel):
    ...
    # 参考: https://github.com/huggingface/transformers/blob/bda1cb02360fc9d290636cfcb6bcbeb4a18484ce/src/transformers/models/visual_bert/configuration_visual_bert.py
...
class VisualBertModel(VisualBertPreTrainedModel):
    ...
    def forward(
        ...
        extended_attention_mask = combined_attention_mask.broadcast_to(
            [batch_size, 1, combined_seq_length, combined_seq_length])
...
class VisualBertForQuestionAnswering(VisualBertPreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.visual_bert(input_ids,
        ...
        index_to_gather = index_to_gather.unsqueeze(0).astype(paddle.int64)
        pooled_output = paddle.concat([
            paddle.gather(sequence_output[b], index_to_gather, axis=0).unsqueeze(0)
        ...
        reshaped_logits = paddle.reshape(logits, shape=[-1, self.num_classes]) #TODO
