...
def _reshard_output(x, group, split_axis=0, concat_axis=2, batch_major_in=True, batch_major_out=True):
    ...
    assert (
        x_tensor.shape[split_axis] % nranks == 0
    ), f"Only support evenly split, x_tensor.shape:{x_tensor.shape}, split_axis:{split_axis} nranks:{nranks}"
...
def _to_head(x, group):
    ...
    comm_tensor = paddle.concat(paddle.split(x, nranks, axis=1), axis=0)
...
def _to_seqlen(x, group):
    ...
    reshard_tensor = paddle.concat(paddle.split(comm_tensor, nranks, axis=0), axis=1)
    reshard_tensor.reshape_([shape[0] // nranks, shape[1], shape[2] * nranks])
    return reshard_tensor
...
class ReshardOutProjection(PyLayer):
    ...
    def backward(ctx, dy):
        ...
        res = _reshard_output(
            ...
            batch_major_in=ctx.batch_major_in,
            batch_major_out=ctx.batch_major_out,
        )
...
def prepare_data_auto(batch_size=2, seq_len=2, num_head=2, h=4):
    ...
    for rank in range(sep):
        t = paddle.to_tensor(
            np.reshape(
                np.arange(rank * num_elem, (rank + 1) * num_elem) + 1,
                [seq_len, batch_size, num_head // sep, h // num_head],
            ),
            dtype=paddle.float32,
        )
        input_data_list.append(t)
    ...
    print(
        f"prepare_data_auto, input_data:{input_data}, bin_bout_expected_output_data:{bin_bout_expected_output_data}, bin_sout_expected_output_data:{bin_sout_expected_output_data}, \
          sin_bout_expected_output_data:{sin_bout_expected_output_data}, sin_sout_expected_output_data:{sin_sout_expected_output_data}"
    )
    return (
        input_data,
...
def prepare_data(batch_major=True, dim_size=4, batch_size=2, seq_len=2, num_head=2, h=4, split_axis=0, concat_axis=2):
    ...
    print(
        f"prepare_data_auto, input_data:{input_data}, expected_output_data:{expected_output_data}, transpose_expected_output_data:{transpose_expected_output_data}"
    )
...
def main():
    ...
    (
        input_data2,
        bin_bout_expected_output_data2,
        bin_sout_expected_output_data2,
        sin_bout_expected_output_data2,
        sin_sout_expected_output_data2,
    ) = prepare_data_auto(batch_size=2)
