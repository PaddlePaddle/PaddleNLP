...
class SPTokenizer:
    def __init__(self, model_path: str):
        ...
        special_tokens = ["[MASK]", "[gMASK]", "[sMASK]", "sop", "eop"] + role_special_tokens + ["</s>", "<unk>"]
        self.special_tokens = {}
...
class ChatGLMv3Tokenizer(PretrainedTokenizer):
    resource_files_names = {"vocab_file": "tokenizer.model"}
    model_input_names = ["input_ids", "attention_mask", "position_ids"]
    pretrained_resource_files_map = {
        "model_file": {
            "THUDM/chatglm3-6b": "https://paddlenlp.bj.bcebos.com/models/community/THUDM/chatglm3-6b/tokenizer.model",
        }
    }
    def __init__(
            self,
    ...
    # difference
    def build_chat_input(self, query, history=None, role="user", max_length=None,
                         padding_strategy=PaddingStrategy.DO_NOT_PAD, return_tensors="pd"):
        if history is None:
            history = []
        input_ids = []
        for item in history:
        ...
        return self._batch_encode_plus([input_ids], return_tensors=return_tensors, is_split_into_words=True,
                                       max_length=max_length, padding_strategy=padding_strategy)
