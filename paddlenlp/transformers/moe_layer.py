...
class MoELayer(nn.Layer):
    ...
    def forward_flex_token(self, hidden_states: paddle.Tensor):
        ...
        if DSV3_USE_FP8_GEMM:
            ...
            print("self.config.recompute_fwd_gate_up: ", self.config.recompute_fwd_gate_up)
            print("self.config.dequant_input: ", self.config.dequant_input)
            print("self.config.is_split_group_gemm: ", self.config.is_split_group_gemm)
            output = FusionMoe.apply(hidden_states, token_indices, token_probs, self, recompute_fwd_gate_up=self.config.recompute_fwd_gate_up, dequant_input=self.config.dequant_input, is_split_group_gemm=self.config.is_split_group_gemm)
        else:
            (
                dispatched_input,
...
class FusionMlpNode:
    ...
    def __init__(self, custom_map, max_topk, recompute_fwd_gate_up=False, dequant_input=False, is_split_group_gemm=True):
...
class FusionMoeNode:
    def __init__(self, custom_map, recompute_fwd_gate_up=False, dequant_input=False, is_split_group_gemm=True, name="fusion_moe_node"):
        self.token_dispatcher = custom_map.token_dispatcher
        ...
        self.mlp_node = FusionMlpNode(custom_map, self.moe_router_topk, recompute_fwd_gate_up=recompute_fwd_gate_up, dequant_input=dequant_input, is_split_group_gemm=is_split_group_gemm)
...
class FusionMoe(paddle.autograd.PyLayer):
    @staticmethod
    def forward(ctx, hidden_states, probs, routing_map, custom_map, recompute_fwd_gate_up=False, dequant_input=False, is_split_group_gemm=True):
        ctx.node = FusionMoeNode(custom_map, recompute_fwd_gate_up=recompute_fwd_gate_up, dequant_input=dequant_input, is_split_group_gemm=is_split_group_gemm)
