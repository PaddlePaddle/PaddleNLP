...
class MlpNode:
    ...
    def forward(self, hs_2d_dispatched, dispatched_indices, dispatched_probs):
        ...
        expert_out = self.experts_group_gemm_node.forward(unzipped_tokens, tokens_per_expert)
        ...
        return expert_out_zipped.to(paddle.bfloat16)

    @paddle.no_grad()
    def backward(self, hidden_states_out_grad):
        # zip backward
        unzipped_grad, probs_grad = self.zip_node.backward(
        ...
        expert_out = self.experts_group_gemm_node.backward(unzipped_grad, tokens_per_expert, expected_m)
...
class FusionMoeNode:
    ...
    @paddle.no_grad()
    def backward(self, output_grad):
        output_combie_grad_bf16 = self.combine_quant_node.backward(output_grad)
