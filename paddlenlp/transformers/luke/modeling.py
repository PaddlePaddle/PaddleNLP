    ...
    def init_weights(self, layer):
        ...
        if isinstance(layer, (nn.Linear, nn.Embedding)):
            ...
            layer._epsilon = self.layer_norm_eps if hasattr(
                self,
                "layer_norm_eps") else self.luke.config["layer_norm_eps"]


class LukeSelfOutput(nn.Layer):
    """Luke self output"""

    def __init__(self, hidden_size, layer_norm_eps=1e-6, hidden_dropout_prob=0.1):
...
class LukeEmbeddings(BaseEmbeddings):
    ...
    def __init__(self,
                 ...
                 layer_norm_eps=1e-6,
                 hidden_dropout_prob=0.1):
        super(LukeEmbeddings, self).__init__(vocab_size,
                                             hidden_size,
                                             max_position_embeddings,
                                             type_vocab_size,
                                             layer_norm_eps,
                                             hidden_dropout_prob)
        self.padding_idx = 1
        self.word_embeddings = nn.Embedding(vocab_size, hidden_size, padding_idx=self.padding_idx)
        self.position_embeddings = nn.Embedding(max_position_embeddings, hidden_size,
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None):
        ...
        if position_ids is None:
            ...
            position_ids = position_ids.unsqueeze(0).expand(input_shape)
        return super(LukeEmbeddings, self).forward(input_ids,
...
class EntityAwareSelfAttention(nn.Layer):
    ...
    def forward(self, word_hidden_states, entity_hidden_states, attention_mask):
        ...
        key_layer = self.transpose_for_scores(
            self.key(paddle.concat([word_hidden_states, entity_hidden_states], axis=1)))
...
class EntityAwareAttention(nn.Layer):
    def __init__(self,
        ...
        self.self = EntityAwareSelfAttention(num_attention_heads=num_attention_heads,
    ...
    def forward(self, word_hidden_states, entity_hidden_states, attention_mask):
        word_self_output, entity_self_output, attention_probs = self.self(word_hidden_states,
...
class EntityAwareLayer(nn.Layer):
    def __init__(self,
        ...
        self.attention = EntityAwareAttention(hidden_size=hidden_size,
                                              layer_norm_eps=layer_norm_eps,
                                              hidden_dropout_prob=hidden_dropout_prob,
                                              num_attention_heads=num_attention_heads,
                                              attention_probs_dropout_prob=attention_probs_dropout_prob)
        self.intermediate = LukeIntermediate(hidden_size=hidden_size,
                                             intermediate_size=intermediate_size)
        self.output = LukeOutput(hidden_size=hidden_size,
                                 intermediate_size=intermediate_size,
                                 layer_norm_eps=layer_norm_eps,
                                 hidden_dropout_prob=hidden_dropout_prob)
...
class EntityAwareEncoder(nn.Layer):
    def __init__(self,
        ...
        self.layer = nn.LayerList([EntityAwareLayer(hidden_size=hidden_size,
                                                    intermediate_size=intermediate_size,
                                                    layer_norm_eps=layer_norm_eps,
                                                    hidden_dropout_prob=hidden_dropout_prob,
                                                    num_attention_heads=num_attention_heads,
...
class LukeModel(LukePretrainedModel):
    ...
    def __init__(self,
        ...
        self.encoder = EntityAwareEncoder(num_hidden_layers=num_hidden_layers,
                                          hidden_size=hidden_size,
                                          intermediate_size=intermediate_size,
                                          layer_norm_eps=layer_norm_eps,
                                          hidden_dropout_prob=hidden_dropout_prob,
                                          num_attention_heads=num_attention_heads,
                                          attention_probs_dropout_prob=attention_probs_dropout_prob)
        self.embeddings = LukeEmbeddings(vocab_size=vocab_size,
                                         hidden_size=hidden_size,
        ...
        self.entity_embeddings = EntityEmbeddings(entity_vocab_size=entity_vocab_size,
    ...
    def forward(
        ...
        word_hidden_state, entity_hidden_state, attention_probs = self.encoder(word_embeddings,
                                                                               entity_embeddings,
                                                                               attention_mask)
...
class EntityPredictionHead(nn.Layer):
    def __init__(self,
                 ...
                 layer_norm_eps):
        super().__init__()
        self.transform = EntityPredictionHeadTransform(hidden_size=hidden_size,
                                                       entity_emb_size=entity_emb_size,
                                                       layer_norm_eps=layer_norm_eps)
...
class LukeForMaskedLM(LukePretrainedModel):
    def __init__(self, luke):
        ...
        self.lm_head = LukeLMHead(vocab_size=self.luke.config['vocab_size'],
                                  hidden_size=self.luke.config['hidden_size'],
                                  layer_norm_eps=self.luke.config['layer_norm_eps'])
        self.entity_predictions = EntityPredictionHead(hidden_size=self.luke.config['hidden_size'],
    ...
    def forward(
        ...
        if labels is not None:
            mlm_loss = self.loss_fn(logits.reshape((-1, self.vocab_size)),
                                    labels.reshape([-1]))
            if loss is None:
                loss = mlm_loss
        ...
        if entity_labels is not None:
            mep_loss = self.loss_fn(entity_logits.reshape((-1, self.entity_vocab_size)),
        ...
        output = (logits, entity_logits, outputs[0], outputs[1], outputs[2], outputs[3])
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if len(labels.shape) == 1:
                ...
                loss = nn.functional.binary_cross_entropy_with_logits(logits.reshape([-1]),
...
class LukeForEntityPairClassification(LukePretrainedModel):
    def __init__(self, luke, num_labels):
        ...
        self.num_labels = num_labels
        self.dropout = nn.Dropout(self.luke.config['hidden_dropout_prob'])
        self.classifier = nn.Linear(self.luke.config['hidden_size'] * 2, num_labels, bias_attr=False)
        self.apply(self.init_weights)
...
class LukeForEntitySpanClassification(LukePretrainedModel):
    def __init__(self, luke, num_labels):
        ...
        self.num_labels = num_labels
        self.dropout = nn.Dropout(self.luke.config['hidden_dropout_prob'])
        self.classifier = nn.Linear(self.luke.config['hidden_size'] * 3, num_labels)
        self.apply(self.init_weights)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if len(labels.shape) == 2:
                loss = nn.functional.cross_entropy(logits.reshape((-1, self.num_labels)),
                                                   labels.reshape([-1]))
            else:
                loss = nn.functional.binary_cross_entropy_with_logits(logits.reshape([-1]),
                                                                      labels.reshape([-1]).astype('float32'))
