...
class QWenAttention(nn.Layer):
    def __init__(self, config, skip_recompte_ops={}):
        super().__init__()
        ...
        if config.sequence_parallel:
            ...
            if config.recompute and not config.recompute_use_reentrant:
                if skip_recompte_ops.get("attention_column_ln", False):
                    ColumnParallelLinear = RRColumnSequenceParallelLinear
                if skip_recompte_ops.get("attention_row_ln", False):
...
class QWenMLP(nn.Layer):
    def __init__(self, config, skip_recompte_ops={}):
        ...
        if config.sequence_parallel:
            ...
            # NOTE: refined_recompute is only supported when `recompute_use_reentrant=False`
            if config.recompute and not config.recompute_use_reentrant:
                if skip_recompte_ops.get("mlp_column_ln", False):
                    ColumnParallelLinear = RRColumnSequenceParallelLinear
                if skip_recompte_ops.get("mlp_row_ln", False):
                    RowParallelLinear = RRRowSequenceParallelLinear
        else:
            ColumnParallelLinear = linear_utils.ColumnParallelLinear
            RowParallelLinear = linear_utils.RowParallelLinear
...
class QWenBlock(nn.Layer):
    def __init__(self, config, skip_recompte_ops={}):
        ...
        self.attn = QWenAttention(config, skip_recompte_ops=skip_recompte_ops)
        self.ln_2 = QWenRMSNorm(config)
        self.mlp = QWenMLP(config, skip_recompte_ops=skip_recompte_ops)
