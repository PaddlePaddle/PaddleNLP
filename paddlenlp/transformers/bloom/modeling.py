        ...
        **kwargs,
        ...
        if position_ids is not None:
            ...
            if use_pre_caches:
                ...
                alibi = build_alibi_tensor(alibi_attention_mask, self.config.n_head, dtype=hidden_states.dtype)
            else:
                alibi = build_alibi_tensor(attention_mask, self.config.n_head, dtype=hidden_states.dtype)
        ...
        if is_decoder:
            ...
            if use_pre_caches:
                pre_caches_attention_mask = paddle.zeros(
                    causal_mask.shape[:-1] + [pre_caches_length], dtype=causal_mask.dtype
                )
            ...
            if use_pre_caches:
                causal_mask = (
                    ...
                    .unsqueeze(0)
                    .unsqueeze(0)
                    .expand([input_ids_shape[0], self.config.n_head, -1, -1])
                )
                pre_caches_attention_mask = paddle.zeros(
                    [input_ids_shape[0], self.config.n_head, input_ids_shape[-1], pre_caches_length],
                    dtype=paddle.get_default_dtype(),
                )
                causal_mask = paddle.concat([pre_caches_attention_mask, causal_mask], axis=-1)
            else:
