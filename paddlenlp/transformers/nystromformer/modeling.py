...
class NystromformerSelfAttention(nn.Layer):
    ...
    def iterative_inv(self, mat, n_iter=6):
        ...
        if self.init_option == "original":
            ...
            value = 1 / paddle.max(
                paddle.sum(key, axis=-2),
                axis=-1).values[:, :, None, None] * trans_matrix(key)
        for _ in range(n_iter):
            key_value = paddle.matmul(key, value)
            value = paddle.matmul(
                0.25 * value,
                13 * identity - paddle.matmul(
                    key_value, 15 * identity - paddle.matmul(
                        key_value, 7 * identity - key_value)), )
        return value
    ...
    def forward(self,
        ...
        if self.num_landmarks == self.seq_len:
            ...
            q_landmarks = query_layer.reshape(
                ...
                self.attention_head_size, ).mean(axis=-2)
            ...
            kernel_1 = nn.functional.softmax(
                paddle.matmul(query_layer, trans_matrix(k_landmarks)), axis=-1)
            kernel_2 = nn.functional.softmax(
                paddle.matmul(q_landmarks, trans_matrix(k_landmarks)), axis=-1)
    ...
    def forward(self,
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                loss = loss_fct(
                    logits.reshape([-1, self.num_labels]), labels.flatten())
