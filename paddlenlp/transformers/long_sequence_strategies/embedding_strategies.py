...
class YaRNScalingRotaryEmbedding(nn.Layer):
    ...
    def yarn(self):
        ...
        low, high = self._yarn_find_correction_range(self.beta_fast, self.beta_slow, self.dim, self.base, self.original_max_position_embeddings)
        inv_freq_mask = (1 - paddle.cast(self._yarn_linear_ramp_mask(low, high, self.dim // 2), dtype=paddle.float32)) * self.extrapolation_factor
