...
class YARNScalingRotaryEmbedding(nn.Layer):
    """RotaryEmbedding extended with Dynamic YARN scaling."""
    ...
    def __init__(self, **init_args):
        ...
        self.max_position_embeddings = init_args["max_position_embeddings"]
        self.base = init_args["base"]
        self.scaling_factor = init_args["scaling_factor"]  # scaling_factor
        self.original_max_position_embeddings = init_args["original_max_position_embeddings"]
        self.extrapolation_factor = init_args["extrapolation_factor"]
        self.attn_factor = init_args["attn_factor"]
        self.beta_fast = init_args["beta_fast"]
        self.beta_slow = init_args["beta_slow"]

        self.yarn()

        self._set_cos_sin_cache(seq_len=self.max_position_embeddings)

    # def __init__(self, dim, max_position_embeddings=2048, base=10000, scaling_factor=1, original_max_position_embeddings=2048, extrapolation_factor=1, attn_factor=1, beta_fast=32, beta_slow=1):
    #     super().__init__()
    #     print("LLaMA YaRN Init!!!!")
    #     self.dim = dim
    ...
    # 修改为类方法
    def _yarn_linear_ramp_mask(self, low, high, dim):
        if low == high:
    ...
    def _yarn_get_mscale(self, scaling_factor=1):
        if scaling_factor <= 1:
            return 1.0
