...
def padding_zeros(paddling_num, origin_num, original_tensor):
    padding_rows = paddling_num - origin_num
    # 创建全零的Tensor
    padding_tensor = paddle.zeros([padding_rows, original_tensor.shape[-1]], dtype=original_tensor.dtype)
    # 行方向拼接
    padded_tensor = paddle.concat([original_tensor, padding_tensor], axis=0)
...
class ExpertsGroupGemmNode:
    ...
    def bwd_dowm_input(self, expert_w2, unzipped_grad, tokens_per_expert, expected_m):
        ...
        # do2
        do2 = paddle.zeros([len(expert_w2), unzipped_grad_fp8.shape[1], bw_w2_quant.shape[1]], dtype="bfloat16")
        deep_gemm.m_grouped_gemm_fp8_fp8_bf16_nt_masked(
    ...
    #     #裁剪o1去掉padding的0
    #     self_o1_1 = self.o1[0][0:tokens_per_expert[0]]
    #     self_o1_2 = self.o1[1][0:tokens_per_expert[1]]
    #     self_o1_3 = self.o1[2][0:tokens_per_expert[2]]
    #     self_o1_4 = self.o1[3][0:tokens_per_expert[3]]

    #     #裁剪do2去掉padding的0
    #     do2_1 = do2[0][0:tokens_per_expert[0]]
    #     do2_2 = do2[1][0:tokens_per_expert[1]]
    #     do2_3 = do2[2][0:tokens_per_expert[2]]
    #     do2_4 = do2[3][0:tokens_per_expert[3]]

    #     #计算四次swiglu_grad
    #     do1_1,_ = paddle._C_ops.swiglu_grad(self_o1_1, None, do2_1)
    #     do1_2,_ = paddle._C_ops.swiglu_grad(self_o1_2, None, do2_2)
    #     do1_3,_ = paddle._C_ops.swiglu_grad(self_o1_3, None, do2_3)
    #     do1_4,_ = paddle._C_ops.swiglu_grad(self_o1_4, None, do2_4)

    #     #算完之后padding 0
    #     do1_1_new = padding_zeros(do2.shape[1],do1_1.shape[0],do1_1)
    #     do1_2_new = padding_zeros(do2.shape[1],do1_2.shape[0],do1_2)
    #     do1_3_new = padding_zeros(do2.shape[1],do1_3.shape[0],do1_3)
    #     do1_4_new = padding_zeros(do2.shape[1],do1_4.shape[0],do1_4)

    #     # concat
    #     do1 = paddle.concat([do1_1_new,do1_2_new,do1_3_new,do1_4_new],axis=0)

    #     return do1

    def bwd_swiglu(self, o1, do2, tokens_per_expert):
        assert len(o1) == len(do2) == len(tokens_per_expert), "输入列表长度必须一致"
        num_experts = len(o1)
        do1_list = []

        for i in range(num_experts):
            # 裁剪 o1 和 do2，去掉 padding 的 0
            expert_o1 = o1[i][: tokens_per_expert[i]]
            expert_do2 = do2[i][: tokens_per_expert[i]]

            # swiglu backward
            do1, _ = paddle._C_ops.swiglu_grad(expert_o1, None, expert_do2)

            # 零填充到原始形状
            padded_do1 = padding_zeros(do2.shape[1], do1.shape[0], do1)

            do1_list.append(padded_do1)

        # 拼接所有专家的梯度
        do1 = paddle.concat(do1_list, axis=0)
        return do1
    ...
    def bwd_down_weight(self, out_grad, o2, expert_w2):
        ...
        o2_t_scale = o2_t_scale.reshape([group_num, o2_t_scale.shape[0], -1])
        ...
        for i in range(len(expert_w2)):
            if hasattr(expert_w2[i], "main_grad"):
                expert_w2[i].main_grad = kitchen_fp8_gemm(
                    o2_t_fp8[i],  # 2048,8448
                    o2_t_scale[i],  # 66,2048
                    out_grad_fp8[i],  # 7168,8448
                    out_grad_scale[i],  # 66,7168
                    True,
                    True,
    ...
    @paddle.no_grad()
    def forward(self, hs_out, tokens_per_expert):
        # get w1
    ...
    @paddle.no_grad()
    def backward(self, out_grad, tokens_per_expert, expected_m):
        # recompute expert_w2 and expert_w1
        expert_w2 = [x.w2 for x in self.custom_map.experts if x is not None]
        expert_w1 = [x.w1 for x in self.custom_map.experts if x is not None]

        # do2
        do2, o2 = self.bwd_dowm_input(expert_w2, out_grad, tokens_per_expert, expected_m)

        # do1
        do1 = self.bwd_swiglu(self.o1, do2, tokens_per_expert)
        ...
        # dw2
        self.bwd_down_weight(out_grad, o2, expert_w2)
