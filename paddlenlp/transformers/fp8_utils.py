...
class ExpertsGroupGemmNode:
    ...
    def bwd_dowm_input(self, expert_w2, unzipped_grad, unzipped_scale, tokens_per_expert):
        ...
        deep_gemm.m_grouped_gemm_fp8_fp8_bf16_nt_masked(
            (unzipped_grad, unzipped_scale), (bw_w2_quant, bw_w2_scale), do2, tokens_per_expert, unzipped_grad.shape[1]
        )
    ...
    def bwd_gate_up_input(self, do1, expert_w1, tokens_per_expert):
    ...
    def bwd_down_weight(self, out_grad, o2, expert_w2):
        ...
        # o2 padding后[4, 8448, 2048]
        o2_t = o2.reshape([o2.shape[1] // 4, -1])  # [8448, 4*2048]

        # o2_t=o2.reshape([-1, H2])
        _, _, o2_t_fp8, o2_t_scale = kitchen_quant(
            o2_t, backend=kitchen.ops.Backend.CUTLASS, is_1d_scaled=True, return_transpose=True
        )

        o2_t_fp8 = o2_t_fp8.reshape([len(expert_w2), H2, -1])  # [4, 2048, 8448]
        o2_t_scale = o2_t_scale.reshape([len(expert_w2), H2, -1])

        # quant out_grad
        H1 = out_grad.shape[-1]
        out_grad = out_grad.reshape([out_grad.shape[0] // 4, -1])  # [7196,33792]
        _, _, out_grad_fp8, out_grad_scale = kitchen_quant(
            out_grad, backend=kitchen.ops.Backend.CUTLASS, is_1d_scaled=True, return_transpose=True
        )

        out_grad_fp8 = out_grad_fp8.reshape([len(expert_w2), H1, -1])  # [4, 8448, 7196]
        out_grad_scale = out_grad_scale.reshape([len(expert_w2), H1, -1])
    ...
    def bwd_gate_up_weight(self, do1, input_x, expert_w1):
        ...
        H1 = input_x.shape[-1]
        input_x = input_x.reshape([input_x.shape[0] // 4, -1])
        _, _, input_x_fp8, input_x_scale = kitchen_quant(
            input_x, backend=kitchen.ops.Backend.CUTLASS, is_1d_scaled=True, return_transpose=True
        )
        input_x_fp8 = input_x_fp8.reshape([len(expert_w1), H1, -1])
        input_x_scale = input_x_scale.reshape([len(expert_w1), H1, -1])

        # transpose do1 and quant do1
        do1 = do1.reshape([do1.shape[1], -1])
        _, _, do1_fp8, do1_scale = kitchen_quant(
            do1, backend=kitchen.ops.Backend.CUTLASS, is_1d_scaled=True, return_transpose=True
        )
        do1_fp8 = do1_fp8.reshape([len(expert_w1), -1, do1_fp8.shape[-1]])
        do1_scale = do1_scale.reshape([len(expert_w1), -1, do1_scale.shape[-1]])
    ...
    @paddle.no_grad()
    def backward(self, out_grad, out_grad_scale, tokens_per_expert, dispatched_indices):
