...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    ...
    def _tie_or_clone_weights(self, output_embeddings, input_embeddings):
        ...
        if hasattr(output_embeddings,
                   "bias") and output_embeddings.bias is not None:
        ...
        if state_dict is None and not from_tf:
            try:
                ...
                for i, (key, value) in enumerate(state_dict.items()):
                    if len(layer_params) == 16:
                        # params' order in encoder layer are different with paddle, thus reorder
                        params.extend(layer_params[:8])
                        params.extend(layer_params[10:14])
                        params.extend(layer_params[8:10])
                        params.extend(layer_params[14:])
                        print("layer params: ",
                              len(layer_params), len(params), layer_params_keys)
                        layer_params = []
                        layer_params_keys = []
                    if ".encoder.layer" not in key:
                        if key == 'cls.predictions.bias':
                            params.extend([params[0], value.numpy()])
                            continue
                        # use weight sharing between pretraing output layer and embedding layer, thus no need to convert this param
                        if key == 'cls.predictions.decoder.weight': continue
                        # weight of linear should be transposed to be consistent
                        if len(value.shape) == 2 and "embeddings" not in key:
                            value = value.transpose(0, 1)
                        params.append(value.numpy())
                    else:
                        if len(value.shape) == 2 and "embeddings" not in key:
                            value = value.transpose(0, 1)
                        layer_params.append(value.numpy())
                        layer_params_keys.append(key)
                import pickle
                with open("params.pd", 'wb') as f:
    ...
    def generate(
        ...
        assert (eos_token_ids is None) or (
            isinstance(eos_token_ids, (list, tuple)) and (
    ...
    def _generate_no_beam_search(
        ...
        for hypo_idx, hypo in enumerate(input_ids):
            decoded[hypo_idx, :sent_lengths[hypo_idx]] = hypo[:sent_lengths[
                hypo_idx]]
        ...
        while cur_len < max_length:
            ...
            if do_sample:
                ...
                next_words = torch.multinomial(
                    F.softmax(
                        scores, dim=-1),
                    num_samples=2)  # (batch_size * num_beams, 2)
                # Compute next scores
            ...
            assert next_scores.size() == next_words.size() == (batch_size, 2 *
                                                               num_beams)
            ...
            for batch_idx in range(batch_size):
                ...
                for idx, score in zip(next_words[batch_idx],
                    ...
                    if eos_token_ids is not None and word_id.item(
                    ) in eos_token_ids:
                        generated_hyps[batch_idx].add(input_ids[
                            batch_idx * num_beams + beam_id, :cur_len].clone(),
                                                      score.item())
                    else:
    ...
    def forward(self,
        ...
        if start_positions is not None and end_positions is not None:
            ...
            end_top_log_probs = end_top_log_probs.view(-1, self.start_n_top *
                                                       self.end_n_top)
            end_top_index = end_top_index.view(-1, self.start_n_top *
                                               self.end_n_top)
...
class SequenceSummary(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig):
        ...
        self.activation = (get_activation(activation_string)
                           if activation_string else
                           Identity())  # type: typing.Callable
