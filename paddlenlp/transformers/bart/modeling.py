...
class BartModel(BartPretrainedModel):
    ...
    def __init__(self,
        ...
        encoder = nn.TransformerEncoder(encoder_layer, encoder_layers)
        decoder = nn.TransformerDecoder(decoder_layer, decoder_layers)
...
class BartForSequenceClassification(BartPretrainedModel):
    ...
    def forward(self, input_ids, decoder_input_ids=None):
        output = self.bart(input_ids, decoder_input_ids)
        eos_mask = paddle.cast(
            output == self.bart.config['eos_token_id'], dtype='float32')
...
class BartForConditionalGeneration(BartPretrainedModel):
    def __init__(self, bart):
        super().__init__()
        self.bart = bart
        self.lm_head = nn.Linear(self.bart.config['d_model'],
                                 self.bart.config['vocab_size'])
    ...
    def forward(self, input_ids, decoder_input_ids=None):
        output = self.bart(input_ids, decoder_input_ids)
        lm_logits = self.lm_head(output) + self.final_logits_bias
        return lm_logits
