...
def _make_causal_mask(input_ids_shape, past_key_values_length, dtype):
    ...
    mask = paddle.full((target_length, target_length), -1000)
...
class TransformerDecoder(Layer):
    ...
    def __init__(self, config: OPTConfig, decoder_layers: List[Layer]):
        ...
        print("self.project_out", self.project_out)
    ...
    def forward(
        ...
        print("output", output)
        array_dict = {'output':output}
        import numpy as np
        np.savez('/zhoukangkang/output.npz', **array_dict)
