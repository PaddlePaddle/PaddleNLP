...
class DynamicCache(Cache):
    ...
    def update(
        ...
        if layer_idx == 0:
            self._seen_tokens += key_states.shape[-3]
...
class OffloadedCache(DynamicCache):
    ...
    def update(
        ...
        if len(self.key_cache) < layer_idx:
            ...
            self.key_cache[layer_idx] = paddle.concat([key_tensor, key_states], axis=-3)
            self.value_cache[layer_idx] = paddle.concat([value_tensor, value_states], axis=-3)
...
class SinkCache(Cache):
    ...
    def _get_rerotation_cos_sin(
        ...
        if key_states.shape[-3] not in self.cos_sin_rerotation_cache:
            # Upcast to float32 temporarily for better accuracy
            cos = cos.to(paddle.float32)
            sin = sin.to(paddle.float32)

            # Compute the cos and sin required for back- and forward-rotating to one position earlier in the sequence
            original_cos = cos[self.num_sink_tokens + key_states.shape[-3] :]
            shifted_cos = cos[self.num_sink_tokens : -key_states.shape[-3]]
            original_sin = sin[self.num_sink_tokens + key_states.shape[-3] :]
            shifted_sin = sin[self.num_sink_tokens : -key_states.shape[-3]]
            rerotation_cos = original_cos * shifted_cos + original_sin * shifted_sin
            rerotation_sin = -original_sin * shifted_cos + original_cos * shifted_sin

            self.cos_sin_rerotation_cache[key_states.shape[-3]] = (
                rerotation_cos.to(key_states.dtype).unsqueeze(0),
                rerotation_sin.to(key_states.dtype).unsqueeze(0),
            )
        return self.cos_sin_rerotation_cache[key_states.shape[-3]]

    def get_seq_length(self, layer_idx: Optional[int] = 0) -> int:
        """Returns the sequence length of the cached states. A layer index can be optionally passed."""
        # TODO: deprecate this function in favor of `cache_position`
        # Workaround to make 'key_states.shape[-3] + past_key_value.get_seq_length(self.layer_idx)' <= window_length
        if len(self.key_cache) <= layer_idx:
            return 0
    ...
    def update(
        ...
        if len(self.key_cache) <= layer_idx:
        ...
        elif key_states.shape[-3] + self.get_seq_length(layer_idx) < self.window_length:
            # Growing cache
            self.key_cache[layer_idx] = paddle.concat([self.key_cache[layer_idx], key_states], axis=-3)
            self.value_cache[layer_idx] = paddle.concat([self.value_cache[layer_idx], value_states], axis=-3)
            ...
            keys_to_keep = self.key_cache[layer_idx][
                :, :, -self.window_length + self.num_sink_tokens + key_states.shape[-3] :
            ]
            ...
            self.key_cache[layer_idx] = paddle.concat([sink_keys, keys_to_keep, key_states], axis=-3)

            sink_values = self.value_cache[layer_idx][:, :, : self.num_sink_tokens]
            values_to_keep = self.value_cache[layer_idx][
                :, :, -self.window_length + self.num_sink_tokens + value_states.shape[-3] :
            ]
            self.value_cache[layer_idx] = paddle.concat([sink_values, values_to_keep, value_states], axis=-3)
...
class StaticCache(Cache):
    ...
    Parameters:
        ...
        layer_device_map(`Dict[int, Union[str, int]]]`, `optional`):
    ...
    def __init__(
        ...
        for idx in range(config.num_hidden_layers):
            ...
            self.register_buffer(f"key_cache_{idx}", paddle.zeros(cache_shape, dtype=dtype))
            self.register_buffer(f"value_cache_{idx}", paddle.zeros(cache_shape, dtype=dtype))
...
class HybridCache(Cache):
    ...
    def __init__(
        ...
        self.is_sliding = paddle.tensor(
            [not bool(i % 2) for i in range(config.num_hidden_layers)], dtype=paddle.bool, device=device
        )
        self.key_cache: List[paddle.Tensor] = []
        self.value_cache: List[paddle.Tensor] = []
...
class MambaCache:
    ...
    def __init__(
        ...
        self.conv_states: paddle.Tensor = paddle.zeros(
        ...
        self.ssm_states: paddle.Tensor = paddle.zeros(
            config.num_hidden_layers,
            self.max_batch_size,
            self.intermediate_size,
            self.ssm_state_size,
...
class OffloadedStaticCache(StaticCache):
    ...
    def __init__(
        ...
        self.max_batch_size = max_batch_size
        self.max_cache_len = config.max_position_embeddings if max_cache_len is None else max_cache_len
        self.device = device if layer_device_map is None else layer_device_map[0]
    ...
    def _create_key_value_cache_tensors(
        self, shape: Tuple[int, ...], device: str
    ) -> Tuple[paddle.Tensor, paddle.Tensor]:
