...
class Mutil_dataset_PretrainingTrainer(Trainer):
    ...
    def pre_process(self, first=False):
        ...
        #         data_cache_path='/root/paddlejob/workspace/env_run/dyl/PaddleNLP/data/index-cache',
        #         load_data=False
        #     )
    ...
    def train(
        ...
        while self.state.global_step < max_steps:
            ...
            print(dataset_id, self.data[dataset_id*2+1])
            if front_dataset_id != dataset_id or self.train_dataset is None:
                if self.train_dataset is not None:
                    del train_dataset
                if self.eval_dataset is not None:
                    del val_dataset
                train_dataset, val_dataset, _ = build_train_valid_test_datasets(
                    # [self.data[dataset_id*2+1]], 
                    self.data[dataset_id*2:dataset_id*2+2], 
                    self.data_args.data_impl,
                    self.data_args.split,
                    self.train_val_test_num_samples,
                    self.data_args.max_seq_length,
                ...
                if args.local_rank == 0:
                    prepare_future = executor.submit(self.pre_process, True)
                    print(prepare_future.running())
                skip_step = 0
