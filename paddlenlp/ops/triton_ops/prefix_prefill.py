...
# Adapted from https://github.com/vllm-project/vllm/blob/main/vllm/attention/ops/prefix_prefill.py
...
if triton.__version__ >= "2.1.0":
    ...
    def _fwd_kernel(
        ...
        for start_n in range(0, cur_batch_ctx_len, BLOCK_N):
            ...
            k_load = tl.load(
                K_cache + off_k, mask=dim_mask[:, None] & ((start_n + offs_n[None, :]) < cur_batch_ctx_len), other=0.0
            )  # [D,N]
            ...
            if SLIDING_WINDOW > 0:
                ...
                # exp().
                qk = tl.where(
                    (cur_batch_ctx_len + offs_m[:, None]) - (start_n + offs_n[None, :]) < SLIDING_WINDOW, qk, -10000
                )
            ...
            # update acc
            v_load = tl.load(
                V_cache + off_v, mask=dim_mask[None, :] & ((start_n + offs_n[:, None]) < cur_batch_ctx_len), other=0.0
            )  # [N,D]
    ...
    def _fwd_kernel_alibi(
        ...
        q = tl.load(
            Q + off_q, mask=dim_mask[None, :] & (offs_m[:, None] < cur_batch_seq_len - cur_batch_ctx_len), other=0.0
        )
    ...
    def context_attention_fwd(
        ...
        # appropriate paddle.dtype for interpretation by Triton
        if "fp8" in kv_cache_dtype:
            assert k_cache.dtype == paddle.uint8
            assert v_cache.dtype == paddle.uint8

            if kv_cache_dtype in ("fp8", "fp8_e4m3"):
                target_dtype = paddle.float8_e4m3fn
            elif kv_cache_dtype == "fp8_e5m2":
                target_dtype = paddle.float8_e5m2
            else:
                raise ValueError("Unsupported FP8 dtype:", kv_cache_dtype)

            k_cache = paddle.cast(k_cache, target_dtype)
            v_cache = paddle.cast(v_cache, target_dtype)

        if k_cache.dtype == paddle.uint8 or v_cache.dtype == paddle.uint8 and kv_cache_dtype == "auto":
            raise ValueError(
