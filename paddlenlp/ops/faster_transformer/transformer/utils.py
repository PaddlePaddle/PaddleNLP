...
def weight_channel_wise_quantize(weight,
                                 max_range=127,
...
def index_CUBLASLT_ORDER_COL4_4R2_8C(col_id, row_id, m_32):
    ...
    new_row = (((((row_id >> 3) << 3) + ((row_id & 1) << 2) +
                 ((col_id & 31) >> 3)) << 5) +
               ((right_half + ((row_id & 7) >> 1)) << 2) + (col_id & 3))
...
def index_CUBLASLT_ORDER_COL32_2R_4R4(col_id, row_id, m_32):
    ...
    new_row = (((row_id >> 5) << 10) + (((((((row_in_tile & 7) >> 1) << 2) +
                                           (row_in_tile >> 3)) << 1) +
                                         (row_in_tile & 1)) << 5) + col_in_tile)
    return new_col * m_32 + new_row


def weight_COL_channel_wise_quantize(weight,
                                     max_range=127,
                                     use_fp16_decoding=False,
                                     sm=75):
    w_shape = weight.shape
    ...
    for i in range(w_shape[0]):
        for j in range(w_shape[1]):
            ...
            ele = tmp_weight[i, j]
            if sm == 75:
                idx_in_COL = index_CUBLASLT_ORDER_COL4_4R2_8C(
                    i, j, 32 * w_shape[1])
            else:
                idx_in_COL = index_CUBLASLT_ORDER_COL32_2R_4R4(
                    i, j, 32 * w_shape[1])
            ret_weight[int(idx_in_COL / w_shape[1]),
                       int(idx_in_COL % w_shape[1])] = round(ele * max_range /
                                                             scale)
