...
public:
  ...
  ~CublasHandle() { cublasDestroy(cublas_handle_); }
};

CublasHandle handle_ = CublasHandle();
...
std::vector<paddle::Tensor> encoder_kernel(
  ...
  if (normalize_before == false) {
    ...
    encoder_param.cublas_handle = handle_.cublas_handle_;
    encoder_param.cublaslt_handle = handle_.cublaslt_handle_;
...
std::vector<paddle::Tensor> EncoderCUDAForward(
  ...
  cublasSetStream(handle_.cublas_handle_, stream);
