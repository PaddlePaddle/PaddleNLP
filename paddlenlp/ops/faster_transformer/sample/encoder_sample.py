...
def parse_args():
    ...
    parser.add_argument(
        "--normalize_before",
        action="store_true",
        help="Use pre-normalization or post-normalization ")
...
def do_predict(args):
    ...
    # random generate seq_len for each sample
    # mem_seq_lens = paddle.randint(1, seq_len + 1, (batch_size, ), dtype="int32")
    mem_seq_lens = paddle.randint(
        args.seq_len, args.seq_len + 1, (batch_size, ), dtype="int32")
    ...
    print("PD src_mask attn:{}".format(src_slf_attn_bias))
    ...
    np.testing.assert_allclose(out1, out2, rtol=1.0, atol=1e-6)
...
if __name__ == "__main__":
    ...
    args.normalize_before = ARGS.normalize_before
