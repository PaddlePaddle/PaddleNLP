...
void sampling_init_kernelLauncher(bool* finished,
  ...
  sampling_init_kernel<<<grid, block, 0, stream>>>(
      finished, sequence_length, word_ids, start_id);
}
...
void embedding_lookup_sine_position_encoding_kernel_launcher(
  ...
  embedding_lookup_sine_position_encoding_kernel<T><<<grid, block, 0, stream>>>(
      from_tensor,
      embedding_table,
      position_encoding,
      word_ids,
...
void embedding_position_lookups_kernel_launcher(T* from_tensor,
  ...
  dim3 grid(min(batch_size, 65536));
  dim3 block(min(hidden_units, 1024));
  embedding_position_lookups_kernel<T><<<grid, block, 0, stream>>>(
      from_tensor,
      embedding_table,
      pos_table,
      word_ids,
...
__global__ void apply_temperature_penalty_kernel(T* logits,
                                                 ...
                                                 const int m,
                                                 const int n) {
  for (int index = blockIdx.x * blockDim.x + threadIdx.x; index < m * n;
       index += blockDim.x * gridDim.x) {
...
void apply_temperature_penalty_kernelLauncher(T* logits,
  ...
  apply_temperature_penalty_kernel<T><<<grid, block, 0, stream>>>(
      logits, temperature_inverse, m, n);
}
...
template <typename T>
__global__ void broadcast_kernel(T* log_probs,
                                 T* cum_log_probs,
  ...
  if (tid < N) log_probs[tid] += cum_log_probs[bid];
}
...
void broadcast_kernelLauncher(float* log_probs,
  ...
  broadcast_kernel<float><<<grid, block, 0, stream>>>(
      log_probs, cum_log_probs, vocab_size, N);
}

template <typename T>
__global__ void update_kernel(T* log_probs,
                              T* cum_log_probs,
                              bool* finished,
                              int* parent_ids,
                              int* sequence_length,
  ...
  int tid = threadIdx.x;
  sequence_length[tid] =
      finished[tid] ? sequence_length[tid] : sequence_length[tid] + 1;
...
void update_kernelLauncher(float* log_probs,
  ...
  update_kernel<float><<<grid, block, 0, stream>>>(log_probs,
                                                   cum_log_probs,
                                                   finished,
                                                   parent_ids,
                                                   sequence_length,
...
template <typename T>
__global__ void update_kernel_v2(bool* finished,
                                 int* parent_ids,
                                 int* sequence_length,
                                 int* word_ids,
                                 int* output_ids,
...
void update_kernelLauncher_v2(bool* finished,
  ...
  update_kernel_v2<float><<<grid, block, 0, stream>>>(finished,
                                                      parent_ids,
                                                      sequence_length,
                                                      word_ids,
                                                      output_ids,
...
__global__ void update_KV_cache_kernel(const T* __restrict key_src_cache,
  ...
  int batch_id =
      (blockIdx.x % (batch_size * beam_width * step)) / (beam_width * step);
...
__global__ void update_KV_cache_kernel(const half* __restrict key_src_cache,
  ...
  int tgt_hidden_id =
      (step_id * batch_size * beam_width * hidden_dim +
       batch_id * beam_width * hidden_dim + beam_id * hidden_dim) /
      2;

  const half2* key_src_ptr =
      (const half2*)key_src_cache + layer_id * cache_size / 2;
  half2* key_tgt_ptr = (half2*)key_tgt_cache + layer_id * cache_size / 2;
  const half2* value_src_ptr =
      (const half2*)value_src_cache + layer_id * cache_size / 2;
...
template <typename T>
__global__ void apply_logit_penalties_kernel(int step,
                                             int vocab_size,
                                             int beam_width,
                                             T* log_probs,
                                             int* current_ids,
  ...
  if (tid == 0 && bid == 0) {
    ...
    if (step > 1) {
      ...
      for (int i = step - 2; i > 0; --i) {
        prev_id =
            previous_ids[bbsize * i + batchid * beam_width + parent_beamid];
        if (log_probs[prev_id + bbid * vocab_size] > T(0)) {
          log_probs[prev_id + bbid * vocab_size] =
              float(log_probs[prev_id + bbid * vocab_size]) / repeat_penalty;
        } else {
...
void apply_logit_penalties(int step,
  ...
  apply_logit_penalties_kernel<T><<<grid, block, 0, stream>>>(
      step,
      vocab_size,
      beam_width,
      log_probs,
...
__global__ void transpose_kernel(data_type* __restrict__ out,
                                 ...
                                 int tW,
                                 int stride)
// int tH, int tW should be template parameters for the best performance, we do
// not do that sine the task is tiny.
// batch  stride (blockIdx.z dimension) for fully packed tensor ==  height *
// width
{
...
void transpose(data_type* out,
  ...
  transpose_kernel<
      data_type><<<grid, tW * tH, tH * tW * sizeof(data_type), stream>>>(
      out, in, height, width, tH, tW, stride);
}
...
__global__ void gather_tree_kernel(const int batch_size,
                                   const int max_time,
                                   const int beam_width,
                                   const int end_token,
                                   const int* step_ids,
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x;
       i < batch_size * beam_width;
       i += gridDim.x * blockDim.x) {
    const int batch = i / beam_width;
    const int beam = i % beam_width;

    const int max_seq_len_b =
        min(max_time, __ldg(max_sequence_lengths + batch));
    if (max_seq_len_b <= 0) {
      continue;
    }
...
void gather_tree_kernel_launcher(int max_time,
  ...
  gather_tree_kernel<<<grid, block, 0, stream>>>(batch_size,
                                                 max_time,
                                                 beam_width,
                                                 end_token,
                                                 step_ids,
...
template void embedding_position_lookups_kernel_launcher(
    float* from_tensor,
    const float* embedding_table,
