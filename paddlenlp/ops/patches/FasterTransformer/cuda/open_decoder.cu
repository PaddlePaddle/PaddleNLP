...
using Copy_half_t = typename std::conditional<
    ...
    typename std::conditional<
        HALF_ELEMENTS_PER_WARP_LOAD == 64,
        int,
        typename std::conditional<HALF_ELEMENTS_PER_WARP_LOAD == 128,
                                  int2,
                                  int4>::type>::type>::type;
...
template <typename T>
__global__ void add_bias_gelu(T* out, const T* bias, int m, int n) {
...
template <>
__global__ void add_bias_gelu(half* out, const half* bias, int m, int n) {
...
template <typename T>
__global__ void add_bias_relu(T* out, const T* bias, int m, int n) {
  for (int id = blockIdx.x * blockDim.x + threadIdx.x; id < m * n;
       id += blockDim.x * gridDim.x) {
    T reg_bias = __ldg(&bias[id % n]);
    T val = out[id] + reg_bias;
...
template <>
__global__ void add_bias_relu(half* out, const half* bias, int m, int n) {
  half2* out_ptr = (half2*)out;
  const half2* bias_ptr = (half2*)bias;
...
__global__ void masked_attention_kernel_opt(T* __restrict key_buf,
                                            T* __restrict value_buf,
                                            T* __restrict query_buf,
                                            const T* __restrict self_Q_bias,
                                            T* __restrict key_cache,
                                            const T* __restrict self_K_bias,
                                            T* __restrict value_cache,
                                            const T* __restrict self_V_bias,
  ...
  __shared__
      typename cub::WarpReduce<float>::TempStorage temp_storage[warp_num];
  ...
  float max_val =
      MaxValBlockReduce(max_val_block_temp_storage).Reduce(local_i, cub::Max());
  ...
  // This optimization introduces discrepancy because of different order in FP32
  // summation
...
__global__ void masked_attention_kernel_opt_half2(
    float* __restrict key_buf,
    float* __restrict value_buf,
    float* __restrict query_buf,
    const float* __restrict self_Q_bias,
    float* __restrict key_cache,
    const float* __restrict self_K_bias,
    float* __restrict value_cache,
    const float* __restrict self_V_bias,
    float* __restrict context_buf,
    int batch_size,
    int head_num,
    const int step,
    const float scalar) {}
...
template <int size_per_head, int block_sz>
__global__ void masked_attention_kernel_opt_half2(
    half* __restrict key_buf,
    half* __restrict value_buf,
    half* __restrict query_buf,
    const half* __restrict self_Q_bias,
    half* __restrict key_cache,
    const half* __restrict self_K_bias,
    half* __restrict value_cache,
    const half* __restrict self_V_bias,
    half* __restrict context_buf,
    int batch_size,
    int head_num,
    const int step,
    const half scalar) {
...
template <typename T>
__global__ void masked_attention_kernel(T* key_buf,
                                        T* value_buf,
                                        T* query_buf,
                                        const T* self_Q_bias,
                                        T* key_cache,
  ...
  for (int ite = 0; ite < step; ++ite) {
    ...
    if (threadIdx.x == 0) logits[ite] = qk;
  ...
  if (tid < step) logits[tid] = local_o / s_sum;
  __syncthreads();
...
template <typename T>
__global__ void masked_attention_kernel_v2(T* query_buf,
                                           const T* self_Q_bias,
                                           T* key_cache,
                                           const T* self_K_bias,
                                           T* value_cache,
  ...
  if (step_id < step) {
    for (int wite = 0; wite < warp_ite; ++wite) {
      T key = key_cache[step_id * offset + bid * head_num * size_per_head +
                        head_id * size_per_head + tid % warp_size +
                        wite * warp_size];
      // for the last step, we should update K + bias_K to the cache
      if (step_id == step - 1) {
        key += self_K_bias[bid * head_num * size_per_head +
                           head_id * size_per_head + tid % warp_size +
                           wite * warp_size];
        key_cache[step_id * offset + bid * head_num * size_per_head +
                  head_id * size_per_head + tid % warp_size +
                  wite * warp_size] = key;
      }
...
template <typename T>
void masked_attention_dispatch(T* key_buf,
                               T* value_buf,
                               T* query_buf,
                               const T* self_Q_bias,
                               T* key_cache,
  ...
  switch (cond) {
    case 32:
      masked_attention_kernel_opt<
          32,
          block_sz,
          T><<<grid, block_sz, sizeof(float) * step, stream>>>(key_buf,
                                                               value_buf,
                                                               query_buf,
                                                               self_Q_bias,
                                                               key_cache,
    ...
    case 64:
      if (sizeof(T) == 2)
        masked_attention_kernel_opt_half2<
            64,
            block_sz><<<grid, block_sz, sizeof(float) * step, stream>>>(
            key_buf,
            value_buf,
            query_buf,
            self_Q_bias,
    ...
    default:
      ...
      if ((int)block_size < size_per_head) block_size = size_per_head;
...
void OpenDecoder<OpType_>::masked_multi_head_attention(
  ...
  masked_attention_dispatch<DataType_>(key_buf_,
                                       value_buf_,
                                       query_buf_,
...
__global__ void cross_attention_kernel_opt(T* __restrict query_buf,
  ...
  int key_value_id =
      bid * (seq_len * head_num * size_per_head) + +head_id * size_per_head;
...
__global__ void cross_attention_kernel(T* query_buf,
  ...
  if (tid < size_per_head) sq[tid] = query_buf[qkv_id] + Q_bias[qkv_bias_id];
  __syncthreads();
  ...
  if (tid == 0) s_sum = val + 1e-6;
  __syncthreads();
  if (tid < length) logits[tid] = local_o / s_sum;
  __syncthreads();
  ...
  if (tid < size_per_head) {
    ...
    context_buf[bid * head_num * size_per_head + head_id * size_per_head +
                tid] = sum;
  }
}
...
void cross_attention_dispatch(T* query_buf,
  ...
  switch (cond) {
    case 32:
      cross_attention_kernel_opt<
          T,
          32,
          block_sz><<<grid, block_sz, sizeof(float) * seq_len, stream>>>(
          query_buf,
          Q_bias,
          key_cache,
          K_bias,
    ...
    default:
      ...
      if (block_size < size_per_head) block_size = size_per_head;
...
void OpenDecoder<OpType_>::cross_multi_head_attention(
  ...
  cross_attention_dispatch<DataType_>(query_buf_,
...
template <typename T>
__global__ void decoder_norm1_kernel_generalize(const T* __restrict input,
...
template <typename T>
__global__ void decoder_norm1_kernel(const T* __restrict input,
                                     const T* __restrict gamma,
                                     const T* __restrict beta,
                                     T* output,
                                     int m,
...
template <>
__global__ void decoder_norm1_kernel(const half* __restrict input,
  ...
  variance = blockReduceSum<float>(
      tid < blockDim.x
          ? (local_out_fp2.x - s_mean) * (local_out_fp2.x - s_mean) +
                (local_out_fp2.y - s_mean) * (local_out_fp2.y - s_mean)
          : 0.0f);
  if (tid == 0) s_variance = rsqrtf(variance / n + 1e-6);
  __syncthreads();
...
template <typename T>
__global__ void decoder_norm2_kernel_generalize(const T* __restrict input,
                                                const T* __restrict gamma,
                                                const T* __restrict beta,
...
template <typename T>
__global__ void decoder_norm2_kernel(const T* __restrict input,
                                     const T* __restrict gamma,
                                     const T* __restrict beta,
                                     const T* __restrict bias,
                                     T* output,
...
template <>
__global__ void decoder_norm2_kernel(const half* __restrict input,
                                     const half* __restrict gamma,
                                     const half* __restrict beta,
  ...
  int id = blockIdx.x * blockDim.x + tid;
  if (tid < blockDim.x) {
    output_ptr[id] = __hadd2(__hadd2(output_ptr[id], __ldg(&input_ptr[id])),
                             __ldg(&bias_ptr[tid]));
...
template <OperationType OpType_>
void OpenDecoder<OpType_>::decoder_norm1(const DataType_* input,
...
template <OperationType OpType_>
void OpenDecoder<OpType_>::decoder_norm2(const DataType_* input,
                                         const DataType_* gamma,
                                         const DataType_* beta,
...
template <OperationType OpType_>
void OpenDecoder<OpType_>::ffn(const DataType_* input,
                               DataType_* ffn_inner,
                               DataType_* output,
                               const int m,
...
template <typename T>
__global__ void add_bias_input_kernel(
...
template <typename T>
__global__ void add_bias_input_kernel_generalize(
    T* output, const T* input, const T* bias, const int m, const int n) {
  // TODO For GPT-3
...
template <OperationType OpType_>
void OpenDecoder<OpType_>::add_bias_input(DataType_* output,
...
template void OpenDecoder<OperationType::FP16>::decoder_norm1(const half* input,
                                                              const half* gamma,
                                                              const half* beta,
                                                              half* output,
                                                              int m,
...
template void OpenDecoder<OperationType::FP16>::decoder_norm2(const half* input,
                                                              const half* gamma,
                                                              const half* beta,
                                                              const half* bias,
                                                              half* output,
