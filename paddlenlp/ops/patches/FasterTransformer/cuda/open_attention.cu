...
__inline__ __device__ int target_index(int id1,
                                       int id2,
                                       int id3,
                                       int id4,
                                       int dim_1,
...
// block(64)
__global__ void mappingRemovePaddingData(int* mapping,
...
// add_QK_bias_transform for batch int8 cublasLtMatmul & per axis quantization
// for weight
// 1.add QK bias
// 2.transform each Q K CUBLASLT_ORDER_COL32 matrixes into a series of
// sub-matrix (with CUBLASLT_ORDER_COL32/CUBLASLT_ORDER_COL4_4R2_8C layout)
//  Q, K are CUBLASLT_ORDER_COL32 matrixes of m = batch_size * seq_len, n =
//  head_num * size_per_head
//  q_buf_ is of batchCount = batch_size * head_num, m = seq_len, n =
//  size_per_head, CUBLASLT_ORDER_COL32
//  k_buf_ is of batchCount = batch_size * head_num, m = seq_len, n =
//  size_per_head, CUBLASLT_ORDER_COL4_4R2_8C
...
__global__ void add_QK_bias_transform(int8_t* q_buf_,
  ...
  const float input_deQFactor_div127 =
      qk_id == 0 ? __ldg(q_input_deQFactor_div127_ptr)
                 : __ldg(k_input_deQFactor_div127_ptr);
  weight_amax = qk_id == 0 ? q_weight_amax : k_weight_amax;
  const float output_scale =
      qk_id == 0 ? __ldg(q_output_scale_ptr) : __ldg(k_output_scale_ptr);
  ...
  int data_id = (((threadIdx4 >> 5) << 5) * m + ((blockIdx.x % m) << 5) +
                 (threadIdx4 & 31));

  float scale;
  float tmp;
  char4 tmp4;
  scale = static_cast<float>(__ldg(data_ptr + data_id)) *
          __ldg(weight_amax + threadIdx4) * input_deQFactor_div127;
  ...
  // new (row, rol) of LtTrans COL32/COL4 sub-matrix, leading dim = (COL32_ *
  // seq_len)
  ...
  buf_ptr4[(((batch_id * head_num + head_id) * stride +
             (new_col << 5) * seq_len + new_row) >>
            2)] = tmp4;
}
...
__global__ void add_QK_bias_transform(int8_t* q_buf_,
  ...
  const float input_deQFactor =
      qk_id == 0 ? __ldg(q_input_deQFactor_ptr) : __ldg(k_input_deQFactor_ptr);
  const float output_scale =
      qk_id == 0 ? __ldg(q_output_scale_ptr) : __ldg(k_output_scale_ptr);
...
// add_QK_bias_transform & rebuild padding for batch int8 cublasLtMatmul & per
// axis quantization for weight
// 1.add QK bias
// 2.transform each Q K CUBLASLT_ORDER_COL32 matrixes into a series of
// sub-matrix (with CUBLASLT_ORDER_COL32/CUBLASLT_ORDER_COL4_4R2_8C layout)
//  Q, K are CUBLASLT_ORDER_COL32 matrixes of m = valid_word_num, n = head_num *
//  size_per_head
//  q_buf_ is of batchCount = batch_size * head_num, m = seq_len, n =
//  size_per_head, CUBLASLT_ORDER_COL32
//  k_buf_ is of batchCount = batch_size * head_num, m = seq_len, n =
//  size_per_head, CUBLASLT_ORDER_COL4_4R2_8C or CUBLASLT_ORDER_COL32_2R_4R4
// only for int32 input & int8 output
// seq_len, size_per_head must be a multiple of 32
...
// using char4
template <typename T>
__global__ void add_QK_bias_transform_rebuild_padding(
    int8_t* q_buf_,
    int8_t* k_buf_,
    const int32_t* Q,
    const T* bias_Q,
  ...
  m_full_idx = (valid_word_num != m)
                   ? (m_full_idx + __ldg(sequence_id_offset + m_full_idx))
                   : m_full_idx;
  ...
  int data_id = (((threadIdx4 >> 5) << 5) * valid_word_num +
                 ((blockIdx.x % valid_word_num) << 5) + (threadIdx4 & 31));
...
__global__ void add_QK_bias_transform_rebuild_padding(
  ...
  const float deQFactor =
      qk_id == 0 ? __ldg(q_deQFactor_ptr) : __ldg(k_deQFactor_ptr);
  const float output_scale =
      qk_id == 0 ? __ldg(q_output_scale_ptr) : __ldg(k_output_scale_ptr);
...
// input matrix a matrix of m = batch_size*seq_len , n = head_num*size_per_head,
// CUBLASLT_ORDER_COL32
// output matrixes are a series of sub-matrixes with size of m = size_per_head,
// n = seq_len , CUBLASLT_ORDER_COL4_4R2_8C or CUBLASLT_ORDER_COL32_2R_4R4
...
__global__ void add_V_bias_transform(int8_t* v_buf_,
  ...
  int inIdx =
      (((col >> 5) << 5) * batch_size * seq_len + ((row << 5) + (col & 31)));
  // for shm row-major
  ...
  scale = __ldg(data_ptr + inIdx) * __ldg(weight_amax + col) *
          input_deQFactor_div127;
  tmp = scale + static_cast<float>(__ldg(bias_ptr + col));  //(tmp2.x);
  shm[sh_row][sh_col] = float_to_int8_rn(tmp * out_scale);

  scale = __ldg(data_ptr + inIdx + 1) * __ldg(weight_amax + col + 1) *
          input_deQFactor_div127;
  ...
  buf_ptr4[(blockIdx.z * stride + (col << 5) * size_per_head + row) >> 2] =
      dataTmp;
}
...
__global__ void add_V_bias_transform(int8_t* v_buf_,
  ...
  int row = batch_id * seq_len + word_id;
  int inIdx =
      ((col & 0xffffffe0) * batch_size * seq_len + ((row << 5) + (col & 31)));
  // for shm row-major
  int sh_col = threadIdx4;
  int sh_row = threadIdx.y;
  ...
  if (use_ORDER_COL32_2R_4R4) {
    ...
    row = (
        ...
        ////id_in_size%2 is even row, otherwise odd row
        ////word_id%COL32_/8 is the number tile of (8 rows 8 columns)
        (((id_in_size & 0xfffffff8) + ((id_in_size & 1) << 2) +
          ((word_id & 31) >> 3))
         << 5) +
        ////word_id%8 >= 4 is the right half of (8 rows 8 columns) tile
        ////(id_in_size%8/2) is (the row id of alternating 4 rows) - 1
...
// input matrix a matrix of m = valid_word_num, n = head_num*size_per_head,
// CUBLASLT_ORDER_COL32
// output matrixes are a series of sub-matrixes with size of m = size_per_head,
// n = seq_len , CUBLASLT_ORDER_COL4_4R2_8C or CUBLASLT_ORDER_COL32_2R_4R4
// only for int32_t Input int8_t Output
...
// per axis quantization for weight
template <typename T>
__global__ void add_V_bias_transform_rebuild_padding(
    int8_t* v_buf_,
    const int32_t* V,
    const T* V_bias,
  ...
  if (row != -1) {
    ...
    int inIdx =
        ((col & 0xffffffe0) * valid_word_num + ((row << 5) + (col & 31)));
    ...
    shm[sh_row][sh_col] = shm[sh_row][sh_col + 1] = shm[sh_row][sh_col + 2] =
        shm[sh_row][sh_col + 3] = 0;
  }
  __syncthreads();
...
__global__ void add_V_bias_transform_rebuild_padding(
  ...
  if (row != -1) {
    col = head_id * size_per_head + id_in_size;
    int inIdx =
        ((col & 0xffffffe0) * valid_word_num + ((row << 5) + (col & 31))) >> 2;
...
__global__ void trt_add_QKV_bias(half2* Q,
                                 const half2* bias_Q,
                                 half2* K,
                                 const half2* bias_K,
                                 half2* V,
  ...
  const int target_offset =
      blockIdx.x * head_num * 3 * size_per_head + head_id * 3 * size_per_head;

  q_buf_[target_offset + 0 * size_per_head + size_id] =
      Q[seq_id * blockDim.x + threadIdx.x] + bias_Q[threadIdx.x];

  q_buf_[target_offset + 1 * size_per_head + size_id] =
      K[seq_id * blockDim.x + threadIdx.x] + bias_K[threadIdx.x];

  q_buf_[target_offset + 2 * size_per_head + size_id] =
      V[seq_id * blockDim.x + threadIdx.x] + bias_V[threadIdx.x];
}

template <OperationType OpType_>
void OpenMultiHeadAttention<OpType_>::trt_add_QKV_bias_kernelLauncher(
    const DataType_* bias_Q, const DataType_* bias_K, const DataType_* bias_V) {
  dim3 grid;
  dim3 block;
  ...
  trt_add_QKV_bias<<<grid, block, 0, param_.stream>>>((half2*)query_buf_,
                                                      (const half2*)bias_Q,
                                                      (half2*)key_buf_,
                                                      (const half2*)bias_K,
                                                      (half2*)value_buf_,
...
void OpenMultiHeadAttention<OpType_>::fused_multiHeadAttr_kernelLauncher() {
  ...
  dispatcher_fp16->run(q_buf_,
                       nullptr,
                       param_.trt_seqlen_offset,
                       trt_attn_workspace_,
                       param_.attr_out,
                       param_.stream);
}
...
__global__ void add_QKV_bias(half* Q,
  ...
  int seq_id =
      (tid % (head_num * seq_len * size_per_head)) / (head_num * size_per_head);
  int head_id = (tid % (head_num * size_per_head)) / size_per_head;
  int id = tid % size_per_head;
  int target_id = target_index(batch_id,
                               seq_id,
                               head_id,
                               id,
                               batch_size,
...
__global__ void softmax_kernel(T* qk_buf_,
  ...
  for (int i = 0; i < seq_len; ++i) {
    ...
    float tmp =
        threadIdx.x < seq_len ? (float)(qk * (float)scalar + mask_val) : -1e20f;
...
template <typename T>
__global__ void softmax_kernel_v2(T* qk_buf_,
                                  const T* attr_mask,
                                  const int batch_size,
                                  const int head_num,
                                  const int seq_len,
...
template <typename T>
__global__ void softmax_kernel_v3(T* qk_buf_,
                                  const T* attr_mask,
                                  const int batch_size,
                                  const int head_num,
                                  const int seq_len,
  ...
  for (int seq_id = blockIdx.x; seq_id < seq_len; seq_id += gridDim.x) {
    ...
    if (qual) {
      qk_offset =
          ((blockIdx.y * head_num + blockIdx.z) * seq_len + seq_id) * seq_len +
          threadIdx.x;
    ...
    if (qual) qk_buf_[qk_offset] = (T)(qk_tmp * s_mean);
  }
}
...
// seq_len % 2 == 0
template <>
__global__ void softmax_kernel_v3(half* qk_buf_,
  ...
  for (int seq_id = blockIdx.x; seq_id < seq_len; seq_id += gridDim.x) {
    ...
    half2 qk;
    if (qual) {
      qk_offset = ((((blockIdx.y * head_num + blockIdx.z) * seq_len + seq_id) *
                    seq_len) >>
                   1) +
                  threadIdx.x;
      int mask_offset =
          (((blockIdx.y * seq_len + seq_id) * seq_len) >> 1) + threadIdx.x;
    ...
    max_val = blockDim.x <= 32 ? warpReduceMax(max_val)
                               : blockReduceMax<float>(max_val);
    ...
    float sum_val = blockDim.x <= 32
                        ? warpReduceSum((float)(tmp.x + tmp.y))
                        : blockReduceSum<float>((float)(tmp.x + tmp.y));
...
template <typename T>
__global__ void softmax_kernel_v3_LE32(T* qk_buf_,
                                       const T* attr_mask,
                                       const int batch_size,
                                       const int head_num,
                                       const int seq_len,
  ...
  for (int seq_id = blockIdx.x; seq_id < seq_len; seq_id += gridDim.x) {
    ...
    if (qual) qk_buf_[qk_offset] = (T)(tmp * s_mean);
  }
}

// int_buf are a series of sub-matrixes of m = seq_len, n = seq_len,
// CUBLASLT_ORDER_COL32
// grid = (seq_len, batch_size, head_num)
// block.x = max(32, (seq_len/4 + 31)/32*32)
// for int32_t I; int8 O;
template <typename T>
__global__ void softmax_COL32(int8_t* qk_buf_,
                              const int32_t* int_buf,
                              const T* attr_mask,
                              const int batch_size,
                              const int head_num,
                              const int seq_len,
  ...
  for (int seq_id = blockIdx.x; seq_id < seq_len; seq_id += gridDim.x) {
    ...
    int inIdx = (blockIdx.y * head_num + blockIdx.z) * (seq_len_x_seq_len) +
                (threadIdx4 & 0xffffffe0) * seq_len + (seq_id << 5) +
                (threadIdx4 & 31);

    // set softmax of padding word to 0
    float mask_in_seq = static_cast<float>(
        __ldg(attr_mask + (blockIdx.y * seq_len_x_seq_len + seq_id)));
    if (mask_in_seq < 0.1f) {
      if (qual) buf4Ptr[inIdx >> 2] = tmp4;
      continue;
    }
    ...
    if (qual) {
      ...
      // for x
      mask_val =
          (1.0f - static_cast<float>(__ldg(attr_mask + mask_id))) * -10000.0f;
      floatTmp4.x = floatTmp4.x + mask_val;
      max_val = fmaxf(max_val, floatTmp4.x);

      // for y
      mask_val = (1.0f - static_cast<float>(__ldg(attr_mask + mask_id + 1))) *
                 -10000.0f;
      floatTmp4.y = floatTmp4.y + mask_val;
      max_val = fmaxf(max_val, floatTmp4.y);

      // for z
      mask_val = (1.0f - static_cast<float>(__ldg(attr_mask + mask_id + 2))) *
                 -10000.0f;
      floatTmp4.z = floatTmp4.z + mask_val;
      max_val = fmaxf(max_val, floatTmp4.z);

      // for w
      mask_val = (1.0f - static_cast<float>(__ldg(attr_mask + mask_id + 3))) *
                 -10000.0f;
    ...
    sum_val = blockDim.x <= 32 ? warpReduceSum(sum_val)
                               : blockReduceSum<float>(sum_val);
...
__global__ void softmax_COL32(int8_t* qk_buf_,
  ...
  for (int seq_id = blockIdx.x; seq_id < seq_len; seq_id += gridDim.x) {
    ...
    // set softmax of padding word to 0
    const float mask_in_seq = static_cast<float>(
        __ldg(attr_mask + (blockIdx.y * seq_len_x_seq_len + seq_id)));
    if (mask_in_seq < 0.1f) {
      if (qual) buf4Ptr[inIdx] = tmp4;
      continue;
    }
...
// for seq_len <= 32
template <typename T>
__global__ void softmax_COL32_LE32(int8_t* qk_buf_,
                                   const int32_t* int_buf,
                                   const T* attr_mask,
                                   const int batch_size,
                                   const int head_num,
  ...
  for (int seq_id = blockIdx.x; seq_id < seq_len; seq_id += gridDim.x) {
    int inIdx = (blockIdx.y * head_num + blockIdx.z) * (seq_len_x_seq_len) +
                (threadIdxx & 0xffffffe0) * seq_len + (seq_id << 5) +
                (threadIdxx & 31);
    ...
    float mask_in_seq = static_cast<float>(
        __ldg(attr_mask + (blockIdx.y * seq_len_x_seq_len + seq_id)));
    if (mask_in_seq < 0.1f) {
      if (qual) qk_buf_[inIdx] = 0;
      continue;
    }

    float floatTmp =
        qual ? static_cast<float>(__ldg(int_buf + inIdx)) * scalar1 : 0.0f;
    ...
    mask_id =
        qual ? threadIdxx + blockIdx.y * seq_len_x_seq_len + seq_id * seq_len
             : 0;
    mask_val = qual
                   ? (1.0f - static_cast<float>(__ldg(attr_mask + mask_id))) *
                         -10000.0f
                   : 0.0f;
    ...
    float sum_val = blockDim.x <= 32 ? warpReduceSum(floatTmp)
                                     : blockReduceSum<float>(floatTmp);
...
template <typename T>
__global__ void softmax_COL32_LE64(int8_t* qk_buf_,
                                   const int32_t* int_buf,
                                   const T* attr_mask,
                                   const int batch_size,
                                   const int head_num,
  ...
  for (int seq_id = blockIdx.x; seq_id < seq_len; seq_id += gridDim.x) {
    char2 tmp2 = {0, 0};
    int inIdx = (blockIdx.y * head_num + blockIdx.z) * (seq_len_x_seq_len) +
                (threadIdx2 & 0xffffffe0) * seq_len + (seq_id << 5) +
                (threadIdx2 & 31);
...
template <>
__global__ void transpose(half* src,
                          half* dst,
                          const int batch_size,
                          const int seq_len,
                          const int head_num,
  ...
  int batch_id = tid / (head_num * seq_len * size_per_head);
  int head_id =
      (tid % (head_num * seq_len * size_per_head)) / (seq_len * size_per_head);
...
void OpenMultiHeadAttention<OpType_>::multiHeadAttr_nofuse_kernelLauncher(
  ...
  if (int8_mode_ != 0) {
    // var for int8
    const float *Qbias_amax_ptr, *Kbias_amax_ptr, *Vbias_amax_ptr,
        *bmm1_amax_ptr, *Softmax_amax_ptr, *bmm2_amax_ptr, *in_amax_ptr,
        *Q_aftergemm_amax_ptr, *K_aftergemm_amax_ptr, *V_aftergemm_amax_ptr;
    ...
    if (param_.sequence_id_offset == nullptr ||
        param_.valid_word_num == batch_size * seq_len) {
      if (int8_mode_ == 1) {
        add_QK_bias_transform<<<dim3(batch_size * seq_len * 2),
                                dim3((head_num * size_per_head) / 4),
                                0,
                                stream>>>((int8_t*)q_buf_,
                                          (int8_t*)k_buf_,
                                          (const int32_t*)Q,
                                          bias_Q,
                                          (const int32_t*)K,
        ...
        add_V_bias_transform<<<
            dim3(size_per_head / 32, seq_len / 32, batch_size * head_num),
            dim3(8, 32),
            0,
            stream>>>((int8_t*)v_buf_,
                      (const int32_t*)V,
                      bias_V,
                      batch_size,
                      seq_len,
        ...
        add_QK_bias_transform<<<dim3(batch_size * seq_len * 2),
                                ...
                                stream>>>((int8_t*)q_buf_,
                                          ...
                                          size_per_head,
                                          seq_len * size_per_head,
                                          Q_aftergemm_amax_ptr + 1,
                                          K_aftergemm_amax_ptr + 1,
                                          Qbias_amax_ptr + 3,
                                          Kbias_amax_ptr + 3,
                                          use_ORDER_COL32_2R_4R4_);
        add_V_bias_transform<<<
            ...
            stream>>>((int8_t*)v_buf_,
                      ...
                      size_per_head,
                      seq_len * size_per_head,
                      V_aftergemm_amax_ptr + 1,
                      Vbias_amax_ptr + 3,
                      use_ORDER_COL32_2R_4R4_);
      }
    } else {
      cudaMemset(sequence_id_map_, -1, batch_size * seq_len * sizeof(int));
      mappingRemovePaddingData<<<dim3((param_.valid_word_num + 63) / 64),
                                 dim3(64)>>>(
          sequence_id_map_, param_.sequence_id_offset, param_.valid_word_num);
      if (int8_mode_ == 1) {
        add_QK_bias_transform_rebuild_padding<<<
            dim3(param_.valid_word_num * 2),
            dim3((head_num * size_per_head) / 4),
            0,
            stream>>>((int8_t*)q_buf_,
                      (int8_t*)k_buf_,
        ...
        add_V_bias_transform_rebuild_padding<<<
            dim3(size_per_head / 32, seq_len / 32, batch_size * head_num),
            dim3(8, 32),
            0,
            stream>>>((int8_t*)v_buf_,
    ...
    if (int8_mode_ == 1) {
      cublasLtMM_withAlgo(qk_int_buf_,
                          batchCount,
                          seq_len,
                          seq_len,
                          size_per_head,
                          size_per_head * seq_len,
                          size_per_head * seq_len,
                          seq_len * seq_len,
                          (int8_t*)q_buf_,
                          (int8_t*)k_buf_,
      ...
      if (seq_len <= 32) {
        if (batch_size * head_num > 960) grid.x = ceil(float(seq_len) / 32.0f);
        block.x = (seq_len + 31) / 32 * 32;
        softmax_COL32_LE32<<<grid, block, 0, stream>>>((int8_t*)qk_buf_,
                                                       qk_int_buf_,
                                                       attr_mask,
                                                       batch_size,
                                                       head_num,
        ...
        if (batch_size * head_num > 960) grid.x = ceil(float(seq_len) / 32.0f);
        softmax_COL32_LE64<<<grid, block, 0, stream>>>((int8_t*)qk_buf_,
                                                       qk_int_buf_,
                                                       attr_mask,
                                                       batch_size,
                                                       head_num,
        ...
        softmax_COL32<<<grid, block, 0, stream>>>((int8_t*)qk_buf_,
                                                  qk_int_buf_,
                                                  attr_mask,
                                                  batch_size,
                                                  head_num,
      ...
      cublasLtMM_withAlgo(transpose_dst_int_buf_,
                          batchCount,
                          seq_len,
                          size_per_head,
                          seq_len,
      ...
      if (param_.sequence_id_offset == nullptr ||
          param_.valid_word_num == batch_size * seq_len) {
        transpose_COL32_kernelLauncher((int8_t*)dst,
                                       (const int*)transpose_dst_int_buf_,
                                       batch_size,
                                       seq_len,
                                       head_num,
                                       size_per_head,
        ...
        transpose_COL32_rebuild_padding_kernelLauncher(
            (int8_t*)dst,
            (const int*)transpose_dst_int_buf_,
            sequence_id_map_,
            param_.valid_word_num,
      ...
      cublasLtMM_withAlgo_int8IO((int8_t*)qk_int_buf_,
                                 batchCount,
                                 seq_len,
                                 seq_len,
                                 size_per_head,
      ...
      cublasLtMM_withAlgo_int8IO((int8_t*)transpose_dst_int_buf_,
                                 batchCount,
                                 seq_len,
                                 size_per_head,
                                 seq_len,
                                 ...
                                 size_per_head * seq_len,
                                 size_per_head * seq_len,
                                 param_.int8O_gemm_deQ_scale_list[4],
                                 (int8_t*)qk_buf_,
                                 (int8_t*)v_buf_,
                                 cublaslt_handle,
                                 stream,
  ...
  // FP32/FP16
    ...
    if (OpType_ == OperationType::FP32) {
      if (param_.sequence_id_offset == nullptr ||
        ...
        dim3 block(k);
        add_QKV_bias<DataType_><<<grid, block, 0, stream>>>(Q,
                                                            bias_Q,
                                                            K,
                                                            bias_K,
                                                            V,
        ...
        std::string message1("FT Q add bias:");
        cuda_print((const float*)q_buf_, m, k, message1);
        std::string message2("FT K add bias:");
        cuda_print((const float*)k_buf_, m, k, message2);
        std::string message3("FT V add bias:");
        cuda_print((const float*)v_buf_, m, k, message3);
      } else {
        add_QKV_bias_rebuild_padding<
            DataType_><<<param_.valid_word_num, k, 0, stream>>>(
            Q,
            bias_Q,
            K,
            bias_K,
      ...
      if (param_.sequence_id_offset == nullptr ||
        ...
        add_QKV_bias_rebuild_padding<
            half2><<<param_.valid_word_num, k / 2, 0, stream>>>(
            (half2*)Q,
            (const half2*)bias_Q,
            (half2*)K,
            (const half2*)bias_K,
    ...
    std::string message4("FT QK_buf:");
    cuda_print((const float*)qk_buf_,
               batch_size * head_num,
               seq_len * seq_len,
               message4);
    ...
    if (seq_len % 2 != 0) {
      ...
      if (batch_size * head_num <= 120) {
        ...
        softmax_kernel_v2<DataType_><<<grid, block, 0, stream>>>(
            qk_buf_, attr_mask, batch_size, head_num, seq_len, scalar);
      } else {
        grid.x = batch_size * head_num;
        softmax_kernel<DataType_><<<grid, block, 0, stream>>>(
            qk_buf_, attr_mask, batch_size, head_num, seq_len, scalar);
      }
    }
    // deal with even seq_len
      ...
      grid.x = seq_len;
      if (batch_size * head_num > 360) grid.x = ceil(float(seq_len) / 32.0f);
      grid.y = batch_size;
      grid.z = head_num;
      if (seq_len <= 32) {
        block.x = 32;
        softmax_kernel_v3_LE32<DataType_><<<grid, block, 0, stream>>>(
            qk_buf_, attr_mask, batch_size, head_num, seq_len, scalar);
        std::string message5("FT QK_softmax output:");
        cuda_print((const float*)qk_buf_,
                   batch_size * head_num,
                   seq_len * seq_len,
                   message5);
        ...
        if (OpType_ == OperationType::FP16) {
          block.x = (seq_len / 2 + 31) / 32 * 32;
          softmax_kernel_v3<<<grid, block, 0, stream>>>(
              qk_buf_, attr_mask, batch_size, head_num, seq_len, scalar);
        } else {
          block.x = (seq_len + 31) / 32 * 32;
          softmax_kernel_v3<DataType_><<<grid, block, 0, stream>>>(
              qk_buf_, attr_mask, batch_size, head_num, seq_len, scalar);
        }
      }
    ...
    std::string message6("FT QKV output:");
    cuda_print((const float*)transpose_dst_,
               batch_size * head_num,
               seq_len * size_per_head,
               message6);
    ...
    if (OpType_ == OperationType::FP16) {
      if (param_.sequence_id_offset == nullptr ||
        ...
        transpose<DataType_><<<grid, block, 0, stream>>>(transpose_dst_,
                                                         dst,
                                                         batch_size,
                                                         seq_len,
                                                         head_num,
        ...
        transpose_rebuild_padding<
            half2><<<param_.valid_word_num, k / 2, 0, stream>>>(
            (half2*)transpose_dst_,
            (half2*)dst,
            batch_size,
            seq_len,
            head_num,
      ...
      if (param_.sequence_id_offset == nullptr ||
        ...
        transpose<DataType_><<<grid, block, 0, stream>>>(
...
template void OpenMultiHeadAttention<OperationType::FP32>::
    multiHeadAttr_nofuse_kernelLauncher(cudaStream_t stream,
...
template void OpenMultiHeadAttention<OperationType::FP16>::
    multiHeadAttr_nofuse_kernelLauncher(cudaStream_t stream,
                                        cublasHandle_t handle,
                                        cublasLtHandle_t cublaslt_handle,
                                        half* Q,
...
template void
OpenMultiHeadAttention<OperationType::FP32>::trt_add_QKV_bias_kernelLauncher(
    const float* bias_Q, const float* bias_K, const float* bias_V);

template void
OpenMultiHeadAttention<OperationType::FP16>::trt_add_QKV_bias_kernelLauncher(
    const half* bias_Q, const half* bias_K, const half* bias_V);

template void OpenMultiHeadAttention<
    OperationType::FP32>::fused_multiHeadAttr_kernelLauncher();
template void OpenMultiHeadAttention<
    OperationType::FP16>::fused_multiHeadAttr_kernelLauncher();

__global__ void trt_add_QKV_bias_2(const half2* Q,
                                   const half2* bias_Q,
                                   const half2* K,
                                   const half2* bias_K,
                                   const half2* V,
  ...
  qkv_buf_[target_offset + 0 * size_per_head + size_id] =
      Q[seq_id * blockDim.x + threadIdx.x] + bias_Q[threadIdx.x];

  qkv_buf_[target_offset + 1 * size_per_head + size_id] =
      K[seq_id * blockDim.x + threadIdx.x] + bias_K[threadIdx.x];

  qkv_buf_[target_offset + 2 * size_per_head + size_id] =
      V[seq_id * blockDim.x + threadIdx.x] + bias_V[threadIdx.x];
}

void add_QKV_bias_transpose_kernelLauncher(const half* query_buf,
                                           const half* bias_Q,
                                           const half* key_buf,
                                           const half* bias_K,
                                           const half* value_buf,
                                           const half* bias_V,
                                           half* context_buf,
                                           const int valid_word_num,
                                           const int head_num,
  ...
  trt_add_QKV_bias_2<<<grid, block, 0, stream>>>((const half2*)query_buf,
                                                 (const half2*)bias_Q,
                                                 (const half2*)key_buf,
                                                 (const half2*)bias_K,
                                                 (const half2*)value_buf,
                                                 (const half2*)bias_V,
                                                 (half2*)context_buf,
                                                 valid_word_num,
                                                 head_num,
