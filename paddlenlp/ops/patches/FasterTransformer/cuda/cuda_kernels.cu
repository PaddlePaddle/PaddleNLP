...
template <>
__global__ void add_bias_act(half* out, const half* bias, int m, int n) {
  half2 val, reg_bias;
  int row_id = blockIdx.x;
...
template <typename T>
__global__ void update_logits_kernel_without_softmax(T* logits,
                                                     const T* bias,
                                                     const int end_id,
                                                     const bool* finished,
                                                     const int n,
  ...
  for (int tid = threadIdx.x; tid < n; tid += blockDim.x) {
    // The default value is -1. Hence, if any id is -1, it cannot be equal to
    // tid.
    if (start_id == tid) {
...
template <typename T>
__global__ void softmax_kernel(T* logits,
                               const T* bias,
                               const int end_id,
                               const bool* finished,
                               const int n) {
...
template <typename T>
__global__ void remove_sequence_length_padding(const T* src,
                                               T* tgt,
...
void remove_sequence_length_padding_kernelLauncher(const T* src,
  ...
  remove_sequence_length_padding<<<m, 256, 0, stream>>>(
...
template void rebuild_sequence_length_padding_kernelLauncher(
    const float* src,
    float* tgt,
    const int* mask_offset,
    const int m,
...
void update_logits(float* logits,
   ...
   */
  update_logits_kernel<float><<<grid, block, 0, stream>>>(
...
void update_logits_without_softmax(T* logits,
  ...
  /*n is the vocab_size, e.g., 30000, 7000.... vocab_size is usually very big.
   */
  update_logits_kernel_without_softmax<<<grid, block, 0, stream>>>(
      logits, bias, end_id, finished, n, start_id, unk_id, mask_id);
}

template void update_logits_without_softmax(float* logits,
                                            const float* bias,
                                            const int end_id,
                                            const bool* finished,
                                            const int m,
...
template void update_logits_without_softmax(half* logits,
                                            const half* bias,
                                            const int end_id,
                                            const bool* finished,
                                            const int m,
...
template void softmax_kernelLauncher(float* logits,
                                     const float* bias,
                                     const int end_id,
                                     const bool* finished,
                                     const int m,
...
template void softmax_kernelLauncher(half* logits,
                                     const half* bias,
                                     const int end_id,
                                     const bool* finished,
                                     const int m,
...
template <typename T>
__global__ void print_abs_mean_kernel(const T* buf, uint size) {
  float sum;
...
template <typename T>
__global__ void print_kernel(const T* buf, uint size) {
  for (int i = 0; i < size; i++) {
...
template <typename T>
__global__ void topK_kernel(const T* log_probs,
                            int* ids,
                            const int batch_size,
                            const int N,
                            const int K) {
...
template <typename T>
__global__ void topK_kernel_2nd(const T* log_probs,
                                int* ids,
                                const int batch_size,
                                const int N,
                                const int K,
...
void topK(const float* log_probs,
  ...
  topK_kernel<float><<<grid, block, 0, stream>>>(
      log_probs, ids, batch_size, N, beam_width);
  /*Second round, for each batch, get the final TopK values out from grid.x * K
   * values. */
  topK_kernel_2nd<float><<<1, block, 0, stream>>>(
      log_probs, ids, batch_size, beam_width * grid.x, beam_width, N);
}

template <typename T>
__global__ void embedding_lookup_kernel(const T* embedding_table,
                                        const int* word_ids,
                                        const int hidden_units,
                                        T* from_tensor) {
  int write_pos = threadIdx.x + blockIdx.x * hidden_units;
  from_tensor[write_pos] =
      embedding_table[word_ids[blockIdx.x] * hidden_units + threadIdx.x];
}
...
void embedding_lookup(const T* embedding_table,
  ...
  embedding_lookup_kernel<<<grid, block, 0, stream>>>(
      embedding_table, word_ids, hidden_units, from_tensor);
}

template <typename T>
__global__ void sine_position_encoder_kernel(T* output, int step, int n) {
  int tid = threadIdx.x;
  int bid = blockIdx.x;
  ...
  float inv_timescales =
      __expf((tid % (int)half_n) * -1 * log_timescale_increment);
  float scaled_time = inv_timescales * step;

  T encoding_val =
      (tid < half_n) ? (T)__sinf(scaled_time) : (T)__cosf(scaled_time);
...
template void embedding_lookup(const float* embedding_table,
                               const int* word_ids,
                               float* from_tensor,
                               const int batch_size,
                               const int beam_width,
...
template void embedding_lookup(const half* embedding_table,
                               const int* word_ids,
                               half* from_tensor,
                               const int batch_size,
                               const int beam_width,
...
template void sine_position_encoder(
    float* output, int step, int m, int n, cudaStream_t stream);

template void sine_position_encoder(
    half* output, int step, int m, int n, cudaStream_t stream);
