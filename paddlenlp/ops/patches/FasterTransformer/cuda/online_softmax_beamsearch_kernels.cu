...
__launch_bounds__(THREADBLOCK_SIZE) __global__
  ...
  if (finish) {
    for (int elem_id = thread_id; elem_id < V; elem_id += THREADBLOCK_SIZE) {
      ...
      // if (elem_id == thread_id) printf("finish vector_id: %d, thread_id: %d,
      // elem: %f, %f, %f\n", vector_id, thread_id, (float)x[elem_id],
      // b[elem_id], elem);
    }
  } else {
    for (int elem_id = thread_id; elem_id < V; elem_id += THREADBLOCK_SIZE) {
      float elem = (float)x[elem_id] + b[elem_id];
      // if (elem_id == thread_id) printf("vector_id: %d, thread_id: %d, elem:
      // %f, %f, %f\n", vector_id, thread_id, (float)x[elem_id], b[elem_id],
      // elem);
      MD new_elem{elem, 1.0F};
      partial.md = reduce_md_op(partial.md, new_elem);
      partial.topk.insert(elem, elem_id);
  ...
  if (thread_id == 0) {
    ...
    for (int i = 0; i < MAX_K; ++i) {
      ...
      if (i < K) {
        ...
        // printf("topk in beam: %d, %d, %d, %f, %f, %f, %f, %d, %d, %d, %d\n",
        // i, z[i], total.topk.p[i], total.topk.u[i], (float)val, (float)c[0],
        // v[i], V, K, MAX_K, vector_id);
      }
    }
  }
}
...
#pragma unroll 1
    for (int elem_id = section_start + thread_id; elem_id < section_end;
         elem_id += THREADBLOCK_SIZE) {
      T bias =
          (T)(b == nullptr ? 0.0f : b[elem_id]);  // gpt-2 does not use bias
  ...
  if (thread_id == 0) {
    ...
    for (int i = 0; i < MAX_K; ++i) {
      ...
      if (i < K) {
        ...
        v[i] = (float)val + (float)c[0];
        // printf("topk in beam: %d, %d, %d, %f, %f, %f, %f, %d, %d, %d, %d,
        // %d\n", i, z[i], total.topk.p[i], total.topk.u[i], (float)val,
        // (float)c[0], v[i], PACKED_TOP_KMD_SIZE, parts_per_beam, K, MAX_K,
        // vector_id);
      }
    }
  }
...
void topK_softMax_kernelLauncher(const T* log_probs,
  ...
  T* topk_tmp_val_buf = reinterpret_cast<T*>(
      topk_tmp_id_buf + batch_size * beam_width * beam_width);
  float* tmp_buffer = reinterpret_cast<float*>(
      topk_tmp_val_buf + batch_size * beam_width * beam_width);
...
template <typename T, int MAX_K, int THREADBLOCK_SIZE>
__launch_bounds__(THREADBLOCK_SIZE) __global__ void batch_topk_update_kernel(
    ...
    int* parent_ids,
    int* output_word_ids,
    // int* output_parent_ids, float* output_cum_log_probs, const int
    // batch_size,
    int* output_parent_ids,
    T* output_cum_log_probs,
    const int batch_size,
    const int beam_width,
    const int vocab_size,
    const int end_id,
    // const int step, const int max_out_len, int V, int K, T diversity_rate) {
    const int step,
  ...
  if (thread_id == 0) {
    ...
    for (int i = 0; i < MAX_K; ++i) {  // K == MAX_K == beam_width*2
      if (i < K) {
        ...
        // int beam_id_in_output =
        //    batch_size * (beam_width * 2) + (beam_id % beam_width) + MAX_K /
        //    2;
        int word_id = abs_id % vocab_size;
        // printf("abs_id: %d, word_id: %d, cum_log: %f, alive_num: %d,
        // finish_num: %d, beam_width: %d\n", abs_id, word_id, cum_log_prob,
        // alive_num, finish_num, beam_width);
        if (word_id == end_id) {  // grow finish
          ...
          // offset(beam_width).
          // finish_candidate.insert(cum_log_prob, beam_id_in_output, step);
          ...
          // int index = vector_id * beam_width + i;
          ...
          alive_num += 1;
          // printf("offset: %d, %d, %d, word_ids: %d, %d, parent_ids:%d, %d\n",
          // vector_id, beam_width, i, abs_id, word_id, beam_id,
          // beam_id_in_output);
        }
      }
    }
    ...
    for (int i = 0; i < MAX_K / 2; ++i) {
      // output_word_ids[i] = finish_candidate.p[i];
      ...
      // finished[i] = 1;
      finished[i] = finish_candidate.u[i] != -MAX_T_VAL ? 1 : 0;
    }
    ...
    // printf("vector_id: %d finish: %d last_prob:%f MAT_T:%f lowest_finish: %f
    // lower_bound:%f\n", vector_id, finish_num,
    // output_cum_log_probs[MAX_K/2-1], -MAX_T_VAL, lowest_finish, lower_bound);
    // output must include both the finish and alive to trace full path
    // if (step == max_out_len) {  // when finishing
    if (step == max_out_len || lowest_finish > lower_bound) {  // when finishing
      for (int i = 0; finish_num < MAX_K / 2; ++finish_num, ++i) {
        // int index = vector_id * beam_width + i;
        // output_word_ids[finish_num] = word_ids[index];
        // output_cum_log_probs[finish_num] =
        //     output_cum_log_probs[index + beam_width];
        // output_parent_ids[finish_num] = output_parent_ids[index +
        // beam_width];
        output_word_ids[finish_num] = word_ids[i];
        // output_cum_log_probs[finish_num] = output_cum_log_probs[i +
        // beam_width];
        output_cum_log_probs[finish_num] =
        ...
        // printf("%d, finish_id: %d, index: %d, word_ids: %d, cum_log: %f,
        // parent_ids:%d\n", MAX_K/2, finish_num, i,
        // output_word_ids[finish_num], output_cum_log_probs[finish_num],
        // output_parent_ids[finish_num]);
      }
      // If early stop, also mark the alive beams finished.
      for (int i = MAX_K / 2; i < MAX_K; ++i) finished[i] = 1;
      // printf("vector_id: %d finish at: %d\n", vector_id, step);
    }
  }
}

template <typename T, int MAX_K>
void topK_softMax_update_kernelLauncher(
    const T* log_probs,
    const float* bias,
    bool* finished,
    int* sequence_length,
    ...
    int* parent_ids,
    int* output_word_ids,
    // int* output_parent_ids, float* cum_log_probs, void* temp_storage,
    int* output_parent_ids,
    T* cum_log_probs,
    void* temp_storage,
    const int temp_storage_size,
  ...
  batch_topk_update_kernel<T, MAX_K, 32><<<batch_size, 32, 0, stream>>>(
      ...
      step,
      max_out_len,
      // beam_width * beam_width * 2, beam_width * 2, diversity_rate);
