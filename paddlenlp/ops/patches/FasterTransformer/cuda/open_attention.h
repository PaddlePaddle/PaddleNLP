...
public:
  ...
  void getBestAlgoFromMap(int batch_size,
    ...
    if (foundAlgo != 4) {
      printf(
          "[WARNING][OpenMultiHeadAttention] Loading GEMM algorithms error, "
      ...
      is_fuse_QKV_ = false;
      if ((split_time > 0) && (fuse_time > 0) && (3 * split_time > fuse_time)) {
        is_fuse_QKV_ = true;
      }
    }
  }
    ...
    try {
      ...
      if (buf_ != NULL) {
        ...
        if (int8_mode_ != 0) {
          buf_ = (DataType_*)allocator_->malloc(
              // query_buf_(Q_int_buf_) key_buf_(K_int_buf_)
              // value_buf_(V_int_buf_) qk_int_buf_
              // transpose_dst_(transpose_dst_int_buf_)
          ...
          if (use_trt_kernel && (mSM_ == kSM_86 || mSM_ == kSM_80 ||
                                 mSM_ == kSM_75 || mSM_ == kSM_72) &&
              size_per_head_ == 64)
            dispatcher_fp16.reset(
                new FusedMHARunnerFP16v2(head_num_, size_per_head_, mSM_));
          buf_ = (DataType_*)allocator_->malloc(
              sizeof(DataType_) * (buf_size * 7 + qk_buf_size) +
                  sizeof(DataType_*) * 9 +
                  (dispatcher_fp16.get() ? dispatcher_fp16->getWorkspaceSize()
                                         : 0),
              false);
          if (buf_ == NULL)
            throw std::runtime_error(
      ...
      if (hasChangedConfig) {
        if (int8_mode_ != 0) {
          ...
          if (isConfigExist == -1)
            printf(
                "[WARNING][OpenMultiHeadAttention] %s is not found; using "
                "default GEMM algo\n",
                IGEMM_CONFIG);
          else {
    ...
    try {
      if (int8_mode_ != 0) {
        ...
        if (dispatcher_fp16.get() && OpType_ == OperationType::FP16) {
          ...
          std::string message1("FT self-attn Q:");
          cuda_print(query_buf_, m, n, message1);

          std::string message2("FT self-attn K:");
          cuda_print(key_buf_, m, n, message2);

          std::string message3("FT self-attn V:");
          cuda_print(value_buf_, m, n, message3);
