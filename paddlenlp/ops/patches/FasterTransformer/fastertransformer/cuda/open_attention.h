...
template <typename T>
void add_QK_bias_transform_kernelLauncher(
    int8_t* q_buf,
    int8_t* k_buf,
    const int32_t* Q,
    const T* bias_Q,
...
template <typename T>
void add_QK_bias_transform_kernelLauncher(int8_t* q_buf,
                                          int8_t* k_buf,
                                          const int8_t* Q,
                                          const T* bias_Q,
                                          const int8_t* K,
...
template <typename T>
void add_V_bias_transform_kernelLauncher(int8_t* v_buf,
                                         const int8_t* V,
                                         const T* V_bias,
                                         const int batch_size,
                                         const int seq_len,
...
void mappingRemovePaddingData_kernelLauncher(const int batch_size,
                                             const int seq_len,
                                             const int valid_word_num,
                                             int* mapping,
...
template <typename T>
void add_QK_bias_transform_rebuild_padding_kernelLauncher(
    int8_t* q_buf,
    int8_t* k_buf,
    const int32_t* Q,
    const T* bias_Q,
...
template <typename T>
void add_V_bias_transform_rebuild_padding_kernelLauncher(
    int8_t* v_buf,
    const int32_t* V,
    const T* V_bias,
...
template <typename T>
void softmax_COL32_kernelLauncher(int8_t* qk_buf,
                                  const int32_t* qk_int_buf,
                                  const T* attr_mask,
                                  const int batch_size,
                                  const int head_num,
                                  const int seq_len,
...
template <typename T>
void add_QKV_bias_rebuild_padding_kernelLauncher(T* Q,
                                                 const T* bias_Q,
                                                 T* K,
                                                 const T* bias_K,
                                                 T* V,
...
template <typename T>
void transpose_kernelLauncher(T* src,
                              T* dst,
                              const int batch_size,
                              const int seq_len,
                              const int head_num,
...
template <typename T>
void transpose_rebuild_padding_kernelLauncher(T* src,
                                              T* dst,
                                              const int valid_word_num,
                                              const int batch_size,
                                              const int seq_len,
...
private:
  ...
  const float *query_weight_amax_list, *key_weight_amax_list,
...
public:
  void getCublasBmmAlgoFromMap() {
    ...
    if (cublasAlgoMap_.find(mark) != cublasAlgoMap_.end()) {
      ...
      cublasBmmAlgo_[0] = dataType == FLOAT_DATATYPE
                              ? CUBLAS_GEMM_DEFAULT
    ...
    if (cublasAlgoMap_.find(mark) != cublasAlgoMap_.end()) {
      ...
      cublasBmmAlgo_[1] = dataType == FLOAT_DATATYPE
                              ? CUBLAS_GEMM_DEFAULT
                              : CUBLAS_GEMM_DEFAULT_TENSOR_OP;
    }
  }
  ...
  void freeBuffer() {
    ...
    if (buf_ != NULL) {
      if (allocator_ == NULL) {
        printf(
            "[ERROR][OpenMultiHeadAttention][freeBuffer] allocator_ is "
            "NULL!\n");
        exit(-1);
      }
  ...
  size_t get_workspace_size() {
    ...
    const int buf_size =
        batch_size_ * head_num_ * from_seq_len_ * size_per_head_;
    const int qk_buf_size =
        batch_size_ * head_num_ * from_seq_len_ * from_seq_len_;
    const int seq_len_padded = (from_seq_len_ + 31) / 32 * 32;
    const int padded_buf_size =
        batch_size_ * head_num_ * seq_len_padded * size_per_head_;
    const int padded_qk_buf_size =
        batch_size_ * head_num_ * seq_len_padded * seq_len_padded;

    if (int8_mode_ != 0) {
      // query_buf_(Q_int_buf_) key_buf_(K_int_buf_) value_buf_(V_int_buf_)
      // qk_int_buf_ transpose_dst_(transpose_dst_int_buf_)
      ...
      size = sizeof(DataType_) * (buf_size * 7 + qk_buf_size) +
             sizeof(DataType_*) * 9 +
  ...
  void allocateBuffer(IAllocator* allocator,
    ...
    if (allocator == NULL) {
      printf(
          "[ERROR][OpenMultiHeadAttention][allocateBuffer] allocator == "
          "NULL!\n");
      exit(-1);
    }
    ...
    try {
      ...
      if (buf_ != NULL) {
        printf(
            "[ERROR][OpenMultiHeadAttention][allocateBuffer] previous buffer "
        ...
        if (int8_mode_ != 0) {
          if ((int8_mode_ == 1 && (batch_size_ * from_seq_len_ >= 512 ||
                                   (from_seq_len_ % 32 != 0))) ||
              int8_mode_ == 2) {
            if (use_trt_kernel && (sm_ == kSM_86 || sm_ == kSM_80 ||
                                   sm_ == kSM_75 || sm_ == kSM_72) &&
                size_per_head_ == 64) {
              // try
              {
                dispatcher_int8.reset(
                    new FusedMHARunnerInt8v2(head_num_, size_per_head_, sm_));
              }
            }
          }
          const int seq_len_padded = (from_seq_len_ + 31) / 32 * 32;
          ...
          trt_attn_workspace_ =
              (void*)(sequence_id_map_ + (batch_size_ * from_seq_len_));
        } else {
          if (use_trt_kernel &&
              (sm_ == kSM_70 || sm_ == kSM_86 || sm_ == kSM_80 ||
               sm_ == kSM_75 || sm_ == kSM_72) &&
              size_per_head_ == 64)
            dispatcher_fp16.reset(new FusedMHARunnerFP16v2(
                head_num_, size_per_head_, sm_, q_scaling_));
          buf_ = (DataType_*)allocator_->malloc(get_workspace_size(), false);
          if (buf_ == NULL)
            throw std::runtime_error(
                std::string("Allocator failed to allocate internal buffer."));
      ...
      if (hasChangedConfig) {
        ...
        if (isConfigExist == -1) {
          // printf("[WARNING][OpenMultiHeadAttention] %s is not found; using
          // default GEMM algo\n", int8_mode_ != 0 ? IGEMM_CONFIG :
          // GEMM_CONFIG);
        } else {
  ...
  // Ctor
  OpenMultiHeadAttention(int int8_mode = 0,
  ...
  OpenMultiHeadAttention(const OpenMultiHeadAttention* attention) {
    ...
    for (int i = 0; i < 2; i++)
      cublasBmmAlgo_[i] = attention->cublasBmmAlgo_[i];
  ...
  void multiHeadAttr_nofuse_kernelLauncher(cudaStream_t stream,
    ...
    if (int8_mode_ != 0) {
      ...
      if (size_per_head % 32 != 0) {
        printf(
            "[ERROR][FT][multiHeadAttr_nofuse_kernelLauncher] int8 unfused mha "
            "kernel only works when size_per_head %% 32 == 0.\n");
        exit(-1);
      }
      if ((seq_len % 32 != 0) && int8_mode_ == 1) {
        printf(
            "[ERROR][FT][multiHeadAttr_nofuse_kernelLauncher] int8 mode1 "
            "unfused mha kernel only works when seq_len %% 32 == 0.\n");
        exit(-1);
      }
      ...
      if (param_.sequence_id_offset == nullptr ||
        ...
        if (int8_mode_ == 1) {
          add_QK_bias_transform_kernelLauncher((int8_t*)q_buf_,
                                               (int8_t*)k_buf_,
                                               (const int32_t*)Q,
                                               bias_Q,
                                               (const int32_t*)K,
        ...
        mappingRemovePaddingData_kernelLauncher(batch_size,
                                                seq_len,
                                                param_.valid_word_num,
                                                sequence_id_map_,
                                                param_.sequence_id_offset,
                                                stream);
        // if we use remove padding, then initialize the q_buf_, k_buf_ and
        // v_buf_ to prevent bugs. v_buf_ will be properly initiaized in
        // add_V_bias_transform_rebuild_padding_kernelLauncher()
        cudaMemsetAsync((int8_t*)q_buf_,
                        0,
                        2 * batch_size_ * seq_len_padded * head_num *
                            size_per_head * sizeof(int8_t),
                        param_.stream);
        if (int8_mode_ == 1) {
          add_QK_bias_transform_rebuild_padding_kernelLauncher(
              (int8_t*)q_buf_,
              (int8_t*)k_buf_,
              (const int32_t*)Q,
              bias_Q,
      ...
      if (int8_mode_ == 1) {
        ...
        softmax_COL32_kernelLauncher((int8_t*)qk_buf_,
                                     qk_int_buf_,
                                     attr_mask,
                                     batch_size,
                                     head_num,
    ...
    // FP32/FP16
      ...
      if (param_.sequence_id_offset == nullptr ||
          param_.valid_word_num == batch_size * seq_len) {
        add_QKV_bias_transpose_kernelLauncher(q_buf_,
                                              k_buf_,
                                              v_buf_,
                                              Q,
                                              bias_Q,
        ...
        // if we use remove padding, then initialize the q_buf_, k_buf_ and
        // v_buf_ to prevent bugs.
        cudaMemsetAsync(q_buf_,
                        0,
                        3 * batch_size_ * seq_len * head_num * size_per_head *
                            sizeof(DataType_),
                        param_.stream);

        add_QKV_bias_rebuild_padding_kernelLauncher(Q,
                                                    bias_Q,
                                                    K,
                                                    bias_K,
                                                    V,
      ...
      attn_softmax_kernelLauncher(
          qk_buf_, attr_mask, batch_size, seq_len, head_num, scalar, stream);
      ...
      if (param_.sequence_id_offset == nullptr ||
          param_.valid_word_num == batch_size * seq_len) {
        transpose_kernelLauncher(transpose_dst_,
                                 dst,
                                 batch_size,
                                 seq_len,
                                 head_num,
        ...
        transpose_rebuild_padding_kernelLauncher(transpose_dst_,
                                                 dst,
                                                 param_.valid_word_num,
                                                 batch_size,
                                                 seq_len,
  ...
  void forward(const DataType_* from_tensor, const DataType_* to_tensor) {
    if (param_.sequence_id_offset != nullptr &&
        param_.valid_word_num != batch_size_ * from_seq_len_) {
      is_fuse_QKV_ = false;
    }
    ...
    try {
      if (int8_mode_ != 0) {
        // K_int_buf_ V_int_buf_ should point to correct buffer according to
        // param_.valid_word_num
        if (int8_mode_ == 1) {
          K_int_buf_ = (int*)Q_int_buf_ +
                       param_.valid_word_num * head_num_ * size_per_head_;
          V_int_buf_ = (int*)K_int_buf_ +
                       param_.valid_word_num * head_num_ * size_per_head_;
        } else if (int8_mode_ == 2 || int8_mode_ == 3) {
          K_int_buf_ =
              (int*)((int8_t*)Q_int_buf_ +
                     param_.valid_word_num * head_num_ * size_per_head_);
          V_int_buf_ =
              (int*)((int8_t*)K_int_buf_ +
                     param_.valid_word_num * head_num_ * size_per_head_);
        }

        int fusedINT8QKV = 0;
        const int8_t* Q_weight =
            (const int8_t*)(param_.self_attention.query_weight.kernel);
        const int8_t* K_weight =
            (const int8_t*)(param_.self_attention.key_weight.kernel);
        const int8_t* V_weight =
            (const int8_t*)(param_.self_attention.value_weight.kernel);
        // for QKV weight are DataType_ & continue
        if ((param_.self_attention.query_weight.kernel + n * k ==
             param_.self_attention.key_weight.kernel) &&
            (param_.self_attention.key_weight.kernel + n * k ==
             param_.self_attention.value_weight.kernel))
          fusedINT8QKV = 1;
        // for QVK weight are int8 & continue
        else if ((Q_weight + n * k == K_weight) &&
                 (K_weight + n * k == V_weight))
          fusedINT8QKV = 2;

        if (int8_mode_ == 1) {
          if (fusedINT8QKV == 0) {
            cublasLtMM_withAlgo(Q_int_buf_,
                                1,
                                m,
                                n,
                                k,
                                ...
                                0,
                                0,
                                param_.int8_from_tensor,
                                Q_weight,
                                param_.cublaslt_handle,
                                param_.stream,
                                cublasAlgoMap_,
                                use_ORDER_COL32_2R_4R4_);
            cublasLtMM_withAlgo(K_int_buf_,
                                1,
                                m,
                                n,
                                k,
                                ...
                                cublasAlgoMap_,
                                use_ORDER_COL32_2R_4R4_);
            cublasLtMM_withAlgo(V_int_buf_,
                                1,
                                m,
                                n,
                                k,
            ...
            int strideFactor =
                (fusedINT8QKV == 1) ? (sizeof(DataType_) / sizeof(int8_t)) : 1;
            cublasLtMM_withAlgo(Q_int_buf_,
                                3,
                                m,
                                n,
          ...
          if (fusedINT8QKV == 0) {
            cublasLtMM_withAlgo_int8IO((int8_t*)Q_int_buf_,
                                       1,
                                       m,
                                       n,
                                       k,
                                       ...
                                       cublasAlgoMap_,
                                       use_ORDER_COL32_2R_4R4_);
            cublasLtMM_withAlgo_int8IO((int8_t*)K_int_buf_,
                                       1,
                                       m,
                                       n,
                                       k,
                                       ...
                                       cublasAlgoMap_,
                                       use_ORDER_COL32_2R_4R4_);
            cublasLtMM_withAlgo_int8IO((int8_t*)V_int_buf_,
                                       1,
                                       m,
                                       n,
                                       k,
        ...
        if (dispatcher_int8.get() && dispatcher_int8->isValid(S) &&
            param_.trt_seqlen_offset != nullptr) {
          // This function is only used when we satisfy the following
          // conditions:
          // 1. INT8
          // 2. GPU SM >= 75
          int8_fused_multiHeadAttr_kernelLauncher(
              (const void*)Q_int_buf_,
              param_.amaxList + 4 + 1,
        ...
        if (is_fuse_QKV_ == true) {
          int algoId = getAlgoIdFromMap(
              cublasAlgoMap_,
              3,
              n,
              m,
              k,
              AType_ == CUDA_R_16F ? HALF_DATATYPE : FLOAT_DATATYPE);
          check_cuda_error(
        ...
        if (dispatcher_fp16.get() && OpType_ == OperationType::FP16 &&
            dispatcher_fp16->isValid(S) &&
            param_.trt_seqlen_offset != nullptr) {
          // This function is only used when we satisfy the following
          // conditions:
          // 1. FP16
          // 2. GPU SM >= 72
          //  3. Temporally add seqlen <= 384 limitation because the current
          //  fused mha cannot handle seqlen > 384.
  ...
  void int8_fused_multiHeadAttr_kernelLauncher(const void* Q,
                                               const float* q_deQFactor_ptr,
                                               const float* k_deQFactor_ptr,
                                               const float* v_deQFactor_ptr,
                                               const float mScaleQkv,
                                               const int S);

  void trt_add_QKV_bias_kernelLauncher(const DataType_* bias_Q,
                                       const DataType_* bias_K,
                                       const DataType_* bias_V);
  ...
  void initialize(MultiHeadInitParam<DataType_> param) {
    ...
    if (int8_mode_ != 0) {
      ...
      if (dispatcher_int8.get()) {
        dispatcher_int8.get()->setScaleList(
            param_.trt_fused_mha_amax_list[0] / 127.0f,
            param_.trt_fused_mha_amax_list[1] / 127.0f,
            param_.trt_fused_mha_amax_list[2] / 127.0f);
      }
    }
  }
  ...
  ~OpenMultiHeadAttention() override {
    if (buf_ != NULL) {
      if (allocator_ == NULL) {
        printf(
            "[ERROR][OpenMultiHeadAttention][~OpenMultiHeadAttention] "
            "allocator_ is NULL!\n");
        exit(-1);
      }
      allocator_->free(buf_);
