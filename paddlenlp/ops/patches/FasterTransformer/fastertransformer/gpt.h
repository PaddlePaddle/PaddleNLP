...
public:
  DecodingGpt(const IAllocator &allocator,
    ...
    decoder_ = new OpenDecoder<OpType_>(args_.head_num_,
                                        size_per_head,
                                        0 /* memory_hidden_units */,
                                        is_fuse_QKV);
    ...
    size_t decoder_workspace_size =
        (size_t)decoder_->getWorkspaceSize();  // type T
    size_t decoder_normed_result_buffer_size =
        args_.batch_size_ * args_.hidden_units_;  // type T
    // cache costs lots of memory, so we only store part of them when we use
    // multi-gpu for inference
    size_t cache_size = args_.batch_size_ * args_.seq_len_ *
                        args_.hidden_units_ / tensor_para_size;  // type T
    size_t logits_buf_size =
        args_.batch_size_ * args_.vocab_size_padded_;  // type T

    size_t topp_id_vals_buf_size =
    ...
    const int MEM_C = 128;
    size_t embedding_kernel_transposed_padded_size =
        args_.hidden_units_ * args_.vocab_size_padded_;
    embedding_kernel_transposed_padded_size =
        div_up(embedding_kernel_transposed_padded_size, MEM_C) * MEM_C;
    ...
    size_t datatype_buf_size =
        ...
        cache_size * 2 * (args_.decoder_layers_ / layer_para_size) +
    ...
    buf_ = reinterpret_cast<void *>(allocator_.malloc(
        ...
        sizeof(int) * (topp_id_vals_buf_size + topp_offset_buf_size +
                       begin_topp_offset_buf_size) +
        topp_workspace_size_ + topk_workspace_size_ +
        topk_topp_workspace_size_ + sizeof(DataType_) * nccl_buf_size_ +
    ...
    from_tensor_[0] = (DataType_ *)(embedding_kernel_padded_ +
                                    embedding_kernel_transposed_padded_size);
    from_tensor_[1] = (DataType_ *)(from_tensor_[0] + from_tensor_size);

    K_cache_[0] = from_tensor_[1] + from_tensor_size +
                  0 * cache_size * args_.decoder_layers_ / layer_para_size;
    V_cache_[0] = from_tensor_[1] + from_tensor_size +
                  1 * cache_size * args_.decoder_layers_ / layer_para_size;

    decoder_buf_ =
        V_cache_[0] + cache_size * args_.decoder_layers_ / layer_para_size;
    ...
    topp_offset_buf_ =
        (int *)((int *)begin_topp_offset_buf_ + begin_topp_offset_buf_size);
    ...
    topk_topp_workspace_ =
        (void *)((char *)topk_workspace_ + topk_workspace_size_);
    nccl_logits_buf_ =
        (DataType_ *)((char *)topk_topp_workspace_ + topk_topp_workspace_size_);
    ...
    cudaMemset(embedding_kernel_padded_,
               0,
               embedding_kernel_transposed_padded_size * sizeof(DataType_));
  ...
  void forward_context(const DecoderInitParam<DataType_> *decoder_param,
    ...
    const int max_len =
        (decoding_params.request_output_len > 0 &&
         input_len + decoding_params.request_output_len <= args_.seq_len_)
            ? input_len + decoding_params.request_output_len
            : args_.seq_len_;
    const int request_batch_size = decoding_params.request_batch_size;
    cudaMemsetAsync(decoding_params.output_ids,
                    0,
                    sizeof(int) * request_batch_size * max_len,
    ...
    if (input_len == 1) {
      cudaMemcpyAsync(decoding_params.output_ids,
                      decoding_params.d_start_ids,
                      sizeof(int) * request_batch_size,
    ...
    for (int ite = 0; ite < ite_num; ite++) {
      ...
      for (int layer = 0; layer < args_.decoder_layers_; ++layer) {
        if (l_parallel_param_.is_valid(layer)) {
          ...
          if (layer ==
                  l_parallel_param_.layers_per_group * l_parallel_param_.rank &&
              layer != 0 && l_parallel_param_.world_size > 1) {
            const int size = m * t_parallel_param_.local_hidden_units_;
            nccl_recv(from_tensor[in_id] + ite * m * h_1 +
                          size * t_parallel_param_.rank,
                      size,
                      l_parallel_param_.rank - 1,
                      l_parallel_param_,
                      decoding_params.stream);
            all2all_gather(from_tensor[in_id] + ite * m * h_1,
            ...
            cache_offset =
                (layer -
                 l_parallel_param_.layers_per_group * l_parallel_param_.rank) *
                    args_.batch_size_ * args_.seq_len_ *
                    t_parallel_param_.local_hidden_units_ +
                ite * local_batch_size * args_.seq_len_ *
                    t_parallel_param_.local_hidden_units_;
          }
          decoder_->forward_context(
              decoder_workspace,
              ...
              decoding_params.d_attn_mask +
                  ite * local_batch_size * input_len * input_len,
              local_batch_size,
              input_len,
              ite,
...
#endif
          if (layer ==
                  l_parallel_param_.layers_per_group *
                          (l_parallel_param_.rank + 1) -
                      1 &&
              layer != args_.decoder_layers_ - 1 &&
              l_parallel_param_.world_size > 1) {
            const int size = m * t_parallel_param_.local_hidden_units_;
            nccl_send(from_tensor[out_id] + ite * m * h_1 +
                          size * t_parallel_param_.rank,
                      size,
                      l_parallel_param_.rank + 1,
                      l_parallel_param_,
  ...
  void forward(const DecoderInitParam<DataType_> *decoder_param,
    ...
    cudaMemsetAsync(finished_buf_,
                    false,
                    sizeof(finished_buf_[0]) * request_batch_size,
                    decoding_params.stream);
    ...
    if (std::is_same<DataType_, float>::value ||
      ...
      cudaMemcpyAsync(
          ...
          sizeof(DataType_) * args_.vocab_size_ * args_.hidden_units_,
          cudaMemcpyDeviceToDevice,
          decoding_params.stream);
      ...
      for (size_t ite = 0; ite < ite_num; ite++) {
        if (l_parallel_param_.rank == 0 && l_parallel_param_.world_size > 1) {
          if (step != (size_t)input_len) {
            ...
            nccl_recv(
                decoding_params.output_ids + (step - 1) * m + ite * local_batch,
                local_batch,
                l_parallel_param_.world_size - 1,
                l_parallel_param_,
                decoding_params.stream);
        ...
        if (l_parallel_param_.rank < l_parallel_param_.world_size - 1 &&
            l_parallel_param_.world_size > 1) {
          if (step != (size_t)input_len) {
            nccl_broadcast(finished_buf_ + ite * local_batch,
                           local_batch,
                           l_parallel_param_.world_size - 1,
                           l_parallel_param_,
                           decoding_params.stream);
        ...
        if (ite == 0) {
          cudaMemcpyAsync(h_finished_buf_,
                          finished_buf_,
                          sizeof(bool) * request_batch_size,
        ...
        if (l_parallel_param_.rank == 0) {
          ...
          embedding_position_lookups_kernel_launcher(
              from_tensor_[0],
              decoding_params.embedding_table,
              decoding_params.position_encoding_table,
              decoding_params.output_ids,
        ...
        for (int layer = 0; layer < args_.decoder_layers_; ++layer) {
          if (l_parallel_param_.is_valid(layer)) {
            ...
            */
            ...
            if (layer ==
                    l_parallel_param_.layers_per_group *
                        l_parallel_param_.rank &&
                layer != 0 && l_parallel_param_.world_size > 1) {
              const int size =
                  local_batch * t_parallel_param_.local_hidden_units_;
              nccl_recv(from_tensor_[from_id] + size * t_parallel_param_.rank,
                        size,
                        l_parallel_param_.rank - 1,
                        l_parallel_param_,
                        decoding_params.stream);
              all2all_gather(from_tensor_[from_id],
            ...
            /*
                ...
                At the beginning of each decoder layer, we initialize the
               decoder object
            ...
            */
            ...
            if (dummy_decoder_max_seq_len == -1) {
              ...
              cache_offset = (layer -
                              l_parallel_param_.layers_per_group *
                                 ...
                                 args_.batch_size_ * args_.seq_len_ *
                                 t_parallel_param_.local_hidden_units_ +
                             ite * local_batch * args_.seq_len_ *
                                 t_parallel_param_.local_hidden_units_;
            }
            decoder_->forward_v2(
                from_tensor_[from_id],
                ...
                V_cache_[0] + cache_offset,
                nullptr,
                nullptr,  // key_mem_cache_ and value_mem_cache_ should be
                          // nullptr
            ...
            if (layer ==
              ...
              const size_t size =
                  local_batch * t_parallel_param_.local_hidden_units_;
              nccl_send(from_tensor_[out_id] + size * t_parallel_param_.rank,
                        size,
                        l_parallel_param_.rank + 1,
                        l_parallel_param_,
                        decoding_params.stream);
        ...
        if (l_parallel_param_.rank == l_parallel_param_.world_size - 1) {
          ...
          if (t_parallel_param_.world_size > 1) {
            ...
            all2all_gather(nccl_logits_buf_,
                           nccl_logits_buf_,
                           local_batch * n,
                           t_parallel_param_,
                           decoding_params.stream);
            ...
            transpose_axis_01_kernelLauncher(logits_buf_,
                                             nccl_logits_buf_,
                                             t_parallel_param_.world_size,
                                             local_batch,
                                             n,
                                             decoding_params.stream);
          }
          ...
          if (args_.repetition_penalty_ != 1.0) {
            ...
            apply_repetition_penalty_kernelLauncher(
                logits_buf_,
          ...
          if (args_.candidate_num_ > 0 && args_.probability_threshold_ == 0.0) {
            ...
            POP_RANGE
                     ...
                     args_.probability_threshold_ > 0.0f) {
            PUSH_RANGE("After Transformer/Sampling")
            // top p sampling
            ...
            POP_RANGE
                     ...
                     args_.probability_threshold_ > 0.0f) {
            PUSH_RANGE("After Transformer/Sampling")
            topK_topP_sampling_kernel_kernelLauncher_v2(
                topk_topp_workspace_,
                topk_topp_workspace_size_,
                decoding_params.output_ids + step * m + ite * local_batch,
                logits_buf_,
        ...
        if (step < (size_t)max_input_len) {
          ...
          set_start_ids_kernelLauncher(decoding_params.output_ids,
                                       decoding_params.d_start_ids,
                                       max_input_len,
                                       step,
                                       ite,
        ...
        if (l_parallel_param_.rank == l_parallel_param_.world_size - 1 &&
          ...
          nccl_send(decoding_params.output_ids + step * m + ite * local_batch,
                    local_batch,
                    0,
                    l_parallel_param_,
                    decoding_params.stream);
        ...
        if (l_parallel_param_.rank == l_parallel_param_.world_size - 1 &&
            l_parallel_param_.world_size > 1 && step < max_len - 1) {
          nccl_broadcast(finished_buf_ + ite * local_batch,
                         local_batch,
                         l_parallel_param_.world_size - 1,
                         l_parallel_param_,
    ...
    if (l_parallel_param_.rank == 0 && l_parallel_param_.world_size > 1) {
      for (size_t ite = 0; ite < request_batch_size / local_batch; ite++) {
        nccl_recv(
            decoding_params.output_ids + (max_len - 1) * m + ite * local_batch,
            local_batch,
            l_parallel_param_.world_size - 1,
            l_parallel_param_,
            decoding_params.stream);
