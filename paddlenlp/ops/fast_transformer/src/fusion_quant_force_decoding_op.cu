...
__global__ void quant_get_trg_length(const int* trg_word,
                               int* trg_length,
                               const int seq_len,
                               const int pad_id) {
  int bid = threadIdx.x;
...
std::vector<paddle::Tensor> decoding_kernel(
  ...
  if (trg_word_shape.size() == 2 && trg_word_shape[0] != 0) {
    ...
    quant_get_trg_length<<<1, trg_word_shape[0], 0, stream>>>(
...
std::vector<paddle::Tensor> QuantForceDecodingCUDAForward(
    const paddle::Tensor& input,
