    ...
    def process(cls, predictor, tokenizer, data, parameters):
        ...
        if 'text_pair' in data and data['text_pair'] is not None:
            ...
            if len(text) != len(text_pair):
                raise ValueError(
                    "The length of text and text_pair must be same.")
            has_pair = True
        ...
        examples = []
        for input_ids, token_type_ids in zip(
                tokenizer_result['input_ids'],
                tokenizer_result['token_type_ids']):
        ...
        for batch in batches:
            ...
            if predictor._predictor_type == 'paddle_inference':
                ...
                outputs = predictor._predictor.run(
                    None, {
                        'input_ids': input_ids,
