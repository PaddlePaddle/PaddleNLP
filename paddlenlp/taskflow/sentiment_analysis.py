...
class UIESentaTask(Task):
    ...
    def check_aspects(self, aspects):
        if aspects is None:
            ...
            raise ValueError(
                    "Invalid aspects, input aspects should not be empty, but {} found!".format(aspects))
        else:
            for i, aspect in enumerate(aspects):
                if not isinstance(aspect, str):
                    raise TypeError(
                        "Invalid aspect, the aspect at index {} should be str, but type of {} found!".format(i, type(aspect)))
                if not aspect.strip():
                    raise ValueError(
