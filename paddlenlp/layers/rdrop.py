...
def rdrop_loss(p, q, pad_mask=None, reduction='none'):
    ...
    """
    p_loss = F.kl_div(F.log_softmax(p, axis=-1), F.softmax(q, axis=-1), reduction=reduction)
    q_loss = F.kl_div(F.log_softmax(q, axis=-1), F.softmax(p, axis=-1), reduction=reduction)
