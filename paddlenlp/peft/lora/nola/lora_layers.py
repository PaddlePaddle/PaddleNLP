...
class LoRALinear(nn.Linear):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if dora:
            self.m = self.create_parameter(
                shape=[1, out_features],  # 修改shape为[1, out_features]以匹配norm的keepdim=True
                dtype=self._dtype,
                is_bias=False,
            )
            # 初始化m为weight的L2范数
            with paddle.no_grad():
                self.m.set_value(paddle.norm(self.weight, p=2, axis=0, keepdim=True))
        if nola:
    ...
    def merge(self):
        if not self.merged:
            if self.nola:
                self.lora_A.data, self.lora_B.data = self.get_nola_lora_matrices()
    ...
    def forward(self, input: paddle.Tensor, *args, **kwargs):
        if not self.apply_pissa and self.pissa:
            ...
            lora = self.lora_A @ self.lora_B  # 计算LoRA的低秩近似
            numerator = self.weight + self.lora_alpha * lora.T
            denominator = paddle.norm(numerator, p=2, axis=0, keepdim=True)
            directional_component = numerator / denominator  # normalize，计算方向组件
            new_weight = self.m * directional_component  # 计算新的权重
            return F.linear(x=input, weight=new_weight, bias=self.bias, name=self.name)
...
class RowParallelLoRALinear(RowParallelLinear):
    def __init__(
        ...
        **kwargs
        ...
        if dora:
            raise ValueError("Dora is not supported in model parallel by now")

        if nola:
            raise ValueError("Nola is not supported in model parallel by now")
