...
from paddlenlp.peft.utils import INCLUDE_LINEAR_LAYERS_SHORTHAND
...
from paddlenlp.peft.utils.peft_types import PeftType, TaskType
...
# from transformers.paddle_utils import Conv1D
...
def onload_layer(layer):
    ...
    offloaded_modules = []
    for name, module in layer.named_sublayers():
    ...
    if hasattr(layer, "base_layer") and (
        ...
        if paddle.device("meta") in layer.base_layer._hf_hook.original_devices.values() and hasattr(
    ...
    for module in offloaded_modules:
        module._hf_hook.post_forward(module, paddle.tensor([]))
    ...
    if base_layer_offload:
        ...
        layer.base_layer._hf_hook.post_forward(layer.base_layer, paddle.tensor([]))
...
class BaseTuner(nn.Layer, ABC):
    ...
    def _pre_injection_hook(self, model: nn.Layer, config: PeftConfig, adapter_name: str) -> None:
        r"""
    ...
    def _prepare_model(self, peft_config: PeftConfig, model: nn.Layer):
        r"""
    ...
    def _cast_adapter_dtype(self, adapter_name: str, autocast_adapter_dtype: bool = True) -> None:
        ...
        dtypes_to_convert_to_fp32 = {paddle.float16, paddle.bfloat16}
        ...
        for module in self.model.sublayers():
            ...
            for submodule in module.sublayers():
                if not isinstance(submodule, (nn.LayerDict, nn.ParameterDict, BufferDict)):
                    continue

                if adapter_name not in submodule:
                    continue

                if isinstance(submodule[adapter_name], paddle.base.framework.Parameter):
                    if submodule[adapter_name].dtype in dtypes_to_convert_to_fp32:
                        submodule[adapter_name].data = submodule[adapter_name].data.to(paddle.float32)
                    continue

                if isinstance(submodule[adapter_name], paddle.Tensor):  # e.g. from a BufferDict
                    if submodule[adapter_name].dtype in dtypes_to_convert_to_fp32:
                        submodule[adapter_name] = submodule[adapter_name].to(paddle.float32)
                    continue
    ...
    def inject_adapter(
        self, model: nn.Layer, adapter_name: str, autocast_adapter_dtype: bool = True, low_cpu_mem_usage: bool = False
    ) -> None:
        r"""
        ...
        key_list = [key for key, _ in model.named_sublayers()]
    ...
    @staticmethod
    def get_model_config(model: nn.Layer) -> dict:
        """
    ...
    def _get_tied_target_modules(self, model: nn.Layer) -> list[str]:
        tied_target_modules = []
        model_config = self.get_model_config(model)
...
class BaseTunerLayer(ABC):
    ...
    Args:
        is_pluggable (`bool`, *optional*):
            Whether the adapter layer can be plugged to any pypaddle module
    ...
    def _get_available_adapters(self) -> set[str]:
        ...
        for layer_name in self.adapter_layer_names:
            ...
            if not isinstance(module, (nn.LayerDict, nn.ParameterDict)):
                continue
        ...
        for layer_name in self.adapter_layer_names:
            ...
            for key, layer in module_dict.items():
                if key in adapter_names:
                    ...
                    layer.stop_gradients = False
                else:
                    # layer.requires_grad_(False)
                    layer.stop_gradients = True
    ...
    def _move_adapter_to_device_of_base_layer(self, adapter_name: str, device: Optional[paddle.device] = None) -> None:
        """
        ...
        for adapter_layer_name in self.adapter_layer_names + self.other_param_names:
            ...
            if not isinstance(adapter_layer, (nn.LayerDict, nn.ParameterDict, BufferDict)):
                continue
            if adapter_name not in adapter_layer:
                continue
            if any(p.place == meta for p in adapter_layer.parameters()):
                continue

            if paddle.is_floating_point(weight) or paddle.is_complex(weight):
                adapter_layer[adapter_name] = adapter_layer[adapter_name].to(device=place_to_str(device), dtype=dtype)
            else:
                adapter_layer[adapter_name] = adapter_layer[adapter_name].to(place_to_str(device))


def place_to_str(place):
    # Place(gpu:0) cuda cpu
    p = str(place)
    p = p.replace("Place", "")
    p = p.replace("(", "")
    p = p.replace(")", "")
...
def load_state_dict(self: paddle.nn.Layer, state_dict: dict, use_structured_name=True, strict=True):
    ...
    self.load_dict(state_dict, use_structured_name=use_structured_name)
...
def inspect_matched_modules(tuner: BaseTuner, adapter_name: str = "default") -> dict:
    ...
    key_list = [key for key, _ in tuner.model.named_sublayers()]
...
def _maybe_include_all_linear_layers(peft_config: PeftConfig, model: nn.Layer) -> PeftConfig:
    """
    ...
    linear_classes = (paddle.nn.Linear, Conv1D)
    ...
    if isinstance(model, PretrainedModel):
        ...
        if output_emb is not None:
            ...
            last_module_name = [name for name, module in model.named_sublayers() if module is output_emb][0]
            ...
            for name in SEQ_CLS_HEAD_NAMES:
                ...
                if cls_head is not None:
                    last_module_name = [name for name, module in model.named_sublayers() if module is cls_head][0]
                    module_names_to_exclude.add(last_module_name)
                    break
...
def clone_module(module: nn.Layer, share_weights=False):
    """Clone a module in a pypaddle model.
    ...
    def _share_weights(src: nn.Layer, dst: nn.Layer):
...
def replicate_layers(model: nn.Layer, layer_map: list[tuple[int, int]]):
    ...
    if hasattr(model, "layers"):
        ...
        model_type = "falcon"
        layers = model.h
    if not model_type or not isinstance(layers, nn.LayerList):
        raise ValueError(
    ...
    for start, end in layer_map:
        for i in range(start, end):
            ...
            for submodule in new_layers[-1].sublayers():
                if hasattr(submodule, "layer_idx"):
                    submodule.layer_idx = current_idx
    layers = nn.LayerList(new_layers)
    if model_type == "llama":
        model.layers = layers
