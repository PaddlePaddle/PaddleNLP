...
class IncrementalPCA:
    ...
    Args:
        ...
        svd_driver (str, optional): name of the cuSOLVER method to be used for paddle.linalg.svd. This keyword
            argument only works on CUDA inputs. Available options are: None, gesvd, gesvdj, and gesvda. Defaults to
            None.
        lowrank (bool, optional): Whether to use paddle.svd_lowrank instead of paddle.linalg.svd which can be faster.
            Defaults to False.
        lowrank_q (int, optional): For an adequate approximation of n_components, this parameter defaults to
            n_components * 2.
        lowrank_niter (int, optional): Number of subspace iterations to conduct for paddle.svd_lowrank.
            Defaults to 4.
        lowrank_seed (int, optional): Seed for making results of paddle.svd_lowrank reproducible.
    """
    ...
    def _svd_fn_full(self, X):
        return paddle.linalg.svd(X, full_matrices=False, driver=self.svd_driver)

    def _svd_fn_lowrank(self, X):
        seed_enabled = self.lowrank_seed is not None
        with paddle.random.fork_rng(enabled=seed_enabled):
            if seed_enabled:
                paddle.manual_seed(self.lowrank_seed)
            U, S, V = paddle.svd_lowrank(X, q=self.lowrank_q, niter=self.lowrank_niter)
            return U, S, V.mH

    def _validate_data(self, X) -> paddle.Tensor:
        """
        Validates and converts the input data `X` to the appropriate tensor format.

        Args:
            X (paddle.Tensor): Input data.

        Returns:
            paddle.Tensor: Converted to appropriate format.
        """
        valid_dtypes = [paddle.float32, paddle.float64]
    ...
    def _incremental_mean_and_var(
        ...
        Args:
            X (paddle.Tensor): The batch input data tensor with shape (n_samples, n_features).
            last_mean (paddle.Tensor): The previous mean tensor with shape (n_features,).
            last_variance (paddle.Tensor): The previous variance tensor with shape (n_features,).
            last_sample_count (paddle.Tensor): The count tensor of samples processed before the current batch.

        Returns:
            Tuple[paddle.Tensor, paddle.Tensor, paddle.Tensor]: Updated mean, variance tensors, and total sample count.
        """
        if X.shape[0] == 0:
        ...
        new_sample_count = paddle.tensor([X.shape[0]], device=X.device)
        updated_sample_count = last_sample_count + new_sample_count

        if last_mean is None:
            last_sum = paddle.zeros(X.shape[1], dtype=paddle.float64, device=X.device)
        else:
            last_sum = last_mean * last_sample_count

        new_sum = X.sum(dim=0, dtype=paddle.float64)

        updated_mean = (last_sum + new_sum) / updated_sample_count

        T = new_sum / new_sample_count
        temp = X - T
        correction = temp.sum(dim=0, dtype=paddle.float64).square()
        temp.square_()
        new_unnormalized_variance = temp.sum(dim=0, dtype=paddle.float64)
    ...
    @staticmethod
    def _svd_flip(u, v, u_based_decision=True) -> Tuple[paddle.Tensor, paddle.Tensor]:
        """
        ...
        Args:
            u (paddle.Tensor): Left singular vectors tensor.
            v (paddle.Tensor): Right singular vectors tensor.
            u_based_decision (bool, optional): If True, uses the left singular vectors to determine the sign flipping.
                Defaults to True.

        Returns:
            Tuple[paddle.Tensor, paddle.Tensor]: Adjusted left and right singular vectors tensors.
        """
        if u_based_decision:
            max_abs_cols = paddle.argmax(paddle.abs(u), dim=0)
            signs = paddle.sign(u[max_abs_cols, range(u.shape[1])])
        else:
            max_abs_rows = paddle.argmax(paddle.abs(v), dim=1)
            signs = paddle.sign(v[range(v.shape[0]), max_abs_rows])
    ...
    def fit(self, X, check_input=True):
        ...
        Args:
            X (paddle.Tensor): The input data tensor with shape (n_samples, n_features).
    ...
    def partial_fit(self, X, check_input=True):
        ...
        if first_pass:
            ...
            self.n_samples_seen_ = paddle.tensor([0], device=X.device)
        ...
        if first_pass:
            ...
            col_batch_mean = paddle.mean(X, dim=0)
            X -= col_batch_mean
            mean_correction_factor = paddle.sqrt((self.n_samples_seen_.double() / n_total_samples) * n_samples)
        ...
        explained_variance_ratio = S**2 / paddle.sum(col_var * n_total_samples)
        ...
        if self.n_components not in (n_samples, n_features):
            ...
            self.noise_variance_ = paddle.tensor(0.0, device=X.device)
        return self

    def transform(self, X) -> paddle.Tensor:
        """
        ...
        Args:
            X (paddle.Tensor): New data tensor with shape (n_samples, n_features) to be transformed.

        Returns:
            paddle.Tensor: Transformed data tensor with shape (n_samples, n_components).
        """
        X = X - self.mean_
        return paddle.mm(X.double(), self.components_.T).to(X.dtype)
