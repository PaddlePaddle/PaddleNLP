...
class DPOTrainer(Trainer):
    ...
    def __init__(
        self, model, data_collator, ref_model=None, disable_dropout: bool = True, **kwargs
    ):
        ...
        if self.reference_free and self.args.dpo_loss_type not in ["sigmoid", "hinge", "ipo"]:
            raise ValueError(
                f"{self.args.dpo_loss_type} is not a valid loss type for DPO reference_free."
            )
        if disable_dropout:
    ...
    def get_batch_logps(self, batch, logits,average_log_prob=False,):
        """DPO logprobs"""
            ...
            if self.reference_free:
                ...
                with paddle.no_grad():
                    ...
                    reference_chosen_logps, reference_rejected_logps = self.get_batch_logps(
                        batch, ref_logits, average_log_prob=self.args.dpo_loss_type == "ipo",)
            policy_logits = model(**dpo_inputs)[0]
            policy_chosen_logps, policy_rejected_logps = self.get_batch_logps(
                batch, policy_logits, average_log_prob=self.args.dpo_loss_type == "ipo",)
