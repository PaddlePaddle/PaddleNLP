    ...
    def set_state_dict(self, state_dict):
        ...
        for idx in range(self.config.num_hidden_layers):
            ...
            self.transformer_block.ln_scales[idx].set_value(
                paddle.to_tensor(state_dict["llama.layers.{}.input_layernorm.weight".format(idx)], dtype=self.transformer_block.ln_scales[idx].dtype)
            )

            self.transformer_block.ffn_ln_scales[idx].set_value(
                paddle.to_tensor(state_dict["llama.layers.{}.post_attention_layernorm.weight".format(idx)], dtype=self.transformer_block.ffn_ln_scales[idx].dtype)
            )
