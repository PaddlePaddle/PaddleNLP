...
class FusedBlockMultiTransformerFP8DynamicQuant(FusedBlockMultiTransformer):
    ...
    def get_scale_shape(self, weight_shape: list, ffn1=False, moe=False):
        ...
        scale_shape = weight_shape[:-2]
        if block_n != 0 and block_k != 0:
            if ffn1:
                scale_shape[-2] = 2 * ((n // 2 + block_k - 1) // block_k)
            else:
                scale_shape[-2] = (n + block_k - 1) // block_k
            scale_shape[-1] = (k + block_n - 1) // block_n
            return scale_shape

        if block_n == 0 and block_k == 0:
            shape_prefix = weight_shape[:-2]
            if moe:
                return shape_prefix + [1]
            else:
                if ffn1:
                    return shape_prefix + [2]
                else:
                    return shape_prefix + [1]
        return scale_shape
        ...
        if self.config.moe_config.topk_method is not None:
            ...
            if self.moe_quant_type in ["weight_only_int4", "weight_only_int8"]:
                ...
                fused_moe_out = fused_moe(
                    ...
                    else self.weight_block_size,  # default block-wise, per-tensor is None
                )
        else:
...
class FusedBlockMultiTransformerFP8TensorWise(FusedBlockMultiTransformer):
    def __init__(self, config: FusedMultiTransformerConfig):
        super().__init__(config)
        self.quant_type = config.quant_type
        self.fp8_type = "float8_e4m3fn"
        ...
        self.ffn1_weights_scale = []
        self.ffn2_weights_scale = []
        self.ffn1_acts_scale = []
        self.ffn2_acts_scale = []
        ...
        for i in range(self.num_layers):
            ...
            if self.config.moe_config.use_moe(i):
                if self.moe_quant_type in ["weight_only_int4", "weight_only_int8"]:
                    ...
                    ffn1_act_attr = paddle.ParamAttr(
                        name=f"fuse.{i}.ffn1_act_scale",
                        initializer=paddle.nn.initializer.Constant(value=0),
                    )
                    ffn1_act_scale = self.create_parameter(
                        shape=[1],
                        attr=ffn1_act_attr,
                        dtype="float32",
                        is_bias=False,
            ...
            if self.config.moe_config.use_moe(i):
                if self.moe_quant_type in ["weight_only_int4", "weight_only_int8"]:
                    ...
                    ffn2_act_attr = paddle.ParamAttr(
                        name=f"fuse.{i}.ffn2_act_scale",
                        initializer=paddle.nn.initializer.Constant(value=0),
                    )
                    ffn2_act_scale = self.create_parameter(
                        shape=[1],
                        attr=ffn2_act_attr,
                        dtype="float32",
                        is_bias=False,
            ...
            self.ffn1_weights_scale.append(ffn1_weight_scale)
            self.ffn2_weights_scale.append(ffn2_weight_scale)
            self.ffn1_acts_scale.append(ffn1_act_scale)
            self.ffn2_acts_scale.append(ffn2_act_scale)
            ...
            self._add_parameter(ffn1_weight_scale)
            self._add_parameter(ffn2_weight_scale)
            self._add_parameter(ffn1_act_scale)
            self._add_parameter(ffn2_act_scale)
    ...
    def get_scale_shape(self, weight_shape: list, ffn1=False, moe=False):
        ...
        if block_n == 0 and block_k == 0:
            ...
            if moe:
                return shape_prefix + [1]
                # if ffn1:
                #     scale_shape[-2] = 2 * ((n // 2 + 128 - 1) // 128)
                # else:
                #     scale_shape[-2] = (n + 128 - 1) // 128
                # scale_shape[-1] = (k + 128 - 1) // 128
            else:
                if ffn1:
                    return shape_prefix + [2]
                else:
    ...
    def compute_layernorm_before_qkv(self, src, i):
        ...
        if i == 0 and not self.config.speculate_config.speculate_method == "eagle":
            ln_out = self.norm_func(
                ...
                self._epsilon,
                begin_norm_axis=1,
                quant_scale=0.02,  # quant_in_scale
                quant_round_type=1,
                quant_max_bound=self.config.quant_max_bound,
                quant_min_bound=self.config.quant_min_bound,
            )[0]
    ...
    def compute_qkv_linear(self, ln_out, i, latent_cache=None, **kwargs):
        if self.config.mla_config.use_mla():
            ...
            key_value = self.cutlass_fp8_gemm(
                x=compressed_kv.clip(min=-448.0, max=448.0).cast(self.fp8_type),
                y=self.kv_b_proj_weights[i],
                bias=None,
                scale=0.02,
                output_dtype=self._dtype,
    ...
    def compute_mla_absorb(
        ...
        **kwargs,
        ...
        fmha_out = paddle.zeros(
            shape=[ln_out.shape[0], self.num_heads * self.config.mla_config.v_head_dim], dtype=self._dtype
        )
        ...
        if kwargs["max_dec_len_this_time"]:  # decode phase
            ...
            compressed_kv = self.cutlass_fp8_gemm(
                x=ln_out if self.config.mla_config.q_lora_rank is not None else ln_out_or_q_c,
                y=self.kv_a_proj_with_mqa_weights[i],
                scale=0.02,
                bias=None,
                output_dtype=self._dtype,
        ...
        return fmha_out.clip(min=-448.0, max=448.0).cast(self.fp8_type)
    ...
    def compute_fused_moe(self, tmp_out, i):
        # tmp_out = tmp_out.cast(self._dtype)
        e_score_correction_bias = self.e_score_correction_biases[i]
        ...
        if self.config.moe_config.topk_method is not None:
            ...
            if self.moe_quant_type in ["weight_only_int4", "weight_only_int8"]:
                ...
                fused_moe_out = fused_moe(
                    ...
                    w2_scale=self.ffn2_weights_scale[i] if hasattr(self, "ffn2_weights_scale") else None,
                    # block_shape=[128, 128],
                    a1_scale=self.ffn1_acts_scale[i] if hasattr(self, "ffn1_acts_scale") else None,
                    a2_scale=self.ffn2_acts_scale[i] if hasattr(self, "ffn2_acts_scale") else None,
                    block_shape=None
                    if sum(self.weight_block_size) == 0
                    else self.weight_block_size,  # default block-wise, per-tensor is None
                )
    ...
    def compute_shared_expert(self, tmp_out, i):
        ...
        if self.config.moe_config.shared_expert_with_gate:
            gate_out = paddle.matmul(tmp_out.cast(self._dtype), self.shared_expert_gate_weights[i])
            gate_out = paddle.nn.functional.sigmoid(gate_out)
            return gate_out * ffn2_out
        return ffn2_out
