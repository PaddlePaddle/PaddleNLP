        ...
        if USE_ABSORB:
            ...
            if query.shape[0] > 1:
                ...
                q_nope_out = paddle.bmm(
                    query_nope.transpose([1, 0]), wk_b  # [num_head, n, qk_nope_head_dim]
                ).transpose([1, 0])
                ...
                fmha_out = fmha_out.reshape([-1, self.num_heads, self.config.mla_config.kv_lora_rank]).transpose(
                    [1, 0]
                )
                print("fmha_out up:", fmha_out.shape)
                fmha_out = (
                    paddle.bmm(fmha_out, wv_b)
                    .transpose([1, 0])
