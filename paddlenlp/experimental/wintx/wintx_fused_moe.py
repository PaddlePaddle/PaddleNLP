...
def fused_moe_kernel_paddle(
    ...
    offs_shift = tl.arange(0, BLOCK_SIZE_K)

    b_shift_bits = ((offs_shift[:, None] % pack_num) * state_bit).to(tl.int32)
...
def fused_moe_wintx_impl(
    hidden_states, w1, w2, topk_weights, topk_ids, w1_scale=None, w2_scale=None, group_size=-1, bit=4
):
...
def fused_moe_wintx(hidden_states, w1, w2, scores, topk: int, w1_scale=None, w2_scale=None, bit=4):

    topk_weights, topk_ids = paddle.topk(scores, k=topk, axis=-1, sorted=False)

    return fused_moe_wintx_impl(
        hidden_states,
        w1,
        w2,
        topk_weights,
...
def fused_moe_wint4(
    ...
    return fused_moe_wintx(hidden_states, w1, w2, scores, topk, w1_scale, w2_scale, bit=4)
