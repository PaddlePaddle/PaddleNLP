...
def convert_model(model_name):
    # dependency for: huggingface diffusers
    from diffusers import StableDiffusionPipeline as PytorchStableDiffusionPipeline

    pytorch_pipe = PytorchStableDiffusionPipeline.from_pretrained(model_name)
...
class StableDiffusionConverter(Converter):

    def convert(self, input_dir: str, output_dir: str):
        paddle_pipe = convert_model(model_name=input_dir)
        paddle_pipe.save_pretrained(output_dir)
