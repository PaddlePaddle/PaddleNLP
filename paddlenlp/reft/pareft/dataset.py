...
class ReftDataset(Dataset):
    ...
    def compute_intervention_and_subspaces(
        self, id: int, data_item, result: dict, last_position: int, **kwargs
    ):
        ...
        if len(self.fields_to_mask) == 1:
            ...
            for field in self.fields_to_mask:
                result[f"{field}_mask"] = (
                    result[field] != self.tokenizer.pad_token_id
                ).int()
        ...
        if "subspaces" in data_item:
            num_interventions = kwargs["num_interventions"]
            share_weights = (
                kwargs["share_weights"] if "share_weights" in kwargs else False
            )
            if share_weights:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i, data_item in enumerate(tqdm(task_dataset)):
            ...
            if data_split == "train":
                result["labels"][-1] = torch.cat(
                    (torch.tensor([IGNORE_INDEX]), result["labels"][-1])
                )
            result["intervention_locations"][-1] = (
                torch.IntTensor(result["intervention_locations"][-1]) + 1
            ).tolist()
            result["attention_mask"].append(
                (result["input_ids"][-1] != tokenizer.pad_token_id).int()
            )
            if "subspaces" in data_item:
                num_interventions = kwargs["num_interventions"]
                share_weights = (
...
class ReftGenerationDataset(ReftDataset):
    ...
    def tokenize(self, data_item):
        ...
        full_input = (
            data_item[self.prompt_field]
            + data_item[self.completion_field]
...
class ReftSupervisedDataset(ReftDataset):
    ...
    def tokenize(self, data_item):
        ...
        # input
        base_input = (
            base_prompt + data_item[self.output_field] + self.tokenizer.eos_token
        )
        input_ids = self.tokenizer(
            base_input,
            max_length=self.tokenizer.model_max_length,
...
def make_last_position_supervised_chat_data_module(
    ...
    for i in range(len(inputs)):
        ...
        all_intervention_locations.append(
            [[base_prompt_length - 1]] * num_interventions
        )
...
class ReftPreferenceDataset(ReftDataset):
    ...
    def tokenize(self, data_item):
        ...
        base_input = (
            base_prompt
            + data_item[self.rejected_output_field]
            + self.tokenizer.eos_token
        )
        ...
        if self.data_split == "train":
            ...
            base_output = (
                base_prompt
                + data_item[self.chosen_output_field]
                + self.tokenizer.eos_token
            )
...
class ReftRewardDataset(ReftDataset):
    ...
    def preprocess(self, kwargs):
        ...
        self.conv_A_reward_field = (
            kwargs["conv_A_reward_field"] if "conv_A_reward_field" in kwargs else None
        )
        self.conv_B_reward_field = (
            kwargs["conv_B_reward_field"] if "conv_B_reward_field" in kwargs else None
        )
        self.fields_to_pad = [
            "chosen_output",
            "rejected_output",
            ...
            merged_features.append(
                {
                    ...
                    "reward": (
                        feature["chosen_reward"] if "chosen_reward" in feature else 1.0
                    ),
            ...
            merged_features.append(
                {
                    ...
                    "reward": (
                        feature["rejected_reward"]
                        if "rejected_reward" in feature
                        else 0.0
                    ),
                    "intervention_locations": feature["intervention_locations"],
                }
