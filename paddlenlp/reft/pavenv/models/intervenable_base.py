...
from .intervention_utils import InterventionState
...
class IntervenableModel(nn.Layer):
        ...
        if _need_tidify:
            for k, v in self.activations.items():
                ...
                for t in range(len(v)):
                    ...
                    states_at_t = paddle.tensor(v[t][1]).bool().to(activations_at_t.device)  # a batched bools
    ...
    def _reconcile_stateful_cached_activations(
        ...
        for unit_location in intervening_unit_locations:
            if self._intervention_state[key].setter_version() in unit_location:
                ...
                state_select_flag += [False]
        state_select_flag = paddle.tensor(state_select_flag).bool().to(intervening_activations.device)
        ...
        for index, select_version in enumerate(self._batched_setter_activation_select[key]):
            if index in selected_indices:
                ...
                reconciled_activations += [paddle.zeros_like(cached_activations[index][0])]
    ...
    def _input_validation(
        ...
        if sources is not None and not (len(sources) == 1 and sources[0] is None):
        ...
        if not self.is_model_stateless and activations_sources is not None:
            for _, v in activations_sources.items():
                if isinstance(v, list) and isinstance(v[0], tuple) and isinstance(v[0][1], list) is not True:
                    raise ValueError("Stateful models need nested activations. See our documentions.")
        ...
        if self.mode == "parallel":
            ...
            for k, v in unit_locations.items():
                ...
                if isinstance(v, int):
                    ...
                    self.use_fast = True
                elif len(v) == 2 and v[0] is None and isinstance(v[1], int):
                    _unit_locations[k] = (
                        None,
                        [[[v[1]]] * batch_size] * len(self.interventions),
                    )
                    self.use_fast = True
                elif len(v) == 2 and isinstance(v[0], int) and v[1] is None:
                    _unit_locations[k] = (
                        [[[v[0]]] * batch_size] * len(self.interventions),
                        None,
                    )
    ...
    def _broadcast_source_representations(self, source_representations):
        ...
        if isinstance(source_representations, dict) or source_representations is None:
            ...
            raise ValueError("Accept input type for source_representations is [Dict, List, paddle.Tensor]")
...
class BoundlessRotatedSpaceIntervention(TrainableIntervention, DistributedRepresentationIntervention):
    ...
    def __init__(self, **kwargs):
        ...
        self.intervention_boundaries = paddle.nn.Parameter(paddle.tensor([0.5]), requires_grad=True)
        self.temperature = paddle.nn.Parameter(paddle.tensor(50.0))
        self.intervention_population = paddle.nn.Parameter(paddle.arange(0, self.embed_dim), requires_grad=False)
    ...
    def forward(self, base, source, subspaces=None):
        ...
        boundary_mask = paddle.ones(batch_size, device=base.device).unsqueeze(dim=-1) * boundary_mask
