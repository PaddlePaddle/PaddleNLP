...
def sigmoid_boundary(_input, boundary_x, boundary_y, temperature):
    ...
    return paddle.nn.functional.sigmoid((_input - boundary_x) / temperature) * paddle.nn.functional.sigmoid(
...
def harmonic_sigmoid_boundary(_input, boundary_x, boundary_y, temperature):
    ...
    return (
        (_input <= boundary_x) * paddle.nn.functional.sigmoid((_input - boundary_x) / temperature)
        + (_input >= boundary_y) * paddle.nn.functional.sigmoid((boundary_y - _input) / temperature)
        + ((_input > boundary_x) & (_input < boundary_y))
        * paddle.nn.functional.sigmoid(
            (0.5 * (paddle.abs(_input - boundary_x) ** (-1) + paddle.abs(_input - boundary_y) ** (-1))) ** (-1)
            / temperature
        )
    )
