...
STOPPING_CRITERIA_INPUTS_DOCSTRING = r"""
    ...
    Return:
        `paddle.Tensor`. (`paddle.Tensor` of shape `(batch_size, 1)`), where `True` indicates we stop generation
...
class MaxLengthCriteria(StoppingCriteria):
    ...
    def __call__(self, input_ids: paddle.Tensor, scores: paddle.Tensor, **kwargs) -> paddle.Tensor:
        ...
        return paddle.full((input_ids.shape[0],), is_done, dtype=paddle.bool)
...
class StopStringCriteria(StoppingCriteria):
    ...
    def __init__(self, tokenizer, stop_strings: Union[str, List[str]]):
        ...
        self.target_lens = paddle.to_tensor([len(stop_string) for stop_string in stop_strings], dtype="int32")
    ...
    @staticmethod
    def _stop_string_create_embedding_vec(token_list, token_indices, stop_strings) -> Dict[str, paddle.Tensor]:
        ...
        gather_vec = paddle.to_tensor(gather_vec, dtype="int32")
    ...
    def __call__(self, input_ids: paddle.Tensor, scores: paddle.Tensor, **kwargs) -> paddle.Tensor:
        ...
        flipped_ids = paddle.flip(input_ids, (1,))

        # Clip out-of-vocab values to the dummy value at the end of the embedding vector
        flipped_ids = paddle.clip(flipped_ids, max=self.embedding_vec.shape[0] - 1)
        ...
        lengths_with_ends = paddle.concat([end_lengths, lengths], axis=1)
...
class StoppingCriteriaList(list):
    @add_start_docstrings(STOPPING_CRITERIA_INPUTS_DOCSTRING)
    def __call__(self, input_ids: paddle.Tensor, scores: paddle.Tensor, **kwargs) -> paddle.Tensor:
        is_done = paddle.full((input_ids.shape[0],), False, dtype=paddle.bool)
