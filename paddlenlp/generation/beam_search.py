...
PROCESS_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`paddle.Tensor` of shape `(batch_size * num_beams, sequence_length)`):
        ...
        next_scores (`paddle.Tensor` of shape `(batch_size, 2 * num_beams)`):
            Current scores of the top `2 * num_beams` non-finished beam hypotheses.
        next_tokens (`paddle.Tensor` of shape `(batch_size, 2 * num_beams)`):
            `input_ids` of the tokens corresponding to the top `2 * num_beams` non-finished beam hypotheses.
        next_indices (`paddle.Tensor` of shape `(batch_size, 2 * num_beams)`):
    ...
    Return:
        `UserDict`: A dictionary composed of the fields as defined above:
            ...
            - **next_beam_scores** (`paddle.Tensor` of shape `(batch_size * num_beams)`) -- Updated scores of all
              non-finished beams.
            - **next_beam_tokens** (`paddle.Tensor` of shape `(batch_size * num_beams)`) -- Next tokens to be added
              to the non-finished beam_hypotheses.
            - **next_beam_indices** (`paddle.Tensor` of shape `(batch_size * num_beams)`) -- Beam indices
...
FINALIZE_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`paddle.Tensor` of shape `(batch_size * num_beams, sequence_length)`):
            ...
            [What are input IDs?](../glossary#input-ids)
        final_beam_scores (`paddle.Tensor` of shape `(batch_size * num_beams)`):
            The final scores of all non-finished beams.
        final_beam_tokens (`paddle.Tensor` of shape `(batch_size * num_beams)`):
            The last tokens to be added to the non-finished beam_hypotheses.
        final_beam_indices (`paddle.Tensor` of shape `(batch_size * num_beams)`):
    ...
    Return:
        `paddle.Tensor` of shape `(batch_size * num_return_sequences, sequence_length)`: The generated sequences.
...
class BeamScorer(ABC):
    ...
    def process(
        ...
        next_scores: paddle.Tensor,
        next_tokens: paddle.Tensor,
        next_indices: paddle.Tensor,
        **kwargs,
    ) -> Tuple[paddle.Tensor]:
...
class BeamSearchScorer(BeamScorer):
    ...
    def process(
        ...
        next_tokens: paddle.Tensor,
        next_indices: paddle.Tensor,
        pad_token_id: Optional[Union[int, paddle.Tensor]] = None,
        eos_token_id: Optional[Union[int, List[int], paddle.Tensor]] = None,
        ...
        if eos_token_id is not None and not isinstance(eos_token_id, paddle.Tensor):
    ...
    def finalize(
        self,
        input_ids: paddle.Tensor,
        final_beam_scores: paddle.Tensor,
        final_beam_tokens: paddle.Tensor,
        final_beam_indices: paddle.Tensor,
        max_length: int,
        pad_token_id: Optional[Union[int, paddle.Tensor]] = None,
        eos_token_id: Optional[Union[int, List[int], paddle.Tensor]] = None,
        beam_indices: Optional[paddle.Tensor] = None,
        ...
        for i, (hypo, best_idx) in enumerate(zip(best, best_indices)):
            ...
            if indices is not None:
                indices[i, : len(best_idx)] = paddle.to_tensor(best_idx)
...
class ConstrainedBeamSearchScorer(BeamScorer):
    ...
    def __init__(
        ...
        self._done = paddle.to_tensor([False for _ in range(batch_size)], dtype=paddle.bool)
    ...
    def process(
        ...
        next_tokens: paddle.Tensor,
        next_indices: paddle.Tensor,
        scores_for_all_vocab: paddle.Tensor,
        pad_token_id: Optional[Union[int, paddle.Tensor]] = None,
        eos_token_id: Optional[Union[int, List[int], paddle.Tensor]] = None,
        beam_indices: Optional[paddle.Tensor] = None,
        decoder_prompt_len: Optional[int] = 0,
        ...
        Args:
            ...
            next_indices (`paddle.Tensor` of shape `(batch_size, 2 * num_beams)`):
                Beam indices indicating to which beam hypothesis the `next_tokens` correspond.
            scores_for_all_vocab (`paddle.Tensor` of shape `(batch_size * num_beams, sequence_length)`):
        ...
        Return:
            `UserDict`: A dictionary composed of the fields as defined above:
                ...
                - **next_beam_scores** (`paddle.Tensor` of shape `(batch_size * num_beams)`) -- Updated scores of
                  all
                non-finished beams.

                - **next_beam_tokens** (`paddle.Tensor` of shape `(batch_size * num_beams)`) -- Next tokens to be
                  added
                to the non-finished beam_hypotheses.
                - **next_beam_indices** (`paddle.Tensor` of shape `(batch_size * num_beams)`) -- Beam indices
                indicating to which beam the next tokens shall be added.
    ...
    def step_sentence_constraint(
        ...
        batch_idx: int,
        input_ids: paddle.Tensor,
        vocab_scores: paddle.Tensor,
        sent_beam_scores: paddle.Tensor,
        sent_beam_tokens: paddle.Tensor,
        sent_beam_indices: paddle.Tensor,
        ...
        full_hypotheses = paddle.concat((input_ids[sent_beam_indices], sent_beam_tokens.unsqueeze(-1)), axis=-1)
        ...
        for seq_idx, pre_seq in enumerate(this_batch_input_ids):
            ...
            if not advance_state.completed:
                ...
                for advance_token in advance_tokens:
                    ...
                    advance_seq = paddle.concat((pre_seq, advance_token.unsqueeze(0)), -1).cpu().tolist()
                ...
                new_score, new_token = paddle.max(this_batch_token_scores[seq_idx], 0)  # some next probable token
                advance_seq = paddle.concat((pre_seq, new_token.unsqueeze(0)), -1)
        ...
        if len(track_new["new_indices"]) > 0:
            ...
            rearrangers = paddle.to_tensor(np.argsort(increments, kind="mergesort"))
