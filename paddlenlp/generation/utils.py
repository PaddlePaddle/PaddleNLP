...
class GenerateDecoderOnlyOutput(ModelOutput):
    ...
    Args:
        ...
        scores (`tuple(paddle.Tensor)` *optional*, returned when `output_scores=True`):
        ...
        logits (`tuple(paddle.Tensor)` *optional*, returned when `output_logits=True`):
            Unprocessed prediction scores of the language modeling head (scores for each vocabulary token before SoftMax)
            at each generation step. Tuple of `paddle.Tensor` with up to `max_new_tokens` elements (one element for
            each generated token), with each tensor of shape `(batch_size, config.vocab_size)`.
        attentions (`tuple(tuple(paddle.Tensor))`, *optional*, returned when `output_attentions=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `paddle.Tensor` of shape `(batch_size, num_heads, generated_length, sequence_length)`.
        hidden_states (`tuple(tuple(paddle.Tensor))`, *optional*, returned when `output_hidden_states=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `paddle.Tensor` of shape `(batch_size, generated_length, hidden_size)`.
        past_key_values (`tuple(tuple(paddle.Tensor)))`, *optional*, returned when `use_cache=True`):
...
class GenerateEncoderDecoderOutput(ModelOutput):
    ...
    Args:
        sequences (`paddle.Tensor` of shape `(batch_size*num_return_sequences, sequence_length)`):
            The generated sequences. The second dimension (sequence_length) is either equal to `max_length` or shorter
            if all batches finished early due to the `eos_token_id`.
        scores (`tuple(paddle.Tensor)` *optional*, returned when `output_scores=True`):
            Processed prediction scores of the language modeling head (scores for each vocabulary token before SoftMax)
        ...
        logits (`tuple(paddle.Tensor)` *optional*, returned when `output_logits=True`):
            ...
            at each generation step. Tuple of `paddle.Tensor` with up to `max_new_tokens` elements (one element for
            each generated token), with each tensor of shape `(batch_size, config.vocab_size)`.
        encoder_attentions (`tuple(paddle.Tensor)`, *optional*, returned when `output_attentions=True`):
            Tuple of `paddle.Tensor` (one for each layer of the decoder) of shape `(batch_size, num_heads,
            sequence_length, sequence_length)`.
        encoder_hidden_states (`tuple(paddle.Tensor)`, *optional*, returned when `output_hidden_states=True`):
            Tuple of `paddle.Tensor` (one for the output of the embeddings + one for the output of each layer) of
            shape `(batch_size, sequence_length, hidden_size)`.
        decoder_attentions (`tuple(tuple(paddle.Tensor))`, *optional*, returned when `output_attentions=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `paddle.Tensor` of shape `(batch_size, num_heads, generated_length, sequence_length)`.
        cross_attentions (`tuple(tuple(paddle.Tensor))`, *optional*, returned when `output_attentions=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `paddle.Tensor` of shape `(batch_size, num_heads, generated_length, sequence_length)`.
        decoder_hidden_states (`tuple(tuple(paddle.Tensor))`, *optional*, returned when `output_hidden_states=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `paddle.Tensor` of shape `(batch_size, generated_length, hidden_size)`.
...
class GenerateBeamDecoderOnlyOutput(ModelOutput):
    ...
    Args:
        sequences (`paddle.Tensor` of shape `(batch_size*num_return_sequences, sequence_length)`):
            The generated sequences. The second dimension (sequence_length) is either equal to `max_length` or shorter
            if all batches finished early due to the `eos_token_id`.
        sequences_scores (`paddle.Tensor` of shape `(batch_size*num_return_sequences)`, *optional*, returned when `output_scores=True`):
            Final beam scores of the generated `sequences`.
        scores (`tuple(paddle.Tensor)` *optional*, returned when `output_scores=True`):
            Beam transition scores for each vocabulary token at each generation step. Beam transition scores consisting
            of log probabilities of tokens conditioned on log softmax of previously generated tokens in this beam.
            Tuple of `paddle.Tensor` with up to `max_new_tokens` elements (one element for each generated token),
            with each tensor of shape `(batch_size*num_beams, config.vocab_size)`.
        logits (`tuple(paddle.Tensor)` *optional*, returned when `output_logits=True`):
            Unprocessed prediction scores of the language modeling head (scores for each vocabulary token before SoftMax)
            at each generation step. Tuple of `paddle.Tensor` with up to `max_new_tokens` elements (one element for
            each generated token), with each tensor of shape `(batch_size, config.vocab_size)`.
        beam_indices (`paddle.Tensor`, *optional*, returned when `output_scores=True`):
            Beam indices of generated token id at each generation step. `paddle.Tensor` of shape
            `(batch_size*num_return_sequences, sequence_length)`.
        attentions (`tuple(tuple(paddle.Tensor))`, *optional*, returned when `output_attentions=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `paddle.Tensor` of shape `(batch_size*num_beams, num_heads, generated_length, sequence_length)`.
        hidden_states (`tuple(tuple(paddle.Tensor))`, *optional*, returned when `output_hidden_states=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `paddle.Tensor` of shape `(batch_size*num_beams*num_return_sequences, generated_length, hidden_size)`.
        past_key_values (`tuple(tuple(paddle.Tensor)))`, *optional*, returned when `use_cache=True`):
            Returns the model cache, used to speed up decoding. Different models have a different cache format, check
            the model's documentation. Usually, a [`~cache_utils.Cache`] instance.
    """
...
class GenerateBeamEncoderDecoderOutput(ModelOutput):
    ...
    Args:
        ...
        decoder_attentions (`tuple(tuple(paddle.Tensor))`, *optional*, returned when `output_attentions=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `paddle.Tensor` of shape `(batch_size*num_beams*num_return_sequences, num_heads, generated_length,
            sequence_length)`.
        cross_attentions (`tuple(tuple(paddle.Tensor))`, *optional*, returned when `output_attentions=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `paddle.Tensor` of shape `(batch_size, num_heads, generated_length, sequence_length)`.
...
class GenerationMixin:
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        #   (this alternative is not as robust as calling `generate` and letting it create `cache_position`)
            ...
            cache_position = paddle.arange(past_length, input_ids.shape[1], dtype="int32")
    ...
    def _prepare_model_inputs(
        ...
        bos_token_id: Optional[paddle.Tensor] = None,
        model_kwargs: Optional[Dict[str, paddle.Tensor]] = None,
    ) -> Tuple[paddle.Tensor, Optional[str], Dict[str, paddle.Tensor]]:
        """
    ...
    def _maybe_initialize_input_ids_for_generation(
        ...
        if "inputs_embeds" in model_kwargs:
            return paddle.ones(shape, dtype="int64")
    ...
    def _prepare_attention_mask_for_generation(
        ...
        default_attention_mask = paddle.ones(inputs_tensor.shape[:2], dtype="int64")
    ...
    def _prepare_decoder_input_ids_for_generation(
        ...
        # decoder_attention_mask if provided)
            ...
            decoder_input_ids = paddle.concat([decoder_start_token_id, decoder_input_ids], axis=-1)
            if "decoder_attention_mask" in model_kwargs:
                decoder_attention_mask = model_kwargs["decoder_attention_mask"]
                decoder_attention_mask = paddle.concat(
                    (paddle.ones_like(decoder_attention_mask)[:, :1], decoder_attention_mask),
                    axis=-1,
                )
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        # Do not call paddle.repeat_interleave if expand_size is 1 because it clones
    ...
    def _get_logits_processor(
        ...
        if generation_config.do_sample:
            ...
            if generation_config.num_beams > 1:
                ...
                elif isinstance(generation_config._eos_token_tensor, paddle.Tensor):
    ...
    def compute_transition_scores(
        self,
        sequences: paddle.Tensor,
        scores: Tuple[paddle.Tensor],
        beam_indices: Optional[paddle.Tensor] = None,
        ...
        Parameters:
            ...
            scores (`tuple(paddle.Tensor)`):
                ...
                Tuple of `paddle.Tensor` with up to `max_new_tokens` elements (one element for each generated token),
                with each tensor of shape `(batch_size*num_beams, config.vocab_size)`.
            beam_indices (`paddle.Tensor`, *optional*):
                Beam indices of generated token id at each generation step. `paddle.Tensor` of shape
        ...
        Return:
            `paddle.Tensor`: A `paddle.Tensor` of shape `(batch_size*num_return_sequences, sequence_length)` containing
        ...
        scores = paddle.stack(scores).reshape(len(scores), -1).transpose(0, 1)
    ...
    def _prepare_generation_config(
        ...
        """
        # TODO joao: when we can detect `fullgraph=True` in `paddle.compile` (https://github.com/pytorch/pytorch/pull/120400)
        ...
        # `paddle.compile` can't compile `copy.deepcopy`, arguments in `kwargs` that are part of `generation_config`
    ...
    def _get_initial_cache_position(self, input_ids, model_kwargs):
        ...
        # `paddle.compile`-friendly `paddle.arange` from a shape -- the lines below are equivalent to `paddle.arange`
        if "inputs_embeds" in model_kwargs and not self.config.is_encoder_decoder:
            cache_position = paddle.ones_like(model_kwargs["inputs_embeds"][0, :, 0], dtype=paddle.int64).cumsum(0) - 1
        elif "decoder_inputs_embeds" in model_kwargs and self.config.is_encoder_decoder:
            cache_position = (
                paddle.ones_like(model_kwargs["decoder_inputs_embeds"][0, :, 0], dtype=paddle.int64).cumsum(0) - 1
            )
        else:
            cache_position = paddle.ones_like(input_ids[0, :], dtype=paddle.int64).cumsum(0) - 1
        ...
        if model_kwargs.get("past_key_values") is not None:
            ...
            # TODO(joao): this is not paddle.compile-friendly, find a work-around. If the cache is not empty,
    ...
    def _prepare_special_tokens(
        ...
        #         paddle.is_floating_point(eos_token_tensor) or (eos_token_tensor < 0).any()
        #     ):
        #         logger.warning(
        ...
        # https://pypaddle.org/docs/stable/paddle.compiler_cudagraph_trees.html#limitations
    ...
    def generate(
        ...
        negative_prompt_attention_mask: Optional[paddle.Tensor] = None,
        **kwargs,
    ) -> Union[GenerateOutput, paddle.Tensor]:
        r"""
        ...
        Parameters:
            inputs (`paddle.Tensor` of varying shape depending on the modality, *optional*):
            ...
            prefix_allowed_tokens_fn (`Callable[[int, paddle.Tensor], List[int]]`, *optional*):
            ...
            negative_prompt_ids (`paddle.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
                The negative prompt needed for some processors such as CFG. The batch size must match the input batch
                size. This is an experimental feature, subject to breaking API changes in future versions.
            negative_prompt_attention_mask (`paddle.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
        ...
        Return:
            [`~utils.ModelOutput`] or `paddle.Tensor`: A [`~utils.ModelOutput`] (if `return_dict_in_generate=True`
            or when `config.return_dict_in_generate=True`) or a `paddle.Tensor`.
        ...
        if not self.config.is_encoder_decoder:
            ...
            if (
                ...
                and paddle.sum(inputs_tensor[:, -1] == generation_config._pad_token_tensor) > 0
            ):
                logger.warning(
    ...
    def _has_unfinished_sequences(
        ...
        """
        # paddle.compile does not support data-dependent control flow. This is a workaround to allow paddle.compile,
        # although we lose the ability to stop when all sequences return an EOS token (and other stopping criteria)
        # TODO (joao): remove this when torch's support for control flow is not experimental (https://pypaddle.org/docs/stable/generated/paddle.cond.html)
    ...
    def heal_tokens(
        self, input_ids: paddle.Tensor, tokenizer: Optional["PreTrainedTokenizerBase"] = None
    ) -> paddle.Tensor:
        r"""
        Generates sequences of token ids for models with a language modeling head.
        Parameters:
            input_ids (`paddle.Tensor`): The sequence used as a prompt for the generation.
            tokenizer (`PreTrainedTokenizerBase`, *optional*): The tokenizer used to decode the input ids.
        Return:
            `paddle.Tensor` where each sequence has its tail token replaced with its appropriate extension.
        """
        if tokenizer is None:
            raise ValueError(
        ...
        for batch_idx, (tail_id, tail_tok) in enumerate(zip(tail_ids, tail_toks)):
            ...
            if paddle.all(batch_ids == pad_token_id).item():
    ...
    def _dola_decoding(
        ...
        **model_kwargs,
        ...
        Return:
            ...
            or `paddle.Tensor`: A `paddle.Tensor` containing the generated tokens (default behaviour) or a
    ...
    def _contrastive_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus):
            ...
            next_probs = nn.functional.softmax(processed_logit_for_next_step, axis=-1)

            top_k_probs, top_k_ids = paddle.topk(next_probs, axis=-1, k=top_k)
            ...
            cosine_matrix_mask = paddle.concat(
            ...
            next_hidden = paddle.stack(paddle.split(next_hidden.squeeze(axis=1), top_k))
            next_hidden = next_hidden[range(batch_size), selected_idx, :]
            last_hidden_states = paddle.concat([last_hidden_states, next_hidden.unsqueeze(1)], axis=1)

            next_decoder_hidden_states = ()
            for layer in full_hidden_states:
                layer = paddle.stack(paddle.split(layer, top_k))[range(batch_size), selected_idx, :]
            ...
            if self.config.is_encoder_decoder:
                ...
                if output_attentions:
                    for layer in outputs.cross_attentions:
                        layer = paddle.stack(paddle.split(layer, top_k, axis=0))[range(batch_size), selected_idx, ...]
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        Return:
            ...
            A `paddle.Tensor` containing the generated tokens (default behaviour) or a
            [`~generation.GenerateDecoderOnlyOutput`] if `model.config.is_encoder_decoder=False` and
            `return_dict_in_generate=True` or a [`~generation.GenerateEncoderDecoderOutput`] if
            `model.config.is_encoder_decoder=True`.
        """
        ...
        origin_len = cur_len  # 保留原始长度以方便后续处理
        this_peer_finished = False
        ...
        if return_dict_in_generate:
            if self.config.is_encoder_decoder:
                ...
                return GenerateDecoderOnlyOutput(
                    sequences=input_ids[:, origin_len:],
                    scores=scores,
                    logits=raw_logits,
                    attentions=decoder_attentions,
                    hidden_states=decoder_hidden_states,
    ...
    def _beam_search(
        ...
        synced_gpus: bool,
        **model_kwargs,
    ) -> Union[GenerateBeamOutput, paddle.Tensor]:
        r"""
        ...
        Return:
            ...
            `paddle.Tensor`: A `paddle.Tensor` containing the generated tokens (default behaviour) or a
    ...
    def _group_beam_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus):
            ...
            if output_scores:
                processed_score = paddle.zeros_like(outputs.logits[:, -1, :])
            if output_logits:
                # Clone is needed to avoid keeping a hanging ref to outputs.logits which may be very large for first iteration
                # (the clone itself is always small)
    ...
    def _constrained_beam_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus):
            ...
            n_eos_tokens = eos_token_id.shape[0] if eos_token_id is not None else 0
            next_token_scores, next_tokens = paddle.topk(
                next_token_scores, max(2, 1 + n_eos_tokens) * num_beams, axis=1, largest=True, sorted=True
            )
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus):
            ...
            # mismatch, or until the max length is reached.
                ...
                if do_sample:
                    ...
                    selected_tokens = paddle.multinomial(probs[0, :, :], num_samples=1).squeeze(1)[None, :]
                else:
...
def _speculative_sampling(
    ...
    r_i = paddle.rand_like(probability_ratio)
    is_accepted = r_i <= probability_ratio
    n_matches = ((~is_accepted).cumsum(axis=-1) < 1).sum()  # this is `n` in algorithm 1
    ...
    if is_done_candidate and n_matches == candidate_length:
        ...
        if n_matches < gamma:
            ...
            p_prime = paddle.clamp((p_n_plus_1 - q_n_plus_1), min=0)
...
def _ranking_fast(
    ...
    cosine_matrix_mask: paddle.Tensor,
    alpha: float,
    beam_width: int,
) -> paddle.Tensor:
    """
    ...
    row in the batch.
    """
    norm_context_hidden = context_hidden / context_hidden.norm(axis=2, keepaxis=True)
    norm_next_hidden = next_hidden / next_hidden.norm(axis=2, keepaxis=True)
    cosine_matrix = paddle.matmul(norm_context_hidden, norm_next_hidden.transpose(1, 2)).squeeze(-1)  # [B*K, S]
    ...
    cosine_matrix_mask = (1 - cosine_matrix_mask) * paddle.finfo(cosine_matrix.dtype).min
...
def stack_model_outputs(model_outputs: List[ModelOutput], config: PretrainedConfig) -> ModelOutput:
    ...
    def _concat(data):
        ...
        if any(data is None for data in data):
            return None
        if isinstance(data[0], paddle.Tensor):
        ...
        # New cache format
            ...
            if isinstance(data[0][0], tuple):
                return tuple(
                    tuple(paddle.concat([attr[i][j] for attr in data], axis=0) for j in range(len(data[0][0])))
...
def _relative_top_filter(
    ...
    """
    scores_normalized = scores.log_softmax(axis=-1)
    baseline_scores_normalized = baseline_scores.log_softmax(axis=-1)
    sorted_logits, sorted_indices = paddle.sort(scores_normalized, descending=True)
    min_thresh = sorted_logits[..., min_tokens_to_keep - 1]
    probs_max = paddle.max(scores_normalized, axis=-1).values
    probs_thresh = probs_max + np.log(relative_top)
    probs_thresh = paddle.min(min_thresh, probs_thresh)
...
def _dola_select_contrast(
    ...
    candidate_premature_logits: Dict[int, paddle.Tensor],
    final_logits: paddle.Tensor,
) -> paddle.Tensor:
    ...
    softmax_mature_layer = F.softmax(final_logits, axis=-1)
    # shape: (num_premature_layers, batch_size, vocab_size)
    softmax_premature_layers = F.softmax(stacked_premature_layers, axis=-1)
    ...
    # shape: (batch_size, vocab_size)
    log_softmax_mature_layer = F.log_softmax(final_logits, axis=-1)
    # shape: (num_premature_layers, batch_size, vocab_size)
    log_softmax_premature_layers = F.log_softmax(stacked_premature_layers, axis=-1)
