...
LOGITS_PROCESSOR_INPUTS_DOCSTRING = r"""
    Args:
        ...
        scores (`paddle.Tensor` of shape `(batch_size, config.vocab_size)`):
...
class LogitsProcessorList(list):
    ...
    def __call__(self, input_ids: paddle.Tensor, scores: paddle.Tensor, **kwargs) -> paddle.Tensor:
        ...
        Return:
            `paddle.Tensor` of shape `(batch_size, config.vocab_size)`:
...
class MinPLogitsWarper(LogitsProcessor):
    ...
    def __call__(self, input_ids: paddle.Tensor, scores: paddle.Tensor) -> paddle.Tensor:
        ...
        top_probs = probs.max(axis=-1, keepdim=True)
...
class TypicalLogitsWarper(LogitsProcessor):
    ...
    def __call__(self, input_ids: paddle.Tensor, scores: paddle.Tensor) -> paddle.Tensor:
        ...
        sorted_indices_to_remove = sorted_scores > paddle.index_sample(sorted_scores, last_ind.view([-1, 1]))
...
class EncoderNoRepeatNGramLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, encoder_ngram_size: int, encoder_input_ids: paddle.Tensor):
...
class SequenceBiasLogitsProcessor(LogitsProcessor):
    ...
    def _prepare_bias_variables(self, scores: paddle.Tensor):
...
class NoBadWordsLogitsProcessor(SequenceBiasLogitsProcessor):
    ...
    Args:
        ...
        eos_token_id (`Union[int, List[int], paddle.Tensor]`, *optional*):
            The id(s) of the *end-of-sequence* token.
    ...
    def __init__(
        self, bad_words_ids: List[List[int]], eos_token_id: Optional[Union[int, List[int], paddle.Tensor]] = None
    ):
    ...
    def __call__(
        ...
        Args:
            ...
            scores (`paddle.Tensor` of shape `(batch_size, config.vocab_size)`):
                Prediction scores of a language modeling head. These can be logits for each vocabulary when not using
                beam search or log softmax for each vocabulary token when using beam search
            current_tokens (`paddle.Tensor` of shape `(batch_size)`):
...
class ForcedEOSTokenLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, max_length: int, eos_token_id: Union[int, List[int], paddle.Tensor]):
        ...
        if paddle.is_floating_point(eos_token_id) or (eos_token_id < 0).any():
...
class InfNanRemoveLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: paddle.Tensor, scores: paddle.Tensor) -> paddle.Tensor:
        ...
        scores_processed = paddle.where(scores != scores, 0.0, scores)

        # set all +/-inf values to max/min possible value
        scores_processed = paddle.where(scores == float("inf"), paddle.finfo(scores.dtype).max, scores_processed)
        scores_processed = paddle.where(scores == -float("inf"), paddle.finfo(scores.dtype).min, scores_processed)
...
class ExponentialDecayLengthPenalty(LogitsProcessor):
    ...
    def __call__(self, input_ids: paddle.Tensor, scores: paddle.Tensor) -> paddle.Tensor:
        ...
        penalties = paddle.zeros_like(scores)
        scores_processed = scores
        if cur_len > self.regulation_start:
            ...
            penalty = paddle.abs(scores[:, self.eos_token_id]) * (pow(self.regulation_factor, penalty_idx) - 1)
...
class LogitNormalization(LogitsProcessor):
    ...
    >>> print(paddle.allclose(paddle.sum(paddle.exp(outputs.scores[-1])), paddle.Tensor((1.000,)), rtol=1e-4))
    False
    ...
    @add_start_docstrings(LOGITS_PROCESSOR_INPUTS_DOCSTRING)
    def __call__(self, input_ids: paddle.Tensor, scores: paddle.Tensor) -> paddle.Tensor:
        scores_processed = paddle.nn.functional.log_softmax(scores, axis=-1)
        return scores_processed
...
class SuppressTokensAtBeginLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: paddle.Tensor, scores: paddle.Tensor) -> paddle.Tensor:
        vocab_tensor = paddle.arange(scores.shape[-1])
        suppress_token_mask = paddle.isin(vocab_tensor, self.begin_suppress_tokens)
        scores_processed = scores
...
class SuppressTokensLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: paddle.Tensor, scores: paddle.Tensor) -> paddle.Tensor:
        ...
        scores = paddle.where(suppress_token_mask, -float("inf"), scores)
        return scores
...
class WhisperTimeStampLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: paddle.Tensor, scores: paddle.Tensor) -> paddle.Tensor:
        ...
        logprobs = paddle.nn.functional.log_softmax(scores_processed.float(), axis=-1)
        for k in range(input_ids.shape[0]):
    ...
    def __call__(self, input_ids: paddle.Tensor, scores: paddle.Tensor) -> paddle.Tensor:
        ...
        if input_ids.shape[1] == self.begin_index:
            ...
            if is_scores_logprobs:
                ...
                probs = paddle.nn.functional.softmax(no_speech_scores.float(), axis=-1)
        ...
        if self.unconditional_context["first_pass"]:
            ...
            if self.unconditional_context["attention_mask"] is None:
                self.unconditional_context["attention_mask"] = paddle.ones_like(
                    self.unconditional_context["input_ids"], dtype="int64"
                )
            ...
            attention_mask = paddle.concat(
                [
                    ...
                    paddle.ones_like(input_ids[:, -1:], dtype="int64"),
                ],
                axis=1,
            )
            if not self.unconditional_context["use_cache"]:
                input_ids = paddle.concat([self.unconditional_context["input_ids"], input_ids[:, -1:]], axis=1)
            else:
...
class BarkEosPrioritizerLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: paddle.Tensor, scores: paddle.Tensor) -> paddle.Tensor:
        ...
        if self.min_eos_p:
            ...
            do_early_stop = paddle.any(do_early_stop, axis=1, keepdim=True)
            scores_processed = paddle.where(do_early_stop, early_stop_scores, scores)
...
class WatermarkLogitsProcessor(LogitsProcessor):
    ...
    def __init__(
        ...
        self.rng = paddle.framework.core.default_cuda_generator(int(paddle.device.get_device()[-1]))
        # self.rng = paddle.Generator(device=device)
        self.hash_key = hashing_key
        self.context_width = context_width

        self.rng.seed(hashing_key)
        self.table_size = 1_000_003
        self.fixed_table = paddle.randperm(self.table_size)

    def set_seed(self, input_seq: paddle.Tensor):
        ...
        self.rng.seed(seed % (2**64 - 1))

    def _get_greenlist_ids(self, input_seq: paddle.Tensor) -> paddle.Tensor:
        self.set_seed(input_seq)
        vocab_permutation = paddle.randperm(self.vocab_size)
        greenlist_ids = vocab_permutation[: self.greenlist_size]
        return greenlist_ids

    def _score_rejection_sampling(self, input_seq: paddle.Tensor, scores: paddle.Tensor) -> paddle.Tensor:
        """
        ...
        for i in range(40):
            greenlist_ids = self._get_greenlist_ids(paddle.concat([input_seq, greedy_predictions[i, None]], axis=-1))
            if greedy_predictions[i] in greenlist_ids:
                final_greenlist.append(greedy_predictions[i])
        return paddle.to_tensor(final_greenlist)
...
class SynthIDTextWatermarkState:
    ...
    def __init__(
        ...
        """
        self.context = paddle.zeros(
            (batch_size, ngram_len - 1),
            dtype=paddle.int64,
        )
        self.context_history = paddle.zeros(
...
class SynthIDTextWatermarkLogitsProcessor(LogitsProcessor):
    ...
    * **Utility Functions:** Provides functions to compute g-values directly, check for context repetition, create
    ...
    def __init__(
        ...
        self.sampling_table = paddle.randint(
            low=0,
            high=2,
    ...
    def update_scores(self, scores: paddle.Tensor, g_values: paddle.Tensor) -> paddle.Tensor:
        ...
        Args:
            scores (`paddle.Tensor`): Scores (batch_size, vocab_size).
            g_values (`paddle.Tensor`): G valus (batch_size, vocab_size, depth).
        ...
        log_probs = paddle.log(probs)
        log_probs = paddle.where(paddle.isfinite(log_probs), log_probs, paddle.finfo(log_probs.dtype).min)
        return log_probs
    ...
    def __call__(self, input_ids: paddle.Tensor, scores: paddle.Tensor) -> paddle.Tensor:
        ...
        if self.debug_mode:
            scores = paddle.ones_like(scores)

        # Currently indices is just a arange to compute watermarking on the desnse logits.
        all_indices = paddle.stack([paddle.arange(vocab_size) for _ in range(batch_size)])
        ...
        if self.state is None:
            ...
            # watermarking.
            self.state.context = paddle.concat(
        ...
        self.state.context_history = paddle.concat(
            (hash_result_with_just_context, self.state.context_history),
            axis=1,
        )[:, :-1]

        updated_watermarked_scores = paddle.where(
            is_repeated_context,
            input=scores,
    ...
    def accumulate_hash(
        ...
        for i in range(data.shape[-1]):
            current_hash = paddle.add(current_hash, data[..., i])
            current_hash = paddle.mul(current_hash, multiplier)
            current_hash = paddle.add(current_hash, increment)
        return current_hash

    def compute_ngram_keys(self, ngrams: paddle.Tensor) -> paddle.Tensor:
        """Computes random keys for each ngram and depth.

        Args:
            ngrams (`paddle.Tensor`):
        ...
        hash_result = paddle.ones(batch_size, dtype="int64")
        # hash_result shape [batch_size,]
        # ngrams shape [batch_size, num_ngrams, ngram_len]
        hash_result = paddle.vmap(self.accumulate_hash, in_dims=(None, 1), out_dims=1)(hash_result, ngrams)
        # hash_result shape [batch_size, num_ngrams]

        keys = self.keys[None, None, :, None]
        # hash_result shape [batch_size, num_ngrams]
        # keys shape [1, 1, depth, 1]
        hash_result = paddle.vmap(self.accumulate_hash, in_dims=(None, 2), out_dims=2)(hash_result, keys)
        # hash_result shape [batch_size, num_ngrams, depth]

        return hash_result

    def _compute_keys(
        self, n_minus_1_grams: paddle.Tensor, indices: paddle.Tensor
    ) -> Tuple[paddle.Tensor, paddle.Tensor]:
        """Computes random keys for each ngram and depth.

        Args:
            n_minus_1_grams (`paddle.Tensor`):
        ...
        hash_result = paddle.vmap(self.accumulate_hash, in_dims=(None, 1), out_dims=1)(
    ...
    def sample_g_values(self, ngram_keys: paddle.Tensor) -> paddle.Tensor:
        """
        Samples g values from Bernoulli distribution.

        It is not possible to pass random keys in a vectorized way in paddle. Instead
        we pre-compute a random sampling table, and use apply modulo table size to
        map from ngram keys (int64) to g values.

        Args:
            ngram_keys (`paddle.Tensor`):
        ...
        return paddle.take_along_axis(sampling_table, indices=ngram_keys, axis=2)

    def _check_input_ids_shape(self, input_ids: paddle.Tensor):
    ...
    def compute_g_values(self, input_ids: paddle.Tensor) -> paddle.Tensor:
        """
        ...
        """
        self._check_input_ids_shape(input_ids)
        ngrams = input_ids.unfold(axis=1, size=self.ngram_len, step=1)
        ngram_keys = self.compute_ngram_keys(ngrams)
        return self.sample_g_values(ngram_keys)

    def compute_context_repetition_mask(self, input_ids: paddle.Tensor) -> paddle.Tensor:
        """
        ...
        for i in range(num_contexts):
            ...
            state.context_history = paddle.concat(
                (context_hash, state.context_history),
                axis=1,
            )[:, :-1]
        are_repeated_contexts = paddle.concat(are_repeated_contexts, axis=1)

        return paddle.logical_not(are_repeated_contexts)

    def compute_eos_token_mask(self, input_ids: paddle.Tensor, eos_token_id: int) -> paddle.Tensor:
        """
        ...
        for eos_equated in all_eos_equated:
            nonzero_idx = paddle.nonzero(eos_equated)
            noneos_mask = paddle.ones_like(eos_equated)
            if nonzero_idx.shape[0] != 0:
                noneos_mask[nonzero_idx[0][0] :] = 0
            noneos_masks.append(noneos_mask)
        return paddle.stack(noneos_masks, axis=0)
