...
class CandidateGenerator:
    ...
    def get_candidates(self, input_ids: paddle.Tensor) -> Tuple[paddle.Tensor, Optional[paddle.Tensor]]:
        ...
        Return:
            `paddle.Tensor` of shape `(batch_size, candidate_length)` containing the candidate sequences to be
            assessed by the model and, optionally, a `paddle.Tensor` of shape `(batch_size, candidate_length,
    ...
    def update_candidate_strategy(self, input_ids: paddle.Tensor, scores: paddle.Tensor, num_matches: int):
        """
        ...
        Args:
            input_ids (`paddle.Tensor` of shape `(batch_size, sequence_length)`):
                Indices of input sequence tokens in the vocabulary. [What are input IDs?](../glossary#input-ids)
            scores (`paddle.Tensor` of shape `(batch_size, candidate_length, config.vocab_size)`):
...
class AssistedCandidateGenerator(CandidateGenerator):
    ...
    Args:
        ...
        model_kwargs (`Dict`):
            ...
            model as well.
        inputs_tensor (`paddle.Tensor`, *optional*):
    ...
    def _calculate_new_tokens(self, input_ids: paddle.Tensor) -> Tuple[int, int]:
    ...
    def _update_past_and_masks(
        self, input_ids: paddle.Tensor, remove_from_pkv: int = 0, num_added_tokens: int = 1
    ) -> bool:
    ...
    def _prepare_generation_args(self, input_ids: paddle.Tensor, min_new_tokens: int, max_new_tokens: int) -> Dict:
    ...
    def _generate_candidates(self, generation_args: Dict) -> Tuple[paddle.Tensor, Optional[paddle.Tensor]]:
        ...
        if (
            ...
            scores_tensor = paddle.concat(assistant_output.scores, axis=0)
            scores_softmax = paddle.nn.functional.softmax(scores_tensor, axis=-1)
            ids = assistant_output.sequences[-1, -len(assistant_output.scores) :]
            p = scores_softmax[range(len(ids)), ids]
            self.probs.extend(p.tolist())
        candidate_logits = paddle.stack(assistant_output.scores, axis=1)
...
class AssistedCandidateGeneratorDifferentTokenizers(AssistedCandidateGenerator):
    ...
    def _get_longest_diag_dict(input_matrix, nonzero_idx):
        ...
        Args:
            input_matrix (paddle.Tensor): The input matrix.
            nonzero_idx (paddle.Tensor): The indices of the non-zero elements in the matrix.
        Returns:
    ...
    def _get_tokens_diag(prompt, prompt_plus_new_tokens):
        ...
        if not paddle.is_tensor(compare_mat):
            compare_mat = paddle.to_tensor(compare_mat)
    ...
    def _prepare_assistant_input_ids(self, input_ids: paddle.Tensor) -> Tuple[paddle.Tensor, int]:
        ...
        if self.prev_assistant_ids is not None and self.prev_target_ids_len > self.target_lookbehind:
            ...
            if new_tokens_only is not None:
                ...
                if new_tokens_only.shape[1] > 0:
                    assistant_input_ids = paddle.concat([assistant_input_ids, new_tokens_only], axis=-1)
            else:
                # edge case: in case of no intersection between prompt and new_assistant_ids
                assistant_input_ids = paddle.concat([assistant_input_ids, new_assistant_ids], axis=-1)
        else:
    ...
    def _process_assistant_outputs(
        self, input_ids: paddle.Tensor, assistant_sequences: paddle.Tensor, assistant_input_ids: paddle.Tensor
    ) -> paddle.Tensor:
        ...
        if target_new_tokens_only is not None:
            if target_new_tokens_only.shape[1] > 0:
                new_target_ids = paddle.concat([new_target_ids, target_new_tokens_only], axis=-1)
        else:
            # edge case: in case of no intersection between prompt and new_target_ids
            new_target_ids = paddle.concat([new_target_ids, new_target_ids_from_window], axis=-1)
...
class AssistantToTargetTranslator:
    ...
    def _get_assistant_to_target_input_ids(self):
        ...
        assistant_to_target_input_ids = paddle.full((max_assistant_index + 1,), self.SUPPRESS_TOKEN_ID, dtype=int)
    ...
    def _get_suppress_input_ids(self) -> list[int]:
        ...
        """
        return paddle.where(self._assistant_to_target_input_ids == self.SUPPRESS_TOKEN_ID)[0]

    def get_target_ids(
        self, assistant_input_ids, target_input_ids, assistant_candidate_ids: paddle.Tensor
    ) -> paddle.Tensor:
        """
        ...
        if num_new_tokens == 0:
            ...
            return paddle.concat((target_input_ids, transformed_slice.unsqueeze(0)), axis=1)

    def get_target_logits(self, assistant_logits: paddle.Tensor) -> paddle.Tensor:
        """
...
class UniversalSpeculativeDecodingGenerator(AssistedCandidateGeneratorDifferentTokenizers):
    ...
    def __init__(
        ...
        self._prev_assistant_ids: Optional[paddle.Tensor] = None
    ...
    def _update_past_and_masks(self, assistant_input_ids: paddle.Tensor, num_added_tokens: int = 1) -> bool:
    ...
    def _prepare_assistant_input_ids(self, target_input_ids: paddle.Tensor) -> paddle.Tensor:
        """
        ...
        if self._prev_assistant_ids is None:
            ...
            assistant_input_ids = paddle.concat([self._prev_assistant_ids, assistant_new_ids], axis=-1)
        assistant_input_ids = assistant_input_ids.to(dtype=paddle.int64)
...
class PromptLookupCandidateGenerator(CandidateGenerator):
    ...
    def __init__(
        self,
        eos_token_id: paddle.Tensor = None,
    ...
    def get_candidates(self, input_ids: paddle.Tensor) -> Tuple[paddle.Tensor, Optional[paddle.Tensor]]:
        ...
        Return:
            `paddle.Tensor` of shape `(num_candidates, candidate_length)`: The candidate sequences to be tried.
        """
        ...
        candidate_input_ids = paddle.concat((input_ids, chosen_ids), axis=1)
...
def _prepare_attention_mask(model_kwargs: Dict[str, Any], new_length: int, is_encoder_decoder: bool) -> Dict[str, Any]:
    ...
    if "cross_attention_mask" in model_kwargs:
        ...
        if mask_length_diff < 0:
            ...
            model_kwargs["cross_attention_mask"] = paddle.concat([cross_mask, new_mask], axis=1)
        ...
        if mask_length_diff < 0:
            ...
            model_kwargs["image_attention_mask"] = paddle.concat([cross_mask, new_mask], axis=1)
...
def _prepare_token_type_ids(model_kwargs: Dict[str, Any], new_length: int) -> Dict[str, Any]:
    ...
    if type_length_diff < 0:
        ...
        model_kwargs["token_type_ids"] = paddle.concat([model_kwargs["token_type_ids"], token_type_copies], axis=-1)
    return model_kwargs
