...
def get_file(
    ...
    for filename in filenames:
        cache_file_name = bos_aistudio_hf_try_to_load_from_cache(repo_id, filename, cache_dir, 
                                                                 subfolder, revision, repo_type, 
                                                                 from_bos, from_aistudio, from_hf_hub)
    ...
    try:
        if filenames[0].startswith("http://") or filenames[0].startswith("https://"):
            ...
            for filename in filenames:
                ...
                is_available = bos_aistudio_hf_file_exist(repo_id, filename, subfolder=subfolder, repo_type=repo_type,
                                                         revision=revision, token=token, endpoint=endpoint, 
                                                          from_bos=from_bos, from_aistudio=from_aistudio,
