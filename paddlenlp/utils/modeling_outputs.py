...
class MoECausalLMOutputWithPast(ModelOutput):
    ...
    Args:
        ...
        attentions (`tuple(paddle.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            Attentions weights after the attention softmax, used to compute the weighted average in the self-attention
            heads.
        z_loss (`paddle.Tensor`, *optional*, returned when `labels` is provided):
            z_loss for the sparse modules.
        aux_loss (`paddle.Tensor`, *optional*, returned when `labels` is provided):
            aux_loss for the sparse modules.
        router_logits (`tuple(paddle.Tensor)`, *optional*, returned when `output_router_logits=True` is passed or when `config.add_router_probs=True`):
    ...
    hidden_states: Optional[Tuple[paddle.Tensor, ...]] = None
    attentions: Optional[Tuple[paddle.Tensor, ...]] = None
    z_loss: paddle.Tensor = None
    aux_loss: paddle.Tensor = None
...
class MoEModelOutput(ModelOutput):
    ...
    Args:
        ...
        attentions (`tuple(paddle.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            Attentions weights after the attention softmax, used to compute the weighted average in the self-attention
            heads.
        router_probs (`tuple(paddle.Tensor)`, *optional*, returned when `output_router_probs=True` and `config.add_router_probs=True` is passed or when `config.output_router_probs=True`):
            Tuple of `paddle.Tensor` (one for each layer) of shape `(batch_size, sequence_length, num_experts)`.
    ...
    hidden_states: Optional[Tuple[paddle.Tensor, ...]] = None
    attentions: Optional[Tuple[paddle.Tensor, ...]] = None
    router_probs: Optional[Tuple[paddle.Tensor]] = None
...
class Seq2SeqMoEModelOutput(ModelOutput):
    ...
    Args:
        ...
        decoder_attentions (`tuple(paddle.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            Attentions weights of the decoder, after the attention softmax, used to compute the weighted average in the
            self-attention heads.
        decoder_router_logits (`tuple(paddle.Tensor)`, *optional*, returned when `output_router_logits=True` is passed or when `config.add_router_probs=True`):
            Tuple of `paddle.Tensor` (one for each layer) of shape `(batch_size, sequence_length, num_experts)`.
        ...
        encoder_attentions (`tuple(paddle.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            Attentions weights of the encoder, after the attention softmax, used to compute the weighted average in the
            self-attention heads.
        encoder_router_logits (`tuple(paddle.Tensor)`, *optional*, returned when `output_router_logits=True` is passed or when `config.add_router_probs=True`):
            Tuple of `paddle.Tensor` (one for each layer) of shape `(batch_size, sequence_length, num_experts)`.
    ...
    decoder_hidden_states: Optional[Tuple[paddle.Tensor, ...]] = None
    decoder_attentions: Optional[Tuple[paddle.Tensor, ...]] = None
    decoder_router_logits: Optional[Tuple[paddle.Tensor]] = None
    cross_attentions: Optional[Tuple[paddle.Tensor, ...]] = None
    encoder_last_hidden_state: Optional[paddle.Tensor] = None
    encoder_hidden_states: Optional[Tuple[paddle.Tensor, ...]] = None
    encoder_attentions: Optional[Tuple[paddle.Tensor, ...]] = None
    encoder_router_logits: Optional[Tuple[paddle.Tensor]] = None
...
class Seq2SeqMoEOutput(ModelOutput):
    ...
    loss: Optional[paddle.Tensor] = None
    logits: paddle.Tensor = None
    encoder_z_loss: paddle.Tensor = None
    decoder_z_loss: paddle.Tensor = None
    encoder_aux_loss: paddle.Tensor = None
    decoder_aux_loss: paddle.Tensor = None
    past_key_values: Optional[Tuple[Tuple[paddle.Tensor]]] = None
    decoder_hidden_states: Optional[Tuple[paddle.Tensor, ...]] = None
    decoder_attentions: Optional[Tuple[paddle.Tensor, ...]] = None
    decoder_router_logits: Optional[Tuple[paddle.Tensor]] = None
...
class ImageSuperResolutionOutput(ModelOutput):
    ...
    loss: Optional[paddle.Tensor] = None
    reconstruction: paddle.Tensor = None
    hidden_states: Optional[Tuple[paddle.Tensor, ...]] = None
    attentions: Optional[Tuple[paddle.Tensor, ...]] = None
...
class BaseModelOutputWithPoolingAndProjection(ModelOutput):
    ...
    Args:
        ...
        attentions (`tuple(paddle.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            Attentions weights after the attention softmax, used to compute the weighted average in the self-attention
            heads.
        projection_state (`tuple(paddle.Tensor)`, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            Tuple of `paddle.Tensor` of shape `(batch_size,config.project_dim)`.
    ...
    hidden_states: Optional[Tuple[paddle.Tensor, ...]] = None
    attentions: Optional[Tuple[paddle.Tensor, ...]] = None
    projection_state: Optional[Tuple[paddle.Tensor]] = None
...
class Seq2SeqTSModelOutput(ModelOutput):
    ...
    Args:
        ...
        encoder_attentions (`tuple(paddle.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            Attentions weights of the encoder, after the attention softmax, used to compute the weighted average in the
            self-attention heads.
        loc (`paddle.Tensor` of shape `(batch_size,)` or `(batch_size, input_size)`, *optional*):
            Shift values of each time series' context window which is used to give the model inputs of the same
            magnitude and then used to shift back to the original magnitude.
        scale (`paddle.Tensor` of shape `(batch_size,)` or `(batch_size, input_size)`, *optional*):
            Scaling values of each time series' context window which is used to give the model inputs of the same
            magnitude and then used to rescale back to the original magnitude.
        static_features (`paddle.Tensor` of shape `(batch_size, feature size)`, *optional*):
    ...
    encoder_hidden_states: Optional[Tuple[paddle.Tensor, ...]] = None
    encoder_attentions: Optional[Tuple[paddle.Tensor, ...]] = None
    loc: Optional[paddle.Tensor] = None
    scale: Optional[paddle.Tensor] = None
    static_features: Optional[paddle.Tensor] = None
...
class Seq2SeqTSPredictionOutput(ModelOutput):
    ...
    Args:
        loss (`paddle.Tensor` of shape `(1,)`, *optional*, returned when a `future_values` is provided):
            Distributional loss.
        params (`paddle.Tensor` of shape `(batch_size, num_samples, num_params)`):
    ...
    loss: Optional[paddle.Tensor] = None
    params: Optional[Tuple[paddle.Tensor]] = None
    past_key_values: Optional[Tuple[Tuple[paddle.Tensor]]] = None
    decoder_hidden_states: Optional[Tuple[paddle.Tensor, ...]] = None
    decoder_attentions: Optional[Tuple[paddle.Tensor, ...]] = None
    cross_attentions: Optional[Tuple[paddle.Tensor, ...]] = None
...
class SampleTSPredictionOutput(ModelOutput):
    ...
    Args:
        sequences (`paddle.Tensor` of shape `(batch_size, num_samples, prediction_length)` or `(batch_size, num_samples, prediction_length, input_size)`):
