...
class QuantizedCacheConfig(CacheConfig):
    ...
    Attributes:
        ...
        residual_length (`Optional[int]`, *optional*, defaults to 128):
            ...
            Defaults to 128.
        compute_dtype (`paddle.dtype`, *optional*, defaults to `paddle.float16`):
    ...
    def __init__(
        ...
        compute_dtype: Optional[paddle.dtype] = paddle.float16,
...
class DynamicCache(Cache):
    ...
    def from_batch_splits(cls, splits: List["DynamicCache"]) -> "DynamicCache":
        ...
        for idx in range(len(splits[0])):
            ...
            if key_cache != []:
                layer_keys = paddle.concat(key_cache, axis=0)
                layer_values = paddle.concat(value_cache, axis=0)
...
class OffloadedCache(DynamicCache):
    ...
    def __init__(self) -> None:
        ...
        self.prefetch_stream = paddle.cuda.Stream()
        self.beam_idx = None  # used to delay beam search operations

    def prefetch_layer(self, layer_idx: int):
        "Starts prefetching the next layer cache"
        if layer_idx < len(self):
            with paddle.cuda.stream(self.prefetch_stream):
    ...
    def __getitem__(self, layer_idx: int) -> List[Tuple[paddle.Tensor]]:
        ...
        if layer_idx < len(self):
            ...
            paddle.cuda.current_stream().synchronize()
    ...
    def reorder_cache(self, beam_idx: paddle.Tensor):
        ...
        # device because performing paddle.index_select on the CPU is very slow
        del self.beam_idx
    ...
    def update(
        ...
        if len(self.key_cache) < layer_idx:
            ...
            self.key_cache[layer_idx] = paddle.concat([key_tensor, key_states], axis=-2)
            self.value_cache[layer_idx] = paddle.concat([value_tensor, value_states], axis=-2)
...
class QuantizedCache(DynamicCache):
    ...
    def __init__(self, cache_config: QuantizedCacheConfig) -> None:
        super().__init__()
        self._quantized_key_cache: List[paddle.Tensor] = []
        self._quantized_value_cache: List[paddle.Tensor] = []
    ...
    def update(
        ...
        if len(self.key_cache) < layer_idx:
            ...
            self.key_cache.append(paddle.zeros(0, dtype=key_states.dtype))
            self.value_cache.append(paddle.zeros(0, dtype=key_states.dtype))
            ...
            keys_to_return = paddle.concat(keys_to_return, axis=-2)
            values_to_return = paddle.concat(values_to_return, axis=-2)
            if (
                ...
                self.key_cache[layer_idx] = paddle.zeros(0, dtype=key_states.dtype)
                self.value_cache[layer_idx] = paddle.zeros(0, dtype=key_states.dtype)
            else:
                self.key_cache[layer_idx] = paddle.concat([self.key_cache[layer_idx], key_states], axis=-2)
                self.value_cache[layer_idx] = paddle.concat([self.value_cache[layer_idx], value_states], axis=-2)
...
class SinkCache(Cache):
    ...
    def _apply_key_rotary_pos_emb(
        self, key_states: paddle.Tensor, cos: paddle.Tensor, sin: paddle.Tensor
    ) -> paddle.Tensor:
    ...
    def _get_rerotation_cos_sin(
        ...
        if key_states.shape[-2] not in self.cos_sin_rerotation_cache:
            ...
            cos = cos.to(paddle.float32)
            sin = sin.to(paddle.float32)
    ...
    def update(
        ...
        if using_rope and layer_idx == 0:
            ...
            if cos.dim() == 2:
                ...
                if self._cos_cache is None:
                    ...
                    self._cos_cache = paddle.concat([self._cos_cache, cos[0, ...]], axis=0)
                    self._sin_cache = paddle.concat([self._sin_cache, sin[0, ...]], axis=0)
        ...
        if len(self.key_cache) <= layer_idx:
            ...
            if using_rope:
                ...
                if partial_rotation_size is not None:
                    keys_to_keep = paddle.concat((keys_to_keep, keys_pass), axis=-1)

            # Concatenate sink tokens, shifted & rotated tokens (if needed), and new tokens
            sink_keys = self.key_cache[layer_idx][:, :, : self.num_sink_tokens]
            self.key_cache[layer_idx] = paddle.concat([sink_keys, keys_to_keep, key_states], axis=-2)

            sink_values = self.value_cache[layer_idx][:, :, : self.num_sink_tokens]
            values_to_keep = self.value_cache[layer_idx][
                :, :, -self.window_length + self.num_sink_tokens + value_states.shape[-2] :
            ]
            self.value_cache[layer_idx] = paddle.concat([sink_values, values_to_keep, value_states], axis=-2)
...
class StaticCache(Cache):
    """
    Static Cache class to be used with `paddle.compile(model)` and `paddle.export()`.
    ...
    Parameters:
        ...
        dtype (`paddle.dtype`, *optional*, defaults to `paddle.float32`):
            The default `dtype` to use when initializing the layer.
        layer_device_map(`Dict[int, Union[str, paddle.device, int]]]`, `optional`):
    ...
    def __init__(
        ...
        self.dtype = dtype
        self.device = paddle.device(device) if device is not None else paddle.device("meta")
    ...
    def update(
        ...
        if self.key_cache[layer_idx].device.type == "meta":
            self.key_cache[layer_idx] = paddle.zeros_like(self.key_cache[layer_idx])
            self.value_cache[layer_idx] = paddle.zeros_like(self.value_cache[layer_idx])
...
class SlidingWindowCache(StaticCache):
    """
    Sliding Window Cache class to be used with `paddle.compile` for models like Mistral that support sliding window attention.
    ...
    def update(
        ...
        slicing = paddle.ones(self.max_cache_len, dtype="int64").cumsum(0)
...
class EncoderDecoderCache(Cache):
    ...
    def get_seq_length(self, layer_idx: Optional[int] = 0) -> int:
        """Returns the sequence length of the cached states. A layer index can be optionally passed."""
        # check if empty list because in case of static cache it will be a tensors and we can't check `if not paddle.Tensor`
    ...
    def from_batch_splits(cls, splits: List["EncoderDecoderCache"]) -> "EncoderDecoderCache":
        ...
        cross_attention_cache = DynamicCache()
        for idx in range(len(splits[0])):
            layer_keys = paddle.concat([current.self_attention_cache.key_cache[idx] for current in splits], axis=0)
            layer_values = paddle.concat([current.self_attention_cache.value_cache[idx] for current in splits], axis=0)
...
class HybridCache(Cache):
    """
    Hybrid Cache class to be used with `paddle.compile` for Gemma2 models that alternate between a local sliding window attention
    ...
    Parameters:
        ...
        max_cache_len (`int`):
            The maximum sequence length with which the model will be used.
        device (`paddle.device` or `str`, *optional*):
            The device on which the cache should be initialized. Should be the same as the layer.
            The recommended way however is not not indicate any `device`, in that case cache will be initialized on `meta`
            device by default, and then moved to input device when updating.
        dtype (paddle.dtype, *optional*, defaults to `paddle.float32`):
    ...
    def __init__(
        ...
        self.device = paddle.device.get_device() if device is not None else paddle.device("meta")
        layer_switch = config.sliding_window_pattern if hasattr(config, "sliding_window_pattern") else 2  # 2 is for BC
        self.is_sliding = paddle.to_tensor(
            [bool((i + 1) % layer_switch) for i in range(config.num_hidden_layers)], dtype=paddle.bool
        )
        self.key_cache: List[paddle.Tensor] = []
        self.value_cache: List[paddle.Tensor] = []
    ...
    def _sliding_update(self, cache_position, layer_idx, key_states, value_states, k_out, v_out, max_cache_len):
        ...
        slicing = paddle.ones(max_cache_len, dtype="int64").cumsum(0)
    ...
    def update(
        ...
        if self.key_cache[layer_idx].device.type == "meta":
            self.key_cache[layer_idx] = paddle.zeros_like(self.key_cache[layer_idx], device=key_states.device)
            self.value_cache[layer_idx] = paddle.zeros_like(self.value_cache[layer_idx], device=value_states.device)
...
class MambaCache:
    ...
    Arguments:
        ...
        batch_size (`int`):
            The batch size with which the model will be used. Note that a new instance must be instantiated if a
            smaller batch size is used.
        dtype (`paddle.dtype`, *optional*, defaults to `paddle.float16`):
            The default `dtype` to use when initializing the layer.
        device (`paddle.device` or `str`, *optional*):
            The device on which the cache should be initialized. Should be the same as the layer.
            The recommended way however is not not indicate any `device`, in that case cache will be initialized on `meta`
    ...
    def __init__(
        ...
        self.conv_states: List[paddle.Tensor] = []
        self.ssm_states: List[paddle.Tensor] = []
        for _ in range(config.num_hidden_layers):
            conv_state: paddle.Tensor = paddle.zeros(
                self.max_batch_size,
            ...
            ssm_state: paddle.Tensor = paddle.zeros(
                self.max_batch_size,
                self.intermediate_size,
                self.ssm_state_size,
                place=self.device,
            ...
            paddle._dynamo.mark_static_address(conv_state)
            paddle._dynamo.mark_static_address(ssm_state)
            self.conv_states.append(conv_state)
            self.ssm_states.append(ssm_state)

    def update_conv_state(
        self, layer_idx: int, new_conv_state: paddle.Tensor, cache_position: paddle.Tensor
    ) -> paddle.Tensor:
        if self.conv_states[layer_idx].device.type == "meta":
            self.conv_states[layer_idx] = paddle.zeros_like(
                self.conv_states[layer_idx],
            )

        conv_state = self.conv_states[layer_idx]
        cache_position = cache_position.clip(0, self.conv_kernel_size - 1)
...
class OffloadedStaticCache(StaticCache):
    """
    Static cache class to be used with `paddle.compile(model)` that offloads to the CPU or
    another device.
    ...
    Args:
        ...
        offload_device (`Union[str, paddle.device]`, *optional*, defaults to `cpu`):
    ...
    Attributes:
        key_cache (`List[paddle.Tensor]`):
            ...
            off-loaded.
        value_cache (`List[paddle.Tensor]`):
    ...
    def __init__(
        ...
        self.max_cache_len = config.max_position_embeddings if max_cache_len is None else max_cache_len
        self.device = paddle.device.get_device() if layer_device_map is None else paddle.device(layer_device_map[0])
        self.offload_device = paddle.device(offload_device)
        ...
        self._device_key_cache: List[paddle.Tensor] = []
        self._device_value_cache: List[paddle.Tensor] = []
        ...
        self._prefetch_stream = paddle.cuda.Stream() if self.device.type == "cuda" else None
    ...
    def update(
        ...
        if layer_idx == 0:
            ...
            if self._prefetch_stream is not None:
                paddle.cuda.default_stream(self.device).wait_stream(self._prefetch_stream)
    ...
    def _create_key_value_cache_tensors(
        self, shape: Tuple[int, ...], device: paddle.device
    ) -> Tuple[paddle.Tensor, paddle.Tensor]:
        ...
        is_cpu_device = device == paddle.device("cpu")

        key_cache = paddle.zeros(shape, dtype=self.dtype, device=device, pin_memory=is_cpu_device)
        value_cache = paddle.zeros(shape, dtype=self.dtype, device=device, pin_memory=is_cpu_device)

        # Note: `mark_static_address` is used to tag the cache as a fixed data pointer,
        # preventing compiled graph breaks when updating the cache.
        paddle._dynamo.mark_static_address(key_cache)
        paddle._dynamo.mark_static_address(value_cache)
    ...
    def _prefetch_layer(self, layer_idx: int) -> None:
        ...
        if self._prefetch_stream is not None:
            with paddle.cuda.stream(self._prefetch_stream):
