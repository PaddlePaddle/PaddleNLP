...
class Trainer:
        ...
        if not self.args.enable_auto_parallel:
            ...
            if self.args.should_load_sharding_stage1_model:
                ...
                if resume_from_checkpoint is not None:
                    ...
                    optimizer_sharded_state_dict_cp = {}
                    for k in optimizer_sharded_state_dict.keys():
                        if "mlp.moe_statics.e_score_correction_bias"  not in k:
                            optimizer_sharded_state_dict_cp[k] = optimizer_sharded_state_dict[k]
                    optimizer_sharded_state_dict = optimizer_sharded_state_dict_cp

                    sharded_state_dict = {**model_sharded_state_dict, **optimizer_sharded_state_dict}
                    logger.info("======> aoa config: ")
                    logger.info(self.args.aoa_config)
                    ...
                    #     md5 = value.local_tensor._md5sum()
                    #     logger.info(f"{key}, {md5}")
                    # logger.info("=======> Save flex checkpoint")
                    # dist.save_state_dict(sharded_state_dict, resume_from_checkpoint)
                    # import sys
                    # sys.exit(0)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step_control + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    # logger.info(f"========> STEP {step_control}  DEBUG ===============")
                    # model_state_dict = self.model.state_dict()
                    # optimizer_state_dict = self.optimizer.state_dict()
                    # master_weights = optimizer_state_dict.pop("master_weights",None)
                    # lr_scheduler = optimizer_state_dict.pop("LR_Scheduler",None)
                    # for key, value in model_state_dict.items():
                    #     logger.info(f"{key}. ==> {value._md5sum()}")
                    ...
                    if save_to_flex:
                        ...
                        sharded_state_dict = {**model_sharded_state_dict, **optimizer_sharded_state_dict}
                        path = f"/workspace/zhuxinming/ERNIE/examples/pre-training/si2fc"
                        dist.save_state_dict(sharded_state_dict, path)
                        logger.info(f"save sharding_io ckpt to flexcheckpoint to {path}")
                        import sys
                        sys.exit(0)
    ...
    def training_step(
        ...
        logger.info(f"Training Step loss md5 {loss._md2sum()} !")
        return loss.detach()
                ...
                if self.dp_group.rank <= 0 or self.args.use_expert_parallel:
                    ...
                    if self.args.unified_checkpoint:
                        ...
                        if not self.args.using_flex_checkpoint:
                            ...
                            logger.info(f"Saving optimizer files.")

                            # for k, v in sharded_state_dict.items():
                            #     logger.info(f"{k}: {v}")
                            #     logger.info(f"stop_gradients: {v.local_tensor.stop_gradient}")
                            ...
                            # master_weights = optimizer_state_dict.pop("master_weights",None)
                            # lr_scheduler = optimizer_state_dict.pop("LR_Scheduler",None)
                            # full_state_dict.update(master_weights)
                            # full_state_dict.update(optimizer_state_dict)
                            # logger.info("LR C:" + str(lr_scheduler))
                            ...
                            # for key, value in sharded_state_dict.items():
                            #     md5 = value.local_tensor._md5sum()
                            #     logger.info(f"{key}: {md5}   stop gradient {value.local_tensor.stop_gradient}")
                            #     before_save_md[key] = md5

                            # logger.info("=======> Load flex checkpoint")
                            # for key, value in sharded_state_dict.items():
                            #     logger.info(f"{key} {value}")
                            ...
                            # before_load_md5 = {}
                            # for k,v in sharded_state_dict.items():
                            #     before_load_md5[k] = v.local_tensor._md5sum()
                            #     paddle.assign(paddle.zeros_like(v.local_tensor),v.local_tensor)
                            #     v.local_tensor.stop_gradient = True
                            #     logger.info(f" {k}  -->  {before_load_md5[k]} ")
                            ...
                            # logger.info("=======>After Load flex checkpoint")
                            # after_load_md5 = {}
                            # for k,v in sharded_state_dict.items():
                            #     after_load_md5[k] = v.local_tensor._md5sum()
                            #     logger.info(f" {k}  -->  {after_load_md5[k]} ")

                            # for k in before_load_md5.keys():
                            #     logger.info(f"============> {k}")
                            #     b_md5 = before_load_md5[k]
                            #     a_md5 = after_load_md5[k]
                            #     logger.info(f"before: {b_md5} => after: {a_md5}")
                            #     assert b_md5 == a_md5, f"{k} before load md5: {b_md5} != after load md5: {a_md5}"
                            ...
                            #     val = value.local_tensor
                            #     zero_t = paddle.zeros_like(val)
                            #     paddle.assign(zero_t, val)
                            #     md5 = val._md5sum()
                            #     val.stop_gradient = True
                            #     logger.info(f"{key}: {md5}")
                            #     after_zero_md[key] = md5
                            ...
                            #     md5 = val._md5sum()
                            #     logger.info(f"{key}: {md5}")
                            #     after_load_md[key] = md5


                            # logger.info("====> begin md5 check")

                            # for key in sharded_state_dict.keys():
                            #     before_save = before_save_md[key]
                            #     # after_zero = after_zero_md[key]
                            #     after_load = after_load_md[key]
                            #     logger.info(f"{key}:\n  before_save: {before_save} \nafter_load: {after_load}")
                            #     # assert before_save != after_zero, f"{key} before_save: {before_save}, after_zero: {after_zero}"
                            #     assert before_save == after_load, f"{key} before_save: {before_save}, after_load: {after_load}"
                            # import sys
                            # logger.info("===>MD5 CHECK PASS !")
                            # sys.exit(0)
