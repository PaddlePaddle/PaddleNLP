...
class MixPrecisionOptimizer:
    ...
    def _obtain_optimizer_parameters_list(self):
        if getattr(self._inner_opt, "_param_groups", None) and isinstance(self._inner_opt._param_groups[0], dict):
            parameters_list = []
            for group in self._inner_opt._param_groups:
                for param in group["params"]:
                    parameters_list.append(param)
        else:
            parameters_list = [param for param in self._inner_opt._parameter_list]
