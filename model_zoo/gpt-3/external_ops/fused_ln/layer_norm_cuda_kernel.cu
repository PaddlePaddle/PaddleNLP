...
template<> __device__
void cuWelfordMuSigma2(
  const phi::dtype::float16* __restrict__ vals,
  const int n1,
  const int n2,
  const int i1,
  float& mu,
    ...
    double epsilon)
{
    DISPATCH_FLOAT_HALF_AND_BFLOAT_INOUT_TYPES(
        input.type(), output->type(), "cuda_layer_norm_kernel",
        HostApplyLayerNorm(
	    output->data<scalar_t_out>(),
	    mean->data<float>(),
	    invvar->data<float>(),
	    const_cast<scalar_t_in *>(input.data<scalar_t_in>()),
	    n1,n2,
	    epsilon,
	    gamma != NULL ? const_cast<scalar_t_out *>(gamma->data<scalar_t_out>()) : NULL,
	    beta != NULL ? const_cast<scalar_t_out *>(beta->data<scalar_t_out>()) : NULL,
        input.stream());
      )
}
    ...
    )
{
	...
	(threads2.x + 1);
      ...
      paddle::Tensor part_grad_gamma = paddle::empty(
	  {part_size,n2}, paddle::DataType::FLOAT32, place);
...
{
    DISPATCH_FLOAT_HALF_AND_BFLOAT_INOUT_TYPES(
        input.type(), gamma->type(),
	"cuda_layer_norm_gradient_kernel",
        HostLayerNormGradient<scalar_t_in, float, scalar_t_out>(
	    dout.data<scalar_t_out>(),
	    mean.data<float>(),
	    invvar.data<float>(),
	    input,
	    n1,n2,
	    gamma != NULL ? gamma->data<scalar_t_out>() : NULL,
	    gamma != NULL ? beta->data<scalar_t_out>() : NULL,
	    epsilon,
	    grad_input->data<scalar_t_in>(),
	    gamma != NULL ? grad_gamma->data<scalar_t_out>() : NULL,
	    gamma != NULL ? grad_beta->data<scalar_t_out>() : NULL,
        input.stream()));
}
