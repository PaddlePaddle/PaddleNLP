...
def test_qic_dataset(client):
    from paddlenlp.datasets import load_dataset
    dev_ds = load_dataset('cblue', "KUAKE-QIC", splits='dev')
    ...
    while idx < len(dev_ds):
        ...
        for i in range(batch_size):
            if idx + i >= len(dev_ds):
                break
            datas.append(dev_ds[idx + i]["text_a"])
...
if __name__ == "__main__":
    client = init_client()
    texts = ["心肌缺血如何治疗与调养呢？", "什么叫痔核脱出？什么叫外痔？"]
    output = test_demo(client, texts)
    print(output)
    test_qic_dataset(client)
