...
def convert_example(example, tokenizer, label_list, max_seq_length=512, is_test=False):
    ...
    if (int(is_test) + len(example)) == 2:
        example = tokenizer(example["sentence"], max_seq_len=max_seq_length, return_token_type_ids=False)
    else:
        example = tokenizer(
            example["sentence1"],
...
def do_train(args):
    ...
    batchify_fn = lambda samples, fn=Tuple(
        Pad(axis=0, pad_val=padding_id), Stack(dtype="int64" if train_ds.label_list else "float32")  # input  # label
    ): fn(samples)
