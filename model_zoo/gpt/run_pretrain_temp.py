...
#
# @dataclass
# class ModelArguments(PdArgumentParser):
...
#     grad_clip: float = field(default=1.0, metadata={"help": "clip of gradient"})
#     to_static: bool = field(default=False, metadata={"help": "whether use static pretraining mode."})
#     lr_decay_style: str = field(default="none", metadata={"help": "use different learning rate decay style."})
#     max_lr: float = field(default=1e-4, metadata={"help": "The initial max learning rate for Adam."})
...
#     training_args.device = get_device(training_args.device)
...
#
#     default_global_batch_size = topo.data_info.size * training_args.per_device_train_batch_size
#     default_global_tokens_num = default_global_batch_size * model_args.max_seq_len
#
...
#             training_args.per_device_train_batch_size * topo.data_info.size,
#             False,
#             False,
#             worker_index,
#         ).lower(),
...
#             max_lr=model_args.max_lr, min_lr=model_args.min_lr, warmup_step=training_args.warmup_steps
#         )
#
#     clip = None
#     if model_args.grad_clip > 0:
#         clip = paddle.nn.ClipGradByGlobalNorm(clip_norm=model_args.grad_clip)
#
#     # Generate parameter names needed to perform weight decay.
#     # All bias and LayerNorm parameters are excluded.
#     decay_params = [p.name for n, p in model.named_parameters() if not any(nd in n for nd in ["bias", "norm"])]
#     optimizer = paddle.optimizer.AdamW(
#         learning_rate=lr_scheduler if lr_scheduler is not None else model_args.max_lr,
...
#         scaler = paddle.amp.GradScaler(init_loss_scaling=model_args.scale_loss)
#
