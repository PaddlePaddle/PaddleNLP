...
def do_train(args):
    ...
    if args.use_multi_layer_strategy:
        ...
        from multi_layer_modeling import ErnieMultiLayerFinetuningForSequenceClassification
        model = ErnieMultiLayerFinetuningForSequenceClassification.from_pretrained(
            args.model_name_or_path, num_classes=num_classes)
    else:
        model = AutoModelForSequenceClassification.from_pretrained(
            args.model_name_or_path, num_classes=num_classes)
    ...
    for epoch in range(num_train_epochs):
        for step, batch in enumerate(train_data_loader):
            ...
            logits = model(**batch)
            if args.use_multi_layer_strategy:
                logits_auxiliary, logits_original = logits
                loss_original = loss_fct(logits_original, labels)
                loss_auxiliary = loss_fct(logits_auxiliary, labels)
                loss = loss_original + loss_auxiliary
            else:
                loss = loss_fct(logits, labels)
