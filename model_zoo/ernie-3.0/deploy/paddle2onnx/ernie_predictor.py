...
class InferBackend(object):
    def __init__(self, model_path, use_fp16):
        ...
        predictor = ort.InferenceSession(model_path, sess_options=sess_options, providers=providers)
        if "CUDAExecutionProvider" in predictor.get_providers():
            ...
            if use_fp16:
                ...
                sess_options = ort.SessionOptions()
                predictor = ort.InferenceSession(fp16_model, sess_options=sess_options, providers=providers)
        else:
            print(">>> [InferBackend] Use CPU to inference ...")
            if use_fp16:
                print(">>> [InferBackend] use_fp16 only takes effect when deploying on gpu ...")
...
class ErniePredictor(object):
    def __init__(self, args):
        ...
        if args.task_name == "seq_cls":
            ...
            print("[ErniePredictor]: task_name only support seq_cls and token_cls now.")
            exit(0)
