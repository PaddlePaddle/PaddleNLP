...
def parse_args():
    ...
    parser.add_argument("--use_multi_layer_strategy",
                        action='store_true',
                        help="Whether to use multiple layer strategy.")
...
def run(args):
    ...
    if args.use_multi_layer_strategy:
        # TODO(LiuChiachi) Needs better way
        from multi_layer_modeling import ErnieMultiLayerFinetuningForQuestionAnswering
        model = ErnieMultiLayerFinetuningForQuestionAnswering.from_pretrained(
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            for step, batch in enumerate(train_data_loader):
                ...
                logits = model(**batch)
                if args.use_multi_layer_strategy:
                    start_logits_auxiliary, end_logits_auxiliary, start_logits, end_logits = logits
                    loss_auxiliary = criterion(
                        (start_logits_auxiliary, end_logits_auxiliary),
                        (start_positions, end_positions))
                    last_layer_loss = criterion(
                        (start_logits, end_logits),
                        (start_positions, end_positions))
                    loss = last_layer_loss + loss_auxiliary
                else:
