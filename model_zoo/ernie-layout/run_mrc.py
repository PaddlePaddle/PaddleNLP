...
def main():
    ...
    train_ds, dev_ds, test_ds = load_dataset(
        data_args.dataset_name, split=["train", "validation", "test"])
    ...
    model = AutoModelForQuestionAnswering.from_pretrained(
        model_args.model_name_or_path, num_classes=num_labels)
    ...
    postprocess_func = partial(postprocessor.postprocess_mrc,
                               tokenizer=tokenizer,
                               lang=data_args.lang)
    ...
    if training_args.do_train:
        if data_args.train_nshard > 1:
            logger.info(
                f"spliting train dataset into {data_args.train_nshard} shard")
            train_shards = []
            for idx in range(data_args.train_nshard):
                train_shards.append(
                    train_ds.shard(
                        num_shards=data_args.train_nshard, index=idx).map(
                            preprocess_func,
                            batched=True,
    ...
    def compute_metrics(eval_preds):
        ...
        def _convert(examples):
            ...
            for example in examples:
                ...
                for i in range(len(example["annotations"])):
                    formatted_example["annotations"]["qid"].append(
                        example["annotations"][i]["qid"])
                    formatted_example["annotations"]["question"].append(
                        example["annotations"][i]["question"])
                    formatted_example["annotations"]["value"].append(
                        example["annotations"][i]["value"])
        ...
        for pred in preds:
            for key, values in zip(pred["annotations"]["qid"],
                                   pred["annotations"]["value"]):
                pred_dict[pred["id"]][key].extend(values)
        for label in labels:
            for key, values in zip(label["annotations"]["qid"],
