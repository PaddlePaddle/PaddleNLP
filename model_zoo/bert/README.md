...
- `batch_size` 表示每次迭代**每张卡**上的样本数目。(非Traner版本)
- `per_device_train_batch_size` 表示用于训练的每个 GPU 核心/CPU 的batch大小.（`int`，可选，默认为 8）（Trainer版本）
...
- `do_train` 是否进行训练任务。(`bool`, 可选, 默认为 `False`)（Trainer版本）
...
- `per_device_train_batch_size` 表示用于训练的每个 GPU 核心/CPU 的batch大小.（`int`，可选，默认为 8）(Trainer版本)
- `per_device_eval_batch_size` 表示用于评估的每个 GPU 核心/CPU 的batch大小.（`int`，可选，默认为 8）(Trainer版本)
- `learning_rate` 表示基础学习率大小，将于learning rate scheduler产生的值相乘作为当前学习率。
- `num_train_epochs` 表示训练轮数。
- `logging_steps` 表示日志打印间隔。
- `save_steps` 表示模型保存及评估间隔。
...
- `fp16` 是否使用 fp16 混合精度训练而不是 fp32 训练。(`bool`, 可选, 默认为 `False`)（Trainer版本）
- `do_train` 是否进行训练任务。(`bool`, 可选, 默认为 `False`)（Trainer版本）
- `do_eval` 是否进行评估任务。同上。(`bool`, 可选, 默认为 `False`)（Trainer版本）
