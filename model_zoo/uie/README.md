...
目前对 UIE 模型采用的压缩方法是量化训练（QAT），硬件需要满足[4.8 模型部署](#模型部署) 中对 INT8 推理的要求才可进行本节的内容。
...
使用 **模型压缩** 功能需要安装最新版本的 `paddleslim`，并且需要保证 PaddlePaddle 版本 >= 2.4.0rc。
...
以报销工单信息抽取任务为例，使用 `uie-base` 进行微调，先得到原始 FP32 模型，然后使用 QAT 策略进一步量化。量化后的模型比原始 FP32 模型的 F1 值高 2.19。但是，量化训练并不一定能保证 UIE 在各数据集效果均有提升，还需要在任务数据上实验。
