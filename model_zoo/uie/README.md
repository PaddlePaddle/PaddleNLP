...
* `use_fast`: 使用C++实现的高性能分词算子FastTokenizer进行文本预处理加速。需要通过`pip install fast_tokenizer`安装FastTokenizer库后方可使用。默认为`False`。更多使用说明可参考[FastTokenizer文档](../../fast_tokenizer)。
<a name="训练定制"></a>
...
#### 4.7 模型压缩(**可选**，看4.8部分看硬件是否支持)

目前对 UIE 模型采用的压缩方法是量化训练（QAT），部署硬件需要满足[4.8 模型部署](#模型部署) 中对 INT8 推理的要求才可进行本节内容。

模型压缩主要用于有模型部署上线需求的场景，可以进一步压缩模型体积、加快推理速度、减少内存占用。模型压缩功能使用的是 **PaddleNLP 模型压缩 API**，文档可[点此](../../docs/compression.md)查看。

使用 **模型压缩** 功能需要安装最新版本的 `paddleslim` 包，并且需要保证 **PaddlePaddle 版本 >= 2.4.0** 以保证量化模型部署成功。
...
压缩与微调共用了一个脚本，压缩时开启 `--do_compress`，并取消设置 `--do_train` 和 `--do_eval`。使用下面的命令，使用上面微调好的模型 `$finetuned_model` 进行压缩：
...
以报销工单信息抽取任务为例，使用 `uie-base` 进行微调，先得到原始 FP32 模型，然后使用 QAT 策略进行量化。量化后的模型比原始 FP32 模型的 F1 值高 2.19。但是，量化训练并不一定能保证 UIE 模型在各数据集效果均有提升。
...
- 环境准备
  ...
  - CPU端
    ...
    如果用户使用 INT8 部署，模型需要先进行量化（参考 4.7 节的内容）。
