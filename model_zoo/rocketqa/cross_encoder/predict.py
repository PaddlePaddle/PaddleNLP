...
from data import convert_example, read_train_set, create_dataloader
from cross_model import CrossEncoder
...
parser.add_argument("--save_dir", type=str, default='output', help="The save path for prediction result.")
...
def predict(model, data_loader):
    ...
    with paddle.no_grad():
        for batch in tqdm(data_loader):
            ...
            loss, probs = F.softmax_with_cross_entropy(logits=logits,
                                                       label=label,
                                                       return_softmax=True)
            probs = probs.numpy()
...
if __name__ == "__main__":
    ...
    file = open('{}/result.txt'.format(args.save_dir), 'w')
