...
from ppdiffusers.initializer import zeros_
...
class ControlNetInputHintBlock(nn.Layer):
    ...
    def forward(self, hint: paddle.Tensor):
        return self.input_hint_block(hint)


class ControlNetZeroConvBlock(nn.Layer):
    def __init__(
        self,
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            if not is_final_block:
                zero_convs.append(zero_conv(output_channel))
        self.zero_convs = nn.LayerList(zero_convs)
        self.mid_zero_conv = zero_conv(block_out_channels[-1])

    def forward(
        self,
        down_block_res_samples: List[paddle.Tensor],
        mid_block_sample: paddle.Tensor,
    ) -> List[paddle.Tensor]:
        outputs = []
