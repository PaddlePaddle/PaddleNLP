...
def index_to_log_onehot(x: paddle.Tensor, num_classes: int) -> paddle.Tensor:
    ...
    log_x = paddle.log(x_onehot.astype("float32").clip(min=1e-30))
    return log_x


def gumbel_noised(logits: paddle.Tensor) -> paddle.Tensor:
    """
    Apply gumbel noise to `logits`
    """
    uniform = paddle.rand(logits.shape)
...
class VQDiffusionScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.log_at = log_at.astype("float32")
        self.log_bt = log_bt.astype("float32")
        self.log_ct = log_ct.astype("float32")
        self.log_cumprod_at = log_cumprod_at.astype("float32")
        self.log_cumprod_bt = log_cumprod_bt.astype("float32")
        self.log_cumprod_ct = log_cumprod_ct.astype("float32")
