...
class VersatileDiffusionDualGuidedPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        if self.text_unet is not None and (
                "dual_cross_attention" not in self.image_unet.config
                or not self.image_unet.config.dual_cross_attention):
    ...
    def _convert_to_dual_attention(self):
        ...
        for name, module in self.image_unet.named_sublayers(include_self=True):
            if isinstance(module, Transformer2DModel):
                ...
                image_transformer = self.image_unet.get_sublayer(
                    parent_name)[index]
                text_transformer = self.text_unet.get_sublayer(
                    parent_name)[index]
                ...
                self.image_unet.get_sublayer(
                    parent_name)[index] = dual_transformer
    ...
    def _encode_image_prompt(self, prompt, num_images_per_prompt,
        ...
        if do_classifier_free_guidance:
            ...
            image_embeddings = paddle.concat(
                [uncond_embeddings, image_embeddings])
