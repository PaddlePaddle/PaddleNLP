...
def _preprocess_image(image: PIL.Image.Image):
    ...
    image = paddle.to_tensor(image).astype("float32") / 127.5 - 1.0
    return image
...
class RePaintPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if seed is not None:
            paddle.seed(seed)
        image = paddle.randn(original_image.shape)
        ...
        for i, t in enumerate(tqdm(self.scheduler.timesteps)):
            if t < t_last:
                ...
                image = self.scheduler.step(model_output, t, image,
                                            original_image,
                                            mask_image).prev_sample

            else:
                # compute the reverse: x_t-1 -> x_t
                image = self.scheduler.undo_step(image, t_last)
            t_last = t
