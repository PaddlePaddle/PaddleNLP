...
class VQDiffusionPipeline(DiffusionPipeline):
    ...
    def _encode_prompt(self, prompt, num_images_per_prompt,
        ...
        if do_classifier_free_guidance:
            if self.learned_classifier_free_sampling_embeddings.learnable:
                ...
                uncond_embeddings = uncond_embeddings.unsqueeze(0).tile(
                    [batch_size, 1, 1])
            else:
                ...
                uncond_embeddings = uncond_embeddings / uncond_embeddings.norm(
                    axis=-1, keepdim=True)
    ...
    def __call__(
        ...
        timesteps_tensor = paddle.to_tensor(timesteps_tensor.numpy())
        sample = latents
        ...
        for i, t in enumerate(self.progress_bar(timesteps_tensor)):
            ...
            sample = self.scheduler.step(torch.tensor(model_output.numpy(),
                                                      device="cuda"),
                                         timestep=torch.tensor(t.numpy(),
                                                               device="cuda"),
                                         sample=torch.tensor(sample.numpy(),
                                                             device="cuda"),
                                         generator=None).prev_sample
            sample = paddle.to_tensor(sample.cpu().numpy())
