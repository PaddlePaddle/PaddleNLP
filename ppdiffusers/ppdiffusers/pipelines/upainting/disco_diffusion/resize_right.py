...
try:
    ...
    warnings.warn("No Paddle found, will work only with Numpy")
    paddle = None
...
try:
    ...
    warnings.warn("No Numpy found, will work only with Paddle")
    numpy = None
...
def apply_convs(input, scale_factor, in_sz, out_sz, weights, dim, pad_sz, pad_mode, fw):
    ...
    tmp_output = fw_empty(tuple(tmp_out_shape), fw, input.place)
...
def fw_swapaxes(x, ax_1, ax_2, fw):
    if fw is numpy:
        ...
        orders = list(range(x.ndim))
        orders[ax_1] = ax_2
        orders[ax_2] = ax_1
        return x.transpose(orders)
...
def fw_pad(x, fw, pad_sz, pad_mode, dim=0):
    ...
    if fw is numpy:
        ...
        orders[-1] = dim
        orders[dim] = x.ndim - 1
        return fw.nn.functional.pad(x.transpose(orders), pad=pad_vec, mode=pad_mode).transpose(orders)
...
def fw_conv(input, filter, stride):
    ...
    reshaped_output = paddle.nn.functional.conv2d(reshaped_input, filter.reshape([1, 1, 1, -1]), stride=(1, stride))
