...
def prepare_mask_and_masked_image(image, mask):
    ...
    The ``image`` will be converted to ``"float32"`` and normalized to be in ``[-1, 1]``. The ``mask`` will be
    binarized (``mask > 0.5``) and cast to ``"float32"`` too.

    Args:
        image (Union[np.array, PIL.Image, paddle.Tensor]): The image to inpaint.
            It can be a ``PIL.Image``, or a ``height x width x 3`` ``np.array`` or a ``channels x height x width``
            ``paddle.Tensor`` or a ``batch x channels x height x width`` ``paddle.T'en'so'r``.
...
class StableDiffusionInpaintPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        if hasattr(
            ...
            deprecate("skip_prk_steps not set",
                      "1.0.0",
                      deprecation_message,
                      standard_warn=False)
            new_config = dict(scheduler.config)
    ...
    def _encode_prompt(self, prompt, num_images_per_prompt,
        ...
        config = self.text_encoder.config if isinstance(
            self.text_encoder.config,
            dict) else self.text_encoder.config.to_dict()
    ...
    def prepare_mask_latents(self, mask, masked_image, batch_size, height,
        ...
        mask = mask.tile([batch_size, 1, 1, 1])
        masked_image_latents = masked_image_latents.tile([batch_size, 1, 1, 1])
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if isinstance(image, PIL.Image.Image) and isinstance(
                mask_image, PIL.Image.Image):
