...
def get_unweighted_text_embeddings(
    ...
    if max_embeddings_multiples > 1:
        ...
        for i in range(max_embeddings_multiples):
            ...
            text_embedding = pipe.text_encoder(input_ids=text_input_chunk.numpy().astype(np.int64))[0]
            text_embedding=paddle.to_tensor(text_embedding)
            if no_boseos_middle:
                if i == 0:
        ...
        input_name = pipe.text_encoder.get_input_info(0).name
        text_embedding = pipe.text_encoder.infer({input_name:text_input.numpy().astype(np.int64)})[0]
        text_embedding=paddle.to_tensor(text_embedding)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if image is None:
            ...
            # TODO wangbojun, vae encoder is not readly for image input
            assert False, "image to image is not implemented"
            if isinstance(image, PIL.Image.Image):
                image = preprocess_image(image)
            # encode the init image into latents and scale the latents
            image = image.astype(dtype=latents_dtype)
            init_latents = self.vae_encode(sample=image.numpy().astype(np.float32))[0]
        ...
        images = self.vae_decoder(latent_sample = latents.numpy().astype(input_dtype))[0]
