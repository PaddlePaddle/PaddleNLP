...
This however can make it difficult to build applications on top of `ppdiffusers` as you will always have to pass the token around. A potential way to solve this issue is by downloading the weights to a local path `"./stable-diffusion-v1-4-paddle"`:
...
```python
# make sure you're logged in with `huggingface-cli login`
from ppdiffusers import StableDiffusionPipeline, LMSDiscreteScheduler
