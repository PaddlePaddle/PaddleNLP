...
def posterior_sample(scheduler, latents, timestep, clean_latents, eta):
    ...
    noise = std_dev_t * paddle.randn(clean_latents.shape,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if source_text_input_ids.shape[-1] > self.tokenizer.model_max_length:
            removed_text = self.tokenizer.batch_decode(
                source_text_input_ids[:, self.tokenizer.model_max_length:])
            logger.warning(
                "The following part of your input was truncated because CLIP can only handle sequences up to"
                f" {self.tokenizer.model_max_length} tokens: {removed_text}")
            source_text_input_ids = source_text_input_ids[:, :self.tokenizer.
                                                          model_max_length]
        attention_mask = paddle.ones_like(text_input_ids)
        text_embeddings = self.text_encoder(text_input_ids,
                                            attention_mask=attention_mask)[0]
        attention_mask = paddle.ones_like(source_text_input_ids)
        source_text_embeddings = self.text_encoder(
            source_text_input_ids, attention_mask=attention_mask)[0]
        ...
        bs_embed, seq_len, _ = source_text_embeddings.shape
        source_text_embeddings = source_text_embeddings.tile(
            [1, num_images_per_prompt, 1])
        source_text_embeddings = source_text_embeddings.reshape(
            [bs_embed * num_images_per_prompt, seq_len, -1])
        ...
        source_uncond_tokens = [""] * batch_size
        ...
        attention_mask = paddle.ones_like(source_uncond_input.input_ids)
        source_uncond_embeddings = self.text_encoder(
            source_uncond_input.input_ids, attention_mask=attention_mask)[0]

        # duplicate unconditional embeddings for each generation per prompt
        bs_embed, seq_len, _ = source_uncond_embeddings.shape
        source_uncond_embeddings = source_uncond_embeddings.tile(
            [1, num_images_per_prompt, 1])
        source_uncond_embeddings = source_uncond_embeddings.reshape(
            [bs_embed * num_images_per_prompt, seq_len, -1])

        # For classifier free guidance, we need to do two forward passes.
        # Here we concatenate the unconditional and text embeddings into a single batch
        # to avoid doing two forward passes
        source_text_embeddings = paddle.concat(
            [source_uncond_embeddings, source_text_embeddings])
