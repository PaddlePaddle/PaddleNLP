...
DO_EXPORT = str2bool(os.getenv("DO_EXPORT", "False"))
    ...
    if getattr(self, "enable_attn", False):
        ...
        if self.attention_auto_machine_weight > self.attn_weight:
            ...
            if DO_EXPORT:
                latent_self_attn1_uc = self.processor(
                    ...
                    encoder_hidden_states=paddle.concat(
                        [latent_hidden_states, image_hidden_states],
                        axis=1,
                    ),
                    attention_mask=attention_mask,
                    **cross_attention_kwargs,
            ...
            if do_classifier_free_guidance and self.current_style_fidelity > 1e-5:
                ...
                if DO_EXPORT:
                    latent_self_attn1_c[:1] = self.processor(
                        self,
                        hidden_states=latent_hidden_states[:1],
                        encoder_hidden_states=latent_hidden_states[:1],
                        attention_mask=attention_mask,
                        **cross_attention_kwargs,
                    )
                else:
