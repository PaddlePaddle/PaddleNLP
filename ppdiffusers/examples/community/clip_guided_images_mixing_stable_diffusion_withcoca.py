...
def spherical_dist_loss(x, y):
    ...
    return paddle.divide((x - y).norm(axis=-1), 2).asin().pow(y=2).multiply(y=2)
...
class CLIPGuidedImagesMixingStableDiffusion(DiffusionPipeline):
    ...
    def cond_fn(
        ...
        if isinstance(latents.dtype, paddle.dtype):
            dtype = latents.dtype
        elif isinstance(latents.dtype, str) and latents.dtype not in ["cpu", "cuda", "ipu", "xpu"]:
            dtype = latents.dtype
        elif isinstance(latents.dtype, paddle.Tensor):
            dtype = latents.dtype.dtype
        else:
            dtype = self.normalize(image).dtype
        image = self.normalize(image).cast(dtype)
