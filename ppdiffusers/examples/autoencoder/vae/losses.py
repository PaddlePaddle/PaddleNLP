...
class ActNorm(nn.Layer):
    ...
    def initialize(self, input):
        ...
        mean = (
            flatten.mean(1).unsqueeze(1).unsqueeze(2).unsqueeze(3).transpose(
                [1, 0, 2, 3]))
    ...
    def forward(self, input, reverse=False):
        ...
        if self.training and self.initialized.item() == 0:
            ...
            self.initialized.set_value(
                paddle.to_tensor(1, dtype=self.initialized.dtype))
...
def hinge_d_loss(logits_real, logits_fake):
    loss_real = paddle.mean(F.relu(1. - logits_real))
    loss_fake = paddle.mean(F.relu(1. + logits_fake))
...
class ScalingLayer(nn.Layer):
    ...
    def __init__(self):
        ...
        self.register_buffer(
            ...
            paddle.to_tensor(
                np.asarray([-.030, -.088,
                            -.188]).astype('float32')[None, :, None, None]))
        self.register_buffer(
            'scale',
            paddle.to_tensor(
                np.asarray([.458, .448, .450]).astype('float32')[None, :, None,
                                                                 None]))
...
class LPIPS(nn.Layer):
    ...
    def __init__(self,
        ...
        if (verbose):
            print('Setting up [%s] perceptual loss: trunk [%s], spatial [%s]' %
                  ('LPIPS' if lpips else 'baseline', net,
    ...
    def forward(self, in0, in1, retPerLayer=False, normalize=False):
        ...
        if (self.lpips):
            ...
            if (self.spatial):
                res = [
                    upsample(diffs[kk].sum(axis=1, keepdim=True),
                             out_HW=in0.shape[2:]) for kk in range(self.L)
                ]
            else:
                res = [
                    spatial_average(diffs[kk].sum(axis=1, keepdim=True),
...
class LPIPSWithDiscriminator(nn.Layer):
    ...
    def calculate_adaptive_weight(self, nll_loss, g_loss, last_layer=None):
        if last_layer is not None:
            nll_grads = paddle.autograd.grad(nll_loss,
                                             last_layer,
                                             retain_graph=True)[0]
            g_grads = paddle.autograd.grad(g_loss,
                                           last_layer,
                                           retain_graph=True)[0]
        else:
            nll_grads = paddle.autograd.grad(nll_loss,
        ...
        if optimizer_idx == 0:
            ...
            if cond is None:
                ...
                logits_fake = self.discriminator(
                    paddle.concat((reconstructions, cond), axis=1))
        ...
        if optimizer_idx == 1:
            ...
            if cond is None:
                ...
                logits_real = self.discriminator(
                    paddle.concat((inputs.detach(), cond), axis=1))
                logits_fake = self.discriminator(
                    paddle.concat((reconstructions.detach(), cond), axis=1))
            disc_factor = adopt_weight(self.disc_factor,
                                       global_step,
                                       threshold=self.discriminator_iter_start)
