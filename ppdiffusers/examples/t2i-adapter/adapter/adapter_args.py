...
class ModelArguments:
    ...
    """
    adapter_config_file: Optional[str] = field(default="./config/openpose_adapter.json", metadata={"help": "adapter_config_file"})
    ...
    pretrained_adapter_name_or_path: str = field(
        default=None,
        metadata={"help": "The pretrained weight of adapter, which is used to facilitate loading the same initialization for training."},
    )
...
class DataArguments:
    ...
    data_format: str = field(
        default="default", metadata={"help": "The data format, must be 'default' or 'img2img'.  The img2img format directly provides control image."}
    )
...
class GenerateArguments:
    ...
    scheduler_type: str = field(
        default="ddim", metadata={"help": "Type of scheduler to use. Should be one of ['pndm', 'lms', 'ddim', 'euler-ancest']"}
    )
    device: str = field(
        default="gpu", metadata={"help": "device"}
    )
    batch_size: int = field(
        default=16, metadata={"help": "batch_size"}
    )
    num_inference_steps: int = field(
        default=50, metadata={"help": "num_inference_steps"}
    )
    save_path: str = field(
        default="output/adapter/", metadata={"help": "Path to the output file."}
    )
    guidance_scales: str = field(
        default_factory=lambda:[3, 4, 5, 6, 7, 8], metadata={"help": "guidance_scales list."}
    )
    height: int = field(
        default=256, metadata={"help": "height."}
    )
    width: int = field(
        default=256, metadata={"help": "width."}
    )