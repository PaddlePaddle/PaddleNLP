...
def process_data(line, filename, data_format):
    try:
        ...
        if resolution != "X":
            try:
                width, height = float(resolution.split("X")[0].replace("X", "")), float(resolution.split("X")[1].replace("X", ""))
            except:
                width, height = float(resolution.split("x")[0].replace("x", "")), float(resolution.split("x")[1].replace("x", ""))
        else:
        ...
        if "laion_aes" in text_id:
            if "AESTHETIC_SCORE" in score_json and score_json["AESTHETIC_SCORE"] and float(score_json["AESTHETIC_SCORE"]) < 6:
                return None
...
def parse_line(line, filename, data_format="default"):
    try:
        ...
        if res is not None:  
            ...
            image = Image.open(
                io.BytesIO(base64.b64decode(image_base64))).convert('RGB')
