...
from ppdiffusers import AltDiffusionPipeline, AutoencoderKL, PNDMScheduler, UNet2DConditionModel, DDIMScheduler
...
class AltDiffusionPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    ...
    def test_alt_diffusion_ddim(self):
        ...
        expected_slice = np.array([
            0.758880615234375, 0.8570979833602905, 0.4303036034107208,
            0.4664992094039917, 0.34632986783981323, 0.2825028896331787,
            0.4622049927711487, 0.4205690622329712, 0.5209154486656189
        ])
    ...
    def test_alt_diffusion_pndm(self):
        ...
        assert image.shape == (1, 128, 128, 3)
        expected_slice = np.array([
            0.8294100761413574, 0.93553626537323, 0.45852237939834595,
            0.4506404995918274, 0.3515189290046692, 0.2775656580924988,
            0.39653486013412476, 0.38815104961395264, 0.535513162612915
        ])
        assert np.abs(image_slice.flatten() - expected_slice).max() < 1e-2
        assert np.abs(image_from_tuple_slice.flatten() -
                      expected_slice).max() < 1e-2
...
class AltDiffusionPipelineIntegrationTests(unittest.TestCase):
    ...
    def test_alt_diffusion(self):
        ...
        expected_slice = np.array([
            0.8720703, 0.87109375, 0.87402344, 0.87109375, 0.8779297, 0.8925781,
            0.8823242, 0.8808594, 0.8613281
        ])
        assert np.abs(image_slice.flatten() - expected_slice).max() < 1e-2

    def test_alt_diffusion_fast_ddim(self):
        scheduler = DDIMScheduler.from_pretrained("BAAI/AltDiffusion",
                                                  subfolder="scheduler")
        ...
        assert image.shape == (1, 512, 512, 3)
        expected_slice = np.array([
            0.9267578, 0.9301758, 0.9013672, 0.9345703, 0.92578125, 0.94433594,
            0.9423828, 0.9423828, 0.9160156
        ])
        assert np.abs(image_slice.flatten() - expected_slice).max() < 1e-2
