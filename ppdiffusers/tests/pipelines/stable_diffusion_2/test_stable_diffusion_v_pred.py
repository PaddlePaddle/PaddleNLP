...
class StableDiffusion2VPredictionPipelineFastTests(PipelineTesterMixin,
                                                   unittest.TestCase):
    ...
    def test_stable_diffusion_v_pred_ddim(self):
        ...
        expected_slice = np.array([
            0.30307483673095703, 0.3711841404438019, 0.2971755266189575,
            0.21124267578125, 0.24938753247261047, 0.4050242006778717,
            0.22494885325431824, 0.21655383706092834, 0.42639416456222534
        ])
    ...
    def test_stable_diffusion_v_pred_k_euler(self):
        ...
        assert image.shape == (1, 64, 64, 3)
        expected_slice = np.array([
            0.28467634320259094, 0.3477485179901123, 0.2706109285354614,
            0.2010326385498047, 0.24505868554115295, 0.36301904916763306,
            0.3366130590438843, 0.32769495248794556, 0.44537732005119324
        ])
        assert np.abs(image_slice.flatten() - expected_slice).max() < 1e-2
        assert np.abs(image_from_tuple_slice.flatten() -
                      expected_slice).max() < 1e-2
...
class StableDiffusion2VPredictionPipelineIntegrationTests(unittest.TestCase):
    ...
    def test_stable_diffusion_v_pred_default(self):
        ...
        expected_slice = np.array([
            0.0567, 0.057, 0.0416, 0.0463, 0.0433, 0.06, 0.0517, 0.0526, 0.0866
        ])
        assert np.abs(image_slice.flatten() - expected_slice).max() < 1e-2
    ...
    def test_stable_diffusion_v_pred_euler(self):
        ...
        assert image.shape == (1, 768, 768, 3)
        expected_slice = np.array([
            0.0351, 0.0376, 0.0505, 0.0424, 0.0551, 0.0656, 0.0471, 0.0276,
            0.0596
        ])
        assert np.abs(image_slice.flatten() - expected_slice).max() < 1e-2
    ...
    def test_stable_diffusion_v_pred_dpm(self):
        ...
        assert image.shape == (1, 768, 768, 3)
        expected_slice = np.array([
            0.2049230933189392, 0.21153515577316284, 0.2323405146598816,
            0.24159285426139832, 0.2559834122657776, 0.2484399378299713,
            0.25171154737472534, 0.2358025312423706, 0.23604032397270203
        ])
        assert np.abs(image_slice.flatten() - expected_slice).max() < 1e-2
    ...
    def test_stable_diffusion_text2img_intermediate_state_v_pred(self):
        ...
        def test_callback_fn(step: int, timestep: int,
            ...
            if step == 0:
                ...
                expected_slice = np.array([
                    -0.2543, -1.2755, 0.4261, -0.9555, -1.173, -0.5892, 2.4159,
                    0.1554, -1.2098
                ])
                ...
                latents_slice = latents[0, -3:, -3:, -1]
                expected_slice = np.array([
                    -0.9572, -0.967, -0.6152, 0.0894, -0.699, -0.2344, 1.5465,
                    -0.0357, -0.1141
                ])
                assert np.abs(latents_slice.flatten() -
                              expected_slice).max() < 1e-2
