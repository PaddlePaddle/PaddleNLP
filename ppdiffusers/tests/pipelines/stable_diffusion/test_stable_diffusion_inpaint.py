...
from ppdiffusers.pipelines.stable_diffusion.pipeline_stable_diffusion_inpaint import prepare_mask_and_masked_image
...
class StableDiffusionInpaintPipelineIntegrationTests(unittest.TestCase):
    ...
    def test_stable_diffusion_inpaint_pipeline_pndm(self):
        ...
        pipe = StableDiffusionInpaintPipeline.from_pretrained(
            model_id, safety_checker=None, scheduler=pndm)
        pipe.set_progress_bar_config(disable=None)
        pipe.enable_attention_slicing()
...
class StableDiffusionInpaintingPrepareMaskAndMaskedImageTests(
    ...
    def test_np_inputs(self):
        ...
        t_mask_pil, t_masked_pil = prepare_mask_and_masked_image(
            im_pil, mask_pil)
    ...
    def test_shape_mismatch(self):
        ...
        with self.assertRaises(AssertionError):
            prepare_mask_and_masked_image(paddle.randn([3, 32, 32]),
                                          paddle.randn([64, 64]))
        # test batch dim
        with self.assertRaises(AssertionError):
            prepare_mask_and_masked_image(paddle.randn([2, 3, 32, 32]),
    ...
    def test_type_mismatch(self):
        ...
        with self.assertRaises(TypeError):
            prepare_mask_and_masked_image(paddle.rand([3, 32, 32]),
                                          paddle.rand([3, 32, 32]).numpy())
        # test tensors-only
        with self.assertRaises(TypeError):
            prepare_mask_and_masked_image(
                paddle.rand([3, 32, 32]).numpy(), paddle.rand([3, 32, 32]))

    def test_channels_first(self):
        # test channels first for 3D tensors
        with self.assertRaises(AssertionError):
            prepare_mask_and_masked_image(paddle.rand([32, 32, 3]),
                                          paddle.rand([3, 32, 32]))
    ...
    def test_tensor_range(self):
        ...
        with self.assertRaises(ValueError):
            prepare_mask_and_masked_image(
                paddle.ones([3, 32, 32]) * 2, paddle.rand([32, 32]))
        # test im >= -1
        with self.assertRaises(ValueError):
            prepare_mask_and_masked_image(
                paddle.ones([3, 32, 32]) * (-2), paddle.rand([32, 32]))
