...
class LDMPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    ...
    def test_inference_uncond(self):
        ...
        image_from_tuple = ldm(generator=generator,
                               num_inference_steps=2,
                               output_type="numpy",
                               return_dict=False)[0]
        ...
        expected_slice = np.array([
            0.8270485401153564, 1.0, 0.6244686841964722, 0.772940456867218, 1.0,
            0.7307173609733582, 0.6108742356300354, 0.9107264280319214,
            0.7249622344970703
        ])
...
class LDMPipelineIntegrationTests(unittest.TestCase):
    ...
    def test_inference_uncond(self):
        ...
        expected_slice = np.array([
            0.5980286598205566, 0.6169854402542114, 0.6275357007980347,
            0.6128235459327698, 0.6096121668815613, 0.6172620058059692,
            0.6060792207717896, 0.6026193499565125, 0.6129077672958374
        ])
        assert np.abs(image_slice.flatten() - expected_slice).max() < 1e-2
