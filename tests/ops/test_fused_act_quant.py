        ...
        for height in [256, 1026, 4098]:
            ...
            for transposing in [True,False]:
                for padding in [True]:
                    ...
                    print("-" * 20 + f"Testing swiglu with {pad_tag} and transposing: {transposing}" + "-" * 20)
                    fused_res, fused_scales = FQO.fused_act_quant(x,transpose_output=transposing,padding_last_dim_to_8x=padding,using_pow2_scaling=False)
                    np_results=[]
                    golden_res = x if not transposing else x.T
                    ...
                    if padding:
                        if transposing:
                            ...
                            dequanted_sliced_result = dequantize_fp8_to_bf16(fused_res, fused_scales)
                            np_results.append(dequanted_sliced_result[:, :width].numpy())
                    else:
