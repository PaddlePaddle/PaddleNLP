...
from test_quant import gen_ref_quant_result
...
def dequantize_fp8_to_bf16(fp8_tensor: paddle.Tensor, scale: paddle.Tensor) -> paddle.Tensor:
...
def compare_tensors(a, b):
    ...
    rel_diff = np.divide(abs_diff, denominator, out=np.zeros_like(abs_diff), where=(denominator != 0))
    ...
    print(
        f"a[{max_abs_idx}] = {a[max_abs_idx]:.6g}"
        + f"\t b[{max_abs_idx}] = {b[max_abs_idx]:.6g}"
        + f"\t 绝对差值: {max_abs_val:.6g}\n"
    )

    print("[最大相对差距]" f"位置: {max_rel_idx}")
    print(
        f"a[{max_rel_idx}] = {a[max_rel_idx]:.6g}"
        + f"\t b[{max_rel_idx}] = {b[max_rel_idx]:.6g}"
        + f"\t 相对差值: {max_rel_val:.6g}\n"
    )
    print("周围元素比较-a:")
        ...
        for height in [256, 1026, 4098]:
            ...
            for transposing in [True, False]:
                for optional_y in [y, None]:
                    ...
                    print("-" * 20 + f"Testing swiglu with {y_tag} and transposing: {transposing}" + "-" * 20)
                    fused_res, fused_scales = FQO.fused_swiglu_act_quant(
                        x, optional_y, transpose_output=transposing, to_e4m3=True, using_pow2_scaling=False
                    )
                    np_results = []
                    ...
                    try:
                        np.testing.assert_allclose(
                            np_results[0], np_results[1], rtol=0.01, atol=1
                        )  # 存在截断误差，atol=1，通常在1e-6
                        print("+++++++ Passed ++++++++")
