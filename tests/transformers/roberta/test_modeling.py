...
class RobertaCompatibilityTest(unittest.TestCase):
    ...
    def test_bert_converter(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            paddle_model = RobertaModel.from_pretrained(
                "hf-internal-testing/tiny-random-RobertaModel", from_hf_hub=True, cache_dir=tempdir
            )
            paddle_model.eval()
            ...
            torch_model = RobertaModel.from_pretrained(
                "hf-internal-testing/tiny-random-RobertaModel", cache_dir=tempdir
            )
            torch_model.eval()
    ...
    def test_bert_converter_from_local_dir_with_enable_torch(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            torch_model = RobertaModel.from_pretrained("hf-internal-testing/tiny-random-RobertaModel")
    ...
    def test_bert_converter_from_local_dir(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            paddle_model = RobertaModel.from_pretrained(tempdir)
            paddle_model.eval()
            paddle_logit = paddle_model(paddle.to_tensor(input_ids))[0]
