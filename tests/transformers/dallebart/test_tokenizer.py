...
class TestTokenizationDalleBart(TokenizerTesterMixin, unittest.TestCase):
    ...
    #                             output = tokenizer(seq_2, seq_1, padding=padding_state, truncation=truncation_state,max_length = model_max_length)
    #                             self.assertEqual(len(output["input_ids"]), model_max_length)

    #                             output = tokenizer(
    #                                 [seq_2], [seq_1], padding=padding_state, truncation=truncation_state,max_length = model_max_length
    #                             )
    #                             self.assertEqual(len(output["input_ids"][0]), model_max_length)

    #                     # Simple
    #                     output = tokenizer(seq_1, seq_2, padding=padding_state, truncation="only_second",max_length = model_max_length)
    #                     self.assertEqual(len(output["input_ids"]), model_max_length)
