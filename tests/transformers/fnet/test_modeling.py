...
class FNetModelTester:
    def __init__(
        ...
        self.parent: FNetModelTest = parent
    ...
    def create_and_check_model(
        self, config: FNetConfig, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels
    ):
    ...
    def create_and_check_for_question_answering(
        ...
        model = FNetForQuestionAnswering(config)
        model.eval()
        result = model(
            input_ids,
            attention_mask=input_mask,
    ...
    def create_and_check_for_token_classification(
        ...
        model = FNetForTokenClassification(config)

        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
        self.parent.assertEqual(result[1].shape, [self.batch_size, self.seq_length, self.num_labels])

    def test_addition_params(self, config: FNetConfig, *args, **kwargs):
        config.num_labels = 7
...
class FNetModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(self, config_class=FNetConfig, vocab_size=256, hidden_size=24)
    ...
    def test_params_compatibility_of_init_method(self):
        ...
        model: FNetForTokenClassification = FNetForTokenClassification.from_pretrained(
            "fnet-base", num_classes=4, dropout=0.3
        )
        assert model.num_labels == 4
        assert model.dropout.p == 0.3


class FNetCompatibilityTest(unittest.TestCase):
    test_model_id = "hf-internal-testing/tiny-random-FNetModel"
    ...
    def setUpClass(cls) -> None:
        ...
        model = FNetModel.from_pretrained(cls.test_model_id)
        model.save_pretrained(cls.torch_model_path)

    def test_model_config_mapping(self):
        config = FNetConfig(num_labels=22, hidden_dropout_prob=0.99)
    ...
    def run_token_for_classification(self, version: str):
        ...
        old_model = FNetModel.from_pretrained("fnet-base")
        old_model_path = os.path.join(tempdir, "old-model")
        old_model.save_pretrained(old_model_path)

        old_model_for_token = FNetForTokenClassification.from_pretrained(
            "fnet-base-uncased", num_classes=4, dropout=0.3
        )
    ...
    def test_fnet_converter(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            from paddlenlp.transformers import FNetModel

            paddle_model = FNetModel.from_pretrained(
                "hf-internal-testing/tiny-random-FNetModel", from_hf_hub=True, cache_dir=tempdir
            )
            paddle_model.eval()
            ...
            torch_model = FNetModel.from_pretrained("hf-internal-testing/tiny-random-FNetModel", cache_dir=tempdir)
            torch_model.eval()
    ...
    def test_fnet_converter_from_local_dir_with_enable_torch(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            torch_model = FNetModel.from_pretrained("hf-internal-testing/tiny-random-FNetModel")
            torch_model.save_pretrained(tempdir)

            # 2. forward the paddle model
            from paddlenlp.transformers import FNetModel, model_utils

            model_utils.ENABLE_TORCH_CHECKPOINT = False

            with self.assertRaises(ValueError) as error:
                FNetModel.from_pretrained(tempdir)
    ...
    def test_fnet_converter_from_local_dir(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            paddle_model = FNetModel.from_pretrained(tempdir)
            paddle_model.eval()
            paddle_logit = paddle_model(paddle.to_tensor(input_ids))[0]
    ...
    @parameterized.expand(
        [
            ("FNetModel",),
            # ("FNetForMaskedLM",),   TODO: need to tie weights
            # ("FNetForPretraining", "FNetForPreTraining"),   TODO: need to tie weights
            ("FNetForMultipleChoice",),
            ("FNetForQuestionAnswering",),
            ("FNetForSequenceClassification",),
            ("FNetForTokenClassification",),
