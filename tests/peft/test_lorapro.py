...
class TestLoRAProLayer(unittest.TestCase):
    ...
    def test_forward(self):
        lora_layer = LoRALinear(in_features=16, out_features=8, r=4, lora_dropout=0.1, lora_alpha=8, lorapro=True)
    ...
    def test_train_eval(self):
        x = paddle.randn([2, 4, 16], "float32")
        lora_layer = LoRALinear(in_features=16, out_features=8, r=4, lorapro=True)
        lora_layer.train()
    ...
    def test_save_load(self):
        with TemporaryDirectory() as tempdir:
            ...
            new_lora_layer = LoRALinear(in_features=16, out_features=8, r=4, lorapro=True)
    ...
    def test_load_regular_linear(self):
        with TemporaryDirectory() as tempdir:
            ...
            lora_layer_r8 = LoRALinear(in_features=16, out_features=8, r=8, lorapro=True)
            lora_layer_r4 = LoRALinear(in_features=16, out_features=8, r=4, lorapro=True)
...
class TestLoRAProModel(unittest.TestCase):
    ...
    def test_lora_model_save_load(self):
        with TemporaryDirectory() as tempdir:
            input_ids = paddle.to_tensor(np.random.randint(100, 200, [1, 20]))
            lora_config = LoRAConfig(target_modules=[".*q_proj.*", ".*v_proj.*"], r=4, lora_alpha=8, lorapro=True)
            model = AutoModel.from_pretrained("__internal_testing__/tiny-random-bert")
            lora_model = LoRAModel(model, lora_config)
            lora_model.eval()
    ...
    def test_lora_module_raise_exception(self):
        lora_config = LoRAConfig(target_modules=[".*norm1.*"], r=4, lora_alpha=8, enable_lora_list=None, lorapro=True)
        model = AutoModel.from_pretrained("__internal_testing__/tiny-random-bert")
        with self.assertRaises(ValueError):
