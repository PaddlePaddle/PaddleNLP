...
class LogitsProcessorTest(unittest.TestCase):
    ...
    def test_temperature_dist_warper(self):
        ...
        processed_scores = temp_dist_warper_smoother(input_ids, scores.clone())
        ...
        self.assertFalse(paddle.all(scores == processed_scores))
    ...
    def test_top_p_dist_warper(self):
        ...
        self.assertFalse(paddle.all(top_p_warp(input_ids, dist) == dist))
    ...
    def test_min_p_dist_warper(self):
        ...
        filtered_dist = paddle.exp(min_p_warp(input_ids, dist))
    ...
    def test_typical_dist_warper(self):
        ...
        dist = paddle.log(paddle.to_tensor([[0.97, 0.01, 0.01, 0.01], [0.4, 0.2, 0.2, 0.2]]))

        typical_warp = TypicalLogitsWarper(0.5)
        filtered_dist = paddle.exp(typical_warp(input_ids, dist))

        # dist should be filtered to keep min num values so that sum is >= 0.7
        # exp (-inf) => 0
        EXPECTED_FILTERED_DIST = paddle.to_tensor([[0.97, 0.0, 0.0, 0.0], [0.0, 0.2, 0.2, 0.2]])
        self.assertTrue(paddle.allclose(filtered_dist, EXPECTED_FILTERED_DIST, atol=1e-3))

        # processor should not change logits in-place
        self.assertFalse(paddle.all(typical_warp(input_ids, dist) == dist))
    ...
    def test_epsilon_dist_warper(self):
        ...
        # create distribution and take log (inverse to Softmax as taken in TopPLogitsWarper)
        dist = paddle.log(paddle.to_tensor([[0.87, 0.099, 0.001, 0.03], [0.4, 0.299, 0.101, 0.2]]))

        epsilon_warp = EpsilonLogitsWarper(0.1)
        filtered_dist = paddle.exp(epsilon_warp(input_ids, dist))

        # dist should be filtered to only keep values with proba >= 0.1
        # exp (-inf) => 0
        EXPECTED_FILTERED_DIST = paddle.to_tensor([[0.87, 0, 0, 0], [0.4, 0.299, 0.101, 0.2]])
        self.assertTrue(paddle.allclose(filtered_dist, EXPECTED_FILTERED_DIST, atol=1e-3))
    ...
    def test_eta_dist_warper(self):
        ...
        # create distribution and take log (inverse to Softmax as taken in TopPLogitsWarper)
        dist = paddle.log(paddle.to_tensor([[0.0, 0.1, 0.8, 0.1], [0.01, 0.04, 0.9, 0.05]]))

        eta_warp = EtaLogitsWarper(0.0625)
        filtered_dist = paddle.exp(eta_warp(input_ids, dist))
        ...
        # exp (-inf) => 0
        EXPECTED_FILTERED_DIST = paddle.to_tensor([[0.0, 0.1, 0.8, 0.1], [0.0, 0.0, 0.9, 0.0]])
        self.assertTrue(paddle.allclose(filtered_dist, EXPECTED_FILTERED_DIST, atol=1e-3))
    ...
    def test_no_repeat_ngram_dist_processor(self):
        ...
        self.assertFalse(paddle.all(scores == filtered_scores_2_gram))
        self.assertFalse(paddle.all(scores == filtered_scores_3_gram))
    ...
    def test_encoder_no_repeat_ngram_dist_processor(self):
        ...
        encoder_input_ids = paddle.to_tensor([1, 2, 1, 1])

        input_ids = paddle.to_tensor([[1, 2, 1], [8, 0, 2]])
        ...
        num_beams = 2
        batch_size = 2
        encoder_input_ids = paddle.to_tensor([[1, 2, 1, 1], [0, 0, 2, 1]])

        input_ids = paddle.to_tensor([[1, 2, 1], [1, 0, 2], [0, 0, 0], [0, 2, 2]])
        scores = self._get_uniform_logits(batch_size * num_beams, vocab_size)
    ...
    def test_no_bad_words_dist_processor(self):
        ...
        self.assertListEqual(
            (filtered_scores == paddle.finfo(scores.dtype).min).tolist(),
            [[True, True, False, True, False], [True, True, True, False, False]],
        )

        # processor should not change logits in-place
        self.assertFalse(paddle.all(scores == filtered_scores))

        # check edge case
        no_bad_words_dist_proc = NoBadWordsLogitsProcessor(bad_words_ids=[[4]], eos_token_id=eos_token_id)
        filtered_scores = no_bad_words_dist_proc(input_ids, scores)
        self.assertTrue(paddle.allclose(scores, filtered_scores, atol=1e-3))
    ...
    def test_forced_bos_token_logits_processor(self):
        ...
        self.assertTrue((processed_scores[:, bos_token_id + 1 :] == paddle.finfo(scores.dtype).min).all())
        self.assertListEqual(
    ...
    def test_forced_eos_token_logits_processor(self):
        ...
        scores = self._get_uniform_logits(batch_size, vocab_size)
        processed_scores = logits_processor(input_ids, scores)
        self.assertFalse((processed_scores == paddle.finfo(scores.dtype).min).any())

    def test_remove_nan_inf_logits_processor(self):
        scores = paddle.to_tensor([[0.0, 0.7, 0.8, float("nan")], [0.1, float("inf"), 0.3, float("-inf")]])
        ...
        self.assertTrue(
            paddle.allclose(
                ...
                paddle.to_tensor(
                    [
                        [0.0, 0.7, 0.8, 0.0],
                        [0.1, paddle.finfo(processed_scores.dtype).max, 0.3, paddle.finfo(processed_scores.dtype).min],
                    ],
                ),
                atol=1e-6,
            )
    ...
    def test_exponential_decay_length_penalty(self):
        ...
        self.assertTrue(paddle.greater_than(scores_after_start[:, eos_token_id], scores[:, eos_token_id]).all())

        # check the penalty increases negative scores
        input_ids = ids_tensor((batch_size, 20), vocab_size=vocab_size)
        scores = paddle.neg(self._get_uniform_logits(batch_size, vocab_size))
        scores_after_start = length_decay_processor(input_ids, scores)
        self.assertTrue(paddle.greater_than(scores_after_start[:, eos_token_id], scores[:, eos_token_id]).all())

        # processor should not change logits in-place
        self.assertFalse(paddle.all(scores == scores_after_start))

    def test_normalization(self):
        input_ids = None

        scores = paddle.to_tensor([[-23.18, -29.96, -43.54, 47.77], [-33.58, -26.87, -32.96, 22.51]])

        logit_normalization = LogitNormalization()
        normalized_scores = logit_normalization(input_ids, scores).exp()

        ones = paddle.ones(scores.shape[0])
        self.assertTrue(normalized_scores.sum(axis=-1).allclose(ones))

        self.assertTrue(normalized_scores.allclose(nn.functional.softmax(scores, axis=-1)))

        # processor should not change logits in-place
        self.assertFalse(paddle.all(scores == normalized_scores))

    def test_classifier_free_guidance(self):
        class Namespace(dict):
            pass

        logits_uncond = paddle.to_tensor([[[1.0, 0, 1.5]]])
        logits_cond = paddle.to_tensor([[[1.0, 1.0, 1.0]]])
    ...
    def test_synthidtext_watermarking_processor_bias_uniformity(self, ngram_len, num_layers, vocab_size):
        ...
        g_values_mean = paddle.mean(paddle.mean(g_values.float(), axis=0))
    ...
    def test_synthidtext_watermark_processor_bias_uniformity_across_vocab(self, vocab_size, num_layers):
        ...
        ngram_keys, _ = logits_processor._compute_keys(
            n_minus_1_grams,
            paddle.stack([paddle.arange(vocab_size) for _ in range(batch_size)]),
        )

        g_values = logits_processor.sample_g_values(ngram_keys)
        # g_values shape should be [batch_size, vocab_size, num_layers]
        g_values_mean = paddle.mean(paddle.mean(g_values.float(), axis=1))
    ...
    def test_synthidtext_watermark_processor_distributional_convergence(self, vocab_size, logits_type):
        ...
        is_close = paddle.all(
            paddle.isclose(
                paddle.to_tensor(updated_softmaxes),
                paddle.nn.functional.softmax()(fixed_logits[0]),  # Take any batch entry, all are same.
                atol=1e-3,
                rtol=0,
            )
        )
