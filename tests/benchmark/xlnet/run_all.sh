...
for model_mode in ${model_mode_list[@]}; do
    for fp_item in ${fp_item_list[@]}; do
          for bs_item in ${bs_item_list[@]}; do
              ...
              run_mode=sp
              CUDA_VISIBLE_DEVICES=0 bash tests/benchmark/xlnet/run_benchmark.sh ${run_mode} ${bs_item} ${fp_item} 1000 ${model_mode}     #  (5min)
              sleep 60
              echo "index is speed, 8gpus, run_mode is multi_process, begin, ${model_name}"
              run_mode=mp
              CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash tests/benchmark/xlnet/run_benchmark.sh ${run_mode} ${bs_item} ${fp_item} 1000 ${model_mode}
              sleep 60
          done
    done
done
