...
class TokenizerTesterMixin:
    ...
    def setUp(self) -> None:
        ...
        tokenizers_list = [(
            ...
            if self.from_pretrained_kwargs is not None else {}, )
                           ...
                           (self.from_pretrained_filter is not None and
                            self.from_pretrained_filter(pretrained_name))]
        self.tokenizers_list = tokenizers_list[:1]

        with open("./sample_text.txt", encoding="utf-8") as f_data:
    ...
    def get_clean_sequence(self,
        ...
        toks = [(i, tokenizer.decode(
            [i], clean_up_tokenization_spaces=False))
                for i in range(len(tokenizer))]
        toks = list(filter(lambda t: re.match(r"^[ a-zA-Z]+$", t[1]), toks))
        toks = list(filter(lambda t: [t[0]] == tokenizer.encode(t[1], return_token_type_ids=None,add_special_tokens=False)['input_ids'], toks))
        ...
        output_ids = tokenizer.encode(
            output_txt, return_token_type_ids=None,
    ...
    def assert_padded_input_match(self,
                                  input_r: list,
                                  input_p: list,
                                  max_length: int,
                                  pad_token_id: int):
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                ...
                added = tokenizer.add_tokens([
                    AddedToken(
                        tok, lstrip=True, rstrip=True) for tok in new_toks
                ])
    ...
    def test_encode_decode_with_spaces(self):
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                ...
                new_toks = [
                    AddedToken(
                        "[ABC]", normalized=False),
                    AddedToken(
                        "[DEF]", normalized=False),
                    AddedToken(
                        "GHI IHG", normalized=False),
                ]
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                ...
                if (tokenizer.build_inputs_with_special_tokens.__qualname__.
                        split(".")[0] != "PretrainedTokenizer" and
                        "token_type_ids" in tokenizer.model_input_names):
    ...
    def test_maximum_encoding_length_single_input(self):
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                ...
                sequence1 = tokenizer(
                    seq_1, return_token_type_ids=None, add_special_tokens=False)
                ...
                padding_strategies = ([False, True, "longest"]
                                      ...
                                      tokenizer.pad_token_id >= 0 else [False])
    ...
    def test_special_tokens_mask_input_pairs(self):
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                ...
                filtered_sequence = [(x if not special_tokens_mask[i] else None)
                                     for i, x in
                                     enumerate(encoded_sequence_w_special)]
                filtered_sequence = [
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                ...
                if "token_type_ids" in tokenizer.model_input_names:
                    ...
                    self.assertEqual(token_type_ids + [token_type_padding_idx] *
            ...
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                ...
                encoded_sequences_padded = [
                    tokenizer.encode(
                        ...
                        padding="max_length") for sequence in sequences
                ]
    ...
    def test_add_tokens(self):
        for tokenizer, pretrained_name, kwargs in self.tokenizers_list:
            with self.subTest(
                ...
                self.assertRaises(
                    AssertionError, tokenizer.add_special_tokens,
    ...
    def test_special_tokens_initialization_with_non_empty_additional_special_tokens(
        ...
        for tokenizer_class, tokenizer_utils in tokenizer_list:
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                new_added_tokens = [
                    AddedToken(
                        "a_new_additional_special_token", lstrip=True)
                ]
