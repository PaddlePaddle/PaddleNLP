...
class PromptModelTestForGeneration(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.tokenizer = AutoTokenizer.from_pretrained("__internal_testing__/tiny-random-gpt")
        cls.tokenizer.pad_token = "<pad>"
        cls.tokenizer.sep_token = "<sep>"
        cls.tokenizer.add_tokens("[Space]", special_tokens=True)
        cls.model = GPTLMHeadModel.from_pretrained("__internal_testing__/tiny-random-gpt")
        cls.num_labels = 2

        cls.template = AutoTemplate.create_from(
            prompt="{'prefix':'文本摘要'}{'text':'text'}{'sep'}{'text':'labels', 'token_type': 1}",
        ...
        cls.data_collator = PromptDataCollatorWithPadding(cls.tokenizer, padding=True, return_tensors="pd")
        cls.prompt_model = PromptModelForGeneration(cls.model, cls.template)
