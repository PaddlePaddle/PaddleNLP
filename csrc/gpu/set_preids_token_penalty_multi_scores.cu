...
__global__ void set_preids_token_penalty_multi_scores_kernel(const bool *stop_flags,
    ...
    const int seq_len_now = ori_seq_lens_encoder[bi];
...
void set_preids_token_penalty_multi_scores(const paddle::Tensor& pre_ids,
                                                  ...
                                                  const paddle::optional<paddle::Tensor>& first_token_ids,
                                                  const paddle::optional<paddle::Tensor>& ori_seq_lens_encoder) {
    ...
    if (first_token_ids && ori_seq_lens_encoder) {
        set_preids_token_penalty_multi_scores_kernel<DataType_><<<bs, 1024, 0, cu_stream>>>(
            ...
            reinterpret_cast<const int64_t*>(first_token_ids.get().data<int64_t>()),
            reinterpret_cast<const int*>(ori_seq_lens_encoder.get().data<int>()),
...
PD_BUILD_OP(set_preids_token_penalty_multi_scores)
    .Inputs({"pre_ids", 
             ...
             "min_len", 
             "eos_token_id",
             paddle::Optional("first_token_ids"),
             paddle::Optional("ori_seq_lens_encoder")})
