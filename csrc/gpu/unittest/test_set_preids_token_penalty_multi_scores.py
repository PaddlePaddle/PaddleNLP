...
class SetPreidsTokenPenaltyMultiScores(unittest.TestCase):
    def test_set_preids_token_penalty_multi_scores_operations(self):
        pre_ids = paddle.to_tensor([[1, 9, 3, 4, 5, 6, 7, 256*1024+1, 256*1024+1, 256*1024+1, 256*1024+3, 256*1024+3, 256*1024+3, 256*1024+3, 256*1024+3, 256*1024+3, -1, -1, -1], [1, 9, 7, 6, 5, 256*1024+1, 256*1024+1, 256*1024+1, 256*1024+3, 256*1024+3, 256*1024+3, 256*1024+3, 256*1024+3, 256*1024+3, 4, -1, -1, -1, -1]], "int64")
        pre_ids_2 = pre_ids.clone()
        input_ids = paddle.to_tensor(
            [[1, 9, 3, 4, 5, 6, 7, 256*1024+1, 256*1024+1, 256*1024+1, 256*1024+3, 256*1024+3, 256*1024+3, 256*1024+3, 256*1024+3, 256*1024+3, -1, -1, -1, -1, -1, -1], [1, 9, 7, 6, 5, 256*1024+1, 256*1024+1, 256*1024+1, 256*1024+3, 256*1024+3, 256*1024+3, 256*1024+3, 256*1024+3, 256*1024+3, 4, -1, -1, -1, -1, -1, -1, -1]], "int64"
        )
        ...
        paddle.seed(2023)
        np.random.seed(2023)
        logits = paddle.to_tensor(np.random.normal(size=(1, 256*1024+4)), dtype='float32')
        # logits = paddle.randn([4, 256*1024], "float32")
        print(logits)
        ...
        temperatures = paddle.to_tensor([0.6, 0.6], "float32")
