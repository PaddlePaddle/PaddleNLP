...
// encoder stage input: seq_len_encoder, none, max_len_this_time, bsz,
// FLAGS_flag_block_shape_q
std::vector<paddle::Tensor> SplitKVBlock(
  ...
  int max_len_this_time_data = max_len_this_time.data<int>()[0];
  int bsz = com_offsets.shape()[0];
  if (max_len_this_time_data <= 0) {
    auto batch_ids =
  ...
  const uint32_t max_tile_size_per_bs =
      div_up(max_len_this_time_data, num_row_per_block);
  auto batch_ids = paddle::empty({bsz * max_tile_size_per_bs},
                                 paddle::DataType::INT32,
                                 paddle::GPUPlace());
  auto tile_ids_per_batch = paddle::empty({bsz * max_tile_size_per_bs},
                                          paddle::DataType::INT32,
                                          paddle::GPUPlace());
  auto num_blocks =
      paddle::empty({1}, paddle::DataType::INT32, paddle::GPUPlace());

  split_kv_block<<<1, 32, 0, stream>>>(sequence_lengths_stage.data<int>(),
...
std::vector<paddle::DataType> SplitKVBlockInferDtype(
    const paddle::DataType& sequence_lengths_stage_dtype,
    const paddle::DataType& sequence_lengths_remove_dtype,
...
std::vector<std::vector<int64_t>> SplitKVBlockInferShape(
    const std::vector<int64_t>& sequence_lengths_stage_shape,
    const std::vector<int64_t>& sequence_lengths_remove_shape,
