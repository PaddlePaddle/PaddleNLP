...
__global__ void KeMatrixTopPBeamTopKFt(const T* src,
  ...
  if (tid == 0) {
    ...
    for(int i = 0; i < TopPBeamTopK; i++) {
      out_id[bid * max_cadidate_len + i] = static_cast<int64_t>(beam_max[i].id);
      out_val[bid * max_cadidate_len + i] = beam_max[i].v;
      float val = static_cast<float>(beam_max[i].v);
      sum_prob += val;
      printf("top %d sum_prob %f, token %lld \n", i, sum_prob, static_cast<int64_t>(beam_max[i].id));

      if(sum_prob >= top_p_value) {
        actual_candidates_lens[bid] = i + 1;
        break;
      }
    }
  }
...
std::vector<paddle::Tensor> LaunchTopPCandidates(const paddle::Tensor& probs,
    ...
    auto verify_scores = paddle::full({token_num, max_candidates_len}, 0, D, probs.place());
    auto verify_tokens = paddle::full({token_num, max_candidates_len}, 0, D, probs.place());
    auto actual_candidate_lens = paddle::full({token_num}, 0, D, probs.place());
...
std::vector<paddle::Tensor> DispatchTopPCandidatesWithDtype(const paddle::Tensor& probs,
    ...
    switch (probs.type()) {
        case paddle::DataType::BFLOAT16: 
            return LaunchTopPCandidates<paddle::DataType::BFLOAT16>(probs, top_p, max_candidates_len);
            break;
        case paddle::DataType::FLOAT16: 
            return LaunchTopPCandidates<paddle::DataType::FLOAT16>(probs, top_p, max_candidates_len);
            break;
        case paddle::DataType::FLOAT32: 
            return LaunchTopPCandidates<paddle::DataType::FLOAT32>(probs, top_p, max_candidates_len);
            break;
        default:
            PD_THROW(
...
std::vector<paddle::Tensor> TopPCandidates(const paddle::Tensor& probs,
    ...
    return DispatchTopPCandidatesWithDtype(probs, top_p, max_candidates_len);
}

std::vector<std::vector<int64_t>> TopPCandidatesInferShape(const std::vector<int64_t>& probs_shape, const std::vector<int64_t>& top_p_shape, int max_candidates_len) {
    int token_num = probs_shape[0];
    return {{token_num, max_candidates_len}, {token_num, max_candidates_len}, {token_num}};
}

std::vector<paddle::DataType> TopPCandidatesInferDtype(const paddle::DataType& probs_dtype, const paddle::DataType& top_p_dtype) {
    return {probs_dtype, paddle::DataType::INT64, paddle::DataType::INT32};
}

PD_BUILD_OP(topp_candidates)
    .Inputs({"probs","top_p"})
    .Outputs({"verify_scores", "verify_tokens", "actual_candidate_lens"})
    .Attrs({"max_candidates_len: int"})
