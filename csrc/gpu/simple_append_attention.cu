...
std::vector<paddle::Tensor> SimpleAppendAttentionKernel(
...
std::vector<paddle::Tensor> SimpleAppendAttention(
    const paddle::Tensor& qkv,
    const paddle::Tensor& key_cache,
    const paddle::Tensor& value_cache,
    const paddle::Tensor& seq_lens_encoder,
  ...
  switch (qkv.dtype()) {
    case paddle::DataType::FLOAT16: {
      return SimpleAppendAttentionKernel<paddle::DataType::FLOAT16>(
          meta_data,
          qkv,
          key_cache,
          value_cache,
    ...
    case paddle::DataType::BFLOAT16: {
      return SimpleAppendAttentionKernel<paddle::DataType::BFLOAT16>(
          meta_data,
          qkv,
          key_cache,
          value_cache,
...
std::vector<std::vector<int64_t>> SimpleAppendAttentionInferShape(
...
std::vector<paddle::DataType> SimpleAppendAttentionInferDtype(
...
PD_BUILD_OP(simple_append_attention)
    .Inputs({"qkv",
             "key_cache",
             "value_cache",
             "seq_lens_encoder",
    ...
    .SetKernelFn(PD_KERNEL(SimpleAppendAttention))
    .SetInferShapeFn(PD_INFER_SHAPE(SimpleAppendAttentionInferShape))
    .SetInferDtypeFn(PD_INFER_DTYPE(SimpleAppendAttentionInferDtype));