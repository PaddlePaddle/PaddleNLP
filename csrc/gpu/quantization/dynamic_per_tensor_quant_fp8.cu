...
std::vector<paddle::Tensor> LaunchDynamicPerTensorQuantFp8Kernel(const paddle::Tensor& x) {
...
std::vector<paddle::Tensor> LaunchDynamicPerTensorQuantFp8(const paddle::Tensor& x) {
    return LaunchDynamicPerTensorQuantFp8Kernel<InType, paddle::DataType::FLOAT8_E4M3FN>(x);
}


std::vector<paddle::Tensor> DynamicPerTensorQuantFp8(const paddle::Tensor& x) {
    if(x.dtype() == paddle::DataType::FLOAT32){
        return LaunchDynamicPerTensorQuantFp8<paddle::DataType::FLOAT32>(x);
    }else if(x.dtype() == paddle::DataType::FLOAT16){
        return LaunchDynamicPerTensorQuantFp8<paddle::DataType::FLOAT16>(x);
    }else if(x.dtype() == paddle::DataType::BFLOAT16){
        return LaunchDynamicPerTensorQuantFp8<paddle::DataType::BFLOAT16>(x);
    }else{
        PD_THROW("Unsupported data type.");
    }
}

std::vector<std::vector<int64_t>> DynamicPerTensorQuantFp8InferShape(const std::vector<int64_t>& input_shape) {
    std::vector<int64_t> scale_shape = {1};
    return {input_shape, scale_shape};
}

std::vector<paddle::DataType> DynamicPerTensorQuantFp8InferDtype(const paddle::DataType& input_dtype) {
    return {paddle::DataType::FLOAT8_E4M3FN, paddle::DataType::FLOAT32};
}

PD_BUILD_OP(dynamic_per_tensor_quant_fp8)
    .Inputs({"x"})
    .Outputs({"output", "scale_out"})
    .SetKernelFn(PD_KERNEL(DynamicPerTensorQuantFp8))
    .SetInferShapeFn(PD_INFER_SHAPE(DynamicPerTensorQuantFp8InferShape))
    .SetInferDtypeFn(PD_INFER_DTYPE(DynamicPerTensorQuantFp8InferDtype));