...
std::vector<paddle::Tensor> top_p_sampling_from_probs(
  ...
  uniform_samples_input =
      paddle::experimental::cast(uniform_samples, paddle::DataType::FLOAT32);
  ...
  cudaError_t status = sampling::TopPSamplingFromProb<float, int>(
      probs_input.data<float>(),
      uniform_samples_input.data<float>(),
      samples.data<int>(),
      success.data<bool>(),
      nullptr,
      batch_size,
...
std::vector<std::vector<int64_t>> top_p_sampling_from_probs_InferShape(
...
std::vector<paddle::DataType> top_p_sampling_from_probs_InferDtype(
