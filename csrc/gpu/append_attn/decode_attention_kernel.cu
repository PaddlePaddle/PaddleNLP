...
template <bool partition_kv, typename T, typename OutT, typename CacheT, uint32_t NUM_STAGES, uint32_t TILE_SIZE_PER_Z, uint32_t DEAL_EACH_TIME, uint32_t GROUP_SIZE, uint32_t HEAD_DIM_QK, uint32_t HEAD_DIM_V, 
          uint32_t BLOCK_SIZE, uint32_t VEC_SIZE, uint32_t CACHE_VEC_SIZE, uint32_t bdx, uint32_t bdy, uint32_t bdz, uint32_t bdxc, PosEncMode pos_enc_mode, CacheType cache_type>
__global__ void multi_query_decode_attention_kernel(T * __restrict__ q, // [token_num, num_heads, head_dim]
  ...
  constexpr uint32_t TILE_SIZE_PER_BDX = num_vec_per_head_qk / bdxc; // 1 2 4
  ...
  T *q_smem;
    ...
    __syncthreads();
#endif
  q_smem = reinterpret_cast<T*>(smem);
#ifdef DEBUG_DEC_ATTN
    __syncthreads();
    if (threadIdx.x == 0 && threadIdx.y == 0 && threadIdx.z == 0 && blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
      printf("qk_tmp start\n");
    ...
    __syncthreads();
#endif
  T *qk_tmp = reinterpret_cast<T*>(smem + NUM_STAGES * DEAL_EACH_TIME * (HEAD_DIM_QK + HEAD_DIM_V) * sizeof(CacheT)
                                   + sizeof(T) * 2 + HEAD_DIM_QK * sizeof(T));
#ifdef DEBUG_DEC_ATTN
    __syncthreads();
    if (threadIdx.x == 0 && threadIdx.y == 0 && threadIdx.z == 0 && blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
      printf("qk_tmp end\n");
...
#pragma unroll
  ...
  T *cache_k_scale_smem = nullptr;
  T *cache_k_zero_point_smem = nullptr;
  T *cache_v_scale_smem = nullptr;
  T *cache_v_zero_point_smem = nullptr;
  T cache_k_scale_reg = 0;
  T cache_v_scale_reg = 0;
  __syncthreads();

  CacheT *k_smem = reinterpret_cast<CacheT*>(smem);
  CacheT *v_smem;
  T *md_smem;
#ifdef DEBUG_DEC_ATTN
  __syncthreads();
  ...
  __syncthreads();
#endif
  v_smem = k_smem + NUM_STAGES * DEAL_EACH_TIME * HEAD_DIM_QK;
#ifdef DEBUG_DEC_ATTN
  __syncthreads();
  if (threadIdx.x == 0 && threadIdx.y == 0 && threadIdx.z == 0 && blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
  ...
  __syncthreads();
#endif
  md_smem = reinterpret_cast<T*>(smem + NUM_STAGES * DEAL_EACH_TIME * (HEAD_DIM_QK + HEAD_DIM_V) * sizeof(CacheT));
#ifdef DEBUG_DEC_ATTN
  __syncthreads();
  if (threadIdx.x == 0 && threadIdx.y == 0 && threadIdx.z == 0 && blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
    printf("md_smem end\n");
...
#ifdef DEBUG_DEC_ATTN
  __syncthreads();
  printf("vid:%d, VEC_SIZE:%d, vid * VEC_SIZE:%d\n",int(vid), int(VEC_SIZE), int(vid * VEC_SIZE));
  __syncthreads();
#endif
  Load<T, VEC_SIZE>(q_smem + vid * VEC_SIZE, &q_vec);
#ifdef DEBUG_DEC_ATTN
  __syncthreads();
  printf("Load q_vec done\n");
  __syncthreads();
#endif
#pragma unroll
  for (int i = 0; i < VEC_SIZE; i++) {
  ...
  AlignedVector<T, VEC_SIZE> k_scale_vec;
  AlignedVector<T, VEC_SIZE> k_zp_vec;
  AlignedVector<T, VEC_SIZE> v_scale_vec;
  AlignedVector<T, VEC_SIZE> v_zp_vec;
  // Load<T, VEC_SIZE>(cache_k_scale_smem + gid * HEAD_DIM + vid * VEC_SIZE, &k_scale_vec);
  // Load<T, VEC_SIZE>(cache_k_zero_point_smem + gid * HEAD_DIM + vid * VEC_SIZE, &k_zp_vec);
  // Load<T, VEC_SIZE>(cache_v_scale_smem + gid * HEAD_DIM + vid * VEC_SIZE, &v_scale_vec);
  // Load<T, VEC_SIZE>(cache_v_zero_point_smem + gid * HEAD_DIM + vid * VEC_SIZE, &v_zp_vec);
  uint32_t stage_idx = 0;  
  constexpr int loop_times = TILE_SIZE_PER_Z / TILE_SIZE_PER_BDX;
#ifdef DEBUG_DEC_ATTN
  __syncthreads();
...
#pragma unroll
    for (int j = 0; j < loop_times; ++j) {
      const uint32_t k_seq_offset = i * DEAL_EACH_TIME + j * TILE_SIZE_PER_BDX + kv_bid;
      const uint32_t k_seq_id = chunk_start + k_seq_offset;
      produce_k<SharedMemFillMode::kNoFill, cache_type, HEAD_DIM_QK, HEAD_DIM_V, VEC_SIZE, HALF_VEC_SIZE, BLOCK_SIZE, bdxc, CACHE_VEC_SIZE>(
        k_smem,
        cache_k,
        block_table_now,
        k_seq_id,
...
#pragma unroll
      for (int j = 0; j < loop_times; ++j) {
        const uint32_t v_seq_offset = i * DEAL_EACH_TIME + j * TILE_SIZE_PER_BDX + kv_bid;
...
//   constexpr int cp_loop_times = TILE_SIZE_PER_Z / (bdy * TILE_SIZE_PER_BDX);
//   // cp bdxc each bdx
//   const uint32_t cp_const_offset = zid * TILE_SIZE_PER_Z + gid * TILE_SIZE_PER_BDX + kv_bid;
  ...
  for (int iter = 0; iter < num_iters; ++iter) {
    wait_group<2 * NUM_STAGES - 1>();
    __syncthreads();

    // compute qk
    compute_qk<VEC_SIZE, HALF_VEC_SIZE, bdx, bdy, bdz, TILE_SIZE_PER_Z, HEAD_DIM_QK, HALF_HEAD_DIM_QK, DEAL_EACH_TIME, pos_enc_mode, cache_type>(
      k_smem,
      q_vec,
      freq_vec,
      k_scale_vec,
...
#pragma unroll
    for (int j = 0; j < loop_times; ++j) {
      const uint32_t k_seq_offset = j * TILE_SIZE_PER_BDX + kv_bid;
      // const uint32_t k_seq_offset = cp_const_offset + j * bdy * TILE_SIZE_PER_BDX;
      produce_k<SharedMemFillMode::kNoFill, cache_type, HEAD_DIM_QK, HEAD_DIM_V, VEC_SIZE, HALF_VEC_SIZE, BLOCK_SIZE, bdxc, CACHE_VEC_SIZE>(
        k_smem,
        cache_k,
        block_table_now,
    ...
    __syncthreads();
    if (threadIdx.x == 0 && threadIdx.y == 0 && threadIdx.z == 0 && blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
      printf("produce_k 2 done\n");
    }
    __syncthreads();
#endif
    // compute sv
    if (kv_vid < num_vec_per_head_v) {
      compute_sv<VEC_SIZE, HALF_VEC_SIZE, bdz, TILE_SIZE_PER_Z, DEAL_EACH_TIME, HEAD_DIM_V, HALF_HEAD_DIM_V, cache_type>(
        s,
        v_smem,
        v_scale_vec,
        v_zp_vec,
...
#pragma unroll
      for (int j = 0; j < loop_times; ++j) {
        const uint32_t v_seq_offset = j * TILE_SIZE_PER_BDX + kv_bid;;
        // const uint32_t v_seq_offset = cp_const_offset + j * bdy * TILE_SIZE_PER_BDX;
        produce_v<SharedMemFillMode::kFillZero, cache_type, HEAD_DIM_QK, HEAD_DIM_V, VEC_SIZE, HALF_VEC_SIZE, BLOCK_SIZE, bdxc, CACHE_VEC_SIZE>(
          v_smem,
          cache_v,
          block_table_now,
...
void MultiQueryDecoderAttention(
  ...
  std::cout << "bsz: " << bsz << ", token_num: " << token_num << ", num_heads: " << num_heads << ", kv_num_heads: " << kv_num_heads << ", group_size: " << GROUP_SIZE;
  ...
  constexpr int tile_size_per_bdx = num_vec_per_head / blockxc;
  // static_assert(blockx <= 32);
  
  constexpr int num_threads = NUM_THREADS > blockx * blocky ? NUM_THREADS : blockx * blocky;
  constexpr int blockz = num_threads / (blockx * blocky);
  constexpr int deal_kv_each_time = DEAL_EACH_TIME * tile_size_per_bdx; // 每次处理64长度的kv cache
  constexpr int tile_size_per_z = deal_kv_each_time;
  // std::cout << "blockx: " << blockx << ", blocky: " << blocky << ", blockz: " << blockz << ", blockxc: " << blockxc << ", deal_kv_each_time: " << deal_kv_each_time << ", tile_size_per_bdx: " << tile_size_per_bdx << ", tile_size_per_z: " << tile_size_per_z << std::endl;
  
  auto splitkv_kernel = multi_query_decode_attention_kernel<true, NV_TYPE, NV_TYPE, NV_CACHE_TYPE, num_stages, tile_size_per_z, deal_kv_each_time, GROUP_SIZE, HEAD_DIM_QK, HEAD_DIM_V,
                                                                        BLOCK_SIZE, vec_size, cache_vec_size, blockx, blocky, blockz, blockxc, pos_enc_mode, cache_type>;
  ...
  if constexpr (cache_type == CacheType::CacheT) {
    cache_smem_bytes = num_stages * deal_kv_each_time * (HEAD_DIM_QK + HEAD_DIM_V) * sizeof(CACHE_TYPE);
  }
  
  const uint32_t chunk_size = get_max_partition_size(bsz);
  const int num_chunks = div_up(max_dec_len, chunk_size);
  // size_t smem_size = blocky * blockz * sizeof(T) * 2 + div_up(max_block_num_per_seq, 4) * 4 * sizeof(int)
  //                    + cache_smem_bytes + blocky * HEAD_DIM * sizeof(T);
  // size_t smem_size = blocky * sizeof(T) * 2 + cache_smem_bytes + HEAD_DIM_QK * sizeof(T) + blocky * sizeof(T) * 2 + blocky * sizeof(T) * 2;
  size_t smem_size = max(cache_smem_bytes + HEAD_DIM_QK * sizeof(T)) + blocky * sizeof(T) * 2 + blocky * sizeof(T) * 2 + blocky * sizeof(T) * 2;
  ...
  if (num_chunks <= 1) {
    ...
    auto no_splitkv_kernel = multi_query_decode_attention_kernel<false, NV_TYPE, NV_TYPE, NV_CACHE_TYPE, num_stages, tile_size_per_z, deal_kv_each_time, GROUP_SIZE, HEAD_DIM_QK, HEAD_DIM_V, BLOCK_SIZE, vec_size, 
                                                                             cache_vec_size, blockx, blocky, blockz, blockxc, pos_enc_mode, cache_type>;
    if (smem_size >= 48 * 1024) {
      cudaFuncSetAttribute(
