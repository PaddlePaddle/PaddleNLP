...
class GetPaddingOffsetV2Test(unittest.TestCase):
    def test_get_padding_offset_v2(self):
        ...
        assert (
            sum(ref_x_remove_padding - x_remove_padding) == 0
        ), "Check x_remove_padding failed."
        assert (
            sum(ref_cum_offsets_out - cum_offsets_out) == 0
        ), "Check cum_offsets_out failed."
        assert (
            sum(ref_padding_offset - padding_offset) == 0
        ), "Check padding_offset failed."
